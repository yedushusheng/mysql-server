...
namespace tdsql{
namespace common {
extern bool tdsql_mtr_mode;
extern bool tdsql_bind_conn_to_thd;
...
extern PSI_mutex_key spd_key_mutex_conn_status;
extern PSI_mutex_key spd_key_mutex_active_conns;
#endif
...
SPIDER_CONN_POOL spd_connect_pools;  //spider's connection pool

/** @brief <key, SPIDER_IP_PORT_CONN>, each server's conn count */
HASH spider_ipport_conns;
/** @brief <key, SPIDER_FOR_STS_CONN>, for table's status */
HASH spider_for_sts_conns;

/**
 * @brief Collection of all active backend connections (SPIDER_CONN objects).
 *
 * A connection is "active" once it's created (by spider_create_conn), and is no
 * longer "active" when it's freed (by spider_free_conn). The hash's read and
 * write is protected by a mutex "spider_active_conns_mutex".
 * */
HASH spider_active_conns;
mysql_mutex_t spider_active_conns_mutex;
...
bool SPIDER_CONN_POOL::put_conn(SPIDER_CONN *conn) {
  ...
  if (tdsql::common::tdsql_bind_conn_to_thd) {
    // do not need to recycle connection to spider pool
    return false;
  }
  void *record = NULL;
  conn_queue *cq;
  ...
  while (
      !(record = my_hash_search_using_hash_value(
    ...
    if (!(cq = (conn_queue *)my_malloc(PSI_NOT_INSTRUMENTED, sizeof(conn_queue),
                                       MY_ZEROFILL | MY_WME)))
      goto alloc_failed;
    mysql_mutex_init(0, &cq->mtx, MY_MUTEX_INIT_FAST);
    cq->mtx_inited = true;

    if (!(cq->q_ptr = (DYNAMIC_ARRAY *)my_malloc(
              PSI_NOT_INSTRUMENTED, sizeof(DYNAMIC_ARRAY), MY_WME)))
      goto alloc_failed;
    if (my_init_dynamic_array(
            cq->q_ptr, PSI_NOT_INSTRUMENTED, sizeof(SPIDER_CONN **), nullptr,
            spider_conn_queue_init_size, spider_conn_queue_increase_size))
      goto alloc_failed;

    cq->key_len = conn->conn_key_length;
    if (!(cq->hash_key = (char *)my_malloc(
              PSI_NOT_INSTRUMENTED, cq->key_len + 1, MY_ZEROFILL | MY_WME)))
      goto alloc_failed;
  ...
  if (!ret) {
    spider_conn_set_state(conn, SPIDER_CONN_STATE_IN_CONN_POOL);
  }
...
*/
SPIDER_CONN *SPIDER_CONN_POOL::get_conn(my_hash_value_type v, const uchar *key,
                                        uint key_len) {
  ...
  mysql_rwlock_rdlock(&rw_lock);
  if (!(cq = (conn_queue *)my_hash_search_using_hash_value(&connections, v, key,
                                                           key_len))) {
    mysql_rwlock_unlock(&rw_lock);
  ...
  if (spd_conn) {
    spider_conn_reset_status(spd_conn);
  }
...
static void insert_active_conn(SPIDER_CONN *conn) {
  DBUG_ENTER("insert_active_conn");
  DBUG_PRINT("info", ("spider conn: %p", conn));
  if (!spider_param_enable_conns_view()) DBUG_VOID_RETURN;

  mysql_mutex_lock(&spider_active_conns_mutex);
  my_hash_insert(&spider_active_conns, (uchar *)conn);
  mysql_mutex_unlock(&spider_active_conns_mutex);

  DBUG_VOID_RETURN;
}

static void delete_active_conn(SPIDER_CONN *conn) {
  DBUG_ENTER("delete_active_conn");
  DBUG_PRINT("info", ("spider conn: %p", conn));
  if (!spider_param_enable_conns_view()) DBUG_VOID_RETURN;

  mysql_mutex_lock(&spider_active_conns_mutex);
  my_hash_delete(&spider_active_conns, (uchar *)conn);
  mysql_mutex_unlock(&spider_active_conns_mutex);
...
uchar *spider_xid_get_hash_key(const uchar *ptr, size_t *length,
                               bool not_used __attribute__((unused))) {
  uchar* ptmp = const_cast<uchar*>(ptr);
  *length = (reinterpret_cast<XID_STATE *>(ptmp))->get_xid()->key_length();
  return (reinterpret_cast<XID_STATE *>(ptmp))->get_xid()->key();
}

void spider_xid_free_hash(void *ptr) {
  if (!(static_cast<XID_STATE *>(ptr))->check_has_uncommitted_xa()) my_free(ptr);
}
...
int spider_free_conn_alloc(SPIDER_CONN *conn) {
  ...
  LogCommInfo("free conn:%p to:%s,host:%s:%ld,user:%s", (void*)conn, conn->server_name,
              conn->tgt_host, conn->tgt_port, conn->conn_key);
  spider_rm_conn_of_thd(conn);
  conn->td_set_vars.reset();
...
void spider_free_conn_from_trx(SPIDER_TRX *trx, SPIDER_CONN *conn, bool another,
  ...
  if (thd->current_global_server_version !=
    ...
    // TODO(shuidu) need check connection change
    ulong server_version = get_server_version_by_name(conn->server_name);
    ulong current_version = conn->current_key_version;
    if (server_version !=
        current_version) {  // conn->server_version is different from
...
SPIDER_CONN *spider_create_conn(SPIDER_SHARE *share, ha_spider *spider,
  ...
  int username_len =
      std::max(share->tgt_usernames_lengths[link_idx],
               (uint)thd->m_main_security_ctx.priv_user().length);

  if (!(conn = (SPIDER_CONN *)spider_bulk_malloc(
            spider_current_trx, 18, MYF(MY_WME | MY_ZEROFILL), &conn,
            sizeof(*conn), &tmp_name,
  ...
  if (tdsql::common::tdsql_pass_client_ip) {
    // do: use client's username
    conn->tgt_username_length = thd->m_main_security_ctx.priv_user().length;
    memcpy(conn->tgt_username, thd->m_main_security_ctx.priv_user().str,
           thd->m_main_security_ctx.priv_user().length);
  } else {
    // in mtr, test case use root@localhost to connect,
    // we can't transparently use root@localhost to connect backend mysql
  ...
  __sync_fetch_and_add(&spider_conn_id, 1LL);
  conn->conn_id = spider_conn_id;

  if (mysql_mutex_init(spd_key_mutex_conn_status, &conn->m_status_mutex,
                       MY_MUTEX_INIT_FAST)) {
    *error_num = HA_ERR_OUT_OF_MEM;
  ...
  spider_conn_reset_status(conn);
  /* TODO: find out if SPIDER_CONNS table is needed */
  if (!spider_add_conn_meta_info(conn)) {
    goto error_end;
  }
  conn->td_set_vars = std::make_shared<TdSetVars>();
  insert_active_conn(conn);

  LogCommInfo("create conn:%p to:%s,host:%s:%ld,user:%s", (void*)conn,
              conn->server_name, conn->tgt_host, conn->tgt_port,
              conn->conn_key);
  DBUG_RETURN(conn);
...
error_end:
  conn->m_last_query.mem_free();
//error_last_query_alloc:
error_conn_status_mutex_init:
  mysql_mutex_destroy(&conn->mta_conn_mutex);
...
SPIDER_CONN *spider_get_conn(SPIDER_SHARE *share, int link_idx, SPIDER_TRX *trx,
                             ha_spider *spider, bool another,
                             bool thd_chg __attribute__((unused)),
  ...
  if (share->modify_server_version != current_server_version) {
    LogCommInfo(
        "server version change, ned update conn hash key, old version: %lu, "
        "new version: %lu",
        share->modify_server_version, current_server_version);
    for (unsigned int i = 0; i < share->all_link_count; i++) {
      spider_update_conn_keys(share, i);
    }
    share->modify_server_version = current_server_version;
  }

  TdString tmp_hash_key;
  my_hash_value_type tmp_hash_value;

  tmp_hash_key.assign(share->conn_keys[link_idx],
                      share->conn_keys_lengths[link_idx]);
  tmp_hash_value = share->conn_keys_hash_value[link_idx];

  if (tdsql::common::tdsql_bind_conn_to_thd) {
    // spider get conn from thd
    conn = spider_get_conn_from_thd(trx->thd, tmp_hash_key.to_string());
    // bind conn to trx
    bool conn_in_trx = !!my_hash_search_using_hash_value(
        &trx->trx_conn_hash, tmp_hash_value,
        reinterpret_cast<const uchar *>(tmp_hash_key.c_str()),
        tmp_hash_key.length());
    if (conn) {
      conn->thd = trx->thd;
      ...
      spider_conn_set_command(conn, SPD_COM_QUERY);
      spider_conn_set_state(conn, SPIDER_CONN_STATE_INIT);
      conn->deadlock = false;

      if (!conn_in_trx) my_hash_insert(&trx->trx_conn_hash, (uchar *)conn);
      DBUG_RETURN(conn);
    }
  }

  bool not_find_other_conn =
      (another) && !(conn = (SPIDER_CONN *)my_hash_search_using_hash_value(
                         &trx->trx_another_conn_hash, tmp_hash_value,
                         reinterpret_cast<const uchar *>(tmp_hash_key.c_str()),
                         tmp_hash_key.length()));
  bool not_find_trx_conn =
      (!another) && !(conn = (SPIDER_CONN *)my_hash_search_using_hash_value(
                          &trx->trx_conn_hash, tmp_hash_value,
                          reinterpret_cast<const uchar *>(tmp_hash_key.c_str()),
                          tmp_hash_key.length()));

  if (not_find_other_conn || not_find_trx_conn) {
    ...
    if (!trx->thd || (spider_param_conn_recycle_mode(trx->thd) & 1) ||
      ...
      LogCommDebug(
          "begin get connect pools get connection, thd: %d, recycle mode: %d, "
          "recycle strict: %d",
          !trx->thd, (spider_param_conn_recycle_mode(trx->thd) & 1),
          spider_param_conn_recycle_strict(trx->thd));
      conn = spd_connect_pools.get_conn(tmp_hash_value,
                                        (const uchar *)tmp_hash_key.c_str(),
                                        tmp_hash_key.length());

      if (!conn) {
        LogCommInfo(
            "get connect pool failed, begin get idle connection get connect, "
            "hash key: %s",
            tmp_hash_key.c_str());
        // pthread_mutex_unlock(&spider_conn_mutex);
        conn = spider_get_conn_from_idle_connection(
            share, link_idx, tmp_hash_key.c_str(), spider, conn_kind,
  ...
  } else if (spider) {// find conn in conn_hash
    spider->conns[base_link_idx] = conn;
    if (spider_bit_is_set(spider->conn_can_fo, base_link_idx))
      conn->use_for_active_standby = TRUE;
  }
  ...
  // put conn in thd cache
  if (tdsql::common::tdsql_bind_conn_to_thd)
    trx->thd->td_conn_map.insert(
        std::make_pair(tmp_hash_key, (void *)conn));
...
/**
 * @brief we cache spider conn in thd, so we first try to get conn from thd
 */
SPIDER_CONN *spider_get_conn_from_thd(THD *thd, TdString key) {

  auto it = thd->td_conn_map.find(key);
  if (it != thd->td_conn_map.end()) {
    SPIDER_CONN *tmp_conn = reinterpret_cast<SPIDER_CONN *>(it->second);
    if (!tmp_conn->db_conn) {
      // all conns are freed by spider_free_conn()
      thd->td_conn_map.erase(key);
      return nullptr;
    }
    return reinterpret_cast<SPIDER_CONN *>(it->second);
  }
  return nullptr;
}

void spider_rm_conn_of_thd(SPIDER_CONN *conn) {
  if (!conn) return;
  THD *thd = current_thd;
  TdString key;
  key.assign(conn->conn_key, conn->conn_key_length);
  thd->td_conn_map.erase(key);
}

int spider_free_conn_of_thd(void *conn) {
  if(!conn) return 0;
  return spider_free_conn(reinterpret_cast<SPIDER_CONN *>(conn));
}

int spider_free_conn(SPIDER_CONN *conn) {
  DBUG_ENTER("spider_free_conn");
  DBUG_PRINT("info", ("spider conn=%p", conn));
  spider_conn_set_command(conn, SPD_COM_FREE);
  SPIDER_IP_PORT_CONN *ip_port_conn = conn->ip_port_conn;
  ...
  /*
    This has to be done BEFORE the spider_free() call, otherwise querying
    I_S.SPIDER_ACTIVE_CONNS table would access conns in the state of freeing.
  */
  delete_active_conn(conn);
...
void spider_conn_queue_connect(SPIDER_SHARE *share  __attribute__((unused)), SPIDER_CONN *conn,
                               int link_idx  __attribute__((unused))) {
...
void spider_conn_queue_start_transaction(SPIDER_CONN *conn) {
  ...
  DBUG_PRINT("info", ("spider conn=%p", conn));
  assert(!conn->trx_start);
  conn->queued_trx_start = TRUE;
  conn->trx_start = TRUE;
  DBUG_VOID_RETURN;
}

void spider_conn_queue_xa_start(SPIDER_CONN *conn, tdsql::Gtid *gtid, bool need_gts) {
  DBUG_ENTER("spider_conn_queue_xa_start");
  DBUG_PRINT("info", ("spider conn=%p", conn));
  conn->queued_xa_start = TRUE;
  conn->queued_xa_start_gtid = gtid;
  conn->need_gts = need_gts;
  DBUG_VOID_RETURN;
}
...
int spider_set_conn_bg_param_for_dml(ha_spider *spider) {
  ...
  if (thd && select_lex &&
      (!(select_lex->has_limit() || select_lex->offset_limit ||
...
int spider_create_conn_thread(SPIDER_CONN *conn) {
  ...
  if (conn && !conn->bg_init) {
    ...
    if (mysql_cond_init(spd_key_cond_bg_conn, &conn->bg_conn_cond))
...
void spider_free_conn_thread(SPIDER_CONN *conn) {
  ...
  if (conn->bg_init) {
    ...
    pthread_cond_wait(&conn->bg_conn_sync_cond, &conn->bg_conn_sync_mutex);
    pthread_mutex_unlock(&conn->bg_conn_sync_mutex);
    pthread_join(conn->bg_thread.thread, NULL);
    pthread_cond_destroy(&conn->bg_conn_cond);
    pthread_cond_destroy(&conn->bg_conn_sync_cond);
    spider_free_mem_calc(
        spider_current_trx, conn->bg_job_stack_id,
...
int spider_bg_all_conn_pre_next(ha_spider *spider, int link_idx) {
  ...
  SPIDER_RESULT_LIST *result_list = &spider->result_list;
  __attribute__((unused))SPIDER_SHARE *share = spider->share;
  THD *thd = current_thd;
...
int spider_bg_conn_search(ha_spider *spider, int link_idx, int first_link_idx,
  ...
  if (first) {
    ...
    if (result_list->current->finish_flg) {
      ...
      result_list->table->m_status = STATUS_NOT_FOUND;
...
void *spider_bg_conn_action(void *arg) {
  ...
  while (TRUE) {
    ...
    if (conn->thd) {
      DBUG_PRINT("info", ("spider query_id=%ld", conn->thd->query_id));
    }
#endif
    if (conn->bg_caller_sync_wait) {
      if (!conn->bg_direct_sql) {
        spider_db_handler *dbton_handler;
        spider = static_cast<ha_spider *>(conn->bg_target);
    ...
    if (conn->bg_direct_sql) {  // if not udf
      ...
      do {
        SPIDER_DIRECT_SQL *direct_sql = static_cast<SPIDER_DIRECT_SQL *>(conn->bg_target);
      ...
      if (is_error) {
        while (spider_bg_conn_get_job(conn))
          spider_udf_free_direct_sql_alloc(static_cast<SPIDER_DIRECT_SQL *>(conn->bg_target),
                                           TRUE);
      }
      conn->bg_direct_sql = FALSE;
      continue;
...
int spider_create_sts_thread(SPIDER_SHARE *share) {
  ...
  if (!share->bg_sts_init) {
    ...
    if (mysql_cond_init(spd_key_cond_bg_sts, &share->bg_sts_cond))
...
void spider_free_sts_thread(SPIDER_SHARE *share) {
  ...
  if (share->bg_sts_init) {
    ...
    pthread_join(share->bg_sts_thread.thread, NULL);
    pthread_cond_destroy(&share->bg_sts_sync_cond);
    pthread_cond_destroy(&share->bg_sts_cond);
...
void *spider_bg_sts_action(void *arg) {
  ...
  uint *conn_link_idx;
  uchar *conn_can_fo;
  char **conn_keys  __attribute__((unused));
...
int spider_create_crd_thread(SPIDER_SHARE *share) {
  ...
  if (!share->bg_crd_init) {
    ...
    if (mysql_cond_init(spd_key_cond_bg_crd, &share->bg_crd_cond))
...
void spider_free_crd_thread(SPIDER_SHARE *share) {
  ...
  if (share->bg_crd_init) {
    ...
    pthread_join(share->bg_crd_thread.thread, NULL);
    pthread_cond_destroy(&share->bg_crd_sync_cond);
    pthread_cond_destroy(&share->bg_crd_cond);
      ...
      for (roop_count = 0; roop_count < (int)share->all_link_count;
        ...
        if (share->monitoring_bg_kind[roop_count] &&
            ...
            mysql_cond_init(spd_key_cond_bg_mon_sleep,
                            &share->bg_mon_sleep_conds[roop_count])
    ...
    for (roop_count = 0; roop_count < (int)share->all_link_count;
      ...
      if (share->monitoring_bg_kind[roop_count]) {
        ...
        pthread_cond_wait(&share->bg_mon_conds[roop_count],
                          &share->bg_mon_mutexes[roop_count]);
        pthread_mutex_unlock(&share->bg_mon_mutexes[roop_count]);
        pthread_join(share->bg_mon_threads[roop_count].thread, NULL);
        pthread_cond_destroy(&share->bg_mon_conds[roop_count]);
        pthread_cond_destroy(&share->bg_mon_sleep_conds[roop_count]);
        pthread_mutex_destroy(&share->bg_mon_mutexes[roop_count]);
      }
...
int spider_conn_first_link_idx(THD *thd, long *link_statuses,
  ...
  for (roop_count = 0; roop_count < link_count; roop_count++) {
    assert((conn_link_idx[roop_count] - roop_count) % link_count == 0);
...
int spider_conn_next_link_idx(THD *thd, long *link_statuses,
  ...
  assert((conn_link_idx[link_idx] - link_idx) % link_count == 0);
  tmp_link_idx =
...
bool spider_conn_need_open_handler(ha_spider *spider, uint idx  __attribute__((unused)), int link_idx) {
...
SPIDER_CONN *spider_get_conn_from_idle_connection(
    SPIDER_SHARE *share, int link_idx, const char *conn_key [[maybe_unused]],
    ha_spider *spider, uint conn_kind, int base_link_idx, int *error_num) {
...
//                        1000 * 1000;  // default 20s
  ...
  ip_port_conn = (SPIDER_IP_PORT_CONN *)my_hash_search_using_hash_value(
           &spider_ipport_conns, tmp_hash_value,
           reinterpret_cast<const uchar*>(tmp_hash_key.c_str()), tmp_hash_key.length());
  if (ip_port_conn) { /* exists */
    mysql_rwlock_unlock(&spider_ipport_conn_rwlock);
    pthread_mutex_lock(&ip_port_conn->mutex);
  } else {
 ...
 // *conn->conn_key = *conn_key; // spider_create_conn already set conn_key
  if (spider) {
    spider->conns[base_link_idx] = conn;
    if (spider_bit_is_set(spider->conn_can_fo, base_link_idx))
      conn->use_for_active_standby = TRUE;
...
SPIDER_IP_PORT_CONN *spider_create_ipport_conn(SPIDER_CONN *conn) {
  ...
  if (conn) {
    SPIDER_IP_PORT_CONN *ret =
        (SPIDER_IP_PORT_CONN *)my_malloc(PSI_NOT_INSTRUMENTED, sizeof(*ret), MY_ZEROFILL | MY_WME);
    if (!ret) {
    ...
    if (mysql_cond_init(spd_key_cond_conn_i, &ret->cond))
    ...
    ret->key = (char *)my_malloc(PSI_NOT_INSTRUMENTED, ret->key_len, MY_ZEROFILL | MY_WME);
    if (!ret->key) {
      pthread_cond_destroy(&ret->cond);
      pthread_mutex_destroy(&ret->mutex);
      goto err_malloc_key;
    ...
    strncpy(ret->remote_ip_str, conn->tgt_host, sizeof(ret->remote_ip_str) - 1);
...
/**
 * @brief new my_polling_last_visited() used by SPIDER_CONN_POOL::iterate()
*/
static void poll_last_visited(uchar *entry, void *data) {
  DBUG_ENTER("my_polling_last_visited");
  conn_queue *cq = (conn_queue *)entry;
  void *buffer = NULL;
  bool buffer_malloced = false;
  if (!cq || !cq->q_ptr) DBUG_VOID_RETURN ; /* queue is empty */
  ...
  */
  for (uint i = 0; i < cq->q_ptr->elements; i++) {
    if (!buffer_malloced) {
      buffer = my_malloc(PSI_NOT_INSTRUMENTED, sizeof(SPIDER_CONN **), MY_WME);
      if (!buffer) DBUG_VOID_RETURN ; /* OOM */
...
static void *spider_conn_recycle_action(
...
static void *spider_conn_recycle_action(
  ...
  while (conn_rcyc_init) {
    ...
    // pthread_mutex_unlock(&spider_conn_mutex);
    spd_connect_pools.iterate(reinterpret_cast<my_hash_delegate_func>(poll_last_visited), &param);
...
void spider_free_conn_recycle_thread() {
  ...
  if (conn_rcyc_init) {
    ...
    pthread_cancel(conn_rcyc_thread.thread);
    pthread_join(conn_rcyc_thread.thread, NULL);
  }
  DBUG_VOID_RETURN;
}
...
SPIDER_CONN_META_INFO *spider_create_conn_meta(SPIDER_CONN *conn) {
  ...
  if (conn) {
    SPIDER_CONN_META_INFO *ret =
        (SPIDER_CONN_META_INFO *)my_malloc(PSI_NOT_INSTRUMENTED, sizeof(*ret), MY_ZEROFILL | MY_WME);
    if (!ret) {
      goto err_return_direct;
    }
...
bool spider_add_conn_meta_info(SPIDER_CONN *conn) {
  DBUG_ENTER("spider_add_conn_meta_info");
  if (!spider_param_enable_conns_view()) DBUG_RETURN(true);
...
void spider_update_conn_meta_info(SPIDER_CONN *conn, uint new_status) {
  if (!spider_param_enable_conns_view()) return;
...
/**
 * @brief get conn status use this func
 */
MYSQL *spider_mysql_connect(char *tgt_host, char *tgt_username,
  ...
  /* tgt_db not use */
  auto m = mysql_real_connect(db_conn, tgt_host, true_username, tgt_password,
                              NULL, tgt_port, tgt_socket, real_connect_option);
...
static void *spider_get_status_action(void *arg __attribute__((unused))) {
  DBUG_ENTER("spider_get_status_action");

  SPIDER_SHARE *share;
  MYSQL *conn;
  my_thread_init();
  std::shared_ptr<THD> thd = std::make_shared<THD>();
  if (!thd) {
    my_thread_end();
    DBUG_RETURN(NULL);
  }
  SPIDER_set_next_thread_id(thd.get());
  thd->thread_stack = (char *)&thd;
  thd->store_globals();
  // thread_safe_decrement32(
  //    &thread_count); /* for shutdonw, don't wait this thread */
  ...
  while (get_status_init) {
    double modify_interval = opt_tdsql_modify_status_interval;
    ...
    ulonglong auto_increment_value  __attribute__((unused));
    time_t create_time;
    time_t update_time;
    time_t check_time;
    MYSQL_TIME mysql_time;
    MYSQL_TIME_STATUS time_status;
    bool not_used_my_bool;
    long not_used_long;
    SPIDER_FOR_STS_CONN *sts_conn = NULL;
    time_t to_tm_time = (time_t)time((time_t *)0);
    struct tm lt;
     __attribute__((unused)) struct tm *l_time = localtime_r(&to_tm_time, &lt);
    my_hrtime_t current_time = my_hrtime();
     __attribute__((unused))long usec = hrtime_sec_part(current_time);
    ...
    for (ulong i = 0; (i < share_records) && get_status_init;
      ...
      if (difftime(cur_time, modify_time) >= modify_interval &&
        ...
        if (conn) {
          int query_length = snprintf(query, 256, "%s %s like '%s'", query_head,
                                      tgt_db, tgt_tb);
          if (!mysql_real_query(conn, query, query_length)) {
            ...
            if (res) {
              ...
              log_spider_mysql_sql(conn, query, query_length, COM_QUERY);
            }
          } else {
            log_spider_mysql_sql(conn, query, query_length, COM_QUERY);
            log_spider_resultf(
...
void spider_free_get_status_thread() {
  ...
  if (get_status_init) {
    ...
    pthread_join(get_status_thread.thread, NULL);
  }
  DBUG_VOID_RETURN;
}
...
SPIDER_FOR_STS_CONN *spider_create_sts_conn(char *key, ulong key_len,
  ...
  sts_conn =
      (SPIDER_FOR_STS_CONN *)my_malloc(PSI_NOT_INSTRUMENTED, sizeof(*sts_conn), MY_ZEROFILL | MY_WME);
  if (!sts_conn) {
    DBUG_RETURN(NULL);
  }
  sts_conn->key_len = key_len;
  sts_conn->key = (char *)my_malloc(PSI_NOT_INSTRUMENTED, key_len, MY_ZEROFILL | MY_WME);
  if (!sts_conn->key) {
    DBUG_RETURN(NULL);
  }
...
void spider_free_conn_by_servername(char *servername) {
  ...
  uint conn_key_len = strlen(servername);
  strmov(conn_key, servername);

  conn = spd_connect_pools.get_conn_by_key((uchar *)conn_key,
      conn_key_len);
  while (conn) {
    spider_free_conn(conn);
  }
...
*/
int spider_send_kill(SPIDER_CONN *to_kill, THD::killed_state kill_signal) {
  ...
  if (kill_signal >= THD::KILL_CONNECTION) {
    is_kill_conn = TRUE;
  ...
  */
  ...
  spider_conn_set_command(killer, SPD_COM_QUERY);
  spider_conn_set_state(killer, SPIDER_CONN_STATE_EXEC_QUERY);
  spider_conn_set_time(killer);
  ...
  spider_conn_set_command(to_kill, SPD_COM_KILLED);
  spider_conn_reset_status(killer);
...
void spider_conn_reset_status(SPIDER_CONN* conn) {
...
void spider_conn_set_time(SPIDER_CONN *conn) {
  ...
  if (conn->dry_run) DBUG_VOID_RETURN;
  if (!spider_param_enable_conns_view()) DBUG_VOID_RETURN;
  conn->m_start_utime = my_interval_timer()/1000;
  DBUG_VOID_RETURN;
}
...
void spider_conn_set_last_query(SPIDER_CONN *conn, const char *query,
  ...
  uint max_info_length = spider_param_active_conns_view_info_length();
  if (!max_info_length) DBUG_VOID_RETURN;

  mysql_mutex_lock(&conn->m_status_mutex);
  conn->m_last_query.copy(query, MY_MIN(length, max_info_length), cs);
  mysql_mutex_unlock(&conn->m_status_mutex);
