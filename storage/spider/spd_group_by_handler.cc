...
spider_fields::spider_fields()
    : dbton_count(0),
      ...
      current_table_num(0),
      table_holder(NULL),
      first_link_idx_chain(NULL),
      last_link_idx_chain(NULL),
      current_link_idx_chain(NULL),
      first_conn_holder(NULL),
      last_conn_holder(NULL),
      current_conn_holder(NULL),
      first_field_holder(NULL),
      last_field_holder(NULL),
      current_field_holder(NULL),
      first_field_chain(NULL),
      last_field_chain(NULL),
      current_field_chain(NULL) {
...
spider_fields::~spider_fields() {
  ...
  if (table_holder) spider_free(spider_current_trx, table_holder, MYF(0));
...
void spider_fields::add_dbton_id(uint dbton_id_arg) {
  uint roop_count;
...
uint spider_fields::get_next_dbton_id() {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  if (current_dbton_num >= dbton_count) DBUG_RETURN(SPIDER_DBTON_SIZE);
  return_dbton_id = dbton_ids[current_dbton_num];
  ++current_dbton_num;
  DBUG_RETURN(return_dbton_id);
}

int spider_fields::make_link_idx_chain(int link_status __attribute__((unused))) {
  uint roop_count, roop_count2;
  SPIDER_CONN *conn;
  int error_num __attribute__((unused));
  ...
  do {
    for (roop_count2 = 0; roop_count2 < table_count; ++roop_count2) {
      ...
      for (roop_count = 0;
           roop_count < conn_holder->link_idx_holder_count_max - 1;
           ++roop_count) {
        if (!link_idx_holder->next) {
          DBUG_PRINT("info",
                     ("spider fill link_idx_holder for %u", roop_count2));
          if (!(add_link_idx_holder = create_link_idx_holder()))
            DBUG_RETURN(HA_ERR_OUT_OF_MEM);
          dup_link_idx_holder = get_dup_link_idx_holder(table_link_idx_holder,
                                                        dup_link_idx_holder);
    ...
    for (roop_count = 0; roop_count < conn_holder->link_idx_holder_count_max;
         ++roop_count) {
  ...
  do {
    table_link_idx_holder = &current_conn_holder->table_link_idx_holder[0];
    table_link_idx_holder->current_link_idx_holder =
        table_link_idx_holder->first_link_idx_holder;
  ...
  for (roop_count = spider_conn_link_idx_next(share->link_statuses,
                                              spider->conn_link_idx, -1,
    ...
    link_idx_holder = table_link_idx_holder->current_link_idx_holder;
    table_link_idx_holder->current_link_idx_holder = link_idx_holder->next;
    assert(link_idx_holder->link_idx == (int)roop_count);
  ...
  do {
    ...
    if (has_remain) {
      ...
      share = spider->share;
      DBUG_PRINT("info",
                 ("spider create link_idx_chain sorted by %d", roop_count2));
      for (roop_count = spider_conn_link_idx_next(
        ...
        DBUG_PRINT("info",
                   ("spider create link_idx_chain for %d", roop_count2));
        table_link_idx_holder[roop_count2].current_link_idx_holder =
...
SPIDER_LINK_IDX_CHAIN *spider_fields::create_link_idx_chain() {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_RETURN((SPIDER_LINK_IDX_CHAIN *)spider_malloc(
      spider_current_trx, 254, sizeof(SPIDER_LINK_IDX_CHAIN),
...
SPIDER_LINK_IDX_HOLDER *spider_fields::get_dup_link_idx_holder(
  ...
  if (!current) {
    ...
    do {
      if (return_link_idx_holder->link_status == SPIDER_LINK_STATUS_OK) break;
      ...
      if (!return_link_idx_holder) {
        ...
        do {
          if (return_link_idx_holder->link_status == SPIDER_LINK_STATUS_OK)
            break;
          assert(return_link_idx_holder != current);
        } while ((return_link_idx_holder = return_link_idx_holder->next));
      }
    } else {
      if (!current->next) {
...
bool spider_fields::check_link_ok_chain() {
  ...
  for (current_link_idx_chain = first_link_idx_chain; current_link_idx_chain;
       current_link_idx_chain = current_link_idx_chain->next) {
    DBUG_PRINT("info",
               ("spider current_link_idx_chain=%p", current_link_idx_chain));
    DBUG_PRINT("info", ("spider current_link_idx_chain->link_status=%d",
                        current_link_idx_chain->link_status));
...
int spider_fields::add_link_idx(SPIDER_CONN_HOLDER *conn_holder_arg,
                                ha_spider *spider_arg, int link_idx) {
  SPIDER_TABLE_LINK_IDX_HOLDER *table_link_idx_holder;
  SPIDER_LINK_IDX_HOLDER *link_idx_holder;
  ...
  if (!table_link_idx_holder->first_link_idx_holder) {
    ...
    if (!link_idx_holder) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  ...
  link_idx_holder->link_status = spider_conn_get_link_status(
      spider_arg->share->link_statuses, spider_arg->conn_link_idx, link_idx);
...
SPIDER_LINK_IDX_HOLDER *spider_fields::create_link_idx_holder() {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_RETURN((SPIDER_LINK_IDX_HOLDER *)spider_malloc(
      spider_current_trx, 253, sizeof(SPIDER_LINK_IDX_HOLDER),
      MYF(MY_WME | MY_ZEROFILL)));
}
...
SPIDER_LINK_IDX_HOLDER *spider_fields::get_next_table_on_link_idx_chain(
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  if (!link_idx_chain_arg->current_link_idx_holder) DBUG_RETURN(NULL);
...
SPIDER_CONN_HOLDER *spider_fields::add_conn(SPIDER_CONN *conn_arg,
                                            long access_balance) {
  SPIDER_CONN_HOLDER *conn_holder;
  ...
  if (!first_conn_holder) {
    ...
    if (!conn_holder) DBUG_RETURN(NULL);
    ...
    conn_holder = first_conn_holder;
    do {
      if (conn_holder->conn == conn_arg) break;
    } while ((conn_holder = conn_holder->next));
    if (!conn_holder) {
      conn_holder = create_conn_holder();
      DBUG_PRINT("info", ("spider conn_holder=%p", conn_holder));
...
SPIDER_CONN_HOLDER *spider_fields::create_conn_holder() {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  return_conn_holder = (SPIDER_CONN_HOLDER *)spider_bulk_malloc(
      spider_current_trx, 252, MYF(MY_WME | MY_ZEROFILL), &return_conn_holder,
      (uint)(sizeof(SPIDER_CONN_HOLDER)), &table_link_idx_holder,
      (uint)(table_count * sizeof(SPIDER_TABLE_LINK_IDX_HOLDER)), NullS);
  if (!return_conn_holder) DBUG_RETURN(NULL);
  DBUG_PRINT("info", ("spider table_count=%u", table_count));
  DBUG_PRINT("info",
             ("spider table_link_idx_holder=%p", table_link_idx_holder));
...
SPIDER_CONN_HOLDER *spider_fields::get_next_conn_holder() {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  if (current_conn_holder) current_conn_holder = current_conn_holder->next;
  DBUG_RETURN(return_conn_holder);
}
...
bool spider_fields::check_conn_same_conn(SPIDER_CONN *conn_arg) {
...
bool spider_fields::remove_conn_if_not_checked() {
  ...
  while (current_conn_holder) {
    if (!current_conn_holder->checked_for_same_conn) {
      removed = TRUE;
      DBUG_PRINT("info",
                 ("spider remove connection %p", current_conn_holder->conn));
...
void spider_fields::check_support_dbton(uchar *dbton_bitmap) {
  SPIDER_CONN_HOLDER *conn_holder;
...
void spider_fields::choose_a_conn() {
  ...
  rand_val = spider_rand(thd_get_thread_id(thd));
  balance_val = (longlong)(rand_val * balance_total);

  current_conn_holder = first_conn_holder;
  while (current_conn_holder) {
    if (balance_val < current_conn_holder->access_balance) break;
    balance_val -= current_conn_holder->access_balance;

    DBUG_PRINT("info",
               ("spider remove connection %p", current_conn_holder->conn));
    first_conn_holder = current_conn_holder->next;
    assert(current_conn_holder->next);
  ...
  DBUG_PRINT("info",
             ("spider choosed connection is %p", current_conn_holder->conn));
...
void spider_fields::free_conn_holder(SPIDER_CONN_HOLDER *conn_holder_arg) {
  uint roop_count;
  DBUG_ENTER("spider_fields::free_conn_holder");
  DBUG_PRINT("info", ("spider this=%p", this));
  for (roop_count = 0; roop_count < table_count; ++roop_count) {
    if (conn_holder_arg->table_link_idx_holder[roop_count]
            .first_link_idx_holder) {
      SPIDER_LINK_IDX_HOLDER *first_link_idx_holder, *current_link_idx_holder;
      first_link_idx_holder = conn_holder_arg->table_link_idx_holder[roop_count]
                                  .first_link_idx_holder;
...
SPIDER_TABLE_HOLDER *spider_fields::add_table(ha_spider *spider_arg) {
  spider_string *str;
  uint length;
  ...
  length =
      my_sprintf(tmp_buf, (tmp_buf, "t%u", spider_arg->idx_for_direct_join));
  ...
  while ((field_holder = get_next_field_holder())) {
    if (!field_holder->spider) {
      ...
      if (field->field_index() < table->s->fields &&
...
int spider_fields::create_table_holder(uint table_count_arg) {
  DBUG_ENTER("spider_fields::create_table_holder");
  DBUG_PRINT("info", ("spider this=%p", this));
  assert(!table_holder);
  table_holder = (SPIDER_TABLE_HOLDER *)spider_malloc(
      spider_current_trx, 249, table_count_arg * sizeof(SPIDER_TABLE_HOLDER),
      MYF(MY_WME | MY_ZEROFILL));
  if (!table_holder) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
...
SPIDER_TABLE_HOLDER *spider_fields::get_next_table_holder() {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  if (current_table_num >= table_count) DBUG_RETURN(NULL);
...
int spider_fields::add_field(Field *field_arg) {
  SPIDER_FIELD_HOLDER *field_holder;
  ...
  if (!first_field_holder) {
    ...
    if (!field_holder) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    ...
    do {
      if (field_holder->field == field_arg) break;
  ...
  if (!field_chain) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
...
SPIDER_FIELD_HOLDER *spider_fields::create_field_holder() {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_RETURN((SPIDER_FIELD_HOLDER *)spider_malloc(spider_current_trx, 250,
                                                   sizeof(SPIDER_FIELD_HOLDER),
                                                   MYF(MY_WME | MY_ZEROFILL)));
}
...
SPIDER_FIELD_HOLDER *spider_fields::get_next_field_holder() {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  if (current_field_holder) current_field_holder = current_field_holder->next;
...
SPIDER_FIELD_CHAIN *spider_fields::create_field_chain() {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_RETURN((SPIDER_FIELD_CHAIN *)spider_malloc(spider_current_trx, 251,
                                                  sizeof(SPIDER_FIELD_CHAIN),
                                                  MYF(MY_WME | MY_ZEROFILL)));
}
...
SPIDER_FIELD_CHAIN *spider_fields::get_next_field_chain() {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  if (current_field_chain) current_field_chain = current_field_chain->next;
...
Field **spider_fields::get_next_field_ptr() {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  if (*current_field_ptr) current_field_ptr++;
  ...
  while ((table_holder = get_next_table_holder())) {
    ...
    if (tmp_share->monitoring_kind[tmp_link_idx]) {
      error_num_buf = spider_ping_table_mon_from_table(
          tmp_spider->trx, tmp_spider->trx->thd, tmp_share, tmp_link_idx,
          (uint32)tmp_share->monitoring_sid[tmp_link_idx],
          tmp_share->table_name, tmp_share->table_name_length,
          tmp_spider->conn_link_idx[tmp_link_idx], NULL, 0,
          tmp_share->monitoring_kind[tmp_link_idx],
          tmp_share->monitoring_limit[tmp_link_idx],
          tmp_share->monitoring_flag[tmp_link_idx], TRUE);
      if (!error_num) error_num = error_num_buf;
    }
  }
...
spider_group_by_handler::spider_group_by_handler(THD *thd_arg, Query *query_arg,
...
int spider_group_by_handler::init_scan() {
  ...
  if (direct_order_limit && select_lex->explicit_limit &&
  ...
  if ((error_num = spider_set_conn_bg_param(spider))) DBUG_RETURN(error_num);
  ...
  while ((dbton_id = fields->get_next_dbton_id()) < SPIDER_DBTON_SIZE) {
    ...
    if ((error_num =
             dbton_hdl->append_select_part(SPIDER_SQL_TYPE_SELECT_SQL))) {
      DBUG_RETURN(error_num);
    }
    fields->set_field_ptr(table->field);
    if ((error_num = dbton_hdl->append_list_item_select_part(
             query.select, NULL, 0, TRUE, fields,
    ...
    if (query.where) {
      ...
      if ((error_num = dbton_hdl->append_item_type_part(
               query.where, NULL, 0, TRUE, fields,
               SPIDER_SQL_TYPE_SELECT_SQL))) {
        DBUG_RETURN(error_num);
      }
    }
    if (query.group_by) {
      if ((error_num = dbton_hdl->append_group_by_part(
               query.group_by, NULL, 0, TRUE, fields,
               SPIDER_SQL_TYPE_SELECT_SQL))) {
        DBUG_RETURN(error_num);
      }
    }
    if (query.having) {
      ...
      if ((error_num = dbton_hdl->append_item_type_part(
               query.having, NULL, 0, TRUE, fields,
               SPIDER_SQL_TYPE_SELECT_SQL))) {
        DBUG_RETURN(error_num);
      }
    }
    ...
    if ((error_num =
             dbton_hdl->append_select_lock_part(SPIDER_SQL_TYPE_SELECT_SQL))) {
      DBUG_RETURN(error_num);
    }
  }
  ...
  while ((link_idx_chain = fields->get_next_link_idx_chain())) {
    ...
    if (result_list->bgs_phase > 0) {
      if ((error_num =
               spider_check_and_init_casual_read(trx->thd, spider, link_idx)))
        DBUG_RETURN(error_num);
      if ((error_num = spider_bg_conn_search(
               spider, link_idx, dbton_hdl->first_link_idx, TRUE, FALSE,
    ...
    if (spider->use_pre_call) {
      ...
      if (spider->result_list.bgs_phase > 0) {
        ...
        while ((link_idx_chain = fields->get_next_link_idx_chain())) {
          ...
          spider->link_idx_chain = link_idx_chain;
          if ((error_num = spider_bg_conn_search(
                   spider, link_idx, dbton_hdl->first_link_idx, TRUE, TRUE,
                   !fields->is_first_link_ok_chain(link_idx_chain)))) {
...
group_by_handler *spider_create_group_by_handler(THD *thd, Query *query) {
  ...
  switch (thd_sql_command(thd)) {
    ...
    case SQLCOM_DELETE_MULTI:
      DBUG_PRINT("info",
                 ("spider update and delete does not support this feature"));
      DBUG_RETURN(NULL);
    default:
      break;
  }
  ...
  do {
    ...
    if (from->table->const_table) continue;
    if (from->table->part_info) {
      ...
      if (bits != 1) {
        DBUG_PRINT("info", ("spider using multiple partitions is not supported "
                            "by this feature yet"));
#else
      DBUG_PRINT("info",
  ...
  for (roop_count = 0; roop_count < (int)share->use_dbton_count; ++roop_count) {
    ...
    if (spider_dbton[dbton_id].support_direct_join &&
  ...
  while ((from = from->next_local)) {
    ...
    for (roop_count = 0; roop_count < spider_bitmap_size(SPIDER_DBTON_SIZE);
         ++roop_count) {
  ...
  for (roop_count = 0; roop_count < SPIDER_DBTON_SIZE; ++roop_count) {
    if (spider_bit_is_set(dbton_bitmap, roop_count)) {
      ...
      while ((item = it++)) {
        ...
        if (spider_db_print_item_type(item, spider, NULL, NULL, 0, roop_count,
                                      TRUE, fields_arg)) {
          DBUG_PRINT("info",
                     ("spider dbton_id=%d can't create select", roop_count));
      ...
      if (keep_going) {
        if (spider_dbton[roop_count].db_util->append_from_and_tables(
          ...
          DBUG_PRINT("info",
                     ("spider dbton_id=%d can't create from", roop_count));
          spider_clear_bit(dbton_bitmap, roop_count);
          keep_going = FALSE;
        }
      }
      if (keep_going) {
        ...
        if (query->where) {
          if (spider_db_print_item_type(query->where, spider, NULL, NULL, 0,
            ...
            DBUG_PRINT("info",
                       ("spider dbton_id=%d can't create where", roop_count));
            spider_clear_bit(dbton_bitmap, roop_count);
            keep_going = FALSE;
          }
        }
        ...
        if (query->group_by) {
          for (order = query->group_by; order; order = order->next) {
            if (spider_db_print_item_type((*order->item), spider, NULL, NULL, 0,
                                          roop_count, TRUE, fields_arg)) {
              DBUG_PRINT("info", ("spider dbton_id=%d can't create group by",
                                  roop_count));
              spider_clear_bit(dbton_bitmap, roop_count);
              keep_going = FALSE;
              break;
      ...
      if (keep_going) {
        ...
        if (query->order_by) {
          for (order = query->order_by; order; order = order->next) {
            if (spider_db_print_item_type((*order->item), spider, NULL, NULL, 0,
                                          roop_count, TRUE, fields_arg)) {
              DBUG_PRINT("info", ("spider dbton_id=%d can't create order by",
                                  roop_count));
              spider_clear_bit(dbton_bitmap, roop_count);
              keep_going = FALSE;
              break;
      ...
      if (keep_going) {
        ...
        if (query->having) {
          if (spider_db_print_item_type(query->having, spider, NULL, NULL, 0,
                                        roop_count, TRUE, fields_arg)) {
            DBUG_PRINT("info",
                       ("spider dbton_id=%d can't create having", roop_count));
            spider_clear_bit(dbton_bitmap, roop_count);
            keep_going = FALSE;
          }
        }
  ...
  DBUG_PRINT("info",
             ("spider s->table_name=%s", from->table->s->table_name.str));
  ...
  for (roop_count = spider_conn_link_idx_next(
       ...
       roop_count = spider_conn_link_idx_next(
           share->link_statuses, spider->conn_link_idx, roop_count,
           share->link_count, tgt_link_status)) {
    ...
    if (conn->table_lock) {
      DBUG_PRINT("info",
                 ("spider direct_join does not support with lock tables yet"));
      if (lock_mode) {
        delete fields;
        DBUG_RETURN(NULL);
      }
    ...
    if (!fields->add_conn(
            conn, share->access_balances[spider->conn_link_idx[roop_count]])) {
      DBUG_PRINT("info", ("spider can not create conn_holder"));
      delete fields;
      DBUG_RETURN(NULL);
    }
    if (fields->add_link_idx(conn->conn_holder_for_direct_join, spider,
                             roop_count)) {
  ...
  while ((from = from->next_local)) {
    ...
    for (roop_count = spider_conn_link_idx_next(
      ...
      if (!fields->check_conn_same_conn(conn)) {
        DBUG_PRINT(
            "info",
            ("spider connection %p can not be used for this query with locking",
             conn));
        if (lock_mode) {
          delete fields;
          DBUG_RETURN(NULL);
    ...
    if (fields->remove_conn_if_not_checked()) {
      if (lock_mode) {
        DBUG_PRINT("info", ("spider some connections can not be used for this "
                            "query with locking"));
        delete fields;
        DBUG_RETURN(NULL);
      }
  ...
  if (!fields->all_query_fields_are_query_table_members()) {
    DBUG_PRINT("info",
               ("spider found a query field that is not a query table member"));
    delete fields;
    DBUG_RETURN(NULL);
  }
