...
#define PTR_BYTE_DIFF(A,B) (my_ptrdiff_t) ((const char*) (A) - (const char*) (B))
//#define SPIDER_DISABLE_STANDBY /* disable one spider shard has multi remote db */
// the max link number 
#define SPIDER_DBTON_SIZE 5
...
class spider_string {
 public:
  ...
  //void q_append(const LEX_CSTRING& data, uint& data_len);
...
class spider_db_util {
 public:
  ...
  virtual int append_xa_start(spider_string *str, tdsql::Gtid *gtid, bool need_gts) = 0;
...
class spider_db_row {
 public:
  ...
  virtual int store_to_field(ha_spider *spider, Field *field, CHARSET_INFO *access_charset) = 0;
...
class spider_db_result {
 ...
 public:
  ...
  virtual int fetch_table_status(
      ...
      time_t &create_time, time_t &update_time, time_t &check_time) = 0;
...
class spider_db_conn {
 ...
 public:
  spider_db_conn(SPIDER_CONN *conn) : conn(conn) {}
  virtual ~spider_db_conn() {}
  virtual int init() = 0;
  virtual bool is_connected() = 0;
  virtual void bg_connect() = 0;
  virtual int connect(const char *tgt_host, const char *tgt_username, const char *tgt_password,
                      long tgt_port, const char *tgt_socket, const char *server_name,
  ...
  virtual int exec_query(const char *query, uint length, int quick_mode,
                         bool sync = true) = 0;
  virtual int get_errno() = 0;
  virtual const char *get_error() = 0;
  virtual const char *get_sqlstate() = 0;
  ...
  virtual int xa_start(tdsql::Gtid *gtid, int *need_mon) = 0;
  virtual int xa_end(tdsql::Gtid *gtid, int *need_mon) = 0;
  virtual int xa_prepare(tdsql::Gtid *gtid, int *need_mon) = 0;
  virtual int xa_end_and_prepare(tdsql::Gtid *gtid, int *need_mon) = 0;
  virtual int xa_commit(tdsql::Gtid *gtid, int *need_mon) = 0;
  virtual int xa_commit_one_phase(tdsql::Gtid *gtid, int *need_mon) = 0;
  virtual int xa_rollback(tdsql::Gtid *gtid, int *need_mon) = 0;
...
class spider_db_share {
 ...
 public:
  ...
  virtual int append_column_name(spider_string *str, uint field_index, bool need_quote = true) = 0;
...
class spider_db_handler {
 ...
 public:
  ...
  virtual int append_explain_select_part(key_range *start_key,
                                         key_range *end_key, ulong sql_type,
                                         int link_idx) = 0;
  ...
  virtual ha_rows explain_select(key_range *start_key, key_range *end_key,
                                 int link_idx) = 0;
...
class spider_db_copy_table {
 public:
  spider_db_share *db_share;
  spider_db_copy_table(spider_db_share *db_share) : db_share(db_share) {}
...
typedef struct st_spider_result {
  ...
  //volatile st_spider_result *prev;
  st_spider_result *prev;
  //volatile st_spider_result *next;
  ...
  Temp_table_param result_tmp_tbl_prm;
...
typedef struct st_spider_result_list {
  //volatile SPIDER_RESULT *first;
  SPIDER_RESULT *first;
  //volatile SPIDER_RESULT *last;
  SPIDER_RESULT *last;
  //volatile SPIDER_RESULT *current;
  ...
  TABLE **upd_tmp_tbls;
  Temp_table_param *upd_tmp_tbl_prms;
  bool tmp_table_join;
