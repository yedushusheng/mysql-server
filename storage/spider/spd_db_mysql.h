...
#include "sql/tdsql/xa/xa_gt.h"
typedef class Temp_table_param TMP_TABLE_PARAM;
 
class spider_db_mysql_util : public spider_db_util {
 public:
  spider_db_mysql_util();
  ~spider_db_mysql_util();
  int append_name(spider_string *str, const char *name, uint name_length)  override;
  int append_name_with_charset(spider_string *str, const char *name,
                               uint name_length, CHARSET_INFO *name_charset)  override;
  bool is_name_quote(const char head_code)  override;
  int append_escaped_name_quote(spider_string *str)  override;
  int append_key_equal_or_like(ha_spider *spider,
                               spider_string *str,
                               Field *field,
                               KEY_PART_INFO *key_part,
                               bool *is_like)  override;
  ...
  int append_trx_isolation(spider_string *str, int trx_isolation)  override;
  int append_autocommit(spider_string *str, bool autocommit)  override;
  int append_sql_log_off(spider_string *str, bool sql_log_off)  override;
  int append_time_zone(spider_string *str, Time_zone *time_zone)  override;
  int append_start_transaction(spider_string *str)  override;
  int append_xa_start(spider_string *str, tdsql::Gtid *gtid, bool need_gts)  override;
  int append_lock_table_head(spider_string *str)  override;
  int append_lock_table_body(spider_string *str, const char *db_name,
                             ...
                             CHARSET_INFO *table_name_charset, int lock_type)  override;
  int append_lock_table_tail(spider_string *str)  override;
  int append_unlock_table(spider_string *str)  override;
  int open_item_func(Item_func *item_func, ha_spider *spider,
                     ...
                     CHARSET_INFO *field_charset)  override;
  ...
  int open_item_sum_func(Item_sum *item_sum, ha_spider *spider,
                         ...
                         spider_fields *fields, CHARSET_INFO *field_charse)  override;
#endif
  int append_escaped_util(spider_string *to, String *from)  override;
  ...
  int append_table(ha_spider *spider, spider_fields *fields, spider_string *str,
                   ...
                   bool top_down, bool first)  override;
  ...
  int append_from_and_tables(ha_spider *spider, spider_fields *fields,
                             spider_string *str, TABLE_LIST *table_list,
                             uint table_count)  override;
  int reappend_tables(spider_fields *fields,
                      SPIDER_LINK_IDX_CHAIN *link_idx_chain,
                      spider_string *str)  override;
  int append_where(spider_string *str)  override;
  int append_having(spider_string *str)  override;
#endif
};
...
class spider_db_mysql_row : public spider_db_row {
 public:
  ...
  spider_db_mysql_row();
  ~spider_db_mysql_row();
  int store_to_field(ha_spider *spider, Field *field, CHARSET_INFO *access_charset) override;
  int append_to_str(spider_string *str) override;
  int append_escaped_to_str(spider_string *str, uint dbton_id) override;
  void first() override;
  void next() override;
  ...
  int store_to_tmp_table(TABLE *tmp_table, spider_string *str) override;
};
...
class spider_db_mysql_result : public spider_db_result {
 public:
  ...
  SPIDER_DB_ROW *fetch_row_from_result_buffer(
      spider_db_result_buffer *spider_res_buf) override;
  SPIDER_DB_ROW *fetch_row_from_tmp_table(TABLE *tmp_table) override;
  int fetch_table_status(int mode, ha_rows &records, ulong &mean_rec_length,
                         ulonglong &data_file_length,
                         ulonglong &max_data_file_length,
                         ulonglong &index_file_length,
                         ulonglong &auto_increment_value, time_t &create_time,
                         time_t &update_time, time_t &check_time) override;
  int fetch_table_records(int mode, ha_rows &records) override;
  int fetch_table_cardinality(int mode, TABLE *table, longlong *cardinality,
                              uchar *cardinality_upd, int bitmap_size) override;
  int fetch_table_mon_status(int &status) override;
...
class spider_db_mysql : public spider_db_conn {
 ...
 public:
  ...
  int connect(const char *tgt_host, const char *tgt_username,
              const char *tgt_password, long tgt_port, const char *tgt_socket,
              const char *server_name, int connect_retry_count,
              longlong connect_retry_interval) override;
  int ping() override;
  ...
  const char *get_sqlstate() override;
  bool is_server_gone_error(int error_num) override;
  bool is_dup_entry_error(int error_num) override;
  bool is_xa_nota_error(int error_num) override;
  void print_warnings();
  ...
  ulonglong affected_rows() override;
  uint matched_rows() override;
  ulonglong last_insert_id() override;
  int set_character_set(const char *csname) override;
  int select_db(const char *dbname) override;
  int consistent_snapshot(int *need_mon) override;
  bool trx_start_in_bulk_sql() override;
  bool trx_transmit_begin_commit() override;
  int start_transaction(int *need_mon) override;
  int commit(int *need_mon) override;
  int rollback(int *need_mon) override;
  bool xa_start_in_bulk_sql() override;
  int xa_start(tdsql::Gtid *gtid, int *need_mon) override;
  int xa_end(tdsql::Gtid *gtid, int *need_mon) override;
  int xa_prepare(tdsql::Gtid *gtid, int *need_mon) override;
  int xa_end_and_prepare(tdsql::Gtid *gtid, int *need_mon) override;
  int xa_commit(tdsql::Gtid *gtid, int *need_mon) override;
  int xa_commit_one_phase(tdsql::Gtid *gtid, int *need_mon) override;
  int xa_rollback(tdsql::Gtid *gtid, int *need_mon) override;
  bool set_trx_isolation_in_bulk_sql() override;
  int set_trx_isolation(int trx_isolation, int *need_mon) override;
  bool set_autocommit_in_bulk_sql() override;
  int set_autocommit(bool autocommit, int *need_mon) override;
  bool set_sql_log_off_in_bulk_sql() override;
  int set_sql_log_off(bool sql_log_off, int *need_mon) override;
  bool set_time_zone_in_bulk_sql() override;
  int set_time_zone(Time_zone *time_zone, int *need_mon) override;
  ...
  int show_master_status(SPIDER_TRX *trx, SPIDER_SHARE *share, int all_link_idx,
                         ...
                         SPIDER_DB_RESULT **res2) override;
  int select_binlog_gtid_pos(SPIDER_TRX *trx, SPIDER_SHARE *share,
                             ...
                             SPIDER_DB_RESULT **res);
  size_t escape_string(char *to, const char *from, size_t from_length) override;
  bool have_lock_table_list() override;
  int append_lock_tables(spider_string *str) override;
  ...
  void set_dup_key_idx(ha_spider *spider, int link_idx) override;
  bool cmp_request_key_to_snd(st_spider_db_request_key *request_key) override;
  ulong thread_id() const override;
  SPIDER_CONN *get_conn() const override { return conn; }
  uint server_status() const override { return (db_conn ? db_conn->server_status : 0); }
  uint warning_count() const override { return (db_conn ? db_conn->warning_count : 0); }
  const char *get_db() const override;
  void set_errno(int) override;
};
...
class spider_mysql_share : public spider_db_share {
 public:
  ...
  ~spider_mysql_share();
  int init() override;
  uint get_column_name_length(uint field_index) override;
  int append_column_name(spider_string *str, uint field_index, bool need_quote = true) override;
  int append_column_name_with_alias(spider_string *str, uint field_index,
                                    const char *alias, uint alias_length) override;
...
class spider_mysql_handler : public spider_db_handler {
 ...
 public:
  ...
  spider_mysql_handler(ha_spider *spider, spider_mysql_share *share);
  ~spider_mysql_handler();
  int init() override;
  int append_index_hint(spider_string *str, int link_idx, ulong sql_type) override;
  int append_table_name_with_adjusting(spider_string *str, int link_idx,
                                       ulong sql_type) override;
  ...
  int append_tmp_table_and_sql_for_bka(const key_range *start_key) override;
  ...
  int append_insert_tmp_bka_table(const key_range *start_key,
                                  spider_string *str, char *tmp_table_name,
                                  int tmp_table_name_length, int *db_name_pos);
  int append_union_table_and_sql_for_bka(const key_range *start_key) override;
  int reuse_union_table_and_sql_for_bka() override;
  int append_insert_for_recovery(ulong sql_type, int link_idx) override;
  int append_update(const TABLE *table, my_ptrdiff_t ptr_diff) override;
  int append_update(const TABLE *table, my_ptrdiff_t ptr_diff, int link_idx) override;
  int append_delete(const TABLE *table, my_ptrdiff_t ptr_diff) override;
  int append_delete(const TABLE *table, my_ptrdiff_t ptr_diff, int link_idx) override;
  ...
  int append_dup_update_pushdown_part(const char *alias, uint alias_length) override;
  int append_update_columns_part(const char *alias, uint alias_length) override;
  int check_update_columns_part() override;
  int append_update_columns(spider_string *str, const char *alias,
                            uint alias_length);
#endif
  int append_select_part(ulong sql_type) override;
  int append_select(spider_string *str, ulong sql_type) override;
  int append_table_select_part(ulong sql_type) override;
  int append_table_select(spider_string *str);
  int append_key_select_part(ulong sql_type, uint idx) override;
  int append_key_select(spider_string *str, uint idx);
  int append_minimum_select_part(ulong sql_type) override;
  ...
  int append_select_columns_with_alias(spider_string *str, const char *alias,
                                       uint alias_length);
  int append_hint_after_table_part(ulong sql_type) override;
  int append_hint_after_table(spider_string *str);
  void set_where_pos(ulong sql_type) override;
  void set_where_to_pos(ulong sql_type) override;
  int check_item_type(Item *item) override;
  int append_values_connector_part(ulong sql_type) override;
  int append_values_connector(spider_string *str);
  int append_values_terminator_part(ulong sql_type) override;
  int append_values_terminator(spider_string *str);
  int append_union_table_connector_part(ulong sql_type) override;
  int append_union_table_connector(spider_string *str);
  int append_union_table_terminator_part(ulong sql_type) override;
  int append_union_table_terminator(spider_string *str);
  int append_key_column_values_part(const key_range *start_key, ulong sql_type) override;
  ...
  int append_key_where_part(const key_range *start_key,
                            const key_range *end_key, ulong sql_type) override;
  ...
  int append_condition_part(const char *alias, uint alias_length,
                            ulong sql_type, bool test_flg) override;
  ...
  int append_sum_select(spider_string *str, const char *alias,
                        uint alias_length);
#endif
  void set_order_pos(ulong sql_type) override;
  ...
  int append_key_order_with_alias(spider_string *str, const char *alias,
                                  uint alias_length);
  int append_limit_part(longlong offset, longlong limit, ulong sql_type) override;
  int reappend_limit_part(longlong offset, longlong limit, ulong sql_type) override;
  int append_limit(spider_string *str, longlong offset, longlong limit);
  int append_select_lock_part(ulong sql_type) override;
  int append_select_lock(spider_string *str);
  int append_union_all_start_part(ulong sql_type) override;
  int append_union_all_start(spider_string *str);
  int append_union_all_part(ulong sql_type) override;
  int append_union_all(spider_string *str);
  int append_union_all_end_part(ulong sql_type) override;
  int append_union_all_end(spider_string *str);
  int append_multi_range_cnt_part(ulong sql_type, uint multi_range_cnt,
                                  bool with_comma) override;
  ...
  int append_multi_range_cnt_with_name_part(ulong sql_type,
                                            uint multi_range_cnt) override;
  int append_multi_range_cnt_with_name(spider_string *str,
                                       uint multi_range_cnt);
  int append_open_handler_part(ulong sql_type, uint handler_id,
                               SPIDER_CONN *conn, int link_idx) override;
  int append_open_handler(spider_string *str, uint handler_id,
                          SPIDER_CONN *conn, int link_idx);
  int append_close_handler_part(ulong sql_type, int link_idx) override;
  int append_close_handler(spider_string *str, int link_idx);
  int append_insert_terminator_part(ulong sql_type) override;
  int append_insert_terminator(spider_string *str);
  int append_insert_values_part(ulong sql_type) override;
  int append_insert_values(spider_string *str);
  int append_into_part(ulong sql_type) override;
  int append_into(spider_string *str);
  void set_insert_to_pos(ulong sql_type) override;
  int append_from_part(ulong sql_type, int link_idx) override;
  ...
  int append_delete_all_rows_part(ulong sql_type) override;
  ...
  int append_explain_select_part(key_range *start_key, key_range *end_key,
                                 ulong sql_type, int link_idx) override;
  int append_explain_select(spider_string *str, key_range *start_key,
                            key_range *end_key, ulong sql_type, int link_idx);
  bool is_sole_projection_field(uint16 field_index) override;
  bool is_bulk_insert_exec_period(bool bulk_end) override;
  bool sql_is_filled_up(ulong sql_type) override;
  bool sql_is_empty(ulong sql_type) override;
  bool support_multi_split_read() override;
  bool support_bulk_update() override;
  int bulk_tmp_table_insert() override;
  int bulk_tmp_table_insert(int link_idx) override;
  ...
  bool need_copy_for_update(int link_idx) override;
  ...
  int insert_lock_tables_list(SPIDER_CONN *conn, int link_idx) override;
  int append_lock_tables_list(SPIDER_CONN *conn, int link_idx, int *appended) override;
  int realloc_sql(ulong *realloced) override;
  int reset_sql(ulong sql_type) override;
  bool need_lock_before_set_sql_for_exec(ulong sql_type) override;
#ifdef SPIDER_HAS_GROUP_BY_HANDLER
  int set_sql_for_exec(ulong sql_type, int link_idx,
                       SPIDER_LINK_IDX_CHAIN *link_idx_chain) override;
#endif
  int set_sql_for_exec(ulong sql_type, int link_idx,
                       bool copy = false  // whether copy to exec_**_sql
  ) override;
  int set_sql_for_exec(spider_db_copy_table *tgt_ct, ulong sql_type) override;
  int execute_sql(ulong sql_type, SPIDER_CONN *conn, int quick_mode,
                  int *need_mon) override;
  int reset() override;
  int sts_mode_exchange(int sts_mode) override;
  int show_table_status(int link_idx, int sts_mode, uint flag) override;
  ...
  ha_rows explain_select(key_range *start_key, key_range *end_key,
                         int link_idx) override;
  int lock_tables(int link_idx) override;
  int unlock_tables(int link_idx) override;
  int disable_keys(SPIDER_CONN *conn, int link_idx) override;
  int enable_keys(SPIDER_CONN *conn, int link_idx) override;
  int check_table(SPIDER_CONN *conn, int link_idx, HA_CHECK_OPT *check_opt) override;
  int repair_table(SPIDER_CONN *conn, int link_idx, HA_CHECK_OPT *check_opt) override;
  int analyze_table(SPIDER_CONN *conn, int link_idx) override;
  int optimize_table(SPIDER_CONN *conn, int link_idx) override;
  int flush_tables(SPIDER_CONN *conn, int link_idx, bool lock) override;
  int flush_logs(SPIDER_CONN *conn, int link_idx) override;
  int insert_opened_handler(SPIDER_CONN *conn, int link_idx) override;
  int delete_opened_handler(SPIDER_CONN *conn, int link_idx) override;
  int sync_from_clone_source(spider_db_handler *dbton_hdl) override;
  bool support_use_handler(int use_handler) override;
  void minimum_select_bitmap_create();
  bool minimum_select_bit_is_set(uint field_index) override;
  void copy_minimum_select_bitmap(uchar *bitmap) override;
  int init_union_table_name_pos() override;
  int set_union_table_name_pos() override;
  int reset_union_table_name(spider_string *str, int link_idx, ulong sql_type) override;
#ifdef SPIDER_HAS_GROUP_BY_HANDLER
  int append_from_and_tables_part(spider_fields *fields, ulong sql_type) override;
  int reappend_tables_part(spider_fields *fields, ulong sql_type) override;
  int append_where_part(ulong sql_type) override;
  int append_having_part(ulong sql_type) override;
  ...
  int append_list_item_select_part(List<Item> *select, const char *alias,
                                   uint alias_length, bool use_fields,
                                   spider_fields *fields, ulong sql_type) override;
  int append_list_item_select(List<Item> *select, spider_string *str,
                              const char *alias, uint alias_length,
                              bool use_fields, spider_fields *fields) override;
...
class spider_mysql_copy_table : public spider_db_copy_table {
 public:
  ...
  spider_string sql;
  uint pos;
  spider_mysql_copy_table(spider_mysql_share *db_share);
  ~spider_mysql_copy_table();
  int init() override;
  void set_sql_charset(CHARSET_INFO *cs) override;
  int append_select_str() override;
  int append_insert_str(int insert_flg) override;
  int append_table_columns(TABLE_SHARE *table_share) override;
  int append_from_str() override;
  int append_table_name(int link_idx) override;
  void set_sql_pos() override;
  void set_sql_to_pos() override;
  int append_copy_where(spider_db_copy_table *source_ct, KEY *key_info,
                        ulong *last_row_pos, ulong *last_lengths) override;
  int append_key_order_str(KEY *key_info, int start_pos, bool desc_flg) override;
  int append_limit(longlong offset, longlong limit) override;
  int append_into_str() override;
  int append_open_paren_str() override;
  int append_values_str() override;
  int append_select_lock_str(int lock_mode) override;
  int exec_query(SPIDER_CONN *conn, int quick_mode, int *need_mon) override;
  ...
  int copy_rows(TABLE *table, SPIDER_DB_ROW *row, ulong **last_row_pos,
                ulong **last_lengths) override;
  int copy_rows(TABLE *table, SPIDER_DB_ROW *row) override;
  int append_insert_terminator() override;
  int copy_insert_values(spider_db_copy_table *source_ct) override;
};
