...
ulonglong spider_mon_table_cache_version = 0;
ulonglong spider_mon_table_cache_version_req = 1;

SPIDER_TABLE_MON_LIST *spider_get_ping_table_mon_list(
    SPIDER_TRX *trx __attribute__((unused)), THD *thd, spider_string *str, uint conv_name_length,
  ...
  if (spider_mon_table_cache_version != spider_mon_table_cache_version_req) {
    ...
    if ((*error_num =
             spider_init_ping_table_mon_cache(thd, &mem_root, need_lock))) {
...
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
  hash_value = my_calc_hash(&spider_udf_table_mon_list_hash[mutex_hash],
...
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
  if (!(table_mon_list =
            (SPIDER_TABLE_MON_LIST *)my_hash_search_using_hash_value(
  ...
  if (!(table_mon_list = (SPIDER_TABLE_MON_LIST *)my_hash_search(
            &spider_udf_table_mon_list_hash[mutex_hash], (uchar *)str->c_ptr(),
            str->length())) ||
      table_mon_list->mon_table_cache_version != mon_table_cache_version)
#endif
  {
    ...
    if (!(table_mon_list = spider_get_ping_table_tgt(
              thd, str->c_ptr(), conv_name_length, link_idx, static_link_id,
              static_link_id_length, server_id, str, need_lock, error_num))) {
    ...
    if (spider_udf_table_mon_list_hash[mutex_hash].array.max_element >
      ...
      spider_alloc_calc_mem(
          spider_current_trx, spider_udf_table_mon_list_hash,
          (spider_udf_table_mon_list_hash[mutex_hash].array.max_element -
           old_elements) *
...
void spider_free_ping_table_mon_list(SPIDER_TABLE_MON_LIST *table_mon_list) {
  ...
  if (!table_mon_list->use_count)
    pthread_cond_broadcast(
        &spider_udf_table_mon_conds[table_mon_list->mutex_hash]);
  pthread_mutex_unlock(
      &spider_udf_table_mon_mutexes[table_mon_list->mutex_hash]);
  DBUG_VOID_RETURN;
}
...
int spider_release_ping_table_mon_list(const char *conv_name,
  ...
  link_idx_str_length =
      my_sprintf(link_idx_str, (link_idx_str, "%010d", link_idx));
...
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
  my_hash_value_type hash_value =
      my_calc_hash(&spider_udf_table_mon_list_hash[mutex_hash],
...
int spider_get_ping_table_mon(THD *thd, SPIDER_TABLE_MON_LIST *table_mon_list,
  ...
  if (!(table_link_mon = spider_open_sys_table(
  ...
  if (table_mon_list->share->static_link_ids[0]) {
    ...
    spider_store_tables_link_idx_str(
        table_link_mon, table_mon_list->share->static_link_ids[0],
        table_mon_list->share->static_link_ids_lengths[0]);
    if (!(error_num =
              spider_ping_table_cache_compare(table_link_mon, mem_root)))
      goto create_table_mon;
    if (error_num == HA_ERR_OUT_OF_MEM) goto error;
...
create_table_mon:
  if ((error_num = spider_get_sys_table_by_idx(
           table_link_mon, table_key, table_link_mon->s->primary_key, 3))) {
    table_link_mon->file->print_error(error_num, MYF(0));
    goto error;
  }

  do {
    if (!(table_mon = (SPIDER_TABLE_MON *)spider_bulk_malloc(
              spider_current_trx, 35, MYF(MY_WME | MY_ZEROFILL), &table_mon,
              sizeof(SPIDER_TABLE_MON), &tmp_share, sizeof(SPIDER_SHARE),
              &tmp_connect_info,
              sizeof(char *) * SPIDER_TMP_SHARE_CHAR_PTR_COUNT,
              &tmp_connect_info_length,
              sizeof(uint) * SPIDER_TMP_SHARE_UINT_COUNT, &tmp_long,
              sizeof(long) * SPIDER_TMP_SHARE_LONG_COUNT, &tmp_longlong,
              sizeof(longlong) * SPIDER_TMP_SHARE_LONGLONG_COUNT, NullS))) {
    ...
    spider_set_tmp_share_pointer(tmp_share, tmp_connect_info,
                                 tmp_connect_info_length, tmp_long,
                                 tmp_longlong);
    ...
    if ((error_num = spider_get_sys_link_mon_server_id(
...
#endif
                                                     NULL)) ||
        (error_num = spider_set_connect_info_default_dbtable(tmp_share, name,
                                                             name_length)) ||
    ...
    DBUG_PRINT("info",
               ("spider table_mon->server_id=%u", table_mon->server_id));
    DBUG_PRINT("info", ("spider server_id=%u", server_id));
    if (table_mon->server_id == server_id) table_mon_list->current = table_mon;
    list_size++;
...
SPIDER_TABLE_MON_LIST *spider_get_ping_table_tgt(
  ...
  SPD_INIT_ALLOC_ROOT(&mem_root, 4096, 0, MYF(MY_WME));
  if (!(table_mon_list = (SPIDER_TABLE_MON_LIST *)spider_bulk_malloc(
            spider_current_trx, 36, MYF(MY_WME | MY_ZEROFILL), &table_mon_list,
            sizeof(SPIDER_TABLE_MON_LIST), &tmp_share, sizeof(SPIDER_SHARE),
            &tmp_connect_info, sizeof(char *) * SPIDER_TMP_SHARE_CHAR_PTR_COUNT,
            &tmp_connect_info_length,
            sizeof(uint) * SPIDER_TMP_SHARE_UINT_COUNT, &tmp_long,
            sizeof(long) * SPIDER_TMP_SHARE_LONG_COUNT, &tmp_longlong,
            sizeof(longlong) * SPIDER_TMP_SHARE_LONGLONG_COUNT, &key_str,
            str->length() + 1, NullS))) {
    my_error(HA_ERR_OUT_OF_MEM, MYF(0));
    goto error;
  }
  ...
  tmp_share->access_charset = const_cast<CHARSET_INFO*>(thd->variables.character_set_client);
  ...
  if (static_link_id) {
    spider_store_tables_static_link_id(table_tables, static_link_id,
                                       static_link_id_length);
    if ((*error_num = spider_get_sys_table_by_idx(
             table_tables, table_key, 2, SPIDER_SYS_TABLES_UIDX1_COL_CNT)) ||
        (*error_num = spider_get_sys_tables_link_idx(table_tables, &link_idx,
                                                     &mem_root))) {
    ...
    if ((*error_num = spider_check_sys_table(table_tables, table_key))) {
      table_tables->file->print_error(*error_num, MYF(0));
      goto error;
    }
  }
  if ((*error_num = spider_get_sys_tables_connect_info(table_tables, tmp_share,
                                                       0, &mem_root)) ||
      (*error_num = spider_get_sys_tables_link_status(table_tables, tmp_share,
                                                      0, &mem_root))) {
    table_tables->file->print_error(*error_num, MYF(0));
    goto error;
  }
  ...
  if ((*error_num = spider_set_connect_info_default(tmp_share,
      ...
      */
      (*error_num = spider_get_ping_table_mon(thd, table_mon_list, name,
                                              name_length, link_idx, server_id,
                                              &mem_root, need_lock)))
    goto error;
  ...
  if (mysql_mutex_init(spd_key_mutex_mon_list_update_status,
                       &table_mon_list->update_status_mutex,
                       MY_MUTEX_INIT_FAST))
...
SPIDER_CONN *spider_get_ping_table_tgt_conn(SPIDER_TRX *trx,
  ...
  if (!(conn = spider_get_conn(share, 0, trx, NULL, FALSE, FALSE,
...
int spider_get_ping_table_gtid_pos(SPIDER_TRX *trx, THD *thd,
  ...
  */
  ...
  if (!(table_gtid_pos = spider_open_sys_table(
  ...
  if ((error_num = spider_get_sys_table_by_idx(
           table_tables, table_key, 0, SPIDER_SYS_TABLES_PK_COL_CNT - 1))) {
  ...
  SPD_INIT_ALLOC_ROOT(&mem_root, 4096, 0, MYF(MY_WME));
  do {
    if ((error_num = spider_get_sys_tables_link_status(
             table_tables, &link_status, &mem_root)) ||
        (error_num = spider_get_sys_tables_static_link_id(
             table_tables, &static_link_id, &static_link_id_length,
             &mem_root)) ||
    ...
    if (link_status == 1 && monitoring_binlog_pos_at_failing > 0) {
      if ((error_num = spider_get_sys_tables_link_idx(
               table_tables, &source_link_idx, &mem_root))) {
        goto error_get_sys_tables_link_idx;
      }
      if ((table_mon_list = spider_get_ping_table_mon_list(
               trx, thd, str, conv_name_length, source_link_idx, static_link_id,
               static_link_id_length, server_id, need_lock, &error_num))) {
        SPIDER_DB_RESULT *res1 = NULL;
        SPIDER_DB_RESULT *res2 = NULL;
        if ((ping_conn = spider_get_ping_table_tgt_conn(
                 trx, table_mon_list->share, &error_num)) &&
            !(error_num = ping_conn->db_conn->show_master_status(
                  trx, table_mon_list->share, 0, &need_mon, table_gtid_pos,
                  tmp_str, monitoring_binlog_pos_at_failing == 1 ? 0 : 1, &res1,
                  &res2))) {
          spider_store_binlog_pos_source_link_idx(table_gtid_pos,
                                                  source_link_idx);
  ...
  if (spider_mon_table_cache_version != spider_mon_table_cache_version_req) {
    ...
    if (!error_num) {
      ...
      do {
        ...
        if (!same) {
          mon_key.sort = spider_calc_for_sort(
              3, mon_key.db_name, mon_key.table_name, mon_key.link_id);
          if (insert_dynamic(&spider_mon_table_cache, (uchar *)&mon_key)) {
            error_num = HA_ERR_OUT_OF_MEM;
...
int spider_ping_table_cache_compare(TABLE *table, MEM_ROOT *mem_root) {
  ...
  for (roop_count = 0; roop_count < spider_mon_table_cache.elements;
       roop_count++) {
    mon_key =
        dynamic_element(&spider_mon_table_cache, roop_count, SPIDER_MON_KEY *);
    ...
    if (!wild_case_compare(system_charset_info, db_name, mon_key->db_name) &&
      ...
      spider_store_db_and_table_name(
          table, mon_key->db_name, mon_key->db_name_length, mon_key->table_name,
          mon_key->table_name_length);
      spider_store_tables_link_idx_str(table, mon_key->link_id,
...
long long spider_ping_table_body(UDF_INIT *initid, UDF_ARGS *args,
                                 char *is_null __attribute__((unused)), char *error) {
  ...
  if (thd->open_tables != 0 || !thd->handler_tables_hash.empty() ||
      /* thd->derived_tables != 0 || */thd->lock != 0 ||
    ...
    } else if (!thd->handler_tables_hash.empty()) {
      my_printf_error(ER_SPIDER_UDF_CANT_USE_IF_OPEN_TABLE_NUM,
                      ER_SPIDER_UDF_CANT_USE_IF_OPEN_TABLE_STR_WITH_NUM, MYF(0),
                      "thd->handler_tables_hash.records",
                      (longlong)thd->handler_tables_hash.size());
    /*} else if (thd->derived_tables != 0) {
      ...
      my_printf_error(ER_SPIDER_UDF_CANT_USE_IF_OPEN_TABLE_NUM,
                      ...
                      (longlong)thd->locked_tables_mode);

    }
    goto error;
  }

  if (args->lengths[0] > SPIDER_CONNECT_INFO_MAX_LEN) {
  ...
  if (args->arg_type[1] == STRING_RESULT) {
    ...
    if (args->lengths[1] > SPIDER_CONNECT_INFO_MAX_LEN) {
      my_printf_error(ER_SPIDER_UDF_PARAM_TOO_LONG_NUM,
  ...
  where_clause = args->args[4] ? args->args[4] : const_cast<char *>("");
  ...
  if (!(table_mon_list = spider_get_ping_table_mon_list(
            trx, trx->thd, &conv_name, conv_name_length, link_idx,
            static_link_id, static_link_id_length, server_id, TRUE,
            &error_num)))
    goto error;
  ...
  if (args->args[5]) tmp_sid = *((longlong *)args->args[5]);
  ...
  if (table_mon_list->mon_status != SPIDER_LINK_MON_NG &&
      !(ping_conn = spider_get_ping_table_tgt_conn(trx, table_mon_list->share,
                                                   &error_num))) {
  ...
  if (table_mon_list->mon_status == SPIDER_LINK_MON_NG || error_num ||
      (tmp_error_num = spider_db_udf_ping_table(
           table_mon_list, table_mon_list->share, trx, ping_conn, where_clause,
           args->lengths[4], (flags & SPIDER_UDF_PING_TABLE_PING_ONLY),
           (flags & SPIDER_UDF_PING_TABLE_USE_WHERE), limit))) {
    DBUG_PRINT(
        "info",
        ...
        if (table_mon_list->mon_status != SPIDER_LINK_MON_NG) {
          ...
          */
          ...
          if (table_mon_list->mon_status != SPIDER_LINK_MON_NG) {
            ...
            spider_sys_update_tables_link_status(trx->thd, conv_name.c_ptr(),
                                                 conv_name_length, link_idx,
                                                 SPIDER_LINK_STATUS_NG, TRUE);
            spider_sys_log_tables_link_failed(trx->thd, conv_name.c_ptr(),
                                              conv_name_length, link_idx, TRUE);
          ...
          */
          ...
          if (status_changed_to_ng) {
            ...
            spider_get_ping_table_gtid_pos(trx, trx->thd, &conv_name,
                                           conv_name_length, link_idx,
                                           server_id, TRUE, &tmp_str);
            if (!is_error && trx->thd->is_error()) trx->thd->clear_error();
          }
        }
        goto end;
      }
  ...
  if (tmp_sid >= 0 || get_lock) {
    table_mon = table_mon_list->current->next;
    while (TRUE) {
      if (!table_mon) table_mon = table_mon_list->first;
      if (table_mon->server_id == first_sid ||
        ...
        if ((flags & SPIDER_UDF_PING_TABLE_USE_ALL_MONITORING_NODES) &&
          ...
          DBUG_PRINT(
              "info",
              ("spider mon_table_result->result_status=SPIDER_LINK_MON_DRAW "
               "1"));
        } else {
          mon_table_result->result_status = SPIDER_LINK_MON_DRAW_FEW_MON;
          DBUG_PRINT("info", ("spider "
                              "mon_table_result->result_status=SPIDER_LINK_MON_"
                              "DRAW_FEW_MON 1"));
        }
      ...
      if ((mon_conn = spider_get_ping_table_tgt_conn(trx, table_mon->share,
        ...
        if (!spider_db_udf_ping_table_mon_next(
                ...
                args->lengths[0], link_idx, where_clause, args->lengths[4],
                first_sid, full_mon_count, current_mon_count, success_count,
                fault_count, flags, limit)) {
    ...
    if (get_lock) pthread_mutex_unlock(&table_mon_list->receptor_mutex);
  } else {
...
bool spider_ping_table_init_body(UDF_INIT *initid, UDF_ARGS *args,
                                    char *message) {
  int error_num;
  THD *thd = current_thd;
  SPIDER_TRX *trx;
  ...
  if (args->arg_type[0] != STRING_RESULT ||
    ...
    strcpy(message,
           "spider_ping_table() requires string 1st "
           "and 5th arguments");
    goto error;
  }
  if (args->arg_type[2] != INT_RESULT || args->arg_type[3] != INT_RESULT ||
      args->arg_type[5] != INT_RESULT || args->arg_type[6] != INT_RESULT ||
      args->arg_type[7] != INT_RESULT || args->arg_type[8] != INT_RESULT ||
      args->arg_type[9] != INT_RESULT) {
    strcpy(message,
           "spider_ping_table() requires integer 3rd, 4,6,7,8,"
           "9th and 10th argument");
    goto error;
  }
  if (args->arg_type[1] != INT_RESULT && args->arg_type[1] != STRING_RESULT) {
    strcpy(message,
           "spider_ping_table() requires string or integer for "
           "2nd argument");
    goto error;
  }
  ...
  if (!(mon_table_result = (SPIDER_MON_TABLE_RESULT *)spider_malloc(
            spider_current_trx, 11, sizeof(SPIDER_MON_TABLE_RESULT),
...
int spider_ping_table_mon_from_table(
    SPIDER_TRX *trx, THD *thd, SPIDER_SHARE *share, int base_link_idx __attribute__((unused)),
  ...
  */
  ...
  if (table_share->tmp_table != NO_TMP_TABLE) {
    my_printf_error(ER_SPIDER_TMP_TABLE_MON_NUM, ER_SPIDER_TMP_TABLE_MON_STR,
                    MYF(0));
  ...
  if (!(table_mon_list = spider_get_ping_table_mon_list(
            trx, thd, &conv_name_str, conv_name_length, link_idx,
            share->static_link_ids[link_idx],
            share->static_link_ids_lengths[link_idx], server_id, need_lock,
            &error_num))) {
    my_afree(buf);
    goto end;
  }
  ...
  if (table_mon_list->mon_status == SPIDER_LINK_MON_NG) {
    ...
    my_printf_error(error_num, ER_SPIDER_LINK_MON_NG_STR, MYF(0),
  ...
  if (!pthread_mutex_trylock(&table_mon_list->caller_mutex)) {
    ...
    while (TRUE) {
      ...
      if ((thd && thd->killed) || *spd_abort_loop) {
        ...
        if (!table_mon) table_mon = table_mon_list->first;
        if (current_mon_count > table_mon_list->list_size ||
          ...
          my_printf_error(error_num, ER_SPIDER_LINK_MON_DRAW_FEW_MON_STR,
                          MYF(0), table_mon_list->share->tgt_dbs[0],
                          table_mon_list->share->tgt_table_names[0]);
          break;
        }
        int prev_error = 0;
        ...
        if ((mon_conn = spider_get_ping_table_tgt_conn(trx, table_mon->share,
          ...
          if (!spider_db_udf_ping_table_mon_next(
                  ...
                  conv_name_length, link_idx, where_clause, where_clause_length,
                  -1, table_mon_list->list_size, 0, 0, 0, flags,
                  monitoring_limit)) {
            if (mon_table_result.result_status == SPIDER_LINK_MON_NG &&
                table_mon_list->mon_status != SPIDER_LINK_MON_NG) {
              /*
              ...
              */
              ...
              if (table_mon_list->mon_status != SPIDER_LINK_MON_NG) {
                ...
                spider_sys_update_tables_link_status(
                    thd, conv_name, conv_name_length, link_idx,
                    SPIDER_LINK_STATUS_NG, need_lock);
                spider_sys_log_tables_link_failed(
                    thd, conv_name, conv_name_length, link_idx, need_lock);
              }
              /*
                            pthread_mutex_unlock(&table_mon_list->update_status_mutex);
              */
            ...
            if (mon_table_result.result_status == SPIDER_LINK_MON_OK) {
              if (prev_error) my_message(prev_error, prev_error_msg, MYF(0));
              error_num = ER_SPIDER_LINK_MON_OK_NUM;
              my_printf_error(error_num, ER_SPIDER_LINK_MON_OK_STR, MYF(0),
                              table_mon_list->share->tgt_dbs[0],
                              table_mon_list->share->tgt_table_names[0]);
              break;
            }
            ...
            my_printf_error(error_num, ER_SPIDER_LINK_MON_DRAW_STR, MYF(0),
                            table_mon_list->share->tgt_dbs[0],
                            table_mon_list->share->tgt_table_names[0]);
            break;
          }
