...
// #include "sql_priv.h"
#include "sql/event_scheduler.h"  // EVENT_SCHEDULER_USER
#include "sql/query_options.h"
//#include "probes_mysql.h"
#include "sql/sql_class.h"
#include "sql/sql_partition.h"
//#include "sql/sql_analyse.h"
...
#include "sql/tdsql/xa/xa_tm.h"
#include "sql/tdsql/xa/mc_gts.h"
#include "spd_param.h"
...
#include "sql/system_variables.h" // remote_queries_executed

extern uint32_t tdsql::common::tdsql_mode;
const uint SIZEOF_STORED_DOUBLE = 8;
extern struct CHARSET_INFO *spd_charset_utf8_bin;
extern std::atomic<int32> volatile *spd_abort_loop;
...
extern bool trx_need_end_gts();
extern bool tdsql::common::tdsql_mtr_mode;
extern bool tdsql::common::tdsql_log_sql_open_flag;
extern bool tdsql::common::tdsql_log_nonquery_commands_flag;
extern bool tdsql::common::tdsql_log_interface_open_flag;

namespace tdsql{
namespace common {
extern bool tdsql_pass_client_ip;
}  // namespace common
}  // namespace
...
#define SPIDER_SQL_ISO_READ_UNCOMMITTED_STR \
  "set session transaction isolation level read uncommitted"
#define SPIDER_SQL_ISO_READ_UNCOMMITTED_LEN \
  sizeof(SPIDER_SQL_ISO_READ_UNCOMMITTED_STR) - 1
#define SPIDER_SQL_ISO_READ_COMMITTED_STR \
  "set session transaction isolation level read committed"
#define SPIDER_SQL_ISO_READ_COMMITTED_LEN \
  sizeof(SPIDER_SQL_ISO_READ_COMMITTED_STR) - 1
#define SPIDER_SQL_ISO_REPEATABLE_READ_STR \
  "set session transaction isolation level repeatable read"
#define SPIDER_SQL_ISO_REPEATABLE_READ_LEN \
  sizeof(SPIDER_SQL_ISO_REPEATABLE_READ_STR) - 1
#define SPIDER_SQL_ISO_SERIALIZABLE_STR \
  "set session transaction isolation level serializable"
#define SPIDER_SQL_ISO_SERIALIZABLE_LEN \
  sizeof(SPIDER_SQL_ISO_SERIALIZABLE_STR) - 1

#define SPIDER_SQL_START_CONSISTENT_SNAPSHOT_STR \
  "start transaction with consistent snapshot"
#define SPIDER_SQL_START_CONSISTENT_SNAPSHOT_LEN \
  sizeof(SPIDER_SQL_START_CONSISTENT_SNAPSHOT_STR) - 1
#define SPIDER_SQL_START_TRANSACTION_STR "start transaction"
#define SPIDER_SQL_START_TRANSACTION_LEN \
  sizeof(SPIDER_SQL_START_TRANSACTION_STR) - 1
...
#define SPIDER_SQL_TMYSQL32_WITH_LOG_HINT "/*!99302 with logs */"
#define SPIDER_SQL_TMYSQL32_WITH_LOG_HINT_LEN \
  sizeof(SPIDER_SQL_TMYSQL32_WITH_LOG_HINT) - 1
...
#define SPIDER_SQL_TDSQL_WITHGTS_STR " tdsql_withgts "
#define SPIDER_SQL_TDSQL_WITHGTS_LEN (sizeof(SPIDER_SQL_TDSQL_WITHGTS_STR) - 1)

#define SPIDER_SQL_SHOW_TABLE_STATUS_STR "show table status from "
#define SPIDER_SQL_SHOW_TABLE_STATUS_LEN \
  sizeof(SPIDER_SQL_SHOW_TABLE_STATUS_STR) - 1
#define SPIDER_SQL_SELECT_TABLES_STATUS_STR                                 \
  "select "                                                                 \
  "`table_rows`,`avg_row_length`,`data_length`,`max_data_length`,`index_"   \
  "length`,`auto_increment`,`create_time`,`update_time`,`check_time` from " \
  "`information_schema`.`tables` where `table_schema` = "
#define SPIDER_SQL_SELECT_TABLES_STATUS_LEN \
  sizeof(SPIDER_SQL_SELECT_TABLES_STATUS_STR) - 1
...
#define SPIDER_SQL_SHOW_MASTER_STATUS_LEN \
  sizeof(SPIDER_SQL_SHOW_MASTER_STATUS_STR) - 1
#define SPIDER_SQL_BINLOG_GTID_POS_STR "select binlog_gtid_pos"
#define SPIDER_SQL_BINLOG_GTID_POS_LEN \
  sizeof(SPIDER_SQL_BINLOG_GTID_POS_STR) - 1
...
#define SPIDER_SQL_SELECT_COLUMNS_STR                                          \
  "select "                                                                    \
  "`column_name`,`column_default`,`is_nullable`,`character_set_name`,`"        \
  "collation_name`,`column_type`,`extra` from `information_schema`.`columns` " \
  "where `table_schema` = "
...
#define SPIDER_SQL_ORDINAL_POSITION_LEN \
  sizeof(SPIDER_SQL_ORDINAL_POSITION_STR) - 1
...
static uchar SPIDER_SQL_LINESTRING_HEAD_STR[] = {0x00, 0x00, 0x00, 0x00, 0x01,
...
static const char *spider_db_table_lock_str[] = {
    " read local,", " read,", " low_priority write,", " write,"};
static const int spider_db_table_lock_len[] = {
    sizeof(" read local,") - 1, sizeof(" read,") - 1,
    sizeof(" low_priority write,") - 1, sizeof(" write,") - 1};
static const char *spider_db_timefunc_interval_str[] = {" year",
                                                        " quarter",
                                                        " month",
                                                        " week",
                                                        " day",
...
void log_spider_sql_tdsql(THD *thd, spider_db_mysql *db_conn, const char *query,
                          uint length, my_hrtime_t &start, bool has_error);

void log_spider_mysql_sql(MYSQL *m, const char *query, uint length,
                          int command);
...
MYSQL *SPIDER_CONN_get_mysql(SPIDER_CONN *spider_conn) {
  return down_cast<spider_db_mysql *>(spider_conn->db_conn)->db_conn;
}

spider_db_share *spider_mysql_create_share(SPIDER_SHARE *share) {
  DBUG_ENTER("spider_mysql_create_share");
  DBUG_RETURN(new spider_mysql_share(share));
}

spider_db_handler *spider_mysql_create_handler(ha_spider *spider,
                                               spider_db_share *db_share) {
  DBUG_ENTER("spider_mysql_create_handler");
  DBUG_RETURN(new spider_mysql_handler(spider, (spider_mysql_share *)db_share));
}

spider_db_copy_table *spider_mysql_create_copy_table(
    spider_db_share *db_share) {
  DBUG_ENTER("spider_mysql_create_copy_table");
  DBUG_RETURN(new spider_mysql_copy_table((spider_mysql_share *)db_share));
}

SPIDER_DB_CONN *spider_mysql_create_conn(SPIDER_CONN *conn) {
...
SPIDER_DBTON spider_dbton_mysql = {0,
...
int spider_db_mysql_row::store_to_field(ha_spider *spider, Field *field,
  ...
  if (!*row) {
    ...
    if (field->all_flags() & BLOB_FLAG) {
      ...
      if (field->charset() == &my_charset_bin ||
        ...
        /* remote db returns json strings, store it in binary format*/
        if (field->type() == MYSQL_TYPE_JSON)
          ((Field_json *)field)->store(*row, *lengths, access_charset);
        else
          ((Field_blob *)field)->set_ptr(*lengths, (uchar *)*row);
      } else {
        DBUG_PRINT("info", ("spider blob convert"));
        if (field->table->file->ht == spider_hton_ptr) {
          if (spider == nullptr) spider = (ha_spider *)field->table->file;
          // not a partition handler
          uint numparts = 0;
          spider->get_no_parts("", &numparts);
          assert(numparts == 0);
          ...
          /* remote db returns json strings, store it in binary format*/
          if (field->type() == MYSQL_TYPE_JSON) 
            ((Field_json *)field)->store(str->ptr(), str->length(), str->charset());
          else
            ((Field_blob *)field)->set_ptr(str->length(), (uchar *)(const_cast<char*>(str->ptr())));
        } else {
...
int spider_db_mysql_row::append_to_str(spider_string *str) {
  DBUG_ENTER("spider_db_mysql_row::append_to_str");
  DBUG_PRINT("info", ("spider this=%p", this));
  if (str->reserve(*lengths)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  str->q_append(*row, *lengths);
  DBUG_RETURN(0);
}

int spider_db_mysql_row::append_escaped_to_str(spider_string *str,
                                               uint dbton_id) {
  ...
  if (str->reserve(*lengths * 2 + 2)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
...
inline int decimal_operation_results(int result, const char *value[[maybe_unused]],
                                     const char *type[[maybe_unused]])
{
  return result;
}

my_decimal *spider_db_mysql_row::val_decimal(my_decimal *decimal_value,
                                             CHARSET_INFO *access_charset) {
  DBUG_ENTER("spider_db_mysql_row::val_decimal");
  DBUG_PRINT("info", ("spider this=%p", this));
  if (!*row) DBUG_RETURN(NULL);

  decimal_operation_results(
      str2my_decimal(0, *row, *lengths, access_charset, decimal_value), "", "");
...
SPIDER_DB_ROW *spider_db_mysql_row::clone() {
  ...
  if (!spider_bulk_malloc(spider_current_trx, 29, MYF(MY_WME), &clone_row->row,
                          sizeof(char *) * field_count, &tmp_char, row_size,
...
int spider_db_mysql_row::store_to_tmp_table(TABLE *tmp_table,
                                            spider_string *str) {
  uint i;
  ...
  tmp_table->field[0]->store((const char *)lengths, sizeof(ulong) * field_count,
                             &my_charset_bin);
  tmp_table->field[1]->set_notnull();
  tmp_table->field[1]->store(str->ptr(), str->length(), &my_charset_bin);
  tmp_table->field[2]->set_notnull();
  tmp_table->field[2]->store((char *)row, (uint)(sizeof(char *) * field_count),
                             &my_charset_bin);
...
SPIDER_DB_ROW *spider_db_mysql_result::fetch_row_from_result_buffer(
    spider_db_result_buffer *spider_res_buf[[maybe_unused]]) {
...
SPIDER_DB_ROW *spider_db_mysql_result::fetch_row_from_tmp_table(
  ...
  tmp_lengths = (ulong *)(const_cast<char*>(tmp_str1.ptr()));
  tmp_row = (MYSQL_ROW)(const_cast<char*>(tmp_str3.ptr()));
  ...
  for (i = 0; i < field_count; i++) {
    if (*tmp_row) {
      *tmp_row = const_cast<char *>(row_ptr);
...
int spider_db_mysql_result::fetch_table_status(
    int mode, ha_rows &records, ulong &mean_rec_length,
    ulonglong &data_file_length, ulonglong &max_data_file_length,
    ulonglong &index_file_length, ulonglong &auto_increment_value,
    time_t &create_time, time_t &update_time, time_t &check_time) {
  int error_num;
  MYSQL_ROW mysql_row;
...
#define SPIDER_HAS_TIME_STATUS
  ...
  if (!(mysql_row = mysql_fetch_row(db_result))) {
    ...
    if ((error_num = mysql_errno(((spider_db_mysql *)db_conn)->db_conn))) {
      my_message(error_num, mysql_error(((spider_db_mysql *)db_conn)->db_conn),
                 MYF(0));
  ...
  if (mode == 1) {
    ...
    if (mysql_row[4])
      records = (ha_rows)my_strtoll10(mysql_row[4], (const char **)NULL, &error_num);
    else
      records = (ha_rows)0;
    DBUG_PRINT("info", ("spider records=%lld", records));
    if (mysql_row[5])
      mean_rec_length =
          (ulong)my_strtoll10(mysql_row[5], (const char **)NULL, &error_num);
    else
      mean_rec_length = 0;
    DBUG_PRINT("info", ("spider mean_rec_length=%lu", mean_rec_length));
    if (mysql_row[6])
      data_file_length =
          (ulonglong)my_strtoll10(mysql_row[6], (const char **)NULL, &error_num);
    else
      data_file_length = 0;
    DBUG_PRINT("info", ("spider data_file_length=%lld", data_file_length));
    if (mysql_row[7])
      max_data_file_length =
          (ulonglong)my_strtoll10(mysql_row[7], (const char **)NULL, &error_num);
    else
    ...
    if (mysql_row[8])
      index_file_length =
          (ulonglong)my_strtoll10(mysql_row[8], (const char **)NULL, &error_num);
    else
      index_file_length = 0;
    DBUG_PRINT("info", ("spider index_file_length=%lld", index_file_length));
    if (mysql_row[10])
      auto_increment_value =
          (ulonglong)my_strtoll10(mysql_row[10], (const char **)NULL, &error_num);
    else
    ...
    if (mysql_row[11]) {
      ...
      str_to_datetime(mysql_row[11], strlen(mysql_row[11]), &mysql_time, 0,
                      &time_status);
#ifdef MARIADB_BASE_VERSION
      create_time = (time_t)my_system_gmt_sec(mysql_time, &not_used_long,
                                              &not_used_uint);
#else
      create_time = (time_t)my_system_gmt_sec(mysql_time, &not_used_long,
                                              &not_used_my_bool);
    ...
    if (mysql_row[12]) {
      ...
      str_to_datetime(mysql_row[12], strlen(mysql_row[12]), &mysql_time, 0,
                      &time_status);
#ifdef MARIADB_BASE_VERSION
      update_time = (time_t)my_system_gmt_sec(mysql_time, &not_used_long,
                                              &not_used_uint);
#else
      update_time = (time_t)my_system_gmt_sec(mysql_time, &not_used_long,
                                              &not_used_my_bool);
    ...
    if (mysql_row[13]) {
      ...
      str_to_datetime(mysql_row[13], strlen(mysql_row[13]), &mysql_time, 0,
                      &time_status);
#ifdef MARIADB_BASE_VERSION
      check_time = (time_t)my_system_gmt_sec(mysql_time, &not_used_long,
                                             &not_used_uint);
#else
      check_time = (time_t)my_system_gmt_sec(mysql_time, &not_used_long,
                                             &not_used_my_bool);
    ...
    if (mysql_row[0])
      records = (ha_rows)my_strtoll10(mysql_row[0], (const char **)NULL, &error_num);
    else
      records = (ha_rows)0;
    DBUG_PRINT("info", ("spider records=%lld", records));
    if (mysql_row[1])
      mean_rec_length =
          (ulong)my_strtoll10(mysql_row[1], (const char **)NULL, &error_num);
    else
      mean_rec_length = 0;
    DBUG_PRINT("info", ("spider mean_rec_length=%lu", mean_rec_length));
    if (mysql_row[2])
      data_file_length =
          (ulonglong)my_strtoll10(mysql_row[2], (const char **)NULL, &error_num);
    else
      data_file_length = 0;
    DBUG_PRINT("info", ("spider data_file_length=%lld", data_file_length));
    if (mysql_row[3])
      max_data_file_length =
          (ulonglong)my_strtoll10(mysql_row[3], (const char **)NULL, &error_num);
    else
      max_data_file_length = 0;
    DBUG_PRINT("info",
               ("spider max_data_file_length=%lld", max_data_file_length));
    if (mysql_row[4])
      index_file_length =
          (ulonglong)my_strtoll10(mysql_row[4], (const char **)NULL, &error_num);
    else
      index_file_length = 0;
    DBUG_PRINT("info", ("spider index_file_length=%lld", index_file_length));
    if (mysql_row[5])
      auto_increment_value =
          (ulonglong)my_strtoll10(mysql_row[5], (const char **)NULL, &error_num);
    else
      auto_increment_value = 1;
    DBUG_PRINT("info",
               ("spider auto_increment_value=%lld", auto_increment_value));
...
int spider_db_mysql_result::fetch_table_records(int mode, ha_rows &records) {
  int error_num;
  MYSQL_ROW mysql_row;
  ...
  if (mode == 1) {
    ...
    if (mysql_row[8]) {
      records = (ha_rows)my_strtoll10(mysql_row[8], (const char **)NULL, &error_num);
    } else
      records = 0;
  }
  DBUG_RETURN(0);
...
int spider_db_mysql_result::fetch_table_cardinality(int mode, TABLE *table,
  ...
  if (mode == 1) {
    ...
    while (mysql_row) {
      if (mysql_row[4] && mysql_row[6] &&
          (field = find_field_in_table_sef(table, mysql_row[4]))) {
        if ((cardinality[field->field_index()] = (longlong)my_strtoll10(
                 mysql_row[6], (const char **)NULL, &error_num)) <= 0)
          cardinality[field->field_index()] = 1;
        spider_set_bit(cardinality_upd, field->field_index());
        DBUG_PRINT("info", ("spider col_name=%s", mysql_row[4]));
        DBUG_PRINT("info", ("spider cardinality=%lld",
                            cardinality[field->field_index()]));
      } else if (mysql_row[4]) {
        DBUG_PRINT("info", ("spider skip col_name=%s", mysql_row[4]));
      } else {
        DBUG_RETURN(ER_SPIDER_INVALID_REMOTE_TABLE_INFO_NUM);
      }
    ...
    while (mysql_row) {
      if (mysql_row[0] && mysql_row[1] &&
          (field = find_field_in_table_sef(table, mysql_row[0]))) {
        if ((cardinality[field->field_index()] = (longlong)my_strtoll10(
                 mysql_row[1], (const char **)NULL, &error_num)) <= 0)
          cardinality[field->field_index()] = 1;
        spider_set_bit(cardinality_upd, field->field_index());
        DBUG_PRINT("info", ("spider col_name=%s", mysql_row[0]));
        DBUG_PRINT("info", ("spider cardinality=%lld",
                            cardinality[field->field_index()]));
      } else if (mysql_row[0]) {
        DBUG_PRINT("info", ("spider skip col_name=%s", mysql_row[0]));
      } else {
        DBUG_RETURN(ER_SPIDER_INVALID_REMOTE_TABLE_INFO_NUM);
      }
      mysql_row = mysql_fetch_row(db_result);
...
int spider_db_mysql_result::fetch_table_mon_status(int &status) {
  int error_num;
  MYSQL_ROW mysql_row;
...
int spider_db_mysql_result::fetch_show_master_status(
    const char **binlog_file_name, const char **binlog_pos) {
  int error_num;
  MYSQL_ROW mysql_row;
...
void spider_db_mysql_result::move_to_pos(longlong pos) {
...
int spider_db_mysql_result::fetch_columns_for_discover_table_structure(
    spider_string *str, CHARSET_INFO *access_charset) {
  int error_num;
  uint length;
  MYSQL_ROW mysql_row;
  DBUG_ENTER(
  ...
  do {
    ...
    if (mysql_row[6] && !strcmp(mysql_row[6], "auto_increment")) {
      if (str->reserve(SPIDER_SQL_AUTO_INCREMENT_LEN))
        DBUG_RETURN(HA_ERR_OUT_OF_MEM);
      str->q_append(SPIDER_SQL_AUTO_INCREMENT_STR,
                    SPIDER_SQL_AUTO_INCREMENT_LEN);
    }
...
int spider_db_mysql_result::fetch_table_for_discover_table_structure(
  ...
  if (!spider_share->table_share->table_charset) {
    spider_share->table_share->table_charset =
        get_charset_by_name(mysql_row[14], MYF(MY_WME));
  }
  DBUG_RETURN(0);
}
#endif

spider_db_mysql::spider_db_mysql(SPIDER_CONN *conn)
    : spider_db_conn(conn),
...
spider_db_mysql::~spider_db_mysql() {
  ...
  if (handler_open_array_inited) {
    ...
    spider_free_mem_calc(
        spider_current_trx, handler_open_array_id,
        handler_open_array.max_element * handler_open_array.size_of_element);
    delete_dynamic(&handler_open_array);
  }
  if (lock_table_hash_inited) {
    spider_free_mem_calc(spider_current_trx, lock_table_hash_id,
...
int spider_db_mysql::init() {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  if (my_hash_init(&lock_table_hash, spd_charset_utf8_bin, 32, 0, 0,
                   (my_hash_get_key)spider_link_get_key, 0, 0)) {
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  }
  spider_alloc_calc_mem_init(lock_table_hash, 140);
  spider_alloc_calc_mem(spider_current_trx, lock_table_hash,
                        lock_table_hash.array.max_element *
                            lock_table_hash.array.size_of_element);
  lock_table_hash_inited = TRUE;

  if (SPD_INIT_DYNAMIC_ARRAY2(&handler_open_array,
  ...
  spider_alloc_calc_mem(
      spider_current_trx, handler_open_array,
      handler_open_array.max_element * handler_open_array.size_of_element);
...
/**
  Check whether the thd context is owned by builtin event scheduler. Event
  scheduler executes event with a special internal user and doesn't set
  privilege user field.

  @return true if the thd context is owned by builtin event scheduler, false
  otherwise
*/
static inline bool builtin_event_scheduler_user(THD *thd) {
  return thd->m_main_security_ctx.priv_user().length == 0 &&
         strcmp(thd->m_main_security_ctx.user().str, EVENT_SCHEDULER_USER) == 0;
}

int spider_db_mysql::connect(const char *tgt_host, const char *tgt_username,
                             const char *tgt_password, long tgt_port,
                             const char *tgt_socket, const char *server_name,
                             int connect_retry_count,
                             longlong connect_retry_interval) {
  int error_num;
  uint real_connect_option = 0;
  bool connect_mutex = spider_param_connect_mutex();
  ...
  while (TRUE) {
    ...
    DBUG_PRINT("info",
               ("spider abort_loop=%s", *spd_abort_loop ? "TRUE" : "FALSE"));
    if ((thd && thd->killed) || *spd_abort_loop) {
    ...
    mysql_options(db_conn, MYSQL_OPT_CONNECT_TIMEOUT, &conn->connect_timeout);
    //mysql_options(db_conn, MYSQL_OPT_USE_REMOTE_CONNECTION, NULL);

    if (conn->tgt_ssl_ca_length | conn->tgt_ssl_capath_length |
        conn->tgt_ssl_cert_length | conn->tgt_ssl_key_length) {
      mysql_ssl_set(db_conn, conn->tgt_ssl_key, conn->tgt_ssl_cert,
                    conn->tgt_ssl_ca, conn->tgt_ssl_capath,
                    conn->tgt_ssl_cipher);
      if (conn->tgt_ssl_vsc) {
        bool verify_flg = TRUE;
        mysql_options(db_conn, MYSQL_OPT_SSL_CERT, &verify_flg);
      }
    }

    if (conn->tgt_default_file) {
      DBUG_PRINT("info",
                 ("spider tgt_default_file=%s", conn->tgt_default_file));
      mysql_options(db_conn, MYSQL_READ_DEFAULT_FILE, conn->tgt_default_file);
    }
    if (conn->tgt_default_group) {
      DBUG_PRINT("info",
                 ("spider tgt_default_group=%s", conn->tgt_default_group));
      mysql_options(db_conn, MYSQL_READ_DEFAULT_GROUP, conn->tgt_default_group);
    }

    // disable the ssl
    enum mysql_ssl_mode ssl_mode = SSL_MODE_DISABLED;
    mysql_options(db_conn, MYSQL_OPT_SSL_MODE, (void*)&ssl_mode);
    real_connect_option = CLIENT_INTERACTIVE | CLIENT_MULTI_STATEMENTS;
    if (connect_mutex) pthread_mutex_lock(&spider_open_conn_mutex);
    const char* true_username = "";
    if (tdsql::common::tdsql_pass_client_ip) {
      tdsql_set_transparent_client(
          db_conn, thd->m_main_security_ctx.ip().str, thd->peer_port,
          thd->m_main_security_ctx.td_hash_stage1().str,
          thd->m_main_security_ctx.td_hash_stage1().length);
      if (td_unlikely(builtin_event_scheduler_user(thd))) {
        // use target set user instead of event_scheduler user for event
        // executing
        true_username = tgt_username;
      } else {
        true_username = thd->m_main_security_ctx.user().str;
      }
    } else {
      true_username = tgt_username;
    }
    ...
    /* tgt_db not use */
    if (!mysql_real_connect(db_conn, tgt_host, true_username, tgt_password, NULL,
                            tgt_port, tgt_socket, real_connect_option)) {
      if (connect_mutex) pthread_mutex_unlock(&spider_open_conn_mutex);
      log_spider_mysql_sql(db_conn, "", 0, COM_CONNECT);
      error_num = mysql_errno(db_conn);
      std::stringstream ss;
      ss << "Failed to connect to host:" << tgt_host << ", port:" << tgt_port <<
        ", error: " << mysql_error(db_conn);
      log_spider_resultf( SPIDER_LOG_RES_ERR_LVL_ERROR, ss.str().c_str());
      disconnect();
      DBUG_PRINT("info", ("spider thd->killed=%s",
                          thd ? (thd->killed ? "TRUE" : "FALSE") : "NULL"));
      DBUG_PRINT("info",
      ...
      if ((error_num != CR_CONN_HOST_ERROR &&
  ...
  if (db_conn) {
    if (!vio_sock_addr(db_conn->net.vio, sock_addr, &sock_port, NI_MAXHOST)) {
      DBUG_PRINT("info", ("Failed vio_sock_addr"));
    }
  }
  DBUG_RETURN(0);
}
...
namespace tdsql {
extern uint32_t deadlock_timeout;
}
...
int spider_db_mysql::exec_query(const char *query, uint length,
                                int quick_mode[[maybe_unused]], bool sync) {
  int error_num = 0;
  THD *thd = conn->thd;

  // assert(thd); //coredump when shutdown
  if (unlikely(!thd)) thd = current_thd;
  thd->status_var.remote_queries_executed++;
  uint log_result_errors = spider_param_log_result_errors(thd);
  my_hrtime_t start = my_hrtime();
  ...
  if (spider_param_general_log()) {
    ...
    if (tmp_query_str.reserve(length + conn->tgt_wrapper_length + tgt_len +
                              (SPIDER_SQL_SPACE_LEN * 3) + tgt_port_len))
      DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    ...
    query_logger.general_log_write(current_thd, COM_QUERY, tmp_query_str.ptr(),
  ...
  if (opt_spider_slow_log)
  {
      ...
      {// more query do not be recorded
          char spider_last_slow_query[128] = "other query spider distributed do
  not be recorded ... ;\n";
          thd->spider_remote_query.append(spider_last_slow_query,
  strlen(spider_last_slow_query));
      }
  }
  *******************************/
  ...
  if (conn->dry_run) {
    ...
    log_spider_sql_tdsql(current_thd, this, query, length, start, false);
    ...
    */
    ...
    if (!sync) {
      // For parallel query, it is needed to send query and receive result
      // later.
      if (mysql_send_query(db_conn, query, length))
        DBUG_RETURN(1);
      else
        DBUG_RETURN(0);
    }
    // to valid heavy deadlock detection of xa trx, use a background thread to
    // find the wait-for cycle if needed, there are two ways to fire
    // the detection: one is xa lockwait command and query wait for
    // deadlock_timeout seconds.
    // if wait-for cycle found, choose the victim trx and kill query of it
    // in remote set, then the wait sesssion exit and rollback the whole trx.
    // FIXME: add a new variable to indicate it's xa trx?
    if (tdsql::deadlock_timeout <= 0 || !conn->join_trx ||
        conn->disable_xa || conn->deadlock) {
      error_num = mysql_real_query(db_conn, query, length);
      // if (conn->deadlock) error_num = 0; // ignore error
    } else { // xa
      uint read_timeout_bak = db_conn->net.read_timeout;
      if (tdsql::deadlock_timeout > 0 &&
          (read_timeout_bak == 0 || // wait forever
          (read_timeout_bak > 0 && 
           tdsql::deadlock_timeout < read_timeout_bak))) { // xa need deadlock check 
        if (mysql_send_query(db_conn, query, length)) {
          error_num = 1;
        } else {
          // first wait for tdsql::deadlock_timeout seconds
          my_net_set_read_timeout(&db_conn->net, tdsql::deadlock_timeout);
          error_num = (int)(*db_conn->methods->read_query_result)(db_conn);
          // deadlock_timeout reached start the check
          if (error_num && get_errno() == ER_NET_READ_INTERRUPTED) {
            // re-read the reault, clear the timeout error
            net_clear_error(&db_conn->net);
            db_conn->net.error = NET_ERROR_UNSET;
            thd->get_stmt_da()->reset_diagnostics_area();
            uint timeout_left = read_timeout_bak > 0 ?
                                read_timeout_bak - tdsql::deadlock_timeout : 0;
            my_net_set_read_timeout(&db_conn->net, timeout_left);
            tdsql::GTM::instance()->start_deadlock_check();
            error_num = (int)(*db_conn->methods->read_query_result)(db_conn);
          }
          my_net_set_read_timeout(&db_conn->net, read_timeout_bak);
          mysql_extension_bind_free(MYSQL_EXTENSION_PTR(db_conn)); // see mysql_real_query
        }
      } else {
        error_num = mysql_real_query(db_conn, query, length);
      }
      if (error_num) {
        assert(!conn->deadlock);
        SPIDER_TRX *trx = (SPIDER_TRX *)thd_get_ha_data(thd, spider_hton_ptr);
        if (trx != nullptr && trx->trx_xa) {
          if (trx->internal_xid_state.get_state() == XID_STATE::XA_ROLLBACK_ONLY) {
            net_clear_error(&db_conn->net);
            if (db_conn->net.error == NET_ERROR_SOCKET_NOT_READABLE) db_conn->net.error = NET_ERROR_UNSET;
            thd->get_stmt_da()->reset_diagnostics_area(); // clear the kill query error
            // tell the spider_db_errorno to rollbac and return deadlock to client
            conn->deadlock = true;
          }
        }
      }
  ...
  log_spider_sql_tdsql(current_thd, this, query, length, start, has_error);
  ...
  if ((has_error && log_result_errors >= SPIDER_LOG_RES_ERR_LVL_ERROR) ||
    ...
    if (log_result_error_with_sql) {
      ...
      if (log_result_error_with_sql & SPIDER_LOG_RES_ERR_SQL_USER) {
        if (query_string.copy(thd->query().str, thd->query().length,
                              &my_charset_bin))
          DBUG_RETURN(HA_ERR_OUT_OF_MEM);
...
void spider_db_mysql::set_errno(int err) {
  // if (err == HA_ERR_MISS_GTS) {
  set_mysql_error(db_conn, err, unknown_sqlstate);
  //}
}
...
/* return the sqlstate specification */
const char *spider_db_mysql::get_sqlstate() {
  const char *sqlstate_ptr;
  DBUG_ENTER("spider_db_mysql::get_sqlstate");
  DBUG_PRINT("info", ("spider this=%p", this));
  if (conn->dry_run) DBUG_RETURN(ER_SPIDER_DRY_STR);
  sqlstate_ptr = /* mysql_sqlstate(db_conn) */
      db_conn ? db_conn->net.sqlstate : "08001" /*cant_connect_sqlstate*/;
  DBUG_PRINT("info", ("spider sqlstate=%s", sqlstate_ptr));
  DBUG_RETURN(sqlstate_ptr);
}

bool spider_db_mysql::is_server_gone_error(int error_num) {
  bool server_gone;
...
bool spider_db_mysql::is_dup_entry_error(int error_num) {
  bool dup_entry;
  DBUG_ENTER("spider_db_mysql::is_dup_entry_error");
  DBUG_PRINT("info", ("spider this=%p", this));
  dup_entry = (error_num == ER_DUP_ENTRY || error_num == ER_DUP_KEY ||
               error_num == HA_ERR_FOUND_DUPP_KEY);
  DBUG_PRINT("info", ("spider dup_entry=%s", dup_entry ? "TRUE" : "FALSE"));
  DBUG_RETURN(dup_entry);
}

bool spider_db_mysql::is_xa_nota_error(int error_num) {
  bool xa_nota;
  DBUG_ENTER("spider_db_mysql::is_xa_nota_error");
  DBUG_PRINT("info", ("spider this=%p", this));
  xa_nota = (error_num == ER_XAER_NOTA || error_num == ER_XA_RBTIMEOUT ||
...
spider_db_result *spider_db_mysql::store_result(
    spider_db_result_buffer **spider_res_buf [[maybe_unused]],
    st_spider_db_request_key *request_key [[maybe_unused]], int *error_num) {
...
bool spider_db_mysql::trx_transmit_begin_commit() {
  ...
  if (thd && !is_with_begin_commit &&
      !thd_test_options(thd, OPTION_NOT_AUTOCOMMIT) &&
      !thd_test_options(thd,
                        OPTION_BEGIN)) { /* 1.autocommit is 1; 2. without
                                            begin/commit. Default don't transmit
                                            begin/commit to remote mysql */
    DBUG_RETURN(FALSE);
  }
  DBUG_RETURN(TRUE);
}
...
int spider_db_mysql::rollback(int *need_mon) {
  bool is_error;
  int error_num;
...
int spider_db_mysql::xa_start(tdsql::Gtid *gtid[[maybe_unused]], int *need_mon[[maybe_unused]]) {
...
int spider_db_mysql::xa_end(tdsql::Gtid *gtid, int *need_mon) {
  ...
  spider_db_append_gtid_str(&sql_str, gtid);
  if (spider_db_query(conn, sql_str.ptr(), sql_str.length(), -1, need_mon))
    DBUG_RETURN(spider_db_errorno(conn));
  spider_mta_conn_mutex_unlock(conn);
  DBUG_RETURN(0);
}

int spider_db_mysql::xa_prepare(tdsql::Gtid *gtid, int *need_mon) {
...
int spider_db_mysql::xa_end_and_prepare(tdsql::Gtid *gtid, int *need_mon) {
  int error_num = 0;
  int error_num1 = 0;
  int error_num2 = 0;
  char sql_buf[SPIDER_SQL_XA_END_LEN + SPIDER_SQL_XA_PREPARE_LEN +
               2 * XIDDATASIZE + 2 * sizeof(long) + 20];
  spider_string sql_str(sql_buf, sizeof(sql_buf), &my_charset_bin);
  ...
  if (conn->db_conn) error_num2 = conn->db_conn->next_result();
  error_num = error_num1 ? error_num1 : error_num2;
  spider_mta_conn_mutex_unlock(conn);
  DBUG_RETURN(error_num);
}

int spider_db_mysql::xa_commit(tdsql::Gtid *gtid, int *need_mon) {
  ...
  if (trx_need_end_gts()) {
    ...
    set_timespec(&abstime, tdsql::meta_cluster_timeout);
    native_mutex_lock(&(trx->m_end_gts_mutex));
    if (!trx->m_end_gts) native_cond_timedwait(&(trx->m_end_gts_cond),
                                                    &(trx->m_end_gts_mutex),
                                                    &abstime);
    native_mutex_unlock(&(trx->m_end_gts_mutex));
    if (trx->m_end_gts == 0) DBUG_RETURN(ER_SPIDER_GET_GTS);
    sql_str.q_append(SPIDER_SQL_TDSQL_WITHGTS_STR, SPIDER_SQL_TDSQL_WITHGTS_LEN);
    std::string tmp_gts = std::to_string(trx->m_end_gts);
    // trx->m_end_gts = 0; // reset the start gts
    sql_str.q_append(tmp_gts.c_str(), tmp_gts.length());  
  }
  if (spider_db_query(conn, sql_str.ptr(), sql_str.length(), -1, need_mon))
    DBUG_RETURN(spider_db_errorno(conn));
  spider_mta_conn_mutex_unlock(conn);
...
int spider_db_mysql::xa_commit_one_phase(tdsql::Gtid *gtid, int *need_mon) {
  int error_num = 0;
  int error_num1 = 0;
  int error_num2 = 0;
  ...
  // SPIDER_SQL_TMYSQL32_WITH_LOG_HINT will lead to xa commit fail
  //sql_str.q_append(SPIDER_SQL_TMYSQL32_WITH_LOG_HINT,
  //                 SPIDER_SQL_TMYSQL32_WITH_LOG_HINT_LEN);
...
int spider_db_mysql::xa_rollback(tdsql::Gtid *gtid, int *need_mon) {
  ...
  switch (trx_isolation) {
    case ISO_READ_UNCOMMITTED:
      if (spider_db_query(conn, SPIDER_SQL_ISO_READ_UNCOMMITTED_STR,
                          SPIDER_SQL_ISO_READ_UNCOMMITTED_LEN, -1, need_mon))
        DBUG_RETURN(spider_db_errorno(conn));
      spider_mta_conn_mutex_unlock(conn);
      break;
    case ISO_READ_COMMITTED:
      if (spider_db_query(conn, SPIDER_SQL_ISO_READ_COMMITTED_STR,
                          SPIDER_SQL_ISO_READ_COMMITTED_LEN, -1, need_mon))
        DBUG_RETURN(spider_db_errorno(conn));
      spider_mta_conn_mutex_unlock(conn);
      break;
    case ISO_REPEATABLE_READ:
      if (spider_db_query(conn, SPIDER_SQL_ISO_REPEATABLE_READ_STR,
                          SPIDER_SQL_ISO_REPEATABLE_READ_LEN, -1, need_mon))
        DBUG_RETURN(spider_db_errorno(conn));
      spider_mta_conn_mutex_unlock(conn);
      break;
    case ISO_SERIALIZABLE:
      if (spider_db_query(conn, SPIDER_SQL_ISO_SERIALIZABLE_STR,
                          SPIDER_SQL_ISO_SERIALIZABLE_LEN, -1, need_mon))
        DBUG_RETURN(spider_db_errorno(conn));
      spider_mta_conn_mutex_unlock(conn);
      break;
    default:
...
int spider_db_mysql::set_autocommit(bool autocommit, int *need_mon) {
  ...
  if (autocommit) {
    ...
    if (spider_db_query(conn, SPIDER_SQL_AUTOCOMMIT_OFF_STR,
                        SPIDER_SQL_AUTOCOMMIT_OFF_LEN, -1, need_mon))
      DBUG_RETURN(spider_db_errorno(conn));
    spider_mta_conn_mutex_unlock(conn);
  }
  DBUG_RETURN(0);
...
int spider_db_mysql::set_sql_log_off(bool sql_log_off, int *need_mon) {
  ...
  if (sql_log_off) {
    ...
    if (spider_db_query(conn, SPIDER_SQL_SQL_LOG_OFF_STR,
                        SPIDER_SQL_SQL_LOG_OFF_LEN, -1, need_mon))
      DBUG_RETURN(spider_db_errorno(conn));
    spider_mta_conn_mutex_unlock(conn);
  }
  DBUG_RETURN(0);
...
int spider_db_mysql::set_time_zone(Time_zone *time_zone, int *need_mon) {
...
int spider_db_mysql::exec_simple_sql_with_result(
    SPIDER_TRX *trx, SPIDER_SHARE *share, const char *sql, uint sql_length,
  ...
  if ((error_num = spider_db_set_names_internal(trx, share, conn, all_link_idx,
                                                need_mon)) ||
      (spider_db_query(conn, sql, sql_length, -1, need_mon) &&
       (error_num = spider_db_errorno(conn)))) {
    if (error_num == ER_SPIDER_REMOTE_SERVER_GONE_AWAY_NUM &&
        !conn->disable_reconnect) {
      /* retry */
      if ((error_num =
               spider_db_ping_internal(share, conn, all_link_idx, need_mon))) {
        spider_mta_conn_mutex_unlock(conn);
      ...
      spider_conn_set_timeout_from_share(conn, all_link_idx, trx->thd, share);
      if (spider_db_query(conn, sql, sql_length, -1, need_mon)) {
  ...
  if (!(*res = store_result(NULL, NULL, &error_num))) {
    if (error_num || (error_num = spider_db_errorno(conn))) {
      ...
      DBUG_PRINT("info",
                 ("spider error_num=%d 6", ER_QUERY_ON_FOREIGN_DATA_SOURCE));
      DBUG_RETURN(ER_QUERY_ON_FOREIGN_DATA_SOURCE);
    }
  }
  spider_mta_conn_mutex_unlock(conn);
...
int spider_db_mysql::show_master_status(SPIDER_TRX *trx, SPIDER_SHARE *share,
                                        int all_link_idx, int *need_mon,
                                        TABLE *table, spider_string *str,
  ...
  if (!(error_num =
            ((spider_db_mysql_result *)*res1)
                ->fetch_show_master_status(&binlog_file_name, &binlog_pos))) {
    binlog_file_name_length = strlen(binlog_file_name);
    binlog_pos_length = strlen(binlog_pos);
    spider_store_binlog_pos_binlog_file(
        table, binlog_file_name, binlog_file_name_length, binlog_pos,
        binlog_pos_length, conn->access_charset);
    if (mode > 0) {
      error_num = select_binlog_gtid_pos(
          trx, share, all_link_idx, need_mon, table, str, binlog_file_name,
          binlog_file_name_length, binlog_pos, binlog_pos_length, res2);
    } else {
...
int spider_db_mysql::select_binlog_gtid_pos(
    SPIDER_TRX *trx, SPIDER_SHARE *share, int all_link_idx, int *need_mon,
    TABLE *table, spider_string *str, const char *binlog_file_name,
    uint binlog_file_name_length, const char *binlog_pos,
    uint binlog_pos_length, SPIDER_DB_RESULT **res) {
  int error_num;
  size_t length;
  const char *gtid_pos;
  ...
  if (str->reserve(SPIDER_SQL_BINLOG_GTID_POS_LEN + SPIDER_SQL_OPEN_PAREN_LEN +
                   ...
                   SPIDER_SQL_VALUE_QUOTE_LEN + SPIDER_SQL_COMMA_LEN +
                   SPIDER_SQL_VALUE_QUOTE_LEN + binlog_pos_length * 2 +
                   SPIDER_SQL_VALUE_QUOTE_LEN + SPIDER_SQL_CLOSE_PAREN_LEN))
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  str->q_append(SPIDER_SQL_BINLOG_GTID_POS_STR, SPIDER_SQL_BINLOG_GTID_POS_LEN);
  str->q_append(SPIDER_SQL_OPEN_PAREN_STR, SPIDER_SQL_OPEN_PAREN_LEN);
  str->q_append(SPIDER_SQL_VALUE_QUOTE_STR, SPIDER_SQL_VALUE_QUOTE_LEN);
  length =
      conn->db_conn->escape_string(const_cast<char *>(str->ptr()) + str->length(),
                                   binlog_file_name, binlog_file_name_length);
  str->length(str->length() + length);
  str->q_append(SPIDER_SQL_VALUE_QUOTE_STR, SPIDER_SQL_VALUE_QUOTE_LEN);
  str->q_append(SPIDER_SQL_COMMA_STR, SPIDER_SQL_COMMA_LEN);
  str->q_append(SPIDER_SQL_VALUE_QUOTE_STR, SPIDER_SQL_VALUE_QUOTE_LEN);
  length = conn->db_conn->escape_string(const_cast<char *>(str->ptr()) + str->length(),
  ...
  if ((error_num =
           exec_simple_sql_with_result(trx, share, str->ptr(), str->length(),
                                       all_link_idx, need_mon, res))) {
    DBUG_PRINT("info", ("spider error_num=%d 1", error_num));
    DBUG_RETURN(error_num);
  }
  if (!(error_num = ((spider_db_mysql_result *)*res)
                        ->fetch_select_binlog_gtid_pos(&gtid_pos))) {
    spider_store_binlog_pos_gtid(table, gtid_pos, strlen(gtid_pos),
                                 conn->access_charset);
  }
  /*
    res->free_result();
...
size_t spider_db_mysql::escape_string(char *to, const char *from,
                                      size_t from_length) {
  DBUG_ENTER("spider_db_mysql::escape_string");
  DBUG_PRINT("info", ("spider this=%p", this));
  if (db_conn->server_status & SERVER_STATUS_NO_BACKSLASH_ESCAPES)
    DBUG_RETURN(
        escape_quotes_for_mysql(db_conn->charset, to, 0, from, from_length,'\''));
  DBUG_RETURN(
      escape_string_for_mysql(db_conn->charset, to, 0, from, from_length));
}
...
int spider_db_mysql::append_lock_tables(spider_string *str) {
  int error_num;
  ...
  while ((tmp_link_for_hash =
    ...
    spider_mysql_share *db_share =
        (spider_mysql_share *)tmp_spider->share->dbton_share[conn->dbton_id];
    ...
    if ((error_num = spider_db_mysql_utility.append_lock_table_body(
             str, db_name, db_name_length, db_name_charset, table_name,
             table_name_length, table_name_charset, lock_type))) {
...
int spider_db_mysql::append_unlock_tables(spider_string *str) {
  int error_num;
...
void spider_db_mysql::set_dup_key_idx(ha_spider *spider, int link_idx) {
  ...
  for (roop_count = 0; roop_count < table->s->keys; roop_count++) {
    if (roop_count == pk_idx) {
      ...
      key_name_length = strlen(table->s->key_info[roop_count].name);
    }
    DBUG_PRINT("info", ("spider key_name=%s", key_name));
    if (max_length < key_name_length &&
        conn->error_length - 1 >= key_name_length &&
        *(conn->error_str + conn->error_length - 2 - key_name_length) == '\'' &&
        !strncasecmp(conn->error_str + conn->error_length - 1 - key_name_length,
                     key_name, key_name_length)) {
      max_length = key_name_length;
      spider->dup_key_idx = roop_count;
    }
  }
  if (max_length == 0) spider->dup_key_idx = (uint)-1;
  DBUG_PRINT("info", ("spider dup_key_idx=%d", spider->dup_key_idx));
  DBUG_VOID_RETURN;
}

bool spider_db_mysql::cmp_request_key_to_snd(
    st_spider_db_request_key *request_key[[maybe_unused]]) {
...
int spider_db_mysql_util::append_name(spider_string *str, const char *name,
                                      uint name_length) {
...
int spider_db_mysql_util::append_name_with_charset(spider_string *str,
                                                   const char *name,
                                                   uint name_length,
  ...
  if (str->reserve(SPIDER_SQL_NAME_QUOTE_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  str->q_append(SPIDER_SQL_NAME_QUOTE_STR, SPIDER_SQL_NAME_QUOTE_LEN);
  DBUG_RETURN(0);
}

bool spider_db_mysql_util::is_name_quote(const char head_code) {
  DBUG_ENTER("spider_db_mysql_util::is_name_quote");
  DBUG_RETURN(head_code == *name_quote_str);
}

int spider_db_mysql_util::append_escaped_name_quote(spider_string *str) {
...
int spider_db_mysql_util::append_key_equal_or_like(ha_spider *spider,
  ...
  assert(spider->trx && spider->trx->thd);
...
int spider_db_mysql_util::append_column_value(ha_spider *spider[[maybe_unused]],
  ...
  if (new_ptr) {
    if (field->type() == MYSQL_TYPE_BLOB ||
      ...
      tmp_str.set_quick((char*)(const_cast<uchar *>(new_ptr)) + HA_KEY_BLOB_LENGTH, length,
                        &my_charset_bin);
      ptr = tmp_str.get_str();
    } else if (field->type() == MYSQL_TYPE_GEOMETRY) {
#ifndef DBUG_OFF
      double xmin, xmax, ymin, ymax;
      xmin = float8get(new_ptr);
      xmax = float8get(new_ptr + 8);
      ymin = float8get(new_ptr + 16);
      ymax = float8get(new_ptr + 24);
      LogDebug("spider geo is %f %f %f %f", xmin, xmax, ymin, ymax);
#endif
      tmp_str.length(0);
      tmp_str.q_append((char*)(const_cast<uchar *>(SPIDER_SQL_LINESTRING_HEAD_STR)),
                       SPIDER_SQL_LINESTRING_HEAD_LEN);
      tmp_str.q_append((char*)(const_cast<uchar *>(new_ptr)), SIZEOF_STORED_DOUBLE);
      tmp_str.q_append((char*)(const_cast<uchar *>(new_ptr)) + SIZEOF_STORED_DOUBLE * 2,
                       SIZEOF_STORED_DOUBLE);
      tmp_str.q_append((char*)(const_cast<uchar *>(new_ptr)) + SIZEOF_STORED_DOUBLE,
                       SIZEOF_STORED_DOUBLE);
      tmp_str.q_append((char*)(const_cast<uchar *>(new_ptr)) + SIZEOF_STORED_DOUBLE * 3,
                       SIZEOF_STORED_DOUBLE);
      ptr = tmp_str.get_str();
    } else {
      ptr = field->val_str(tmp_str.get_str(), const_cast<uchar*>(new_ptr));
      tmp_str.mem_calc();
    }
  } else {
  ...
  DBUG_PRINT("info", ("spider ptr->length() is %lu", ptr->length()));
  if (field->result_type() == STRING_RESULT) {
    ...
    if (field->type() == MYSQL_TYPE_VARCHAR ||
      ...
      tmp_str2.length(0);
      if (tmp_str2.append(ptr->ptr(), ptr->length(), const_cast<CHARSET_INFO*>(field->charset())))
        DBUG_RETURN(HA_ERR_OUT_OF_MEM);

      if (is_like &&
          tmp_str2.append(SPIDER_SQL_PERCENT_STR, SPIDER_SQL_PERCENT_LEN))
        DBUG_RETURN(HA_ERR_OUT_OF_MEM);

      if (str->reserve(tmp_str2.length() * 2) ||
...
int spider_db_mysql_util::append_from_with_alias(
    spider_string *str, const char **table_names, uint *table_name_lengths,
    const char **table_aliases, uint *table_alias_lengths, uint table_count,
...
int spider_db_mysql_util::append_trx_isolation(spider_string *str,
                                               int trx_isolation) {
...
int spider_db_mysql_util::append_autocommit(spider_string *str,
                                            bool autocommit) {
  ...
  if (autocommit) {
    str->q_append(SPIDER_SQL_AUTOCOMMIT_ON_STR, SPIDER_SQL_AUTOCOMMIT_ON_LEN);
  } else {
    str->q_append(SPIDER_SQL_AUTOCOMMIT_OFF_STR, SPIDER_SQL_AUTOCOMMIT_OFF_LEN);
  }
  DBUG_RETURN(0);
}

int spider_db_mysql_util::append_sql_log_off(spider_string *str,
                                             bool sql_log_off) {
...
int spider_db_mysql_util::append_time_zone(spider_string *str,
                                           Time_zone *time_zone) {
  const String *tz_str = time_zone->get_name();
...
int spider_db_mysql_util::append_start_transaction(spider_string *str) {
  DBUG_ENTER("spider_db_mysql_util::append_start_transaction");
  DBUG_PRINT("info", ("spider this=%p", this));
  if (str->reserve(SPIDER_SQL_SEMICOLON_LEN + SPIDER_SQL_START_TRANSACTION_LEN))
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  if (str->length()) {
    str->q_append(SPIDER_SQL_SEMICOLON_STR, SPIDER_SQL_SEMICOLON_LEN);
  }
...
//todo: here use gtid to replace xid
int spider_db_mysql_util::append_xa_start(spider_string *str, tdsql::Gtid *gtid, bool need_gts) {
  DBUG_ENTER("spider_db_mysql_util::append_xa_start");
  DBUG_PRINT("info", ("spider this=%p", this));
  if (str->reserve(SPIDER_SQL_SEMICOLON_LEN + SPIDER_SQL_XA_START_LEN +
                   XIDDATASIZE + sizeof(long) + 9))
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  if (str->length()) {
    str->q_append(SPIDER_SQL_SEMICOLON_STR, SPIDER_SQL_SEMICOLON_LEN);
  }
  str->q_append(SPIDER_SQL_XA_START_STR, SPIDER_SQL_XA_START_LEN);
  spider_db_append_gtid_str(str, gtid);
  // append gts if needed
  if (need_gts) {
    THD *thd = current_thd;
    assert(thd);
    SPIDER_TRX *trx = (SPIDER_TRX *)thd_get_ha_data(thd, spider_hton_ptr);
    ...
    struct timespec abstime; 
    set_timespec(&abstime, tdsql::meta_cluster_timeout);
    native_mutex_lock(&(trx->m_start_gts_mutex));
    if (!trx->m_start_gts) native_cond_timedwait(&(trx->m_start_gts_cond),
                                                    &(trx->m_start_gts_mutex),
                                                    &abstime);
    native_mutex_unlock(&(trx->m_start_gts_mutex));
    if (trx->m_start_gts == 0) DBUG_RETURN(ER_SPIDER_GET_GTS);
    str->q_append(SPIDER_SQL_TDSQL_WITHGTS_STR, SPIDER_SQL_TDSQL_WITHGTS_LEN);
    std::string tmp_gts = std::to_string(trx->m_start_gts);
    // trx->m_start_gts = 0; // reset the start gts
    str->q_append(tmp_gts.c_str(), tmp_gts.length()); 
  }
  DBUG_RETURN(0);
}

int spider_db_mysql_util::append_lock_table_head(spider_string *str) {
  DBUG_ENTER("spider_db_mysql_util::append_lock_table_head");
  DBUG_PRINT("info", ("spider this=%p", this));
  if (str->reserve(SPIDER_SQL_LOCK_TABLE_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  str->q_append(SPIDER_SQL_LOCK_TABLE_STR, SPIDER_SQL_LOCK_TABLE_LEN);
  DBUG_RETURN(0);
}

int spider_db_mysql_util::append_lock_table_body(
    spider_string *str, const char *db_name, uint db_name_length,
    CHARSET_INFO *db_name_charset, const char *table_name,
    uint table_name_length, CHARSET_INFO *table_name_charset, int lock_type) {
  ...
  str->q_append(SPIDER_SQL_NAME_QUOTE_STR, SPIDER_SQL_NAME_QUOTE_LEN);
  if (str->append(table_name, table_name_length, table_name_charset) ||
...
int spider_db_mysql_util::append_lock_table_tail(spider_string *str) {
...
int spider_db_mysql_util::append_unlock_table(spider_string *str) {
...
int spider_db_mysql_util::open_item_func(Item_func *item_func,
  ...
  if (str) {
    if (str->reserve(SPIDER_SQL_OPEN_PAREN_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    str->q_append(SPIDER_SQL_OPEN_PAREN_STR, SPIDER_SQL_OPEN_PAREN_LEN);
  }
  DBUG_PRINT("info", ("spider functype = %d", item_func->functype()));
  switch (item_func->functype()) {
    case Item_func::ROWNUM_FUNC:
      last_str = SPIDER_SQL_ROWNUM_STR;
      last_str_length = SPIDER_SQL_ROWNUM_LEN;
      break;
    ...
    case Item_func::DATEADD_FUNC:
      func_name = const_cast<char *>(item_func->func_name());
      ...
      if (func_name_length == 1 &&
        ...
        if (!strncasecmp("rand", func_name, func_name_length) &&
          ...
          DBUG_RETURN(spider_db_open_item_int(item_func, spider, str, alias,
                                              ...
                                              use_fields, fields));
        } else if (!strncasecmp("case", func_name, func_name_length)) {
          ...
          if (item_func_case->first_expr_num != -1) {
            if ((error_num = spider_db_print_item_type_and_check_charset(
          ...
          for (roop_count = 0; roop_count < item_func_case->ncases;
            ...
            if ((error_num = spider_db_print_item_type_and_check_charset(
                     item_list[roop_count + 1], spider, str, alias,
                     alias_length, dbton_id, use_fields, fields)))
              DBUG_RETURN(error_num);
          }
          ...
          if (str) {
            if (item_tmp->type() == Item::STRING_ITEM) {
              ...
              /* 2. append "remove_str"*/
              if ((error_num = spider_db_print_item_type_and_check_charset(
                       item_tmp, spider, str, alias, alias_length, dbton_id,
              ...
              /* 4. append `field` */
              if ((error_num = spider_db_print_item_type_and_check_charset(
                       item, spider, str, alias, alias_length, dbton_id,
                       use_fields, fields, field_charset)))
                DBUG_RETURN(error_num);
              /* 5. append ')' */
              if (str->reserve(SPIDER_SQL_CLOSE_PAREN_LEN))
        ...
        if (!strncasecmp("currval", func_name, func_name_length) || 
            !strncasecmp("nextval", func_name, func_name_length)) {
          // sequence operators should never pushdown.
          DBUG_RETURN(ER_SPIDER_COND_SKIP_NUM);
        } else if (!strncasecmp("isfalse", func_name, func_name_length)) {
          last_str = SPIDER_SQL_IS_FALSE_STR;
          last_str_length = SPIDER_SQL_IS_FALSE_LEN;
          break;
        } else if (!strncasecmp("sysdate", func_name, func_name_length) ||
        ...
        } else if (!strncasecmp("convert", func_name, func_name_length)) {
          if (str) {
        ...
        if (str) str->length(str->length() - SPIDER_SQL_OPEN_PAREN_LEN);
        DBUG_RETURN(spider_db_print_item_type_and_check_charset(
            item_func, spider, str, alias, alias_length, dbton_id, use_fields,
            fields, field_charset));
        ...
        if (!strncasecmp("cast_as_date", func_name, func_name_length)) {
          ...
          if (item->type() == Item::FUNC_ITEM) {
            ...
            if (ifunc->functype() == Item_func::UNKNOWN_FUNC) {
              ...
              child_func_name = const_cast<char *>(ifunc->func_name());
              child_func_name_length = strlen(child_func_name);
              DBUG_PRINT("info",
                         ("spider child func_name is %s", child_func_name));
              if (child_func_name_length == 10 &&
                  !strncasecmp("column_get", child_func_name,
                               child_func_name_length)) {
...
// #ifdef ITEM_FUNC_TIMESTAMPDIFF_ARE_PUBLIC
          ...
          if (str) {
            ...
            uint interval_len;
            switch (item_func_timestamp_diff->intervaltype()) {
              case INTERVAL_YEAR:
          ...
          if ((error_num = spider_db_print_item_type_and_check_charset(
                   item_list[0], spider, str, alias, alias_length, dbton_id,
                   use_fields, fields, field_charset)))
            DBUG_RETURN(error_num);
          if (str) {
            if (str->reserve(SPIDER_SQL_COMMA_LEN))
          ...
          if ((error_num = spider_db_print_item_type_and_check_charset(
                   item_list[1], spider, str, alias, alias_length, dbton_id,
                   use_fields, fields, field_charset)))
            DBUG_RETURN(error_num);
          if (str) {
            if (str->reserve(SPIDER_SQL_CLOSE_PAREN_LEN))
...
/*#else
          if (str) {
            if (str->reserve(func_name_length + SPIDER_SQL_OPEN_PAREN_LEN +
                             strlen(item_func->print_type()) +
                             SPIDER_SQL_COMMA_LEN))
              DBUG_RETURN(HA_ERR_OUT_OF_MEM);
            str->q_append(func_name, func_name_length);
            str->q_append(SPIDER_SQL_OPEN_PAREN_STR, SPIDER_SQL_OPEN_PAREN_LEN);
            str->q_append(item_func->print_type(),
                          strlen(item_func->print_type()));
            str->q_append(SPIDER_SQL_COMMA_STR, SPIDER_SQL_COMMA_LEN);
          }
          func_name = SPIDER_SQL_COMMA_STR;
        ...
        if (!strncasecmp("cast_as_binary", func_name, func_name_length)) {
          ...
          if (str) {
            ...
            item_func->print(current_thd, tmp_str.get_str(), QT_TO_SYSTEM_CHARSET);

            tmp_str.mem_calc();
            if (tmp_str.reserve(1)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
          ...
          break;
        } else if (!strncasecmp("cast_as_signed", func_name,
                                func_name_length)) {
          item = item_list[0];
          if (item->type() == Item::FUNC_ITEM) {
            DBUG_PRINT("info", ("spider child is FUNC_ITEM"));
        ...
        if (!strncasecmp("date_add_interval", func_name, func_name_length)) {
          ...
          func_name =
              spider_db_timefunc_interval_str[item_date_add_interval->int_type];
          func_name_length = strlen(func_name);
          if ((error_num = spider_db_print_item_type_and_check_charset(
                   item_list[0], spider, str, alias, alias_length, dbton_id,
                   use_fields, fields, field_charset)))
    ...
    case Item_func::NOW_FUNC:
      if (str) str->length(str->length() - SPIDER_SQL_OPEN_PAREN_LEN);
      DBUG_RETURN(spider_db_open_item_string(item_func, spider, str, alias,
...
#endif
      if (str) str->length(str->length() - SPIDER_SQL_OPEN_PAREN_LEN);
      DBUG_RETURN(spider_db_open_item_cond((Item_cond *)item_func, spider, str,
    ...
    case Item_func::FT_FUNC:
      ...
      start_item = 1;
      if (str) {
        if (str->reserve(SPIDER_SQL_MATCH_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    ...
    case Item_func::SP_EQUALS_FUNC:
      if (str) {
        ...
        DBUG_PRINT("info", ("spider func_name = %s", func_name));
        DBUG_PRINT("info", ("spider func_name_length = %d", func_name_length));
        if (str->reserve(func_name_length)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
        str->q_append(func_name, func_name_length);
      }
      func_name = SPIDER_SQL_COMMA_STR;
      func_name_length = SPIDER_SQL_COMMA_LEN;
  ...
  if (item_count) {
    ...
    for (roop_count = start_item; roop_count < item_count; roop_count++) {
      ...
      if ((error_num = spider_db_print_item_type_and_check_charset(
               item, spider, str, alias, alias_length, dbton_id, use_fields,
               fields, field_charset)))
        DBUG_RETURN(error_num);
      if (roop_count == 1) {
  ...
  if (item_func->functype() == Item_func::FT_FUNC) {
    ...
    if (str) {
      if (str->reserve(SPIDER_SQL_AGAINST_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    ...
    if ((error_num = spider_db_print_item_type_and_check_charset(
      ...
      DBUG_RETURN(error_num);
    if (str) {
      if (str->reserve(((item_func_match->flags & FT_BOOL)
    ...
    if (func_name_length == 7 &&
      ...
      if (str) {
        ...
        CHARSET_INFO *conv_charset =
            const_cast<CHARSET_INFO *>(item_func_conv_charset->SPIDER_Item_func_conv_charset_conv_charset);
  ...
  if (str) {
    if (merge_func) str->length(str->length() - SPIDER_SQL_CLOSE_PAREN_LEN);
...
int spider_db_mysql_util::open_item_sum_func(
    Item_sum *item_sum, ha_spider *spider, spider_string *str,
    const char *alias, uint alias_length, bool use_fields,
    spider_fields *fields, CHARSET_INFO *field_charse) {
  uint dbton_id = spider_dbton_mysql.dbton_id;
  uint roop_count, item_count = item_sum->argument_count();
  int error_num;
  ...
  switch (item_sum->sum_func()) {
    ...
    case Item_sum::MAX_FUNC: {
      ...
      Item *item, **args = item_sum->arguments();
      if (str) {
        if (str->reserve(func_name_length)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
        str->q_append(func_name, func_name_length);
      }
...
int spider_db_mysql_util::append_escaped_util(spider_string *to, String *from) {
...
int spider_db_mysql_util::append_table(
    ha_spider *spider, spider_fields *fields, spider_string *str,
    TABLE_LIST *table_list, TABLE_LIST **used_table_list, uint *current_pos,
    TABLE_LIST **cond_table_list_ptr, bool top_down, bool first) {
  int error_num;
  ...
  DBUG_PRINT("info",
             ("spider table_list->outer_join=%u", table_list->outer_join));
  DBUG_PRINT("info", ("spider table_list->on_expr=%p", table_list->on_expr));
  DBUG_PRINT("info", ("spider table_list->join_using_fields=%p",
                      table_list->join_using_fields));
  DBUG_PRINT("info", ("spider table_list->table=%p", table_list->table));
  if (!top_down && table_list->embedding) {
    if ((error_num = append_embedding_tables(
             spider, fields, str, table_list->embedding, used_table_list,
             current_pos, cond_table_list_ptr)))
      DBUG_RETURN(error_num);
    ...
    if (table_list->outer_join || table_list->on_expr ||
        table_list->join_using_fields) {
      ...
      if (table_list->outer_join & JOIN_TYPE_LEFT) {
        ...
        if (str) {
          ...
          str->q_append(SPIDER_SQL_JOIN_STR, SPIDER_SQL_JOIN_LEN);
               ...
               (cond_table_list->outer_join || cond_table_list->on_expr ||
    ...
    if (str) {
      ...
      db_share = (spider_mysql_share *)spd->share->dbton_share[dbton_id];
      dbton_hdl = (spider_mysql_handler *)spd->dbton_handler[dbton_id];

      dbton_hdl->table_name_pos = str->length();

      if (str->reserve(db_share->db_nm_max_length + SPIDER_SQL_DOT_LEN +
      ...
      if ((error_num = db_share->append_table_name_with_adjusting(
    ...
    if (on_expr) {
      ...
      if ((error_num = spider_db_print_item_type(on_expr, spider, str, NULL, 0,
  ...
  do {
    if (cur_table_list->outer_join & JOIN_TYPE_RIGHT) {
      ...
      if ((error_num = append_table(spider, fields, str, cur_table_list,
                                    used_table_list, current_pos,
                                    cond_table_list_ptr, TRUE, first)))
        DBUG_RETURN(error_num);
      first = FALSE;
      if (prev_table_list) {
        ...
        if ((error_num = append_table(spider, fields, str, prev_table_list,
                                      used_table_list, current_pos,
                                      cond_table_list_ptr, TRUE, FALSE))) {
...
int spider_db_mysql_util::append_embedding_tables(
  ...
  if (embedding) {
    ...
    DBUG_PRINT("info",
               ("spider embedding->outer_join=%u", embedding->outer_join));
    DBUG_PRINT("info", ("spider embedding->on_expr=%p", embedding->on_expr));
    DBUG_PRINT("info", ("spider embedding->join_using_fields=%p",
                        embedding->join_using_fields));
    DBUG_PRINT("info", ("spider embedding->table=%p", embedding->table));
    ...
    if (table_list->outer_join & JOIN_TYPE_RIGHT) {
      if ((error_num = append_tables_top_down_check(table_list, used_table_list,
                                                    current_pos)))
        DBUG_RETURN(error_num);
      assert(!(*cond_table_list_ptr));
      (*cond_table_list_ptr) = table_list;
...
int spider_db_mysql_util::append_from_and_tables(ha_spider *spider,
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  used_table_list =
      (TABLE_LIST **)my_alloca(sizeof(TABLE_LIST *) * table_count);
  if (!used_table_list) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  ...
  do {
    ...
    if (table->const_table) continue;

    for (roop_count = 0; roop_count < current_pos; ++roop_count) {
      if (used_table_list[roop_count] == table_list) break;
    }
    if (roop_count < current_pos) continue;
    ...
    if ((error_num =
             append_table(spider, fields, str, table_list, used_table_list,
...
int spider_db_mysql_util::reappend_tables(spider_fields *fields,
  ...
  while ((table_holder = fields->get_next_table_holder())) {
    ...
    db_share = (spider_mysql_share *)spider->share->dbton_share[dbton_id];
    if (!db_share->same_db_table_name) {
      ...
      if ((error_num = db_share->append_table_name_with_adjusting(
               str, spider->conn_link_idx[link_idx_holder->link_idx]))) {
        DBUG_RETURN(error_num);
      }
    }
  }
...
int spider_db_mysql_util::append_where(spider_string *str) {
...
int spider_db_mysql_util::append_having(spider_string *str) {
  DBUG_ENTER("spider_db_mysql_util::append_having");
  DBUG_PRINT("info", ("spider this=%p", this));
  if (str->reserve(SPIDER_SQL_HAVING_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  str->q_append(SPIDER_SQL_HAVING_STR, SPIDER_SQL_HAVING_LEN);
  DBUG_RETURN(0);
}
#endif

spider_mysql_share::spider_mysql_share(st_spider_share *share)
    : spider_db_share(share),
      table_select(NULL),
      table_select_pos(0),
      key_select(NULL),
      key_select_pos(NULL),
...
spider_mysql_share::~spider_mysql_share() {
  DBUG_ENTER("spider_mysql_share::~spider_mysql_share");
  DBUG_PRINT("info", ("spider this=%p", this));
  if (table_select) delete[] table_select;
  if (key_select) delete[] key_select;
  if (key_hint) delete[] key_hint;
...
int spider_mysql_share::init() {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  if (!(key_select_pos = (int *)spider_bulk_alloc_mem(
            spider_current_trx, 112, __func__, __FILE__, __LINE__,
            MYF(MY_WME | MY_ZEROFILL), &key_select_pos, sizeof(int) * keys,
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
  ...
  if (keys > 0 && !(key_hint = new spider_string[keys])) {
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  }
  ...
  if (!(table_select = new spider_string[1]) ||
      (keys > 0 && !(key_select = new spider_string[keys])) ||
      (error_num = create_table_names_str()) ||
      (table_share && ((error_num = create_column_name_str()) ||
  ...
  for (roop_count = 0; roop_count < keys; roop_count++) {
    ...
    if ((error_num = append_key_select(roop_count))) DBUG_RETURN(error_num);
  }

  DBUG_RETURN(error_num);
}

uint spider_mysql_share::get_column_name_length(uint field_index) {
  DBUG_ENTER("spider_mysql_share::get_column_name_length");
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_RETURN(column_name_str[field_index].length());
}

int spider_mysql_share::append_column_name(spider_string *str,
                                           uint field_index,
                                           bool need_quote) {
  int error_num;
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  if (need_quote) {
    error_num = spider_db_mysql_utility.append_name(
        str, column_name_str[field_index].ptr(),
        column_name_str[field_index].length());
  } else {
    error_num = str->append(column_name_str[field_index].ptr(),
                            column_name_str[field_index].length());
  }
  DBUG_RETURN(error_num);
}

int spider_mysql_share::append_column_name_with_alias(spider_string *str,
                                                      uint field_index,
                                                      const char *alias,
                                                      uint alias_length) {
  DBUG_ENTER("spider_mysql_share::append_column_name_with_alias");
  DBUG_PRINT("info", ("spider this=%p", this));
  if (str->reserve(alias_length + column_name_str[field_index].length() +
...
int spider_mysql_share::append_table_name(spider_string *str,
                                          int all_link_idx) {
...
int spider_mysql_share::append_table_name_with_adjusting(spider_string *str,
                                                         int all_link_idx) {
  const char *db_nm = db_names_str[all_link_idx].ptr();
  uint db_nm_len = db_names_str[all_link_idx].length();
  ...
  str->q_append(SPIDER_SQL_DOT_STR, SPIDER_SQL_DOT_LEN);
  spider_db_mysql_utility.append_name(str, table_nm, table_nm_len);
  uint length = db_nm_max_len - db_nm_len + table_nm_max_len - table_nm_len;
  memset(const_cast<char *>(str->ptr()) + str->length(), ' ', length);
  str->length(str->length() + length);
  DBUG_RETURN(0);
}

int spider_mysql_share::append_from_with_adjusted_table_name(
    spider_string *str, int *table_name_pos) {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  if (str->reserve(SPIDER_SQL_FROM_LEN + db_nm_max_length + SPIDER_SQL_DOT_LEN +
                   table_nm_max_length +
                   /* SPIDER_SQL_NAME_QUOTE_LEN */ 4)) {
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  }
...
int spider_mysql_share::create_table_names_str() {
  ...
  for (roop_count = 0; roop_count < (int)spider_share->all_link_count;
    ...
    if (spider_share->sql_dbton_ids[roop_count] != dbton_id) continue;
    if (first_all_link_idx == -1) first_all_link_idx = roop_count;

    str = &table_names_str[roop_count];
    if (roop_count != 0 && same_db_table_name &&
      ...
      if ((error_num = spider_db_append_name_with_quote_str(
               str, spider_share->tgt_table_names[roop_count], dbton_id)))
        goto error;
      if (roop_count) {
    ...
    if (roop_count != 0 && same_db_table_name &&
        ...
        !memcmp(first_db_nm, spider_share->tgt_dbs[roop_count], db_nm_len)) {
      ...
      str->set_charset(spider_share->access_charset);
      if ((error_num = spider_db_append_name_with_quote_str(
               str, spider_share->tgt_dbs[roop_count], dbton_id)))
        goto error;
      if (roop_count) {
        same_db_table_name = FALSE;
        DBUG_PRINT("info", ("spider found different db name %s",
                            spider_share->tgt_dbs[roop_count]));
        if (str->length() > db_nm_max_length) db_nm_max_length = str->length();
      } else
    ...
    db_table_str_hash_value[roop_count] = spd_connect_pools.calc_hash(
      (uchar *)(const_cast<char*>(str->ptr())), str->length());
#endif
  }
  DBUG_RETURN(0);
...
int spider_mysql_share::create_column_name_str() {
  ...
  if (table_share->fields &&
    ...
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  for (field = table_share->field, str = column_name_str; *field;
       field++, str++) {
    str->init_calc_mem(89);
    str->set_charset(spider_share->access_charset);
    if ((*field)->gcol_info && (*field)->gcol_info->expr_str.str) {
      if (str->append((*field)->gcol_info->expr_str.str,
                      (*field)->gcol_info->expr_str.length, system_charset_info))
        goto error;
    } else {
      if ((error_num = spider_db_append_name_with_quote_str(
             str, const_cast<char *>((*field)->field_name), dbton_id)))
        goto error;
    }
  }
  DBUG_RETURN(0);
...
int spider_mysql_share::append_show_table_status() {
  ...
  for (roop_count = 0; roop_count < (int)spider_share->all_link_count;
    ...
    if (show_table_status[0 + (2 * roop_count)].reserve(
            SPIDER_SQL_SHOW_TABLE_STATUS_LEN +
            db_names_str[roop_count].length() + SPIDER_SQL_LIKE_LEN +
        ...
        show_table_status[1 + (2 * roop_count)].reserve(
            ...
            db_names_str[roop_count].length() + SPIDER_SQL_AND_LEN +
            SPIDER_SQL_TABLE_NAME_LEN + SPIDER_SQL_EQUAL_LEN +
            table_names_str[roop_count].length() +
            ((SPIDER_SQL_VALUE_QUOTE_LEN)*4)))
      goto error;
    str = &show_table_status[0 + (2 * roop_count)];
    str->q_append(SPIDER_SQL_SHOW_TABLE_STATUS_STR,
                  SPIDER_SQL_SHOW_TABLE_STATUS_LEN);
    str->q_append(SPIDER_SQL_NAME_QUOTE_STR, SPIDER_SQL_NAME_QUOTE_LEN);
    ...
    str->q_append(SPIDER_SQL_SELECT_TABLES_STATUS_STR,
                  SPIDER_SQL_SELECT_TABLES_STATUS_LEN);
    str->q_append(SPIDER_SQL_VALUE_QUOTE_STR, SPIDER_SQL_VALUE_QUOTE_LEN);
    str->q_append(db_names_str[roop_count].ptr(),
                  db_names_str[roop_count].length());
...
int spider_mysql_share::append_show_records() {
  ...
  for (roop_count = 0; roop_count < (int)spider_share->all_link_count;
    ...
    if (show_records[roop_count].reserve(
            SPIDER_SQL_SHOW_RECORDS_LEN + db_names_str[roop_count].length() +
...
int spider_mysql_share::append_show_index() {
  ...
  for (roop_count = 0; roop_count < (int)spider_share->all_link_count;
    ...
    if (show_index[0 + (2 * roop_count)].reserve(
        ...
        show_index[1 + (2 * roop_count)].reserve(
            ...
            SPIDER_SQL_TABLE_NAME_LEN + SPIDER_SQL_EQUAL_LEN +
            table_names_str[roop_count].length() +
            ((SPIDER_SQL_VALUE_QUOTE_LEN)*4) + SPIDER_SQL_GROUP_LEN +
            SPIDER_SQL_COLUMN_NAME_LEN))
      goto error;
    str = &show_index[0 + (2 * roop_count)];
    str->q_append(SPIDER_SQL_SHOW_INDEX_STR, SPIDER_SQL_SHOW_INDEX_LEN);
    append_table_name(str, roop_count);
    str = &show_index[1 + (2 * roop_count)];
    str->q_append(SPIDER_SQL_SELECT_STATISTICS_STR,
                  SPIDER_SQL_SELECT_STATISTICS_LEN);
    str->q_append(SPIDER_SQL_VALUE_QUOTE_STR, SPIDER_SQL_VALUE_QUOTE_LEN);
    str->q_append(db_names_str[roop_count].ptr(),
                  db_names_str[roop_count].length());
...
int spider_mysql_share::append_table_select() {
  ...
  if (!*table_share->field) DBUG_RETURN(0);
  ...
  for (field = table_share->field; *field; field++) {
    ...
    if (str->reserve(field_length +
      ...
      DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    append_column_name(str, (*field)->field_index(), !(*field)->gcol_info);
    str->q_append(SPIDER_SQL_COMMA_STR, SPIDER_SQL_COMMA_LEN);
  }
  str->length(str->length() - SPIDER_SQL_COMMA_LEN);
...
int spider_mysql_share::append_key_select(uint idx) {
  KEY_PART_INFO *key_part;
  Field *field;
  uint part_num;
  uint field_length;
  ...
  if (!spider_user_defined_key_parts(key_info)) DBUG_RETURN(0);
...
#ifdef SPIDER_HAS_DISCOVER_TABLE_STRUCTURE
int spider_mysql_share::discover_table_structure(SPIDER_TRX *trx,
  ...
  for (roop_count = 0; roop_count < (int)spider_share->all_link_count;
    ...
    if (sql_str.reserve(
            ...
            SPIDER_SQL_AND_LEN + SPIDER_SQL_TABLE_NAME_LEN +
            SPIDER_SQL_EQUAL_LEN + table_names_str[roop_count].length() +
            SPIDER_SQL_ORDER_LEN + SPIDER_SQL_ORDINAL_POSITION_LEN +
            /* SPIDER_SQL_VALUE_QUOTE_LEN */ 8 + SPIDER_SQL_SEMICOLON_LEN +
            SPIDER_SQL_SHOW_INDEX_LEN + db_names_str[roop_count].length() +
            SPIDER_SQL_DOT_LEN + table_names_str[roop_count].length() +
            /* SPIDER_SQL_NAME_QUOTE_LEN */ 4 + SPIDER_SQL_SEMICOLON_LEN +
            SPIDER_SQL_SHOW_TABLE_STATUS_LEN +
            db_names_str[roop_count].length() + SPIDER_SQL_LIKE_LEN +
            table_names_str[roop_count].length() +
            /* SPIDER_SQL_NAME_QUOTE_LEN */ 4)) {
    ...
    sql_str.q_append(SPIDER_SQL_SEMICOLON_STR, SPIDER_SQL_SEMICOLON_LEN);
    sql_str.q_append(SPIDER_SQL_SHOW_TABLE_STATUS_STR,
                     SPIDER_SQL_SHOW_TABLE_STATUS_LEN);
    ...
    SPIDER_CONN *conn;
    int need_mon;
    if (!(conn = spider_get_conn(spider_share, 0, trx, NULL, FALSE, FALSE,
    ...
    if ((error_num = res->fetch_columns_for_discover_table_structure(
    ...
    if ((error_num = res->fetch_index_for_discover_table_structure(
             str, spider_share->access_charset))) {
    ...
    if ((error_num = res->fetch_table_for_discover_table_structure(
             str, spider_share, spider_share->access_charset))) {
...
spider_mysql_handler::spider_mysql_handler(ha_spider *spider,
                                           spider_mysql_share *db_share)
    : spider_db_handler(spider, db_share),
      where_pos(0),
      order_pos(0),
      limit_pos(0),
      table_name_pos(0),
...
int spider_mysql_handler::init() {
  ...
  // func_count is field count now
  upd_tmp_tbl_prm.func_count = 1;
  if (!(link_for_hash = (SPIDER_LINK_FOR_HASH *)spider_bulk_alloc_mem(
            spider_current_trx, 141, __func__, __FILE__, __LINE__,
            MYF(MY_WME | MY_ZEROFILL), &link_for_hash,
...
int spider_mysql_handler::append_index_hint(spider_string *str, int link_idx[[maybe_unused]],
                                            ulong sql_type) {
    ...
    if (sql_type != SPIDER_SQL_TYPE_HANDLER) {
      switch (hint->type) {
        case INDEX_HINT_IGNORE:
          if (str->reserve(hint->key_name.length + SPIDER_SQL_INDEX_IGNORE_LEN +
                           SPIDER_SQL_OPEN_PAREN_LEN +
                           SPIDER_SQL_CLOSE_PAREN_LEN))
            DBUG_RETURN(HA_ERR_OUT_OF_MEM);
          str->q_append(SPIDER_SQL_INDEX_IGNORE_STR,
          ...
          break;
        case INDEX_HINT_USE:
          if (str->reserve(hint->key_name.length + SPIDER_SQL_INDEX_USE_LEN +
                           SPIDER_SQL_OPEN_PAREN_LEN +
                           SPIDER_SQL_CLOSE_PAREN_LEN))
            DBUG_RETURN(HA_ERR_OUT_OF_MEM);
        ...
        case INDEX_HINT_FORCE:
          if (str->reserve(hint->key_name.length + SPIDER_SQL_INDEX_FORCE_LEN +
                           SPIDER_SQL_OPEN_PAREN_LEN +
                           SPIDER_SQL_CLOSE_PAREN_LEN))
            DBUG_RETURN(HA_ERR_OUT_OF_MEM);
...
int spider_mysql_handler::append_table_name_with_adjusting(spider_string *str,
                                                           int link_idx,
                                                           ulong sql_type) {
  int error_num = 0;
  ...
  if (sql_type == SPIDER_SQL_TYPE_HANDLER) {
    ...
    error_num = mysql_share->append_table_name_with_adjusting(
        str, spider->conn_link_idx[link_idx]);
  }
  DBUG_RETURN(error_num);
}

int spider_mysql_handler::append_key_column_types(const key_range *start_key,
                                                  spider_string *str) {
  ...
  if (!start_key_part_map) DBUG_RETURN(0);

  for (key_part = key_info->key_part, key_count = 0; start_key_part_map;
       start_key_part_map >>= 1, key_part++, key_count++) {
    field = key_part->field;
    ...
    if (str->reserve(SPIDER_SQL_COMMA_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    str->q_append(SPIDER_SQL_COMMA_STR, SPIDER_SQL_COMMA_LEN);
  }
  str->length(str->length() - SPIDER_SQL_COMMA_LEN);
...
int spider_mysql_handler::append_key_join_columns_for_bka(
    const key_range *start_key, spider_string *str, const char **table_aliases,
  ...
  for (key_part = key_info->key_part, key_count = 0; start_key_part_map;
       start_key_part_map >>= 1, key_part++, key_count++) {
    field = key_part->field;
    key_name_length = mysql_share->column_name_str[field->field_index()].length();
    length = my_sprintf(tmp_buf, (tmp_buf, "c%u", key_count));
    if (str->reserve(length + table_alias_lengths[0] + key_name_length +
                     /* SPIDER_SQL_NAME_QUOTE_LEN */ 2 +
                     table_alias_lengths[1] + SPIDER_SQL_PF_EQUAL_LEN +
                     SPIDER_SQL_AND_LEN))
      DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    ...
    mysql_share->append_column_name(str, field->field_index(), !field->gcol_info);
    str->q_append(SPIDER_SQL_AND_STR, SPIDER_SQL_AND_LEN);
  }
...
int spider_mysql_handler::append_tmp_table_and_sql_for_bka(
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  char tmp_table_name[MAX_FIELD_WIDTH * 2], tgt_table_name[MAX_FIELD_WIDTH * 2];
  ...
  create_tmp_bka_table_name(tmp_table_name, &tmp_table_name_length,
                            first_link_idx);
  if ((error_num = append_table_name_with_adjusting(
           &tgt_table_name_str, first_link_idx, SPIDER_SQL_TYPE_SELECT_SQL))) {
    DBUG_RETURN(error_num);
  }
  ...
  if ((error_num = append_drop_tmp_bka_table(
           &tmp_sql, tmp_table_name, tmp_table_name_length, &tmp_sql_pos1,
           &tmp_sql_pos5, TRUE)) ||
      (error_num = append_create_tmp_bka_table(
           start_key, &tmp_sql, tmp_table_name, tmp_table_name_length,
           &tmp_sql_pos2, const_cast<CHARSET_INFO*>((spider->share->table_share->table_charset)))) ||
      (error_num =
           append_insert_tmp_bka_table(start_key, &tmp_sql, tmp_table_name,
                                       tmp_table_name_length, &tmp_sql_pos3)))
    DBUG_RETURN(error_num);
  tmp_sql_pos4 = tmp_sql.length();
  if ((error_num = spider_db_append_select(spider))) DBUG_RETURN(error_num);
  ...
  if ((error_num = append_select_columns_with_alias(&sql, SPIDER_SQL_B_DOT_STR,
                                                    SPIDER_SQL_B_DOT_LEN)) ||
      (error_num = spider_db_mysql_utility.append_from_with_alias(
           &sql, table_names, table_name_lengths, table_aliases,
           table_alias_lengths, 2, &table_name_pos, FALSE)))
    DBUG_RETURN(error_num);
  if (mysql_share->key_hint &&
      (error_num = spider_db_append_hint_after_table(
           spider, &sql, &mysql_share->key_hint[spider->active_index])))
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  where_pos = sql.length();
  if ((error_num = append_key_join_columns_for_bka(
           start_key, &sql, table_dot_aliases, table_dot_alias_lengths)) ||
      (error_num =
           append_condition_part(SPIDER_SQL_B_DOT_STR, SPIDER_SQL_B_DOT_LEN,
  ...
  if (spider->result_list.direct_order_limit) {
    if ((error_num = append_key_order_for_direct_order_limit_with_alias(
             &sql, SPIDER_SQL_B_DOT_STR, SPIDER_SQL_B_DOT_LEN)))
      DBUG_RETURN(error_num);
  }
#ifdef HANDLER_HAS_DIRECT_AGGREGATE
...
void spider_mysql_handler::create_tmp_bka_table_name(char *tmp_table_name,
                                                     int *tmp_table_name_length,
                                                     int link_idx) {
  uint adjust_length, length;
  DBUG_ENTER("spider_mysql_handler::create_tmp_bka_table_name");
  if (spider_param_bka_table_name_type(
          current_thd,
    ...
    *tmp_table_name_length =
        mysql_share->db_nm_max_length + mysql_share->table_nm_max_length;
    ...
    length =
        my_sprintf(tmp_table_name,
                   (tmp_table_name, "%s%s%p%s", SPIDER_SQL_DOT_STR,
                    SPIDER_SQL_TMP_BKA_STR, spider, SPIDER_SQL_UNDERSCORE_STR));
    ...
    *tmp_table_name_length = mysql_share->db_nm_max_length;
    ...
    memcpy(tmp_table_name, mysql_share->db_names_str[link_idx].c_ptr(),
           mysql_share->db_names_str[link_idx].length());
    tmp_table_name += mysql_share->db_names_str[link_idx].length();
    length = my_sprintf(tmp_table_name,
                        (tmp_table_name, "%s%s%p", SPIDER_SQL_DOT_STR,
                         SPIDER_SQL_TMP_BKA_STR, spider));
    *tmp_table_name_length += length;
  }
  DBUG_VOID_RETURN;
}
...
int spider_mysql_handler::append_create_tmp_bka_table(
    const key_range *start_key, spider_string *str, char *tmp_table_name,
    int tmp_table_name_length, int *db_name_pos, CHARSET_INFO *table_charset) {
  int error_num;
  SPIDER_SHARE *share = spider->share;
  THD *thd = spider->trx->thd;
  ...
  if (str->reserve(SPIDER_SQL_CREATE_TMP_LEN + tmp_table_name_length +
                   SPIDER_SQL_OPEN_PAREN_LEN + SPIDER_SQL_ID_LEN +
                   SPIDER_SQL_ID_TYPE_LEN + SPIDER_SQL_COMMA_LEN))
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  ...
  *db_name_pos = str->length();
  ...
  if (str->reserve(SPIDER_SQL_ENGINE_LEN + bka_engine_length +
                   SPIDER_SQL_DEF_CHARSET_LEN + cset_length +
                   SPIDER_SQL_COLLATE_LEN + coll_length +
...
int spider_mysql_handler::append_drop_tmp_bka_table(
    spider_string *str, char *tmp_table_name, int tmp_table_name_length,
    int *db_name_pos, int *drop_table_end_pos, bool with_semicolon) {
...
int spider_mysql_handler::append_insert_tmp_bka_table(
  ...
  if (str->reserve(SPIDER_SQL_INSERT_LEN + SPIDER_SQL_INTO_LEN +
                   tmp_table_name_length + SPIDER_SQL_OPEN_PAREN_LEN +
...
int spider_mysql_handler::append_union_table_and_sql_for_bka(
  ...
  if ((error_num = spider_db_mysql_utility.append_from_with_alias(
           &tmp_sql, table_names, table_name_lengths, table_aliases,
           table_alias_lengths, 2, &table_name_pos, FALSE)))
    DBUG_RETURN(error_num);
  if (mysql_share->key_hint &&
      (error_num = spider_db_append_hint_after_table(
           spider, &tmp_sql, &mysql_share->key_hint[spider->active_index])))
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  where_pos = tmp_sql.length();
  if ((error_num = append_key_join_columns_for_bka(
           start_key, &tmp_sql, table_dot_aliases, table_dot_alias_lengths)) ||
      (error_num =
           append_condition_part(SPIDER_SQL_B_DOT_STR, SPIDER_SQL_B_DOT_LEN,
  ...
  if (spider->result_list.direct_order_limit) {
    ...
    if ((error_num = append_group_by(&tmp_sql, SPIDER_SQL_B_DOT_STR,
                                     SPIDER_SQL_B_DOT_LEN)))
      DBUG_RETURN(error_num);
  }
#endif
...
int spider_mysql_handler::append_insert_for_recovery(ulong sql_type,
                                                     int link_idx) {
  ...
  for (field = table->field; *field; field++) {
    ...
    if (insert_sql->reserve(field_name_length +
                            ...
                            SPIDER_SQL_COMMA_LEN))
      DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    mysql_share->append_column_name(insert_sql, (*field)->field_index(), !(*field)->gcol_info);
  ...
  for (field = table->field; *field; field++) {
    ...
    if ((*field)->is_null()) {
      ...
      if (spider_db_mysql_utility.append_column_value(
              spider, insert_sql, *field, NULL, false, share->access_charset) ||
...
int spider_mysql_handler::append_update(const TABLE *table,
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  if (str->length() > 0) {
    if (str->reserve(SPIDER_SQL_SEMICOLON_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    str->q_append(SPIDER_SQL_SEMICOLON_STR, SPIDER_SQL_SEMICOLON_LEN);
  }

  if ((error_num = append_update(str, 0)) ||
...
int spider_mysql_handler::append_update(const TABLE *table,
                                        my_ptrdiff_t ptr_diff, int link_idx) {
  int error_num;
  SPIDER_SHARE *share = spider->share;
  ...
  if ((error_num = append_update(str, link_idx)) ||
      (error_num = append_update_set(str)) ||
      (error_num = append_update_where(str, table, ptr_diff)))
    DBUG_RETURN(error_num);
  ...
  if (spider->pk_update &&
    ...
    str->q_append(SPIDER_SQL_SEMICOLON_STR, SPIDER_SQL_SEMICOLON_LEN);
    if ((error_num =
             append_insert_for_recovery(SPIDER_SQL_TYPE_UPDATE_SQL, link_idx)))
      DBUG_RETURN(error_num);
  }
...
int spider_mysql_handler::append_delete(const TABLE *table,
                                        my_ptrdiff_t ptr_diff) {
  int error_num;
  spider_string *str = &update_sql;
  ...
  if ((error_num = append_delete(str)) ||
      (error_num =
           append_from(str, SPIDER_SQL_TYPE_DELETE_SQL, first_link_idx)) ||
      (error_num = append_update_where(str, table, ptr_diff)))
    DBUG_RETURN(error_num);
  filled_up = (str->length() >= (uint)spider->result_list.bulk_update_size);
  DBUG_RETURN(0);
...
int spider_mysql_handler::append_insert(spider_string *str, int link_idx[[maybe_unused]]) {
  SPIDER_SHARE *share = spider->share;
  ...
  if ((spider->write_can_replace ||
       ...
       (spider->sql_command == SQLCOM_LOAD &&
        spider->lex_duplicates == DUP_REPLACE)) &&
      spider->direct_dup_insert) {
    if (str->reserve(SPIDER_SQL_REPLACE_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    str->q_append(SPIDER_SQL_REPLACE_STR, SPIDER_SQL_REPLACE_LEN);
  } else {
    if (str->reserve(SPIDER_SQL_INSERT_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    str->q_append(SPIDER_SQL_INSERT_STR, SPIDER_SQL_INSERT_LEN);
  }
  ...
  if (spider->ignore_dup_key && !spider->write_can_replace &&
    (spider->direct_dup_insert && spider->direct_insert_ignore) &&
      ...
      !(spider->sql_command == SQLCOM_LOAD &&
        spider->lex_duplicates == DUP_REPLACE)) {
    if (str->reserve(SPIDER_SQL_SQL_IGNORE_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
...
int spider_mysql_handler::append_update(spider_string *str, int link_idx) {
  DBUG_ENTER("spider_mysql_handler::append_update");
  if (str->reserve(SPIDER_SQL_UPDATE_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  ...
  if (spider->ignore_dup_key && !spider->insert_with_update) {
    if (str->reserve(SPIDER_SQL_SQL_IGNORE_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    str->q_append(SPIDER_SQL_SQL_IGNORE_STR, SPIDER_SQL_SQL_IGNORE_LEN);
  }
  if (str->reserve(mysql_share->db_nm_max_length + SPIDER_SQL_DOT_LEN +
                   mysql_share->table_nm_max_length +
                   /* SPIDER_SQL_NAME_QUOTE_LEN */ 4))
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  table_name_pos = str->length();
...
int spider_mysql_handler::append_delete(spider_string *str) {
  DBUG_ENTER("spider_mysql_handler::append_delete");
  if (str->reserve(SPIDER_SQL_DELETE_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  ...
  if (spider->quick_mode) {
    if (str->reserve(SPIDER_SQL_SQL_QUICK_MODE_LEN))
      DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    str->q_append(SPIDER_SQL_SQL_QUICK_MODE_STR, SPIDER_SQL_SQL_QUICK_MODE_LEN);
  }
...
int spider_mysql_handler::append_update_set(spider_string *str) {
  uint field_name_length;
  SPIDER_SHARE *share = spider->share;
  TABLE *table = spider->get_table();
  Field **fields;
  DBUG_ENTER("spider_mysql_handler::append_update_set");
  if (str->reserve(SPIDER_SQL_SET_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
...
int spider_mysql_handler::append_direct_update_set(spider_string *str) {
  ...
  if (spider->direct_update_kinds == SPIDER_SQL_KIND_SQL &&
    ...
    DBUG_RETURN(spider_db_append_update_columns(
        spider, str, NULL, 0, spider_dbton_mysql.dbton_id, FALSE, NULL));
  }
...
int spider_mysql_handler::append_dup_update_pushdown_part(const char *alias,
                                                          uint alias_length) {
  int error_num;
...
int spider_mysql_handler::append_update_columns_part(const char *alias,
                                                     uint alias_length) {
  int error_num;
...
int spider_mysql_handler::append_update_columns(spider_string *str,
                                                const char *alias,
                                                uint alias_length) {
  int error_num;
  DBUG_ENTER("spider_mysql_handler::append_update_columns");
  error_num =
      spider_db_append_update_columns(spider, str, alias, alias_length,
                                      spider_dbton_mysql.dbton_id, FALSE, NULL);
  DBUG_RETURN(error_num);
}
#endif

int spider_mysql_handler::append_select_part(ulong sql_type) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_select(spider_string *str, ulong sql_type) {
  SPIDER_RESULT_LIST *result_list = &spider->result_list;
  DBUG_ENTER("spider_mysql_handler::append_select");
  if (sql_type == SPIDER_SQL_TYPE_HANDLER) {
    if (str->reserve(SPIDER_SQL_HANDLER_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    str->q_append(SPIDER_SQL_HANDLER_STR, SPIDER_SQL_HANDLER_LEN);
  } else {
    if (str->reserve(SPIDER_SQL_SELECT_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    str->q_append(SPIDER_SQL_SELECT_STR, SPIDER_SQL_SELECT_LEN);
    if (result_list->direct_distinct) {
      if (str->reserve(SPIDER_SQL_DISTINCT_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
      str->q_append(SPIDER_SQL_DISTINCT_STR, SPIDER_SQL_DISTINCT_LEN);
    }
    if (result_list->lock_type != F_WRLCK && spider->lock_mode < 1) {
      /* no lock */
      /*
      Query_block *select_lex = &spider->trx->thd->lex->query_block;
      if (select_lex->sql_cache == SELECT_LEX::SQL_CACHE &&
      ...
      } else if (select_lex->sql_cache == SELECT_LEX::SQL_NO_CACHE &&
...
int spider_mysql_handler::append_table_select_part(ulong sql_type) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_table_select(spider_string *str) {
  DBUG_ENTER("spider_mysql_handler::append_table_select");
  table_name_pos = str->length() + mysql_share->table_select_pos;
  if (str->append(*(mysql_share->table_select))) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  DBUG_RETURN(0);
}

int spider_mysql_handler::append_key_select_part(ulong sql_type, uint idx) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_key_select(spider_string *str, uint idx) {
  DBUG_ENTER("spider_mysql_handler::append_key_select");
  table_name_pos = str->length() + mysql_share->key_select_pos[idx];
  if (str->append(mysql_share->key_select[idx])) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  DBUG_RETURN(0);
}

int spider_mysql_handler::append_minimum_select_part(ulong sql_type) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_minimum_select(spider_string *str,
                                                ulong sql_type) {
  TABLE *table = spider->get_table();
  Field **field;
  int field_length;
  ...
  if (appended)
    ...
    if (str->reserve(SPIDER_SQL_ONE_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    str->q_append(SPIDER_SQL_ONE_STR, SPIDER_SQL_ONE_LEN);
  }
  DBUG_RETURN(append_from(str, sql_type, first_link_idx));
}

int spider_mysql_handler::append_table_select_with_alias(spider_string *str,
                                                         const char *alias,
                                                         uint alias_length) {
  TABLE *table = spider->get_table();
  Field **field;
...
int spider_mysql_handler::append_key_select_with_alias(spider_string *str,
                                                       const KEY *key_info,
                                                       const char *alias,
                                                       uint alias_length) {
  KEY_PART_INFO *key_part;
...
int spider_mysql_handler::append_minimum_select_with_alias(spider_string *str,
                                                           const char *alias,
                                                           uint alias_length) {
  TABLE *table = spider->get_table();
  Field **field;
...
int spider_mysql_handler::append_select_columns_with_alias(spider_string *str,
                                                           const char *alias,
                                                           uint alias_length) {
  int error_num;
  SPIDER_RESULT_LIST *result_list = &spider->result_list;
  ...
  if (!spider->select_column_mode) {
    if (result_list->keyread)
      DBUG_RETURN(append_key_select_with_alias(str, result_list->key_info,
                                               alias, alias_length));
    else
      DBUG_RETURN(append_table_select_with_alias(str, alias, alias_length));
  }
  DBUG_RETURN(append_minimum_select_with_alias(str, alias, alias_length));
}

int spider_mysql_handler::append_hint_after_table_part(ulong sql_type) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_hint_after_table(spider_string *str) {
  int error_num;
  DBUG_ENTER("spider_mysql_handler::append_hint_after_table");
  DBUG_PRINT("info", ("spider this=%p", this));
  if (mysql_share->key_hint &&
      (error_num = spider_db_append_hint_after_table(
           spider, str, &mysql_share->key_hint[spider->active_index])))
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  DBUG_RETURN(0);
}

void spider_mysql_handler::set_where_pos(ulong sql_type) {
...
void spider_mysql_handler::set_where_to_pos(ulong sql_type) {
...
int spider_mysql_handler::check_item_type(Item *item) {
  int error_num;
  DBUG_ENTER("spider_mysql_handler::check_item_type");
  DBUG_PRINT("info", ("spider this=%p", this));
  error_num = spider_db_print_item_type(
      item, spider, NULL, NULL, 0, spider_dbton_mysql.dbton_id, FALSE, NULL);
  DBUG_RETURN(error_num);
}

int spider_mysql_handler::append_values_connector_part(ulong sql_type) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_values_connector(spider_string *str) {
...
int spider_mysql_handler::append_values_terminator_part(ulong sql_type) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_values_terminator(spider_string *str) {
  DBUG_ENTER("spider_mysql_handler::append_values_terminator");
  DBUG_PRINT("info", ("spider this=%p", this));
  str->length(str->length() - SPIDER_SQL_COMMA_LEN - SPIDER_SQL_OPEN_PAREN_LEN);
  DBUG_RETURN(0);
}

int spider_mysql_handler::append_union_table_connector_part(ulong sql_type) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_union_table_connector(spider_string *str) {
...
int spider_mysql_handler::append_union_table_terminator_part(ulong sql_type) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_union_table_terminator(spider_string *str) {
...
int spider_mysql_handler::append_key_column_values_part(
    const key_range *start_key, ulong sql_type) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_key_column_values(spider_string *str,
                                                   const key_range *start_key) {
  int error_num;
  const uchar *ptr;
  SPIDER_RESULT_LIST *result_list = &spider->result_list;
  ...
  for (key_part = key_info->key_part, length = 0; start_key_part_map;
       start_key_part_map >>= 1, key_part++, length += store_length) {
    store_length = key_part->store_length;
    ptr = start_key->key + length;
    field = key_part->field;
    if ((error_num = spider_db_append_null_value(str, key_part, &ptr))) {
      if (error_num > 0) DBUG_RETURN(error_num);
    } else {
      if (spider_db_mysql_utility.append_column_value(
...
int spider_mysql_handler::append_key_column_values_with_name(
    spider_string *str, const key_range *start_key) {
  int error_num;
  const uchar *ptr;
  SPIDER_RESULT_LIST *result_list = &spider->result_list;
  SPIDER_SHARE *share = spider->share;
  ...
  for (key_part = key_info->key_part, length = 0, key_count = 0;
       start_key_part_map; start_key_part_map >>= 1, key_part++,
      length += store_length, key_count++) {
    store_length = key_part->store_length;
    ptr = start_key->key + length;
    field = key_part->field;
    if ((error_num = spider_db_append_null_value(str, key_part, &ptr))) {
...
int spider_mysql_handler::append_key_where_part(const key_range *start_key,
...
int spider_mysql_handler::append_key_where(spider_string *str,
  ...
  error_num = spider_db_append_key_where_internal(
      str, str_part, str_part2, start_key, end_key, spider, set_order, sql_type,
      spider_dbton_mysql.dbton_id);
  DBUG_RETURN(error_num);
}

int spider_mysql_handler::append_is_null_part(ulong sql_type,
                                              KEY_PART_INFO *key_part,
                                              const key_range *key,
                                              const uchar **ptr, bool key_eq,
                                              bool tgt_final) {
  int error_num;
  spider_string *str, *str_part = NULL, *str_part2 = NULL;
  ...
  error_num = append_is_null(sql_type, str, str_part, str_part2, key_part, key,
                             ptr, key_eq, tgt_final);
  DBUG_RETURN(error_num);
}

int spider_mysql_handler::append_is_null(
    ulong sql_type, spider_string *str, spider_string *str_part,
    spider_string *str_part2, KEY_PART_INFO *key_part, const key_range *key,
  ...
  if (key_part->null_bit) {
    if (*(*ptr)++) {
      if (sql_type == SPIDER_SQL_TYPE_HANDLER) {
        if (key_eq || key->flag == HA_READ_KEY_EXACT ||
          ...
          str = str_part;
          if (str->reserve(SPIDER_SQL_NULL_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
          str->q_append(SPIDER_SQL_NULL_STR, SPIDER_SQL_NULL_LEN);
        } else {
          if (str_part->length() == SPIDER_SQL_OPEN_PAREN_LEN) {
            ...
            if (str->reserve(SPIDER_SQL_GT_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
      ...
      if (key_eq || key->flag == HA_READ_KEY_EXACT ||
        ...
        if (str->reserve(
                ...
                /* SPIDER_SQL_NAME_QUOTE_LEN */ 2 +
                mysql_share->column_name_str[key_part->field->field_index()]
                    .length()))
          DBUG_RETURN(HA_ERR_OUT_OF_MEM);
...
int spider_mysql_handler::append_where_terminator_part(ulong sql_type,
                                                       bool set_order,
                                                       int key_count) {
  int error_num;
  spider_string *str, *str_part = NULL, *str_part2 = NULL;
...
int spider_mysql_handler::append_where_terminator(
    ulong sql_type, spider_string *str, spider_string *str_part,
    spider_string *str_part2, bool set_order, int key_count) {
  SPIDER_RESULT_LIST *result_list = &spider->result_list;
  ...
  if (sql_type != SPIDER_SQL_TYPE_HANDLER) {
    ...
    if (!result_list->ha_read_kind)
      str_part->q_append(SPIDER_SQL_CLOSE_PAREN_STR,
                         SPIDER_SQL_CLOSE_PAREN_LEN);
    if (str->append(*str_part)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    ...
    if (clause_length < SPIDER_SQL_NEXT_LEN) {
      ...
      if (str->reserve(clause_length)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
...
int spider_mysql_handler::append_match_where_part(ulong sql_type) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_match_where(spider_string *str) {
  int error_num;
  bool first = TRUE;
  ...
  if (spider->ft_current) {
    while (TRUE) {
      ...
      if (ft_info == spider->ft_current) break;
      ft_info = ft_info->next;
    }
    if (!first) str->length(str->length() - SPIDER_SQL_AND_LEN);
  }
  DBUG_RETURN(0);
}

int spider_mysql_handler::append_update_where(spider_string *str,
                                              const TABLE *table,
                                              my_ptrdiff_t ptr_diff) {
  uint field_name_length;
  ...
  str->length(str->length() - SPIDER_SQL_AND_LEN);
  if (str->reserve(SPIDER_SQL_LIMIT1_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  str->q_append(SPIDER_SQL_LIMIT1_STR, SPIDER_SQL_LIMIT1_LEN);
  DBUG_RETURN(0);
}

int spider_mysql_handler::append_condition_part(const char *alias,
                                                uint alias_length,
  ...
  error_num = append_condition(str, alias, alias_length, start_where, sql_type);
  DBUG_RETURN(error_num);
}

int spider_mysql_handler::append_condition(spider_string *str,
                                           const char *alias, uint alias_length,
                                           bool start_where, ulong sql_type[[maybe_unused]]) {
...
int spider_mysql_handler::append_match_against_part(ulong sql_type,
...
int spider_mysql_handler::append_match_against(spider_string *str,
                                               st_spider_ft_info *ft_info,
                                               const char *alias,
                                               uint alias_length) {
  SPIDER_SHARE *share = spider->share;
  ...
  for (key_part = key_info->key_part, key_count = 0;
  ...
  tmp_str.length(0);
  if (tmp_str.append(ft_init_key->ptr(), ft_init_key->length(),
                     const_cast<CHARSET_INFO*>((ft_init_key->charset()))) ||
  ...
  if (str->reserve(
          ...
          ((ft_info->flags & FT_EXPAND) ? SPIDER_SQL_WITH_QUERY_EXPANSION_LEN
                                        : 0)))
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  str->q_append(SPIDER_SQL_VALUE_QUOTE_STR, SPIDER_SQL_VALUE_QUOTE_LEN);
...
int spider_mysql_handler::append_match_select_part(ulong sql_type,
                                                   const char *alias,
                                                   uint alias_length) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_match_select(spider_string *str,
                                              const char *alias,
                                              uint alias_length) {
  int error_num;
  ...
  if (spider->ft_current) {
    st_spider_ft_info *ft_info = spider->ft_first;
    while (TRUE) {
      if ((error_num = append_match_against(str, ft_info, alias, alias_length)))
        DBUG_RETURN(error_num);
      if (str->reserve(SPIDER_SQL_COMMA_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
      str->q_append(SPIDER_SQL_COMMA_STR, SPIDER_SQL_COMMA_LEN);
      if (ft_info == spider->ft_current) break;
...
#ifdef HANDLER_HAS_DIRECT_AGGREGATE
int spider_mysql_handler::append_sum_select_part(ulong sql_type,
                                                 const char *alias,
                                                 uint alias_length) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_sum_select(spider_string *str,
                                            const char *alias,
                                            uint alias_length) {
  int error_num;
  ...
  for (item_sum_ptr = join->sum_funcs; *item_sum_ptr; ++item_sum_ptr) {
    if ((error_num = spider_db_mysql_utility.open_item_sum_func(
             *item_sum_ptr, spider, str, alias, alias_length, FALSE, NULL,
             NULL))) {
      DBUG_RETURN(error_num);
    }
    if (str->reserve(SPIDER_SQL_COMMA_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
...
void spider_mysql_handler::set_order_pos(ulong sql_type) {
...
void spider_mysql_handler::set_order_to_pos(ulong sql_type) {
...
#ifdef HANDLER_HAS_DIRECT_AGGREGATE
int spider_mysql_handler::append_group_by_part(const char *alias,
                                               uint alias_length,
                                               ulong sql_type) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_group_by(spider_string *str, const char *alias,
                                          uint alias_length) {
  int error_num;
  Query_block *select_lex;
  ...
  if (group) {
    if (str->reserve(SPIDER_SQL_GROUP_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    str->q_append(SPIDER_SQL_GROUP_STR, SPIDER_SQL_GROUP_LEN);
    for (; group; group = group->next) {
      if ((error_num = spider_db_print_item_type(
               (*group->item), spider, str, alias, alias_length,
               spider_dbton_mysql.dbton_id, FALSE, NULL))) {
        DBUG_RETURN(error_num);
      }
      if (str->reserve(SPIDER_SQL_COMMA_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
...
int spider_mysql_handler::append_key_order_for_merge_with_alias_part(
  ...
  spider_string *str;
  DBUG_ENTER(
      "spider_mysql_handler::append_key_order_for_merge_with_alias_part");
  DBUG_PRINT("info", ("spider this=%p", this));
  switch (sql_type) {
    case SPIDER_SQL_TYPE_SELECT_SQL:
    case SPIDER_SQL_TYPE_TMP_SQL:
...
int spider_mysql_handler::append_key_order_for_merge_with_alias(
  ...
  if (table->s->primary_key < MAX_KEY) {
    ...
    KEY_PART_INFO *key_part;
    for (key_part = key_info->key_part, length = 1;
      ...
      key_name_length =
          mysql_share->column_name_str[field->field_index()].length();
      if (length == 1) {
        if (str->reserve(SPIDER_SQL_ORDER_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    ...
    /* sort by all columns */
    Field **fieldp;
    for (fieldp = table->field, length = 1; *fieldp; fieldp++, length++) {
      key_name_length =
      ...
      if (str->reserve(alias_length + key_name_length +
        ...
        DBUG_RETURN(HA_ERR_OUT_OF_MEM);
      str->q_append(alias, alias_length);
      mysql_share->append_column_name(str, (*fieldp)->field_index(), !(*fieldp)->gcol_info);
      str->q_append(SPIDER_SQL_COMMA_STR, SPIDER_SQL_COMMA_LEN);
    }
    if (length > 1) {
      str->length(str->length() - SPIDER_SQL_COMMA_LEN);
...
int spider_mysql_handler::
    append_key_order_for_direct_order_limit_with_alias_part(const char *alias,
                                                            uint alias_length,
                                                            ulong sql_type) {
  int error_num;
  spider_string *str;
  DBUG_ENTER(
      "spider_mysql_handler::append_key_order_for_direct_order_limit_with_"
      "alias_part");
  DBUG_PRINT("info", ("spider this=%p", this));
  switch (sql_type) {
    case SPIDER_SQL_TYPE_SELECT_SQL:
  ...
  error_num = append_key_order_for_direct_order_limit_with_alias(str, alias,
                                                                 alias_length);
  DBUG_RETURN(error_num);
}
...
int spider_mysql_handler::append_key_order_for_direct_order_limit_with_alias(
  ...
  if (select_lex->order_list.first) {
    ...
    for (order = (ORDER *)select_lex->order_list.first; order;
         order = order->next) {
      if ((error_num = spider_db_print_item_type(
               (*order->item), spider, str, alias, alias_length,
               spider_dbton_mysql.dbton_id, FALSE, NULL))) {
...
int spider_mysql_handler::append_key_order_with_alias_part(const char *alias,
                                                           uint alias_length,
                                                           ulong sql_type) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_key_order_for_handler(spider_string *str,
                                                       const char *alias,
                                                       uint alias_length) {
  ...
  str->q_append(alias, alias_length);
  memset(const_cast<char *>(str->ptr()) + str->length(), ' ',
         ha_where_pos - ha_next_pos - alias_length);
  DBUG_RETURN(0);
}

int spider_mysql_handler::append_key_order_with_alias(spider_string *str,
                                                      const char *alias,
                                                      uint alias_length) {
  SPIDER_RESULT_LIST *result_list = &spider->result_list;
  KEY *key_info = result_list->key_info;
  ...
  if (result_list->sorted == TRUE) {
    if (result_list->desc_flg == TRUE) {
      for (key_part = key_info->key_part + result_list->key_order, length = 1;
      ...
      if (length + result_list->key_order <=
              (int)spider_user_defined_key_parts(key_info) &&
          length <= result_list->max_order) {
        field = key_part->field;
        key_name_length =
...
int spider_mysql_handler::append_limit_part(longlong offset, longlong limit,
                                            ulong sql_type) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::reappend_limit_part(longlong offset, longlong limit,
                                              ulong sql_type) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_limit(spider_string *str, longlong offset,
                                       longlong limit) {
...
int spider_mysql_handler::append_select_lock_part(ulong sql_type) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_select_lock(spider_string *str) {
  ...
  if (lock_mode == SPIDER_LOCK_MODE_EXCLUSIVE) {
    if (str->reserve(SPIDER_SQL_FOR_UPDATE_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
...
int spider_mysql_handler::append_union_all_start_part(ulong sql_type) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_union_all_start(spider_string *str) {
...
int spider_mysql_handler::append_union_all_part(ulong sql_type) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_union_all(spider_string *str) {
  DBUG_ENTER("spider_mysql_handler::append_union_all");
  DBUG_PRINT("info", ("spider this=%p", this));
  if (str->reserve(SPIDER_SQL_UNION_ALL_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  str->q_append(SPIDER_SQL_UNION_ALL_STR, SPIDER_SQL_UNION_ALL_LEN);
  DBUG_RETURN(0);
}

int spider_mysql_handler::append_union_all_end_part(ulong sql_type) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_union_all_end(spider_string *str) {
  DBUG_ENTER("spider_mysql_handler::append_union_all_end");
  DBUG_PRINT("info", ("spider this=%p", this));
  str->length(str->length() - SPIDER_SQL_UNION_ALL_LEN +
              SPIDER_SQL_CLOSE_PAREN_LEN);
  DBUG_RETURN(0);
}

int spider_mysql_handler::append_multi_range_cnt_part(ulong sql_type,
...
int spider_mysql_handler::append_multi_range_cnt(spider_string *str,
                                                 uint multi_range_cnt,
                                                 bool with_comma) {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  range_cnt_length =
      my_sprintf(range_cnt_str, (range_cnt_str, "%u", multi_range_cnt));
  if (with_comma) {
    ...
    if (str->reserve(range_cnt_length)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    str->q_append(range_cnt_str, range_cnt_length);
  }
  DBUG_RETURN(0);
}
...
int spider_mysql_handler::append_multi_range_cnt_with_name(
    spider_string *str, uint multi_range_cnt) {
  int range_cnt_length;
  char range_cnt_str[SPIDER_SQL_INT_LEN];
  ...
  range_cnt_length =
      my_sprintf(range_cnt_str, (range_cnt_str, "%u", multi_range_cnt));
  if (str->reserve(range_cnt_length + SPIDER_SQL_SPACE_LEN + SPIDER_SQL_ID_LEN +
                   SPIDER_SQL_COMMA_LEN))
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  str->q_append(range_cnt_str, range_cnt_length);
  str->q_append(SPIDER_SQL_SPACE_STR, SPIDER_SQL_SPACE_LEN);
...
int spider_mysql_handler::append_open_handler_part(ulong sql_type,
                                                   uint handler_id,
                                                   SPIDER_CONN *conn,
                                                   int link_idx) {
  int error_num;
...
int spider_mysql_handler::append_open_handler(spider_string *str,
                                              uint handler_id[[maybe_unused]],
                                              SPIDER_CONN *conn[[maybe_unused]], int link_idx) {
  int error_num;
  DBUG_ENTER("spider_mysql_handler::append_open_handler");
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_PRINT("info", ("spider link_idx=%d", link_idx));
  DBUG_PRINT("info",
             ("spider m_handler_cid=%s", spider->m_handler_cid[link_idx]));
  if (str->reserve(SPIDER_SQL_HANDLER_LEN)) {
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  }
  str->q_append(SPIDER_SQL_HANDLER_STR, SPIDER_SQL_HANDLER_LEN);
  if ((error_num = mysql_share->append_table_name(
           str, spider->conn_link_idx[link_idx])))
    DBUG_RETURN(error_num);
...
int spider_mysql_handler::append_close_handler_part(ulong sql_type,
                                                    int link_idx) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_close_handler(spider_string *str,
                                               int link_idx) {
...
int spider_mysql_handler::append_insert_terminator_part(ulong sql_type) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_insert_terminator(spider_string *str) {
  DBUG_ENTER("spider_mysql_handler::append_insert_terminator");
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_PRINT("info",
             ("spider dup_update_sql.length=%u", dup_update_sql.length()));
  if (spider->result_list.insert_dup_update_pushdown &&
    ...
    if (str->append(dup_update_sql)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  } else {
    str->length(str->length() - SPIDER_SQL_COMMA_LEN);
  }
  DBUG_RETURN(0);
...
int spider_mysql_handler::append_insert_values_part(ulong sql_type) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_insert_values(spider_string *str) {
  SPIDER_SHARE *share = spider->share;
  TABLE *table = spider->get_table();
  Field **field;
  bool add_value = FALSE;
  ...
  if (add_value) str->length(str->length() - SPIDER_SQL_COMMA_LEN);
  if (str->reserve(SPIDER_SQL_CLOSE_PAREN_LEN + SPIDER_SQL_COMMA_LEN)) {
    str->length(0);
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  }
...
int spider_mysql_handler::append_into_part(ulong sql_type) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_into(spider_string *str) {
  const TABLE *table = spider->get_table();
  Field **field;
  uint field_name_length = 0;
  ...
  if (field_name_length) str->length(str->length() - SPIDER_SQL_COMMA_LEN);
  if (str->reserve(SPIDER_SQL_CLOSE_PAREN_LEN + SPIDER_SQL_VALUES_LEN))
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  str->q_append(SPIDER_SQL_CLOSE_PAREN_STR, SPIDER_SQL_CLOSE_PAREN_LEN);
  str->q_append(SPIDER_SQL_VALUES_STR, SPIDER_SQL_VALUES_LEN);
...
void spider_mysql_handler::set_insert_to_pos(ulong sql_type) {
...
int spider_mysql_handler::append_from_part(ulong sql_type, int link_idx) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_from(spider_string *str, ulong sql_type,
                                      int link_idx) {
...
int spider_mysql_handler::append_flush_tables_part(ulong sql_type, int link_idx,
                                                   bool lock) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_flush_tables(spider_string *str, int link_idx[[maybe_unused]],
                                              bool lock) {
  ...
  if (lock) {
    ...
    str->q_append(SPIDER_SQL_WITH_READ_LOCK_STR, SPIDER_SQL_WITH_READ_LOCK_LEN);
  } else {
    if (str->reserve(SPIDER_SQL_FLUSH_TABLES_LEN))
      DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    str->q_append(SPIDER_SQL_FLUSH_TABLES_STR, SPIDER_SQL_FLUSH_TABLES_LEN);
...
int spider_mysql_handler::append_optimize_table_part(ulong sql_type,
                                                     int link_idx) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_optimize_table(spider_string *str,
                                                int link_idx) {
  SPIDER_SHARE *share = spider->share;
  int conn_link_idx = spider->conn_link_idx[link_idx];
  int local_length = spider_param_internal_optimize_local(
                         spider->trx->thd, share->internal_optimize_local) *
...
int spider_mysql_handler::append_analyze_table_part(ulong sql_type,
                                                    int link_idx) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_analyze_table(spider_string *str,
                                               int link_idx) {
  SPIDER_SHARE *share = spider->share;
  int conn_link_idx = spider->conn_link_idx[link_idx];
  int local_length = spider_param_internal_optimize_local(
...
int spider_mysql_handler::append_repair_table_part(ulong sql_type, int link_idx,
...
int spider_mysql_handler::append_repair_table(spider_string *str, int link_idx,
                                              HA_CHECK_OPT *check_opt) {
  SPIDER_SHARE *share = spider->share;
  int conn_link_idx = spider->conn_link_idx[link_idx];
  int local_length = spider_param_internal_optimize_local(
  ...
  if (check_opt->flags & T_QUICK) {
    if (str->reserve(SPIDER_SQL_SQL_QUICK_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
...
int spider_mysql_handler::append_check_table_part(ulong sql_type, int link_idx,
                                                  HA_CHECK_OPT *check_opt) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_check_table(spider_string *str, int link_idx,
                                             HA_CHECK_OPT *check_opt) {
  int conn_link_idx = spider->conn_link_idx[link_idx];
  ...
  if (str->reserve(SPIDER_SQL_SQL_CHECK_TABLE_LEN +
                   ...
                   /* SPIDER_SQL_NAME_QUOTE_LEN */ 4))
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  str->q_append(SPIDER_SQL_SQL_CHECK_TABLE_STR, SPIDER_SQL_SQL_CHECK_TABLE_LEN);
  mysql_share->append_table_name(str, conn_link_idx);
  if (check_opt->flags & T_QUICK) {
    if (str->reserve(SPIDER_SQL_SQL_QUICK_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    str->q_append(SPIDER_SQL_SQL_QUICK_STR, SPIDER_SQL_SQL_QUICK_LEN);
  ...
  if (check_opt->flags & T_FAST) {
    if (str->reserve(SPIDER_SQL_SQL_FAST_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    str->q_append(SPIDER_SQL_SQL_FAST_STR, SPIDER_SQL_SQL_FAST_LEN);
  }
  if (check_opt->flags & T_MEDIUM) {
    if (str->reserve(SPIDER_SQL_SQL_MEDIUM_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
...
int spider_mysql_handler::append_enable_keys_part(ulong sql_type,
                                                  int link_idx) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_enable_keys(spider_string *str, int link_idx) {
  int conn_link_idx = spider->conn_link_idx[link_idx];
  ...
  if (str->reserve(SPIDER_SQL_SQL_ALTER_TABLE_LEN +
    ...
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  str->q_append(SPIDER_SQL_SQL_ALTER_TABLE_STR, SPIDER_SQL_SQL_ALTER_TABLE_LEN);
  mysql_share->append_table_name(str, conn_link_idx);
  str->q_append(SPIDER_SQL_SQL_ENABLE_KEYS_STR, SPIDER_SQL_SQL_ENABLE_KEYS_LEN);
  DBUG_RETURN(0);
}

int spider_mysql_handler::append_disable_keys_part(ulong sql_type,
                                                   int link_idx) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_disable_keys(spider_string *str,
                                              int link_idx) {
  int conn_link_idx = spider->conn_link_idx[link_idx];
...
int spider_mysql_handler::append_delete_all_rows_part(ulong sql_type) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_delete_all_rows(spider_string *str,
                                                 ulong sql_type) {
  int error_num;
...
int spider_mysql_handler::append_truncate(spider_string *str, ulong sql_type,
                                          int link_idx) {
  DBUG_ENTER("spider_mysql_handler::append_truncate");
  if (str->reserve(
          SPIDER_SQL_TRUNCATE_TABLE_LEN + mysql_share->db_nm_max_length +
          SPIDER_SQL_DOT_LEN + mysql_share->table_nm_max_length +
          /* SPIDER_SQL_NAME_QUOTE_LEN */ 4 + SPIDER_SQL_OPEN_PAREN_LEN))
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
...
int spider_mysql_handler::append_explain_select_part(key_range *start_key,
                                                     key_range *end_key,
                                                     ulong sql_type,
                                                     int link_idx) {
  int error_num;
...
int spider_mysql_handler::append_explain_select(spider_string *str,
                                                key_range *start_key,
                                                key_range *end_key,
                                                ulong sql_type, int link_idx) {
  int error_num;
  ...
  if ((error_num = append_from(str, sql_type, link_idx)) ||
...
bool spider_mysql_handler::is_sole_projection_field(uint16 field_index) {
  // Determine whether the projection list consists solely of the field of
  // interest
...
bool spider_mysql_handler::is_bulk_insert_exec_period(bool bulk_end) {
  ...
  if ((bulk_end || (int)insert_sql.length() >= spider->bulk_size) &&
...
bool spider_mysql_handler::sql_is_filled_up(ulong sql_type[[maybe_unused]]) {
  DBUG_ENTER("spider_mysql_handler::sql_is_filled_up");
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_RETURN(filled_up);
}

bool spider_mysql_handler::sql_is_empty(ulong sql_type) {
  bool is_empty;
...
int spider_mysql_handler::bulk_tmp_table_insert(int link_idx) {
  int error_num;
  DBUG_ENTER("spider_mysql_handler::bulk_tmp_table_insert");
  DBUG_PRINT("info", ("spider this=%p", this));
  error_num =
      store_sql_to_bulk_tmp_table(&spider->result_list.update_sqls[link_idx],
...
int spider_mysql_handler::bulk_tmp_table_rnd_init() {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  upd_tmp_tbl->file->ha_extra(HA_EXTRA_CACHE);
...
bool spider_mysql_handler::need_copy_for_update(int link_idx) {
  int all_link_idx = spider->conn_link_idx[link_idx];
  DBUG_ENTER("spider_mysql_handler::need_copy_for_update");
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_RETURN(!mysql_share->same_db_table_name ||
              spider->share->link_statuses[all_link_idx] ==
...
int spider_mysql_handler::mk_bulk_tmp_table_and_bulk_start() {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  if (!upd_tmp_tbl) {
    if (!(upd_tmp_tbl = spider_mk_sys_tmp_table(thd, table, &upd_tmp_tbl_prm,
                                                "a", update_sql.charset()))) {
      DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    }
    upd_tmp_tbl->file->ha_extra(HA_EXTRA_WRITE_CACHE);
...
int spider_mysql_handler::store_sql_to_bulk_tmp_table(spider_string *str,
                                                      TABLE *tmp_table) {
  int error_num;
...
int spider_mysql_handler::restore_sql_from_bulk_tmp_table(spider_string *str,
                                                          TABLE *tmp_table) {
...
int spider_mysql_handler::insert_lock_tables_list(SPIDER_CONN *conn,
                                                  int link_idx) {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  uint old_elements = db_conn->lock_table_hash.array.max_element;
  if (my_hash_insert(&db_conn->lock_table_hash, (uchar *)tmp_link_for_hash2)) {
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  }
  if (db_conn->lock_table_hash.array.max_element > old_elements) {
    spider_alloc_calc_mem(
        spider_current_trx, db_conn->lock_table_hash,
...
int spider_mysql_handler::append_lock_tables_list(SPIDER_CONN *conn,
                                                  int link_idx, int *appended) {
  int error_num;
  ...
  tmp_link_for_hash2->db_table_str = &mysql_share->db_table_str[conn_link_idx];
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
  tmp_link_for_hash2->db_table_str_hash_value =
      mysql_share->db_table_str_hash_value[conn_link_idx];
  if (!(tmp_link_for_hash =
            (SPIDER_LINK_FOR_HASH *)my_hash_search_using_hash_value(
                &db_conn->lock_table_hash,
                tmp_link_for_hash2->db_table_str_hash_value,
                (uchar *)(const_cast<char*>(tmp_link_for_hash2->db_table_str->ptr())),
...
#endif
  {
    ...
    *appended = 1;
    ...
    if (tmp_link_for_hash->spider->lock_type < spider->lock_type) {
      my_hash_delete(&db_conn->lock_table_hash, (uchar *)tmp_link_for_hash);
      uint old_elements = db_conn->lock_table_hash.array.max_element;
...
int spider_mysql_handler::realloc_sql(ulong *realloced) {
  THD *thd = spider->trx->thd;
  st_spider_share *share = spider->share;
  int init_sql_alloc_size =
      spider_param_init_sql_alloc_size(thd, share->init_sql_alloc_size);
  ...
  if ((int)sql.alloced_length() > init_sql_alloc_size * 2) {
    sql.free();
    if (sql.real_alloc(init_sql_alloc_size)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    *realloced |= SPIDER_SQL_TYPE_SELECT_SQL;
  }
  if ((int)ha_sql.alloced_length() > init_sql_alloc_size * 2) {
    ha_sql.free();
    if (ha_sql.real_alloc(init_sql_alloc_size)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    *realloced |= SPIDER_SQL_TYPE_SELECT_SQL;
  }
  ...
  if ((int)tmp_sql.alloced_length() > init_sql_alloc_size * 2) {
    tmp_sql.free();
    if (tmp_sql.real_alloc(init_sql_alloc_size)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    *realloced |= SPIDER_SQL_TYPE_TMP_SQL;
  }
  DBUG_RETURN(0);
}

int spider_mysql_handler::reset_sql(ulong sql_type) {
...
bool spider_mysql_handler::need_lock_before_set_sql_for_exec(ulong sql_type[[maybe_unused]]) {
...
int spider_mysql_handler::set_sql_for_exec(
    ulong sql_type, int link_idx, SPIDER_LINK_IDX_CHAIN *link_idx_chain) {
  int error_num;
...
int spider_mysql_handler::set_sql_for_exec(ulong sql_type, int link_idx,
                                           bool copy) {
  int error_num;
  uint tmp_pos;
  SPIDER_SHARE *share = spider->share;
    ...
    if ((mysql_share->same_db_table_name || link_idx == first_link_idx) &&
      ...
      if (sql_type & SPIDER_SQL_TYPE_SELECT_SQL) {
        ...
        if (exec_sql->copy(sql))
          ...
          if (result_list->tmp_table_join && spider->bka_mode != 2) {
            if ((error_num = spider_db_mysql_utility.append_from_with_alias(
                     exec_sql, table_names, table_name_lengths, table_aliases,
                     table_alias_lengths, 2, &table_name_pos, TRUE)))
              DBUG_RETURN(error_num);
  ...
  if (sql_type & SPIDER_SQL_TYPE_INSERT_SQL) {
    if ((mysql_share->same_db_table_name || link_idx == first_link_idx) &&
      ...
      if (exec_insert_sql->copy(insert_sql)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
      DBUG_PRINT("info",
                 ("spider exec_insert_sql=%s", exec_insert_sql->c_ptr_safe()));
      tmp_pos = exec_insert_sql->length();
      exec_insert_sql->length(insert_table_name_pos);
      append_table_name_with_adjusting(exec_insert_sql, link_idx, sql_type);
  ...
  if (sql_type & SPIDER_SQL_TYPE_BULK_UPDATE_SQL) {
    if (reading_from_bulk_tmp_table) {
      if (mysql_share->same_db_table_name &&
        ...
        if ((error_num = restore_sql_from_bulk_tmp_table(
                 exec_update_sql,
                 spider->result_list.upd_tmp_tbls[link_idx]))) {
          DBUG_RETURN(error_num);
        }
    ...
    if (mysql_share->same_db_table_name || link_idx == first_link_idx)
      ...
      exec_update_sql = &spider->result_list.update_sqls[link_idx];
      if (exec_update_sql->copy(update_sql)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
      tmp_pos = exec_update_sql->length();
      exec_update_sql->length(table_name_pos);
      append_table_name_with_adjusting(exec_update_sql, link_idx, sql_type);
...
int spider_mysql_handler::execute_sql(ulong sql_type, SPIDER_CONN *conn,
  ...
  switch (sql_type) {
    ...
    case SPIDER_SQL_TYPE_BULK_UPDATE_SQL:
      DBUG_PRINT("info",
                 ("spider %s", sql_type == SPIDER_SQL_TYPE_UPDATE_SQL
...
int spider_mysql_handler::sts_mode_exchange(int sts_mode) {
  DBUG_ENTER("spider_mysql_handler::sts_mode_exchange");
  DBUG_PRINT("info", ("spider sts_mode=%d", sts_mode));
  DBUG_RETURN(sts_mode);
}

int spider_mysql_handler::show_table_status(int link_idx, int sts_mode,
                                            uint flag[[maybe_unused]]) {
  int error_num;
  ...
  if (sts_mode == 1) {
    ...
    spider_conn_set_timeout_from_share(conn, link_idx, spider->trx->thd, share);
    if ((error_num = spider_db_set_names(spider, conn, link_idx)) ||
        (spider_db_query(conn, mysql_share->show_table_status[0 + pos].ptr(),
                         mysql_share->show_table_status[0 + pos].length(), -1,
      ...
      if (error_num == ER_SPIDER_REMOTE_SERVER_GONE_AWAY_NUM &&
        ...
        spider_conn_set_timeout_from_share(conn, link_idx, spider->trx->thd,
                                           share);
        if (spider_db_query(conn, mysql_share->show_table_status[0 + pos].ptr(),
                            mysql_share->show_table_status[0 + pos].length(),
                            -1, &spider->need_mons[link_idx])) {
          conn->disable_connect_retry = FALSE;
          DBUG_RETURN(spider_db_errorno(conn));
        }
      } else {
    ...
    if (!(res = conn->db_conn->store_result(NULL, &request_key, &error_num))) {
      ...
      if (error_num || (error_num = spider_db_errorno(conn)))
        ...
        my_printf_error(
            ...
            mysql_share->table_names_str[spider->conn_link_idx[link_idx]]
                .ptr());
        DBUG_RETURN(ER_SPIDER_REMOTE_TABLE_NOT_FOUND_NUM);
      }
    }
    ...
    error_num = res->fetch_table_status(
        sts_mode, share->records, share->mean_rec_length,
        share->data_file_length, share->max_data_file_length,
        share->index_file_length, auto_increment_value, share->create_time,
        share->update_time, share->check_time);
    res->free_result();
    delete res;
    if (error_num) {
      switch (error_num) {
    ...
    spider_conn_set_timeout_from_share(conn, link_idx, spider->trx->thd, share);
    if ((error_num = spider_db_set_names(spider, conn, link_idx)) ||
        (spider_db_query(conn, mysql_share->show_table_status[1 + pos].ptr(),
                         mysql_share->show_table_status[1 + pos].length(), -1,
                         &spider->need_mons[link_idx]) &&
         (error_num = spider_db_errorno(conn)))) {
      if (error_num == ER_SPIDER_REMOTE_SERVER_GONE_AWAY_NUM &&
          !conn->disable_reconnect) {
        ...
        spider_conn_set_timeout_from_share(conn, link_idx, spider->trx->thd,
                                           share);
        if (spider_db_query(conn, mysql_share->show_table_status[1 + pos].ptr(),
...
int spider_mysql_handler::crd_mode_exchange(int crd_mode) {
  DBUG_ENTER("spider_mysql_handler::crd_mode_exchange");
  DBUG_PRINT("info", ("spider crd_mode=%d", crd_mode));
  DBUG_RETURN(crd_mode);
}

int spider_mysql_handler::show_index(int link_idx, int crd_mode) {
  int error_num;
  SPIDER_CONN *conn = spider->spider_get_conn_by_idx(link_idx);
  SPIDER_SHARE *share = spider->share;
  TABLE *table = spider->get_table();
  ...
  if (crd_mode == 1) {
    ...
    spider_conn_set_timeout_from_share(conn, link_idx, spider->trx->thd, share);
    if ((error_num = spider_db_set_names(spider, conn, link_idx)) ||
        (spider_db_query(conn, mysql_share->show_index[0 + pos].ptr(),
                         mysql_share->show_index[0 + pos].length(), -1,
                         &spider->need_mons[link_idx]) &&
         (error_num = spider_db_errorno(conn)))) {
      if (error_num == ER_SPIDER_REMOTE_SERVER_GONE_AWAY_NUM &&
          !conn->disable_reconnect) {
        ...
        spider_conn_set_timeout_from_share(conn, link_idx, spider->trx->thd,
                                           share);
        if (spider_db_query(conn, mysql_share->show_index[0 + pos].ptr(),
                            mysql_share->show_index[0 + pos].length(), -1,
                            &spider->need_mons[link_idx])) {
          DBUG_RETURN(spider_db_errorno(conn));
        }
    ...
    for (roop_count = 0, tmp_cardinality = share->cardinality;
         roop_count < (int)table->s->fields; roop_count++, tmp_cardinality++) {
      if (!spider_bit_is_set(share->cardinality_upd, roop_count)) {
        DBUG_PRINT("info", ("spider uninitialized column cardinality id=%d",
                            roop_count));
        *tmp_cardinality = -1;
      }
    }
    ...
    spider_conn_set_timeout_from_share(conn, link_idx, spider->trx->thd, share);
    if ((error_num = spider_db_set_names(spider, conn, link_idx)) ||
        (spider_db_query(conn, mysql_share->show_index[1 + pos].ptr(),
                         mysql_share->show_index[1 + pos].length(), -1,
                         &spider->need_mons[link_idx]) &&
         (error_num = spider_db_errorno(conn)))) {
      if (error_num == ER_SPIDER_REMOTE_SERVER_GONE_AWAY_NUM &&
          !conn->disable_reconnect) {
        ...
        spider_conn_set_timeout_from_share(conn, link_idx, spider->trx->thd,
                                           share);
        if (spider_db_query(conn, mysql_share->show_index[1 + pos].ptr(),
                            mysql_share->show_index[1 + pos].length(), -1,
                            &spider->need_mons[link_idx])) {
          DBUG_RETURN(spider_db_errorno(conn));
        }
...
int spider_mysql_handler::show_records(int link_idx) {
  int error_num;
  SPIDER_CONN *conn = spider->spider_get_conn_by_idx(link_idx);
  SPIDER_DB_RESULT *res;
  SPIDER_SHARE *share = spider->share;
  ...
  spider_conn_set_timeout_from_share(conn, link_idx, spider->trx->thd, share);
  if ((error_num = spider_db_set_names(spider, conn, link_idx)) ||
      (spider_db_query(conn, mysql_share->show_records[pos].ptr(),
                       mysql_share->show_records[pos].length(), -1,
                       &spider->need_mons[link_idx]) &&
       (error_num = spider_db_errorno(conn)))) {
    if (error_num == ER_SPIDER_REMOTE_SERVER_GONE_AWAY_NUM &&
        !conn->disable_reconnect) {
      ...
      spider_conn_set_timeout_from_share(conn, link_idx, spider->trx->thd,
                                         share);
      if (spider_db_query(conn, mysql_share->show_records[pos].ptr(),
                          mysql_share->show_records[pos].length(), -1,
                          &spider->need_mons[link_idx])) {
        DBUG_PRINT("info", ("spider error_num=%d 3", error_num));
        DBUG_RETURN(spider_db_errorno(conn));
...
int spider_mysql_handler::show_last_insert_id(int link_idx,
...
ha_rows spider_mysql_handler::explain_select(key_range *start_key,
                                             key_range *end_key, int link_idx) {
  int error_num;
  SPIDER_CONN *conn = spider->spider_get_conn_by_idx(link_idx);
  SPIDER_RESULT_LIST *result_list = &spider->result_list;
  ...
  spider_conn_set_timeout_from_share(conn, link_idx, spider->trx->thd,
                                     spider->share);
  if ((error_num = spider_db_set_names(spider, conn, link_idx)) ||
      (spider_db_query(conn, str->ptr(), str->length(), -1,
                       &spider->need_mons[link_idx]) &&
       (error_num = spider_db_errorno(conn)))) {
    if (error_num == ER_SPIDER_REMOTE_SERVER_GONE_AWAY_NUM &&
        !conn->disable_reconnect) {
      /* retry */
      if ((error_num = spider_db_ping(spider, conn, link_idx))) {
        if (spider->check_error_mode(error_num)) set_my_errno(error_num);
        spider_mta_conn_mutex_unlock(conn);
        DBUG_RETURN(HA_POS_ERROR);
      }
      if ((error_num = spider_db_set_names(spider, conn, link_idx))) {
  ...
  if (!(res = conn->db_conn->store_result(NULL, &request_key, &error_num))) {
    if (error_num || (error_num = spider_db_errorno(conn))) {
      ...
      set_my_errno(ER_QUERY_ON_FOREIGN_DATA_SOURCE);
      spider_mta_conn_mutex_unlock(conn);
      DBUG_RETURN(HA_POS_ERROR);
    }
  }
  spider_mta_conn_mutex_unlock(conn);
  error_num = res->fetch_table_records(2, rows);
  res->free_result();
  delete res;
  if (error_num) {
    set_my_errno(error_num);
...
int spider_mysql_handler::lock_tables(int link_idx) {
  int error_num;
  SPIDER_CONN *conn = spider->spider_get_conn_by_idx(link_idx);
...
int spider_mysql_handler::unlock_tables(int link_idx) {
  int error_num;
  SPIDER_CONN *conn = spider->spider_get_conn_by_idx(link_idx);
...
int spider_mysql_handler::disable_keys(SPIDER_CONN *conn, int link_idx) {
  int error_num;
  SPIDER_SHARE *share = spider->share;
  ...
  str->length(0);
  if ((error_num =
           append_disable_keys_part(SPIDER_SQL_TYPE_OTHER_HS, link_idx))) {
    DBUG_RETURN(error_num);
  }
  spider_mta_conn_mutex_lock(conn);
  conn->need_mon = &spider->need_mons[link_idx];
...
int spider_mysql_handler::enable_keys(SPIDER_CONN *conn, int link_idx) {
  int error_num;
  SPIDER_SHARE *share = spider->share;
  spider_string *str = &spider->result_list.sqls[link_idx];
  ...
  str->length(0);
  if ((error_num =
           append_enable_keys_part(SPIDER_SQL_TYPE_OTHER_HS, link_idx))) {
    DBUG_RETURN(error_num);
  }
  spider_mta_conn_mutex_lock(conn);
  conn->need_mon = &spider->need_mons[link_idx];
...
int spider_mysql_handler::check_table(SPIDER_CONN *conn, int link_idx,
                                      HA_CHECK_OPT *check_opt) {
  int error_num;
  SPIDER_SHARE *share = spider->share;
  spider_string *str = &spider->result_list.sqls[link_idx];
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  str->length(0);
  if ((error_num = append_check_table_part(SPIDER_SQL_TYPE_OTHER_HS, link_idx,
                                           check_opt))) {
    DBUG_RETURN(error_num);
  }
  spider_mta_conn_mutex_lock(conn);
...
int spider_mysql_handler::repair_table(SPIDER_CONN *conn, int link_idx,
                                       HA_CHECK_OPT *check_opt) {
  int error_num;
  SPIDER_SHARE *share = spider->share;
  spider_string *str = &spider->result_list.sqls[link_idx];
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  str->length(0);
  if ((error_num = append_repair_table_part(SPIDER_SQL_TYPE_OTHER_HS, link_idx,
                                            check_opt))) {
    DBUG_RETURN(error_num);
  }
  spider_mta_conn_mutex_lock(conn);
...
int spider_mysql_handler::analyze_table(SPIDER_CONN *conn, int link_idx) {
  int error_num;
  SPIDER_SHARE *share = spider->share;
  spider_string *str = &spider->result_list.sqls[link_idx];
  ...
  str->length(0);
  if ((error_num =
           append_analyze_table_part(SPIDER_SQL_TYPE_OTHER_HS, link_idx))) {
    DBUG_RETURN(error_num);
  }
  spider_mta_conn_mutex_lock(conn);
  conn->need_mon = &spider->need_mons[link_idx];
...
int spider_mysql_handler::optimize_table(SPIDER_CONN *conn, int link_idx) {
  int error_num;
  SPIDER_SHARE *share = spider->share;
  spider_string *str = &spider->result_list.sqls[link_idx];
  ...
  str->length(0);
  if ((error_num =
           append_optimize_table_part(SPIDER_SQL_TYPE_OTHER_HS, link_idx))) {
    DBUG_RETURN(error_num);
  }
  spider_mta_conn_mutex_lock(conn);
  conn->need_mon = &spider->need_mons[link_idx];
...
int spider_mysql_handler::flush_tables(SPIDER_CONN *conn, int link_idx,
                                       bool lock) {
  int error_num;
  SPIDER_SHARE *share = spider->share;
  spider_string *str = &spider->result_list.sqls[link_idx];
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  str->length(0);
  if ((error_num = append_flush_tables_part(SPIDER_SQL_TYPE_OTHER_HS, link_idx,
                                            lock))) {
    DBUG_RETURN(error_num);
  }
  spider_conn_set_timeout_from_share(conn, link_idx, spider->trx->thd, share);
...
int spider_mysql_handler::flush_logs(SPIDER_CONN *conn, int link_idx) {
  int error_num;
  SPIDER_SHARE *share = spider->share;
  DBUG_ENTER("spider_mysql_handler::flush_logs");
  DBUG_PRINT("info", ("spider this=%p", this));
  spider_conn_set_timeout_from_share(conn, link_idx, spider->trx->thd, share);
  if (spider_db_query(conn, SPIDER_SQL_FLUSH_LOGS_STR,
                      SPIDER_SQL_FLUSH_LOGS_LEN, -1,
                      &spider->need_mons[link_idx])) {
    error_num = spider_db_errorno(conn);
    DBUG_RETURN(error_num);
  }
...
int spider_mysql_handler::insert_opened_handler(SPIDER_CONN *conn,
                                                int link_idx) {
  spider_db_mysql *db_conn = (spider_db_mysql *)conn->db_conn;
  SPIDER_LINK_FOR_HASH *tmp_link_for_hash = &link_for_hash[link_idx];
  assert(tmp_link_for_hash->spider == spider);
  assert(tmp_link_for_hash->link_idx == link_idx);
  ...
  if (db_conn->handler_open_array.max_element > old_elements) {
    spider_alloc_calc_mem(
        spider_current_trx, db_conn->handler_open_array,
...
int spider_mysql_handler::delete_opened_handler(SPIDER_CONN *conn,
                                                int link_idx) {
  spider_db_mysql *db_conn = (spider_db_mysql *)conn->db_conn;
  ...
  assert(roop_count < elements);
  DBUG_RETURN(0);
}

int spider_mysql_handler::sync_from_clone_source(spider_db_handler *dbton_hdl[[maybe_unused]]) {
  DBUG_ENTER("spider_mysql_handler::sync_from_clone_source");
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_RETURN(0);
}

bool spider_mysql_handler::support_use_handler(int use_handler[[maybe_unused]]) {
...
void spider_mysql_handler::minimum_select_bitmap_create() {
  ...
  for (field_p = table->field; *field_p; field_p++) {
    ...
    DBUG_PRINT(
        "info",
        ("spider ft_discard_bitmap=%s",
         spider_bit_is_set(spider->ft_discard_bitmap, field_index) ? "TRUE"
                                                                   : "FALSE"));
    ...
    DBUG_PRINT("info", ("spider read_set=%s",
                        bitmap_is_set(table->read_set, field_index) ? "TRUE"
                                                                    : "FALSE"));
    DBUG_PRINT(
        "info",
        ("spider write_set=%s",
         bitmap_is_set(table->write_set, field_index) ? "TRUE" : "FALSE"));
    if (spider_bit_is_set(spider->ft_discard_bitmap, field_index) &
        (spider_bit_is_set(spider->searched_bitmap, field_index) |
...
bool spider_mysql_handler::minimum_select_bit_is_set(uint field_index) {
  DBUG_ENTER("spider_mysql_handler::minimum_select_bit_is_set");
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_PRINT("info", ("spider field_index()=%u", field_index));
  DBUG_PRINT("info",
             ("spider minimum_select_bitmap=%s",
              spider_bit_is_set(minimum_select_bitmap, field_index) ? "TRUE"
                                                                    : "FALSE"));
  DBUG_RETURN(spider_bit_is_set(minimum_select_bitmap, field_index));
}

void spider_mysql_handler::copy_minimum_select_bitmap(uchar *bitmap) {
  int roop_count;
  TABLE *table = spider->get_table();
  DBUG_ENTER("spider_mysql_handler::copy_minimum_select_bitmap");
  for (roop_count = 0; roop_count < (int)((table->s->fields + 7) / 8);
       roop_count++) {
    bitmap[roop_count] = minimum_select_bitmap[roop_count];
    DBUG_PRINT("info", ("spider roop_count=%d", roop_count));
    DBUG_PRINT("info", ("spider bitmap=%d", bitmap[roop_count]));
  }
  DBUG_VOID_RETURN;
}
  ...
  if (union_table_name_pos_current->tgt_num >= SPIDER_INT_HLD_TGT_SIZE) {
    if (!union_table_name_pos_current->next) {
      if (!spider_bulk_malloc(spider_current_trx, 237, MYF(MY_WME),
                              &union_table_name_pos_current->next,
                              sizeof(SPIDER_INT_HLD), NullS)) {
        DBUG_RETURN(HA_ERR_OUT_OF_MEM);
      }
...
int spider_mysql_handler::reset_union_table_name(spider_string *str,
                                                 int link_idx, ulong sql_type) {
  DBUG_ENTER("spider_mysql_handler::reset_union_table_name");
  DBUG_PRINT("info", ("spider this=%p", this));
  if (!union_table_name_pos_current) DBUG_RETURN(0);
  ...
  while (TRUE) {
    ...
    if (tmp_pos == union_table_name_pos_current) break;
...
#ifdef SPIDER_HAS_GROUP_BY_HANDLER
int spider_mysql_handler::append_from_and_tables_part(spider_fields *fields,
                                                      ulong sql_type) {
  int error_num;
  spider_string *str;
  SPIDER_TABLE_HOLDER *table_holder;
  ...
  error_num = spider_db_mysql_utility.append_from_and_tables(
      table_holder->spider, fields, str, table_list, fields->get_table_count());
  DBUG_RETURN(error_num);
}

int spider_mysql_handler::reappend_tables_part(spider_fields *fields,
                                               ulong sql_type) {
  int error_num;
  spider_string *str;
  ...
  error_num =
      spider_db_mysql_utility.reappend_tables(fields, link_idx_chain, str);
  DBUG_RETURN(error_num);
}

int spider_mysql_handler::append_where_part(ulong sql_type) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_having_part(ulong sql_type) {
  int error_num;
  spider_string *str;
...
int spider_mysql_handler::append_item_type_part(Item *item, const char *alias,
                                                uint alias_length,
                                                bool use_fields,
                                                spider_fields *fields,
                                                ulong sql_type) {
  ...
  error_num = spider_db_print_item_type(item, spider, str, alias, alias_length,
                                        spider_dbton_mysql.dbton_id, use_fields,
                                        fields);
  DBUG_RETURN(error_num);
}

int spider_mysql_handler::append_list_item_select_part(
    List<Item> *select, const char *alias, uint alias_length, bool use_fields,
...
int spider_mysql_handler::append_list_item_select(
    List<Item> *select, spider_string *str, const char *alias,
    uint alias_length, bool use_fields, spider_fields *fields) {
  int error_num;
  uint dbton_id = spider_dbton_mysql.dbton_id, length;
  ...
  while ((item = it++)) {
    if ((error_num =
             spider_db_print_item_type(item, spider, str, alias, alias_length,
                                       dbton_id, use_fields, fields))) {
      DBUG_RETURN(error_num);
    }
    ...
    if (str->reserve(SPIDER_SQL_COMMA_LEN + /* SPIDER_SQL_NAME_QUOTE_LEN */ 2 +
                     SPIDER_SQL_SPACE_LEN + length))
      DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    str->q_append(SPIDER_SQL_SPACE_STR, SPIDER_SQL_SPACE_LEN);
    if ((error_num =
             spider_db_mysql_utility.append_name(str, item_name, length))) {
      DBUG_RETURN(error_num);
    }
    str->q_append(SPIDER_SQL_COMMA_STR, SPIDER_SQL_COMMA_LEN);
  }
...
int spider_mysql_handler::append_group_by_part(ORDER *order, const char *alias,
                                               uint alias_length,
                                               bool use_fields,
                                               spider_fields *fields,
                                               ulong sql_type) {
  ...
  error_num =
      append_group_by(order, str, alias, alias_length, use_fields, fields);
  DBUG_RETURN(error_num);
}

int spider_mysql_handler::append_group_by(ORDER *order, spider_string *str,
                                          const char *alias, uint alias_length,
                                          bool use_fields,
                                          spider_fields *fields) {
  int error_num;
...
int spider_mysql_handler::append_order_by_part(ORDER *order, const char *alias,
                                               uint alias_length,
                                               bool use_fields,
                                               spider_fields *fields,
                                               ulong sql_type) {
  ...
  error_num =
      append_order_by(order, str, alias, alias_length, use_fields, fields);
  DBUG_RETURN(error_num);
}

int spider_mysql_handler::append_order_by(ORDER *order, spider_string *str,
                                          const char *alias, uint alias_length,
                                          bool use_fields,
                                          spider_fields *fields) {
  int error_num;
...
spider_mysql_copy_table::spider_mysql_copy_table(spider_mysql_share *db_share)
    : spider_db_copy_table(db_share), mysql_share(db_share) {
...
void spider_mysql_copy_table::set_sql_charset(CHARSET_INFO *cs) {
...
int spider_mysql_copy_table::append_select_str() {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  if (sql.reserve(SPIDER_SQL_SELECT_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  sql.q_append(SPIDER_SQL_SELECT_STR, SPIDER_SQL_SELECT_LEN);
  DBUG_RETURN(0);
}

int spider_mysql_copy_table::append_insert_str(int insert_flg) {
  DBUG_ENTER("spider_mysql_copy_table::append_insert_str");
  DBUG_PRINT("info", ("spider this=%p", this));
  if (insert_flg & SPIDER_DB_INSERT_REPLACE) {
    if (sql.reserve(SPIDER_SQL_REPLACE_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    sql.q_append(SPIDER_SQL_REPLACE_STR, SPIDER_SQL_REPLACE_LEN);
  } else {
    if (sql.reserve(SPIDER_SQL_INSERT_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  ...
  if (insert_flg & SPIDER_DB_INSERT_LOW_PRIORITY) {
    ...
    if (sql.reserve(SPIDER_SQL_SQL_DELAYED_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  ...
  if (insert_flg & SPIDER_DB_INSERT_IGNORE) {
    if (sql.reserve(SPIDER_SQL_SQL_IGNORE_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    sql.q_append(SPIDER_SQL_SQL_IGNORE_STR, SPIDER_SQL_SQL_IGNORE_LEN);
  }
  DBUG_RETURN(0);
}

int spider_mysql_copy_table::append_table_columns(TABLE_SHARE *table_share) {
  int error_num;
  Field **field;
  DBUG_ENTER("spider_mysql_copy_table::append_table_columns");
  DBUG_PRINT("info", ("spider this=%p", this));
  for (field = table_share->field; *field; field++) {
    if (sql.reserve(SPIDER_SQL_NAME_QUOTE_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    sql.q_append(SPIDER_SQL_NAME_QUOTE_STR, SPIDER_SQL_NAME_QUOTE_LEN);
    if ((error_num = spider_db_append_name_with_quote_str(
             &sql, const_cast<char *>((*field)->field_name),
             spider_dbton_mysql.dbton_id)))
      DBUG_RETURN(error_num);
...
int spider_mysql_copy_table::append_from_str() {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  if (sql.reserve(SPIDER_SQL_FROM_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  sql.q_append(SPIDER_SQL_FROM_STR, SPIDER_SQL_FROM_LEN);
  DBUG_RETURN(0);
}

int spider_mysql_copy_table::append_table_name(int link_idx) {
  int error_num;
...
int spider_mysql_copy_table::append_copy_where(spider_db_copy_table *source_ct,
                                               KEY *key_info,
                                               ulong *last_row_pos,
                                               ulong *last_lengths) {
  ...
  for (roop_count = spider_user_defined_key_parts(key_info) - 1;
    ...
    for (roop_count2 = 0; roop_count2 < roop_count; roop_count2++) {
      ...
      if ((error_num =
               copy_key_row(source_ct, field, &last_row_pos[field->field_index()],
    ...
    if ((error_num =
             copy_key_row(source_ct, field, &last_row_pos[field->field_index()],
                          &last_lengths[field->field_index()], SPIDER_SQL_GT_STR,
                          SPIDER_SQL_GT_LEN))) {
      DBUG_RETURN(error_num);
    }
    sql.length(sql.length() - SPIDER_SQL_AND_LEN);
    if (sql.reserve(SPIDER_SQL_CLOSE_PAREN_LEN + SPIDER_SQL_OR_LEN +
                    SPIDER_SQL_OPEN_PAREN_LEN)) {
      DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    }
...
int spider_mysql_copy_table::append_key_order_str(KEY *key_info, int start_pos,
                                                  bool desc_flg) {
  ...
  if ((int)spider_user_defined_key_parts(key_info) > start_pos) {
    if (sql.reserve(SPIDER_SQL_ORDER_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    sql.q_append(SPIDER_SQL_ORDER_STR, SPIDER_SQL_ORDER_LEN);
    if (desc_flg == TRUE) {
      for (key_part = key_info->key_part + start_pos, length = 0;
...
int spider_mysql_copy_table::append_limit(longlong offset, longlong limit) {
  char buf[SPIDER_LONGLONG_LEN + 1];
  uint32 length;
...
int spider_mysql_copy_table::append_into_str() {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  if (sql.reserve(SPIDER_SQL_INTO_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
...
int spider_mysql_copy_table::append_open_paren_str() {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  if (sql.reserve(SPIDER_SQL_OPEN_PAREN_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  sql.q_append(SPIDER_SQL_OPEN_PAREN_STR, SPIDER_SQL_OPEN_PAREN_LEN);
  DBUG_RETURN(0);
}
...
int spider_mysql_copy_table::append_select_lock_str(int lock_mode) {
  DBUG_ENTER("spider_mysql_copy_table::append_select_lock_str");
  DBUG_PRINT("info", ("spider this=%p", this));
  if (lock_mode == SPIDER_LOCK_MODE_EXCLUSIVE) {
    if (sql.reserve(SPIDER_SQL_FOR_UPDATE_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    sql.q_append(SPIDER_SQL_FOR_UPDATE_STR, SPIDER_SQL_FOR_UPDATE_LEN);
  } else if (lock_mode == SPIDER_LOCK_MODE_SHARED) {
    if (sql.reserve(SPIDER_SQL_SHARED_LOCK_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    sql.q_append(SPIDER_SQL_SHARED_LOCK_STR, SPIDER_SQL_SHARED_LOCK_LEN);
  }
  DBUG_RETURN(0);
}

int spider_mysql_copy_table::exec_query(SPIDER_CONN *conn, int quick_mode,
                                        int *need_mon) {
  int error_num;
  DBUG_ENTER("spider_mysql_copy_table::exec_query");
  DBUG_PRINT("info", ("spider this=%p", this));
  error_num =
      spider_db_query(conn, sql.ptr(), sql.length(), quick_mode, need_mon);
  DBUG_RETURN(error_num);
}

int spider_mysql_copy_table::copy_key_row(spider_db_copy_table *source_ct,
                                          Field *field, ulong *row_pos,
                                          ulong *length, const char *joint_str,
  ...
  sql.q_append(SPIDER_SQL_NAME_QUOTE_STR, SPIDER_SQL_NAME_QUOTE_LEN);
  if ((error_num = spider_db_append_name_with_quote_str(
           &sql, const_cast<char *>(field->field_name), spider_dbton_mysql.dbton_id)))
    DBUG_RETURN(error_num);
...
int spider_mysql_copy_table::copy_row(Field *field, SPIDER_DB_ROW *row) {
  int error_num;
  ...
  if (row->is_null()) {
    ...
    if (sql.reserve(SPIDER_SQL_VALUE_QUOTE_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    sql.q_append(SPIDER_SQL_VALUE_QUOTE_STR, SPIDER_SQL_VALUE_QUOTE_LEN);
    if ((error_num =
             row->append_escaped_to_str(&sql, spider_dbton_mysql.dbton_id)))
      DBUG_RETURN(error_num);
    if (sql.reserve(SPIDER_SQL_VALUE_QUOTE_LEN + SPIDER_SQL_COMMA_LEN))
      DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    sql.q_append(SPIDER_SQL_VALUE_QUOTE_STR, SPIDER_SQL_VALUE_QUOTE_LEN);
  } else {
    if ((error_num = row->append_to_str(&sql))) DBUG_RETURN(error_num);
    if (sql.reserve(SPIDER_SQL_COMMA_LEN)) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  }
  sql.q_append(SPIDER_SQL_COMMA_STR, SPIDER_SQL_COMMA_LEN);
  DBUG_RETURN(0);
}

int spider_mysql_copy_table::copy_rows(TABLE *table, SPIDER_DB_ROW *row,
                                       ulong **last_row_pos,
                                       ulong **last_lengths) {
  int error_num;
  Field **field;
  ...
  for (field = table->field; *field; field++, lengths2++) {
    *row_pos2 = sql.length();
    if ((error_num = copy_row(*field, row))) DBUG_RETURN(error_num);
    *lengths2 = sql.length() - *row_pos2 - SPIDER_SQL_COMMA_LEN;
    row->next();
    row_pos2++;
  }
  sql.length(sql.length() - SPIDER_SQL_COMMA_LEN);
  if (sql.reserve(SPIDER_SQL_CLOSE_PAREN_LEN + SPIDER_SQL_COMMA_LEN +
                  SPIDER_SQL_OPEN_PAREN_LEN)) {
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  }
  sql.q_append(SPIDER_SQL_CLOSE_PAREN_STR, SPIDER_SQL_CLOSE_PAREN_LEN);
...
void log_spider_sql_tdsql(THD *thd, spider_db_mysql *db_conn, const char *query,
                          uint length, my_hrtime_t &start, bool has_error) {
  DBUG_ENTER("log_spider_sql_tdsql");
  my_hrtime_t end = my_hrtime();
  size_t query_string_length = tdsql::log::tdsql_log_sql_size;
  if (query_string_length == 0 || query_string_length > length) {
    query_string_length = length;
  }
  // mode mysql spider does not log, mtr will failed
  if (tdsql::common::tdsql_log_sql_open_flag &&
      tdsql::common::tdsql_mode != tdsql::common::MODE_MYSQL) {
    tdsql::log::tdsql_log_sql_error_verbosity = loglevel::DEBUG_LEVEL;
    if (!has_error) {
      LogSqlDebug(
          "tid:%u cid:%lu con:%p user:%.*s C:%.*s:%d G:%s:%d S:%.*s:%ld "
          "timecost:%.3f(ms) err(%d):\"%s\" sql:%d,%d \"%.*s\"",
          thd->thread_id(), (ulong)db_conn->thread_id(),
          (void *)db_conn->get_conn(),
          db_conn->get_conn()->tgt_username_length,
          db_conn->get_conn()->tgt_username,
          int(thd->m_main_security_ctx.host_or_ip().length),
          thd->m_main_security_ctx.host_or_ip().str, thd->peer_port,
          db_conn->sock_addr, db_conn->sock_port,
          int(db_conn->get_conn()->tgt_host_length),
          db_conn->get_conn()->tgt_host, db_conn->get_conn()->tgt_port,
          db_conn->get_conn()->dry_run ? 0.0
                                       : float(end.val - start.val) / 1000.0,
          db_conn->get_errno(), db_conn->get_error(), (int)COM_QUERY, length,
          int(query_string_length), query);
    } else {
      LogSqlError(
          "tid:%u cid:%lu con:%p user:%.*s C:%.*s:%d G:%s:%d S:%.*s:%ld "
          "timecost:%.3f(ms) err(%d):\"%s\" sql:%d,%d \"%.*s\"",
...
void log_spider_mysql_sql(MYSQL *m, const char *query, uint length,
                          int command) {
  DBUG_ENTER("log_spider_mysql_sql");
  tdsql::log::log_raw_mysql_query(m, query, length, command);
  DBUG_VOID_RETURN;
}
