...
int spider_udf_set_copy_tables_param_default(SPIDER_COPY_TABLES *copy_tables) {
  ...
  if (!copy_tables->database) {
    ...
    if (!(copy_tables->database = spider_create_string(
              copy_tables->trx->thd->db().str, copy_tables->database_length))) {
...
#define SPIDER_PARAM_STR(title_name, param_name)                         \
  if (!strncasecmp(tmp_ptr, title_name, title_length)) {                 \
    ...
    if (!copy_tables->param_name) {                                      \
      ...
      DBUG_PRINT("info",                                                 \
                 ("spider " title_name "=%s", copy_tables->param_name)); \
    }                                                                    \
    break;                                                               \
  }
...
#define SPIDER_PARAM_INT_WITH_MAX(title_name, param_name, min_val, max_val)  \
  if (!strncasecmp(tmp_ptr, title_name, title_length)) {                     \
    ...
    if (copy_tables->param_name == -1) {                                     \
      ...
      DBUG_PRINT("info",                                                     \
                 ("spider " title_name "=%d", copy_tables->param_name));     \
    }                                                                        \
    break;                                                                   \
  }
...
#define SPIDER_PARAM_LONGLONG(title_name, param_name, min_val)               \
  if (!strncasecmp(tmp_ptr, title_name, title_length)) {                     \
    ...
    if (copy_tables->param_name == -1) {                                     \
      ...
      DBUG_PRINT("info",                                                     \
                 ("spider " title_name "=%lld", copy_tables->param_name));   \
    }                                                                        \
    break;                                                                   \
  }
  ...
  while (sprit_ptr[0]) {
    ...
    switch (title_length) {
      ...
      case 16:
        ...
        goto error;
      case 17:
        SPIDER_PARAM_INT_WITH_MAX("use_table_charset", use_table_charset, 0, 1);
        error_num = param_string_parse.print_param_error();
        goto error;
      case 20:
...
set_default:
  if ((error_num = spider_udf_set_copy_tables_param_default(copy_tables)))
    goto error;

  if (param_string) spider_free(spider_current_trx, param_string, MYF(0));
  DBUG_RETURN(0);
...
int spider_udf_get_copy_tgt_tables(THD *thd, SPIDER_COPY_TABLES *copy_tables,
  ...
  spider_store_db_and_table_name(table_tables, copy_tables->spider_db_name,
                                 copy_tables->spider_db_name_length,
                                 copy_tables->spider_table_name,
                                 copy_tables->spider_table_name_length);
  if ((error_num = spider_get_sys_table_by_idx(
           table_tables, table_key, table_tables->s->primary_key, 2))) {
    table_tables->file->print_error(error_num, MYF(0));
    goto error;
  }
  do {
    if (!(table_conn = (SPIDER_COPY_TABLE_CONN *)spider_bulk_malloc(
              spider_current_trx, 25, MYF(MY_WME | MY_ZEROFILL), &table_conn,
              sizeof(SPIDER_COPY_TABLE_CONN), &tmp_share, sizeof(SPIDER_SHARE),
              &tmp_connect_info,
    ...
    if ((error_num = spider_set_connect_info_default(tmp_share,
        ...
        (error_num = spider_set_connect_info_default_db_table(
             tmp_share, copy_tables->spider_db_name,
             copy_tables->spider_db_name_length, copy_tables->spider_table_name,
             copy_tables->spider_table_name_length)) ||
    ...
    */
    ...
    for (roop_count = 0; roop_count < (int)tmp_share->use_dbton_count;
      ...
      if (!spider_dbton[dbton_id].create_db_copy_table) continue;
      ...
      if ((error_num = table_conn->copy_table->init())) goto error;
      break;
    }

    if (!copy_tables->use_auto_mode[0]) {
    ...
    if (table_conn) {
      ...
      if (table_conn->copy_table) delete table_conn->copy_table;
...
int spider_udf_get_copy_tgt_conns(SPIDER_COPY_TABLES *copy_tables) {
  ...
  for (roop_count = 0; roop_count < 2; roop_count++) {
    ...
    while (table_conn) {
      ...
      if (!(table_conn->conn =
                spider_get_conn(share, 0, trx, NULL, FALSE, FALSE,
...
int spider_udf_copy_tables_create_table_list(SPIDER_COPY_TABLES *copy_tables,
                                             ...
                                             uint src_link_idx_list_length __attribute__((unused)),
                                             char *dst_link_idx_list,
                                             uint dst_link_idx_list_length __attribute__((unused))) {
  ...
  if (!(copy_tables->link_idxs[0] = (int *)spider_bulk_malloc(
            spider_current_trx, 26, MYF(MY_WME | MY_ZEROFILL),
            &copy_tables->link_idxs[0],
            sizeof(int) * copy_tables->link_idx_count[0],
            &copy_tables->link_idxs[1],
            sizeof(int) * copy_tables->link_idx_count[1], &tmp_name_ptr,
            sizeof(char) * (spider_table_name_length * 2 +
                            copy_tables->database_length + 3),
            NullS))) {
    my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  }
  ...
  DBUG_PRINT("info",
             ("spider spider_table_name=%s", copy_tables->spider_table_name));
  ...
  for (roop_count2 = 0; roop_count2 < 2; roop_count2++) {
    ...
    while (*tmp_ptr) {
      ...
      DBUG_PRINT("info", ("spider link_idx[%d][%d]=%d", roop_count2, roop_count,
...
long long spider_copy_tables_body(UDF_INIT *initid __attribute__((unused)), UDF_ARGS *args,
  ...
  Reprepare_observer *reprepare_observer_backup __attribute__((unused));
  ...
  if (!(copy_tables = (SPIDER_COPY_TABLES *)spider_bulk_malloc(
            spider_current_trx, 27, MYF(MY_WME | MY_ZEROFILL), &copy_tables,
            sizeof(SPIDER_COPY_TABLES), NullS))) {
    my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
    goto error;
  }
  ...
  if (args->arg_count == 4) {
    if (spider_udf_parse_copy_tables_param(
            copy_tables, args->args[3] ? args->args[3] : const_cast<char *>(""),
            args->args[3] ? args->lengths[3] : 0))
      goto error;
  } else {
    if (spider_udf_parse_copy_tables_param(copy_tables, const_cast<char *>(""), 0))
      goto error;
  }
  if (spider_udf_copy_tables_create_table_list(
          copy_tables, args->args[0], args->lengths[0],
          args->args[1] ? args->args[1] : const_cast<char *>(""),
          args->args[1] ? args->lengths[1] : 0,
          args->args[2] ? args->args[2] : const_cast<char *>(""),
          args->args[2] ? args->lengths[2] : 0))
    goto error;

  SPD_INIT_ALLOC_ROOT(&mem_root, 4096, 0, MYF(MY_WME));
  if (spider_udf_get_copy_tgt_tables(thd, copy_tables, &mem_root, TRUE)) {
    free_root(&mem_root, MYF(0));
    goto error;
  }
  free_root(&mem_root, MYF(0));

  if (spider_udf_get_copy_tgt_conns(copy_tables)) goto error;

  table_list = &copy_tables->spider_table_list;
#if 0  
  table_list->db = const_cast<const char*>(thd->mem_root->Alloc(copy_tables->spider_db_name_length + 1));
  memcpy(const_cast<char *>(table_list->db), copy_tables->spider_db_name, copy_tables->spider_db_name_length);
  table_list->db[copy_tables->spider_db_name_length] = '\0'; 

  table_list->table_name = const_cast<const char*>(thd->mem_root->Alloc(copy_tables->spider_real_table_name_length + 1));
  memcpy(const_cast<char*>(table_list->table_name), copy_tables->spider_real_table_name,
         copy_tables->spider_real_table_name_length);
  table_list->table_name[copy_tables->spider_real_table_name_length] = '\0';
#endif
  ...
  // table_list->alias.str = table_list->table_name;
  // table_list->alias.length = table_list->table_name.length;
  table_list->db = copy_tables->spider_db_name;
  table_list->alias =  table_list->table_name = copy_tables->spider_real_table_name;
  ...
  // DBUG_PRINT("info",
  //           ("spider table_name_length=%zd", table_list->table_name.length));
  ...
  table_list->mdl_request.init(MDL_key::TABLE, table_list->db.str,
                               table_list->table_name.str, MDL_SHARED_READ,
                               MDL_TRANSACTION);
#endif
  MDL_REQUEST_INIT(&table_list->mdl_request, MDL_key::TABLE, table_list->db,
                               table_list->table_name, MDL_SHARED_READ,
                               MDL_TRANSACTION); 
  ...
  // thd->m_reprepare_observer = reprepare_observer_backup;
  copy_tables->trx->trx_start = FALSE;
  copy_tables->trx->is_set_gtid = FALSE;
  copy_tables->trx->updated_in_this_trx = FALSE;
  DBUG_PRINT("info", ("spider trx->updated_in_this_trx=FALSE"));
  ...
  use_table_charset =
      spider_param_use_table_charset(copy_tables->use_table_charset);
  if (use_table_charset)
    copy_tables->access_charset = const_cast<CHARSET_INFO*>(table_share->table_charset);
  else
    copy_tables->access_charset = system_charset_info;

  bulk_insert_rows =
      spider_param_udf_ct_bulk_insert_rows(copy_tables->bulk_insert_rows);
  for (src_tbl_conn = copy_tables->table_conn[0]; src_tbl_conn;
    ...
    if (select_ct->append_select_str() ||
        select_ct->append_table_columns(table_share)) {
      my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
      goto error;
    }

    if (select_ct->append_from_str() || select_ct->append_table_name(0)) {
      my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
      goto error;
    }
  ...
  for (dst_tbl_conn = copy_tables->table_conn[1]; dst_tbl_conn;
    ...
    if (insert_ct->append_insert_str(SPIDER_DB_INSERT_IGNORE) ||
        insert_ct->append_into_str() || insert_ct->append_table_name(0) ||
  ...
  if (!(tmp_sql = new spider_string[all_link_cnt]) ||
  ...
  for (roop_count = 0, table_conn = copy_tables->table_conn[0]; table_conn;
       roop_count++, table_conn = table_conn->next) {
    tmp_spider = &spider[roop_count];
    if (!(tmp_spider->dbton_handler =
              (spider_db_handler **)spider_bulk_alloc_mem(
                  spider_current_trx, 205, __func__, __FILE__, __LINE__,
                  MYF(MY_WME | MY_ZEROFILL), &tmp_spider->dbton_handler,
    ...
    */
    ...
    if (!(tmp_spider->dbton_handler[dbton_id] =
              spider_dbton[dbton_id].create_db_handler(
                  tmp_spider, tmp_spider->share->dbton_share[dbton_id]))) {
      my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
  ...
  for (table_conn = copy_tables->table_conn[1]; table_conn;
       roop_count++, table_conn = table_conn->next) {
    tmp_spider = &spider[roop_count];
    if (!(tmp_spider->dbton_handler =
              (spider_db_handler **)spider_bulk_alloc_mem(
                  spider_current_trx, 206, __func__, __FILE__, __LINE__,
                  MYF(MY_WME | MY_ZEROFILL), &tmp_spider->dbton_handler,
                  sizeof(spider_db_handler *) * SPIDER_DBTON_SIZE, NullS))) {
      my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
      goto error;
...
bool spider_copy_tables_init_body(UDF_INIT *initid __attribute__((unused)), UDF_ARGS *args,
                                     char *message) {
  ...
  if (args->arg_type[0] != STRING_RESULT ||
      ...
      (args->arg_count == 4 && args->arg_type[3] != STRING_RESULT)) {
...
void spider_copy_tables_deinit_body(UDF_INIT *initid __attribute__((unused))) {
  int error_num;
  THD *thd = current_thd;
  SPIDER_TRX *trx;
