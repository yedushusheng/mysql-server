...
void spider_free_mem_calc(SPIDER_TRX *trx, uint id, size_t size) {
  ...
  assert(id < SPIDER_MEM_CALC_LIST_NUM);
  DBUG_PRINT("info",
             ("spider trx=%p id=%u size=%llu", trx, id, (ulonglong)size));
  if (spider_param_enable_mem_calc()) {
    if (trx) {
...
void spider_alloc_mem_calc(SPIDER_TRX *trx, uint id, const char *func_name,
  ...
  if (spider_param_enable_mem_calc()) {
    if (trx) {
      DBUG_PRINT("info", ("spider calc into trx"));
      assert(!trx->alloc_func_name[id] ||
                  trx->alloc_func_name[id] == func_name);
      assert(!trx->alloc_file_name[id] ||
                  trx->alloc_file_name[id] == file_name);
      assert(!trx->alloc_line_no[id] || trx->alloc_line_no[id] == line_no);
      trx->alloc_func_name[id] = func_name;
      trx->alloc_file_name[id] = file_name;
      ...
      DBUG_PRINT("info", ("spider calc into global"));
      pthread_mutex_lock(&spider_mem_calc_mutex);
      assert(!spider_alloc_func_name[id] ||
                  spider_alloc_func_name[id] == func_name);
      assert(!spider_alloc_file_name[id] ||
                  spider_alloc_file_name[id] == file_name);
      assert(!spider_alloc_line_no[id] ||
...
void spider_free_mem(SPIDER_TRX *trx, void *ptr, myf my_flags __attribute__((unused))) {
  uint id, size;
...
void *spider_alloc_mem(SPIDER_TRX *trx, uint id, const char *func_name,
  ...
  if (!(ptr = (uchar *)my_malloc(PSI_NOT_INSTRUMENTED, size, my_flags))) DBUG_RETURN(NULL);
...
void *spider_bulk_alloc_mem(SPIDER_TRX *trx, uint id, const char *func_name,
  ...
  while (va_arg(args, char **)) total_size += ALIGN_SIZE(va_arg(args, uint));
  va_end(args);

  if (!(top_ptr = (uchar *)my_malloc(PSI_NOT_INSTRUMENTED, total_size, my_flags))) DBUG_RETURN(NULL);
...
spider_string::spider_string() : str(), next(NULL) {
...
spider_string::spider_string(uint32 length_arg) : str(length_arg), next(NULL) {
  DBUG_ENTER("spider_string::spider_string");
  DBUG_PRINT("info", ("spider this=%p", this));
  mem_calc_inited = FALSE;
  DBUG_VOID_RETURN;
...
spider_string::spider_string(const char *str, CHARSET_INFO *cs)
...
spider_string::spider_string(const char *str, uint32 len, CHARSET_INFO *cs)
...
spider_string::spider_string(char *str, uint32 len, CHARSET_INFO *cs)
    : str(str, len, cs), next(NULL) {
  DBUG_ENTER("spider_string::spider_string");
  DBUG_PRINT("info", ("spider this=%p", this));
  mem_calc_inited = FALSE;
...
spider_string::spider_string(const String &str) : str(str), next(NULL) {
  DBUG_ENTER("spider_string::spider_string");
  DBUG_PRINT("info", ("spider this=%p", this));
  mem_calc_inited = FALSE;
  DBUG_VOID_RETURN;
...
spider_string::~spider_string() {
  DBUG_ENTER("spider_string::~spider_string");
  DBUG_PRINT("info", ("spider this=%p", this));
  if (mem_calc_inited) free();
  DBUG_VOID_RETURN;
}

void spider_string::init_mem_calc(uint id, const char *func_name,
                                  const char *file_name, ulong line_no) {
  DBUG_ENTER("spider_string::init_mem_calc");
  DBUG_PRINT("info", ("spider this=%p", this));
  assert(!mem_calc_inited);
  this->id = id;
  ...
  if (str.is_alloced()) {
    ...
    spider_alloc_mem_calc(spider_current_trx, id, func_name, file_name, line_no,
                          current_alloc_mem);
  } else
    current_alloc_mem = 0;
...
void spider_string::set_charset(CHARSET_INFO *charset_arg) {
...
CHARSET_INFO *spider_string::charset() const {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_RETURN(const_cast<CHARSET_INFO*>(str.charset()));
}
...
const char &spider_string::operator[](uint32 i) const {
  DBUG_ENTER("spider_string::operator []");
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_RETURN(str[i]);
}

void spider_string::length(uint32 len) {
  DBUG_ENTER("spider_string::length");
  DBUG_PRINT("info", ("spider this=%p", this));
  str.length(len);
  DBUG_VOID_RETURN;
...
char *spider_string::c_ptr() {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  assert(mem_calc_inited);
  assert((!current_alloc_mem && !str.is_alloced()) ||
              current_alloc_mem == str.alloced_length());
  char *res = str.c_ptr();
  SPIDER_STRING_CALC_MEM;
  DBUG_RETURN(res);
...
LEX_STRING spider_string::lex_string() {
  DBUG_ENTER("spider_string::lex_string");
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_RETURN(str.lex_string());
}

void spider_string::set(String &str, uint32 offset, uint32 arg_length) {
...
void spider_string::set(char *str, uint32 arg_length, CHARSET_INFO *cs) {
  DBUG_ENTER("spider_string::set");
  DBUG_PRINT("info", ("spider this=%p", this));
  assert(mem_calc_inited);
  assert((!current_alloc_mem && !this->str.is_alloced()) ||
...
void spider_string::set(const char *str, uint32 arg_length, CHARSET_INFO *cs) {
  DBUG_ENTER("spider_string::set");
  DBUG_PRINT("info", ("spider this=%p", this));
  assert(mem_calc_inited);
  assert((!current_alloc_mem && !this->str.is_alloced()) ||
...
bool spider_string::set_ascii(const char *str, uint32 arg_length) {
...
void spider_string::set_quick(char *str, uint32 arg_length, CHARSET_INFO *cs) {
...
bool spider_string::set_int(longlong num, bool unsigned_flag,
                            CHARSET_INFO *cs) {
...
bool spider_string::set(longlong num, CHARSET_INFO *cs) {
  DBUG_ENTER("spider_string::set");
  DBUG_PRINT("info", ("spider this=%p", this));
  assert(mem_calc_inited);
  assert((!current_alloc_mem && !str.is_alloced()) ||
...
bool spider_string::set(ulonglong num, CHARSET_INFO *cs) {
  DBUG_ENTER("spider_string::set");
  DBUG_PRINT("info", ("spider this=%p", this));
  assert(mem_calc_inited);
  assert((!current_alloc_mem && !str.is_alloced()) ||
...
bool spider_string::set_real(double num, uint decimals, CHARSET_INFO *cs) {
...
bool spider_string::realloc(uint32 arg_length) {
  ...
  if (mem_calc_inited && !res && current_alloc_mem < str.alloced_length()) {
    spider_alloc_mem_calc(spider_current_trx, id, func_name, file_name, line_no,
                          str.alloced_length() - current_alloc_mem);
    current_alloc_mem = str.alloced_length();
  }
  DBUG_RETURN(res);
}

void spider_string::shrink(uint32 arg_length) {
...
spider_string &spider_string::operator=(const String &s) {
  DBUG_ENTER("spider_string::operator =");
  DBUG_PRINT("info", ("spider this=%p", this));
  assert(mem_calc_inited);
  assert((!current_alloc_mem && !str.is_alloced()) ||
...
bool spider_string::copy(const spider_string &s) {
  DBUG_ENTER("spider_string::copy");
  DBUG_PRINT("info", ("spider this=%p", this));
  assert(mem_calc_inited);
  assert((!current_alloc_mem && !str.is_alloced()) ||
...
bool spider_string::copy(const String &s) {
  DBUG_ENTER("spider_string::copy");
  DBUG_PRINT("info", ("spider this=%p", this));
  assert(mem_calc_inited);
  assert((!current_alloc_mem && !str.is_alloced()) ||
...
bool spider_string::copy(const char *s, uint32 arg_length, CHARSET_INFO *cs) {
  DBUG_ENTER("spider_string::copy");
  DBUG_PRINT("info", ("spider this=%p", this));
  assert(mem_calc_inited);
  assert((!current_alloc_mem && !str.is_alloced()) ||
...
bool spider_string::needs_conversion(uint32 arg_length, CHARSET_INFO *cs_from,
                                     CHARSET_INFO *cs_to, uint32 *offset) {
  DBUG_ENTER("spider_string::needs_conversion");
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_RETURN(str.needs_conversion(arg_length, cs_from, cs_to, (size_t*)offset));
}

bool spider_string::copy_aligned(const char *s, uint32 arg_length,
                                 uint32 offset, CHARSET_INFO *cs) {
...
bool spider_string::set_or_copy_aligned(const char *s, uint32 arg_length,
                                        CHARSET_INFO *cs) {
...
bool spider_string::copy(const char *s, uint32 arg_length, CHARSET_INFO *csfrom,
...
bool spider_string::append(const spider_string &s) {
...
bool spider_string::append(const String &s) {
  DBUG_ENTER("spider_string::append");
  DBUG_PRINT("info", ("spider this=%p", this));
  assert(mem_calc_inited);
  assert((!current_alloc_mem && !str.is_alloced()) ||
...
bool spider_string::append(const char *s) {
  DBUG_ENTER("spider_string::append");
  DBUG_PRINT("info", ("spider this=%p", this));
  assert(mem_calc_inited);
  assert((!current_alloc_mem && !str.is_alloced()) ||
...
bool spider_string::append(LEX_STRING *ls) {
  DBUG_ENTER("spider_string::append");
  DBUG_PRINT("info", ("spider this=%p", this));
  assert(mem_calc_inited);
  assert((!current_alloc_mem && !str.is_alloced()) ||
...
bool spider_string::append(const char *s, uint32 arg_length) {
  DBUG_ENTER("spider_string::append");
  DBUG_PRINT("info", ("spider this=%p", this));
  assert(mem_calc_inited);
  assert((!current_alloc_mem && !str.is_alloced()) ||
...
bool spider_string::append(const char *s, uint32 arg_length, CHARSET_INFO *cs) {
  DBUG_ENTER("spider_string::append");
  DBUG_PRINT("info", ("spider this=%p", this));
  assert(mem_calc_inited);
  assert((!current_alloc_mem && !str.is_alloced()) ||
...
bool spider_string::append_ulonglong(ulonglong val) {
...
/*
bool spider_string::append(IO_CACHE *file, uint32 arg_length) {
  DBUG_ENTER("spider_string::append");
  DBUG_PRINT("info", ("spider this=%p", this));
  assert(mem_calc_inited);
  assert((!current_alloc_mem && !str.is_alloced()) ||
...
bool spider_string::append_with_prefill(const char *s, uint32 arg_length,
  ...
  assert((!current_alloc_mem && !str.is_alloced()) ||
              current_alloc_mem == str.alloced_length());
  bool res = str.append_with_prefill(s, arg_length, full_length, fill_char);
  SPIDER_STRING_CALC_MEM;
  DBUG_RETURN(res);
}

int spider_string::strstr(const String &search, uint32 offset) {
  DBUG_ENTER("spider_string::strstr");
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_RETURN(str.strstr(search, offset));
}

int spider_string::strrstr(const String &search, uint32 offset) {
  DBUG_ENTER("spider_string::strrstr");
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_RETURN(str.strrstr(search, offset));
}

bool spider_string::replace(uint32 offset, uint32 arg_length, const char *to,
                            uint32 length) {
...
bool spider_string::replace(uint32 offset, uint32 arg_length,
                            const String &to) {
  DBUG_ENTER("spider_string::replace");
  DBUG_PRINT("info", ("spider this=%p", this));
  assert(mem_calc_inited);
...
bool spider_string::append(char chr) {
  DBUG_ENTER("spider_string::append");
  DBUG_PRINT("info", ("spider this=%p", this));
  assert(mem_calc_inited);
  assert((!current_alloc_mem && !str.is_alloced()) ||
...
bool spider_string::fill(uint32 max_length, char fill) {
...
int spider_string::charpos(int i, uint32 offset) {
  DBUG_ENTER("spider_string::charpos");
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_RETURN(str.charpos(i, offset));
}

int spider_string::reserve(uint32 space_needed) {
...
int spider_string::reserve(uint32 space_needed, uint32 grow_by) {
  DBUG_ENTER("spider_string::reserve");
  DBUG_PRINT("info", ("spider this=%p", this));
  assert(mem_calc_inited);
  assert((!current_alloc_mem && !str.is_alloced()) ||
...
void spider_string::q_append(const char c) {
...
void spider_string::q_append(const uint32 n) {
  DBUG_ENTER("spider_string::q_append");
  DBUG_PRINT("info", ("spider this=%p", this));
  str.q_append(n);
  SPIDER_STRING_CALC_MEM;
...
void spider_string::q_append(double d) {
  DBUG_ENTER("spider_string::q_append");
  DBUG_PRINT("info", ("spider this=%p", this));
  str.q_append(d);
  SPIDER_STRING_CALC_MEM;
...
void spider_string::q_append(const char *data, uint32 data_len) {
  DBUG_ENTER("spider_string::q_append");
  DBUG_PRINT("info", ("spider this=%p", this));
  str.append(data, data_len);
  SPIDER_STRING_CALC_MEM;
  DBUG_VOID_RETURN;
}

void spider_string::write_at_position(int position, uint32 value) {
...
void spider_string::qs_append(const char *str, uint32 len) {
...
void spider_string::qs_append(double d) {
  DBUG_ENTER("spider_string::qs_append");
  DBUG_PRINT("info", ("spider this=%p", this));
  str.q_append(d);
  SPIDER_STRING_CALC_MEM;
...
void spider_string::qs_append(const char c) {
  DBUG_ENTER("spider_string::qs_append");
  DBUG_PRINT("info", ("spider this=%p", this));
  str.append(c);
  SPIDER_STRING_CALC_MEM;
...
void spider_string::qs_append(int i) {
  DBUG_ENTER("spider_string::qs_append");
  DBUG_PRINT("info", ("spider this=%p", this));
  str.q_append((uint)i);
  SPIDER_STRING_CALC_MEM;
...
void spider_string::qs_append(uint i) {
  DBUG_ENTER("spider_string::qs_append");
  DBUG_PRINT("info", ("spider this=%p", this));
  str.q_append(i);
  SPIDER_STRING_CALC_MEM;
...
char *spider_string::prep_append(uint32 arg_length, uint32 step_alloc) {
...
void spider_string::append_escape_string(const char *st, uint len) {
...
bool spider_string::append_for_single_quote(const char *st, uint len) {
...
bool spider_string::append_for_single_quote(const String *s) {
  DBUG_ENTER("spider_string::append_for_single_quote");
  DBUG_PRINT("info", ("spider this=%p", this));
  assert(mem_calc_inited);
  assert((!current_alloc_mem && !str.is_alloced()) ||
...
#else
  bool res = append_escaped(&str, const_cast<String *>(s));
#endif
  SPIDER_STRING_CALC_MEM;
  DBUG_RETURN(res);
}

bool spider_string::append_for_single_quote(const char *st) {
  DBUG_ENTER("spider_string::append_for_single_quote");
  DBUG_PRINT("info", ("spider this=%p", this));
  assert(mem_calc_inited);
  assert((!current_alloc_mem && !str.is_alloced()) ||
...
void spider_string::swap(spider_string &s) {
...
bool spider_string::uses_buffer_owned_by(const String *s) const {
