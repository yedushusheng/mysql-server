...
#include "sql/sql_insert.h" // Sql_cmd_update
...
ha_spider::ha_spider() : handler(spider_hton_ptr, NULL) {
...
ha_spider::ha_spider(handlerton *hton, TABLE_SHARE *table_arg)
...
handler *ha_spider::clone(const char *name, MEM_ROOT *mem_root) {
  ha_spider *spider;
  DBUG_ENTER("ha_spider::clone");
  DBUG_PRINT("info", ("spider this=%p", this));
  if (!(spider = (ha_spider *)get_new_handler(table->s, (table->s->m_part_info != nullptr), mem_root,
                                              spider_hton_ptr)) ||
      !(spider->ref =
  ...
  // if (spider->ha_open(table, name, table->db_stat, HA_OPEN_IGNORE_IF_LOCKED))
  if (spider->ha_open(table, name, table->db_stat, HA_OPEN_IGNORE_IF_LOCKED, nullptr /* dd::TABLE */))
    DBUG_RETURN(NULL);
...
static const char *ha_spider_exts[] = {NullS};

const char **ha_spider::bas_ext() const { return ha_spider_exts; }

int ha_spider::open(const char *name, int mode[[maybe_unused]], uint test_if_locked[[maybe_unused]], const dd::Table *table_def [[maybe_unused]]) {
  THD *thd = ha_thd();
  int error_num, roop_count;
  int init_sql_alloc_size;
#ifdef WITH_PARTITION_STORAGE_ENGINE
  SPIDER_PARTITION_SHARE *partition_share;
  uchar *idx_read_bitmap, *idx_write_bitmap, *rnd_read_bitmap,
      *rnd_write_bitmap;
  uint part_num;
...
*/
    ...
    if (!(partition_handler_share =
              (SPIDER_PARTITION_HANDLER_SHARE *)my_hash_search_using_hash_value(
                  &partition_share->pt_handler_hash, hash_value,
                  (uchar *)&table, sizeof(TABLE *))))
#else
    if (!(partition_handler_share =
              (SPIDER_PARTITION_HANDLER_SHARE *)my_hash_search(
                  &partition_share->pt_handler_hash, (uchar *)&table,
                  sizeof(TABLE *))))
#endif
    {
      create_pt_handler_share = TRUE;
  ...
  if (create_pt_handler_share) {
    if (!(searched_bitmap = (uchar *)spider_bulk_malloc(
              spider_current_trx, 15, MYF(MY_WME), &searched_bitmap,
              sizeof(uchar) * no_bytes_in_map(table->read_set),
              &ft_discard_bitmap,
              sizeof(uchar) * no_bytes_in_map(table->read_set),
              &position_bitmap,
              sizeof(uchar) * no_bytes_in_map(table->read_set),
...
#endif
    if (!(searched_bitmap = (uchar *)spider_bulk_malloc(
              spider_current_trx, 16, MYF(MY_WME), &searched_bitmap,
              sizeof(uchar) * no_bytes_in_map(table->read_set),
              &ft_discard_bitmap,
              sizeof(uchar) * no_bytes_in_map(table->read_set),
              &position_bitmap,
              sizeof(uchar) * no_bytes_in_map(table->read_set), NullS))) {
      error_num = HA_ERR_OUT_OF_MEM;
      goto error_searched_bitmap_alloc;
    }
#ifdef WITH_PARTITION_STORAGE_ENGINE
      ...
      if (part_num) {
        if (partition_handler_share->use_count >= part_num)
          ...
          partition_handler_share
              ->handlers[partition_handler_share->use_count] = this;
  ...
  for (roop_count = 0; roop_count < (int)share->link_count; roop_count++) {
    ...
    if (share->dbton_share[dbton_id]->need_change_db_table_name()) {
      if (result_list.sqls[roop_count].real_alloc(init_sql_alloc_size) ||
  ...
  if (table_share->blob_fields) {
    ...
    for (roop_count = 0; roop_count < (int)table_share->fields; roop_count++) {
      ...
      blob_buff[roop_count].set_charset(const_cast<CHARSET_INFO*>(table->field[roop_count]->charset()));
    }
  }
  ...
  if (is_clone) {
    for (roop_count = 0; roop_count < (int)share->link_count; roop_count++) {
      if ((error_num2 = close_opened_handler(roop_count, FALSE))) {
        if (check_error_mode(error_num2)) error_num = error_num2;
      }
    }
  }
...
#ifdef HA_CAN_BULK_ACCESS
int ha_spider::additional_lock(THD *thd, enum thr_lock_type lock_type) {
...
THR_LOCK_DATA **ha_spider::store_lock(THD *thd, THR_LOCK_DATA **to,
  ...
  if ((error_num =
           check_access_kind(thd, (lock_type >= TL_WRITE_ALLOW_WRITE)))) {
  ...
  selupd_lock_mode =
      spider_param_selupd_lock_mode(thd, share->selupd_lock_mode);
  ...
  switch (sql_command) {
    ...
    case SQLCOM_HA_READ:
      if (lock_type == TL_READ_WITH_SHARED_LOCKS)
        ...
        lock_mode = 0;
        SPIDER_SET_CONNS_PARAM(
            semi_trx_isolation_chk, TRUE, conns, share->link_statuses,
  ...
  if (lock_type != TL_IGNORE && lock.type == TL_UNLOCK) {
    if (sql_command == SQLCOM_DROP_TABLE || sql_command == SQLCOM_ALTER_TABLE ||
    ...
    } else if (sql_command == SQLCOM_LOCK_TABLES ||
               (spider_param_lock_exchange(thd) == 1 &&
                share->semi_table_lock)) {
      DBUG_PRINT("info", ("spider lock exchange route"));
      DBUG_PRINT("info", ("spider lock_type=%u", this->lock_type));
      if ((this->lock_type == TL_READ || this->lock_type == TL_READ_NO_INSERT ||
        ...
        for (roop_count = spider_conn_link_idx_next(
                 share->link_statuses, conn_link_idx, -1, share->link_count,
                 SPIDER_LINK_STATUS_RECOVERY);
             roop_count < (int)share->link_count;
             roop_count = spider_conn_link_idx_next(
                 share->link_statuses, conn_link_idx, roop_count,
          ...
          if ((error_num =
                   dbton_handler[conn->dbton_id]->append_lock_tables_list(
                       conn, roop_count, &appended))) {
            store_error_num = error_num;
            DBUG_RETURN(to);
          }
          if (appended) {
      ...
      if (this->lock_type == TL_READ || this->lock_type == TL_READ_NO_INSERT ||
        ...
        for (roop_count = spider_conn_link_idx_next(
             ...
             roop_count = spider_conn_link_idx_next(
                 share->link_statuses, conn_link_idx, roop_count,
                 share->link_count, SPIDER_LINK_STATUS_RECOVERY)) {
          if (conns[roop_count] && conns[roop_count]->table_lock != 1 &&
      ...
      if (lock_type == TL_READ_NO_INSERT && !thd->in_lock_tables)
        lock_type = TL_READ;
      if (lock_type >= TL_WRITE_CONCURRENT_INSERT && lock_type <= TL_WRITE &&
          /*lock_type != TL_WRITE_DELAYED &&*/ !thd->in_lock_tables &&
...
int ha_spider::external_lock(THD *thd, int lock_type) {
  ...
  sql_command = thd_sql_command(thd);
  if (sql_command == SQLCOM_BEGIN) sql_command = SQLCOM_UNLOCK_TABLES;
  if (sql_command == SQLCOM_UNLOCK_TABLES &&
      (error_num = spider_check_trx_and_get_conn(thd, this, FALSE))) {
    DBUG_RETURN(error_num);
  }
  ...
  external_lock_cnt++;
#endif
  if (lock_type == F_UNLCK && sql_command != SQLCOM_UNLOCK_TABLES)
    DBUG_RETURN(0);
  if (store_error_num) DBUG_RETURN(store_error_num);

  if (
      /* SQLCOM_RENAME_TABLE and SQLCOM_DROP_DB don't come here */
      sql_command == SQLCOM_DROP_TABLE || sql_command == SQLCOM_ALTER_TABLE) {
  ...
  */
  ...
  while (tmp_bulk_access_link) {
    ...
    DBUG_PRINT("info", ("spider ptr bulk_access_link->spider->dbton_handler=%p",
                        &tmp_bulk_access_link->spider->dbton_handler));
    if (!tmp_bulk_access_link->used) break;
...
int ha_spider::extra(enum ha_extra_function operation) {
  ...
  switch (operation) {
...
#ifdef WITH_PARTITION_STORAGE_ENGINE
        if (update_request) {
          if (check_partitioned()) keyread = FALSE;
        }
#endif
      }
      break;
    ...
    defined(HA_EXTRA_HAS_HA_EXTRA_USE_CMP_REF)
...
int ha_spider::index_init(uint idx, bool sorted) {
  int error_num;
  DBUG_ENTER("ha_spider::index_init");
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_PRINT("info", ("spider idx=%u", idx));
#ifdef HA_CAN_BULK_ACCESS
  assert(!bulk_access_started);
  if (bulk_access_executing) {
    if (!is_bulk_access_clone && bulk_access_link_exec_tgt->called) {
      DBUG_RETURN(
          bulk_access_link_exec_tgt->spider->ha_index_init(idx, sorted));
    }
  }
#endif
  pushed_pos = NULL;
  ...
  if (pre_bitmap_checked)
    ...
    if (result_list.lock_type == F_WRLCK) {
      ...
      */
      if (update_request && share->have_recovery_link &&
    ...
    if (!is_clone) set_select_column_mode();
  }

  if ((error_num =
           reset_sql_sql(SPIDER_SQL_TYPE_SELECT_SQL | SPIDER_SQL_TYPE_HANDLER)))
    DBUG_RETURN(error_num);
...
#ifdef HA_CAN_BULK_ACCESS
int ha_spider::pre_index_init(uint idx, bool sorted) {
...
int ha_spider::index_end() {
  ...
  if (bulk_access_executing) {
    if (!is_bulk_access_clone && bulk_access_link_exec_tgt->called) {
      ...
      if (error_num) DBUG_RETURN(check_error_mode(error_num));
      DBUG_RETURN(0);
    }
  }
#endif
  ...
  */
  if ((error_num = drop_tmp_tables())) DBUG_RETURN(check_error_mode(error_num));
  result_list.use_union = FALSE;
  DBUG_RETURN(0);
}
...
int ha_spider::index_read_map_internal(uchar *buf, const uchar *key,
  ...
  do_direct_update = FALSE;
#endif
  if (find_flag >= HA_READ_MBR_CONTAIN && find_flag <= HA_READ_MBR_EQUAL)
  ...
  if ((error_num = spider_db_append_select(this)) ||
    ...
    DBUG_RETURN(error_num);
  if (share->key_hint && (error_num = append_hint_after_table_sql_part(
  ...
  result_list.limit_num = result_list.internal_limit >= result_list.split_read
                              ? result_list.split_read
                              : result_list.internal_limit;
  DBUG_PRINT("info", ("spider result_list.internal_limit=%lld",
                      result_list.internal_limit));
  DBUG_PRINT("info",
             ("spider result_list.split_read=%lld", result_list.split_read));
  DBUG_PRINT("info",
             ("spider result_list.limit_num=%lld", result_list.limit_num));
  if ((error_num = spider_db_append_key_where(&start_key, NULL, this)))
    DBUG_RETURN(error_num);
  DBUG_PRINT("info", ("spider result_list.internal_limit=%lld",
                      result_list.internal_limit));
  DBUG_PRINT("info",
  ...
  if (sql_kinds & SPIDER_SQL_KIND_SQL) {
    ...
    if ((error_num = append_limit_sql_part(result_list.internal_offset,
                                           result_list.limit_num,
                                           SPIDER_SQL_TYPE_SELECT_SQL))) {
      DBUG_RETURN(error_num);
    }
    if ((error_num = append_select_lock_sql_part(SPIDER_SQL_TYPE_SELECT_SQL))) {
      DBUG_RETURN(error_num);
    }
  }
  ...
  if (lock_mode) {
    ...
    link_ok =
        spider_conn_link_idx_next(share->link_statuses, conn_link_idx, -1,
                                  share->link_count, SPIDER_LINK_STATUS_OK);
    roop_start = spider_conn_link_idx_next(share->link_statuses, conn_link_idx,
                                           -1, share->link_count,
                                           SPIDER_LINK_STATUS_RECOVERY);
  ...
  for (roop_count = roop_start; roop_count < roop_end;
       roop_count = spider_conn_link_idx_next(
           share->link_statuses, conn_link_idx, roop_count, share->link_count,
           SPIDER_LINK_STATUS_RECOVERY)) {
    if (result_list.bgs_phase > 0) {
      if ((error_num =
               spider_check_and_init_casual_read(trx->thd, this, roop_count)))
        DBUG_RETURN(error_num);
      if ((error_num = spider_bg_conn_search(this, roop_count, roop_start, TRUE,
                                             FALSE, (roop_count != link_ok)))) {
        if (error_num != HA_ERR_END_OF_FILE &&
            share->monitoring_kind[roop_count] && need_mons[roop_count]) {
          error_num = spider_ping_table_mon_from_table(
              trx, trx->thd, share, roop_count,
              (uint32)share->monitoring_sid[roop_count], share->table_name,
              share->table_name_length, conn_link_idx[roop_count], NULL, 0,
              share->monitoring_kind[roop_count],
              share->monitoring_limit[roop_count],
              share->monitoring_flag[roop_count], TRUE);
        }
        DBUG_RETURN(check_error_mode_eof(error_num));
      }
    } else {
...
#ifdef HA_CAN_BULK_ACCESS
      if (is_bulk_access_clone && !bulk_access_executing &&
          conn_kind[roop_count] != SPIDER_CONN_KIND_MYSQL) {
        ...
        if ((error_num = spider_db_set_names(this, conn, roop_count))) {
          ...
          spider_mta_conn_mutex_unlock(conn);
          if (share->monitoring_kind[roop_count] && need_mons[roop_count]) {
            error_num = spider_ping_table_mon_from_table(
                trx, trx->thd, share, roop_count,
                (uint32)share->monitoring_sid[roop_count], share->table_name,
                share->table_name_length, conn_link_idx[roop_count], NULL, 0,
...
int ha_spider::pre_index_read_map(const uchar *key, key_part_map keypart_map,
                                  enum ha_rkey_function find_flag,
                                  bool use_parallel) {
#ifdef HA_CAN_BULK_ACCESS
  int error_num;
  ...
  if (bulk_access_started) {
    error_num = bulk_access_link_current->spider->pre_index_read_map(
        key, keypart_map, find_flag, TRUE);
    bulk_access_link_current->spider->bulk_access_pre_called = TRUE;
    bulk_access_link_current->called = TRUE;
    DBUG_RETURN(error_num);
  } else if (bulk_access_executing && !is_bulk_access_clone &&
             bulk_access_link_exec_tgt->called) {
    DBUG_RETURN(bulk_access_link_exec_tgt->spider->pre_index_read_map(
        key, keypart_map, find_flag, TRUE));
  }
#endif
...
#ifdef HA_CAN_BULK_ACCESS
    if (!store_error_num && bulk_access_executing && is_bulk_access_clone &&
...
int ha_spider::index_read_map(uchar *buf, const uchar *key,
                              key_part_map keypart_map,
                              enum ha_rkey_function find_flag) {
  int error_num;
  ...
  if (bulk_access_executing) {
    if (is_bulk_access_clone) {
      ...
      DBUG_RETURN(bulk_access_link_exec_tgt->spider->index_read_map(
...
int ha_spider::index_read_last_map_internal(uchar *buf, const uchar *key,
                                            key_part_map keypart_map) {
  int error_num;
  key_range start_key;
  SPIDER_CONN *conn;
  ...
  /*
    spider_db_free_one_result_for_start_next(this);
  */
  if (result_list.current && (error_num = spider_db_free_result(this, FALSE)))
    DBUG_RETURN(error_num);
...
int ha_spider::pre_index_read_last_map(const uchar *key,
                                       key_part_map keypart_map,
                                       bool use_parallel) {
  ...
  check_pre_call(use_parallel);
  if (use_pre_call) {
    store_error_num = index_read_last_map_internal(NULL, key, keypart_map);
    DBUG_RETURN(store_error_num);
  }
  DBUG_RETURN(0);
}

int ha_spider::index_read_last_map(uchar *buf, const uchar *key,
                                   key_part_map keypart_map) {
  int error_num;
...
int ha_spider::index_next(uchar *buf) {
  int error_num;
  backup_error_status();
...
int ha_spider::index_prev(uchar *buf) {
  int error_num;
  backup_error_status();
...
int ha_spider::index_first_internal(uchar *buf) {
  int error_num;
  SPIDER_CONN *conn;
  backup_error_status();
  ...
  if (sql_is_empty(SPIDER_SQL_TYPE_HANDLER) ||
    ...
    */
    ...
    result_list.limit_num = result_list.internal_limit >= result_list.split_read
                                ? result_list.split_read
                                : result_list.internal_limit;
    if ((error_num = spider_db_append_key_where(NULL, NULL, this)))
      DBUG_RETURN(error_num);
    if (sql_kinds & SPIDER_SQL_KIND_SQL) {
      if (result_list.direct_order_limit) {
        if ((error_num =
      ...
      if ((error_num =
               append_select_lock_sql_part(SPIDER_SQL_TYPE_SELECT_SQL))) {
        DBUG_RETURN(error_num);
      }
    }
    if (sql_kinds & SPIDER_SQL_KIND_HANDLER) {
    ...
    for (roop_count = roop_start; roop_count < roop_end;
      ...
      if (result_list.bgs_phase > 0) {
        if ((error_num =
          ...
          DBUG_RETURN(error_num);
        if ((error_num =
                 spider_bg_conn_search(this, roop_count, roop_start, TRUE,
                                       FALSE, (roop_count != link_ok)))) {
          if (error_num != HA_ERR_END_OF_FILE &&
              share->monitoring_kind[roop_count] && need_mons[roop_count]) {
            error_num = spider_ping_table_mon_from_table(
            ...
            if ((error_num = spider_db_store_result(this, roop_count, table))) {
              if (error_num != HA_ERR_END_OF_FILE &&
                ...
                error_num = spider_ping_table_mon_from_table(
                    ...
                    share->table_name, share->table_name_length,
                    conn_link_idx[roop_count], NULL, 0,
                    share->monitoring_kind[roop_count],
                    share->monitoring_limit[roop_count],
                    share->monitoring_flag[roop_count], TRUE);
              }
...
int ha_spider::pre_index_first(bool use_parallel) {
  DBUG_ENTER("ha_spider::pre_index_first");
  DBUG_PRINT("info", ("spider this=%p", this));
  check_pre_call(use_parallel);
  if (use_pre_call) {
    store_error_num = index_first_internal(NULL);
    DBUG_RETURN(store_error_num);
  }
  DBUG_RETURN(0);
}

int ha_spider::index_first(uchar *buf) {
  int error_num;
...
int ha_spider::index_last_internal(uchar *buf) {
  int error_num;
  SPIDER_CONN *conn;
  backup_error_status();
  ...
  if (sql_is_empty(SPIDER_SQL_TYPE_HANDLER) ||
    ...
    */
    ...
    if (sql_kinds & SPIDER_SQL_KIND_SQL) {
      ...
      if (!(result_list.is_get_increment) &&
          (error_num =
               append_select_lock_sql_part(SPIDER_SQL_TYPE_SELECT_SQL))) {
        DBUG_RETURN(error_num);
      }
...
int ha_spider::pre_index_last(bool use_parallel) {
  DBUG_ENTER("ha_spider::pre_index_last");
  DBUG_PRINT("info", ("spider this=%p", this));
  check_pre_call(use_parallel);
  if (use_pre_call) {
    store_error_num = index_last_internal(NULL);
    DBUG_RETURN(store_error_num);
  }
  DBUG_RETURN(0);
}

int ha_spider::index_last(uchar *buf) {
  int error_num;
...
int ha_spider::index_next_same(uchar *buf, const uchar *key[[maybe_unused]], uint keylen[[maybe_unused]]) {
  int error_num;
  backup_error_status();
  ...
  if (bulk_access_executing) {
    if (!is_bulk_access_clone && bulk_access_link_exec_tgt->called) {
      error_num =
          bulk_access_link_exec_tgt->spider->index_next_same(buf, key, keylen);
      if (error_num) DBUG_RETURN(check_error_mode(error_num));
      DBUG_RETURN(0);
    }
  }
...
int ha_spider::read_range_first_internal(uchar *buf, const key_range *start_key,
  ...
  do_direct_update = FALSE;
#endif
  if (start_key && start_key->flag >= HA_READ_MBR_CONTAIN &&
      start_key->flag <= HA_READ_MBR_EQUAL)
    use_spatial_index = TRUE;

  if (end_key) {
    set_key_compare_result_on_equal(
  ...
  if ((error_num = spider_db_append_key_where(start_key,
                                              eq_range ? NULL : end_key, this)))
    DBUG_RETURN(error_num);
  if (sql_kinds & SPIDER_SQL_KIND_SQL) {
    if (result_list.direct_order_limit) {
      if ((error_num =
...
int ha_spider::pre_read_range_first(const key_range *start_key,
                                    const key_range *end_key, bool eq_range,
                                    bool sorted, bool use_parallel) {
...
int ha_spider::read_range_first(const key_range *start_key,
                                const key_range *end_key, bool eq_range,
                                bool sorted) {
  int error_num;
  ...
  if (use_pre_call) {
    ...
    if ((error_num = spider_bg_all_conn_pre_next(this, search_link_idx)))
      DBUG_RETURN(error_num);
    use_pre_call = FALSE;
    if ((error_num = read_range_next())) DBUG_RETURN(error_num);
...
int ha_spider::read_range_next() {
  ...
  if ((error_num =
           spider_db_seek_next(table->record[0], this, search_link_idx, table)))
    DBUG_RETURN(check_error_mode_eof(error_num));
  DBUG_RETURN(check_ha_range_eof());
}
...
ha_rows ha_spider::multi_range_read_info_const(uint keyno, RANGE_SEQ_IF *seq,
                                               void *seq_init_param,
...
ha_rows ha_spider::multi_range_read_info(uint keyno, uint n_ranges, uint keys,
                                         uint *bufsz,
                                         uint *flags, Cost_estimate *cost)
...
int ha_spider::multi_range_read_init(RANGE_SEQ_IF *seq, void *seq_init_param,
...
int ha_spider::multi_range_read_next_first(char **range_info)

#else
int ha_spider::read_multi_range_first_internal(uchar *buf,
...
#endif
{
  ...
  if (
...
#endif
    {
      ...
      for (roop_count = roop_start; roop_count < roop_end;
        ...
        if (result_list.bgs_phase > 0) {
          if ((error_num = spider_check_and_init_casual_read(trx->thd, this,
                                                             roop_count)))
            DBUG_RETURN(error_num);
          error_num = spider_bg_conn_search(this, roop_count, roop_start, TRUE,
                                            FALSE, (roop_count != link_ok));
      ...
      if (error_num) {
        if (error_num != HA_ERR_END_OF_FILE && check_error_mode(error_num))
          DBUG_RETURN(error_num);
        DBUG_PRINT("info", ("spider result_list.finish_flg = FALSE"));
        result_list.finish_flg = FALSE;
        result_list.record_num = 0;
...
#endif
    if (error_num) DBUG_RETURN(check_error_mode_eof(error_num));
  } else {
    ...
    if (!(multi_range_keys = (char **)spider_malloc(
              spider_current_trx, 1,
              sizeof(char *) * (multi_range_num < result_list.multi_split_read
                                    ? multi_range_num
    ...
    if (first_mrr_start_key_length) {
      if (mrr_key_buff[0].reserve(first_mrr_start_key_length))
        DBUG_RETURN(HA_ERR_END_OF_FILE);
      mrr_key_buff[0].append((const char *)first_mrr_start_key,
    ...
    if (first_mrr_end_key_length) {
      if (mrr_key_buff[1].reserve(first_mrr_end_key_length))
        DBUG_RETURN(HA_ERR_END_OF_FILE);
      mrr_key_buff[1].append((const char *)first_mrr_end_key,
    ...
    do {
      ...
      if (bka_mode && have_multi_range &&
        ...
        memset(result_list.tmp_table_join_first, 0xFF, share->link_bitmap_size);
      } else {
        result_list.tmp_table_join = FALSE;
        if (result_list.have_sql_kind_backup) {
          for (roop_count = 0; roop_count < (int)share->link_count;
               roop_count++) {
            sql_kind[roop_count] = result_list.sql_kind_backup[roop_count];
          }
      ...
      if (result_list.tmp_table_join) {
...
#endif
        {
          ...
          multi_range_cnt++;
          if (multi_range_cnt >= (uint)result_list.multi_split_read) break;
#ifdef HA_MRR_USE_DEFAULT_IMPL
        ...
        if ((error_num = init_union_table_name_pos_sql()) ||
            (error_num =
...
#endif
        {
          ...
          if ((error_num = append_limit_sql_part(
                   0, result_list.internal_offset + result_list.limit_num,
                   SPIDER_SQL_TYPE_SELECT_SQL))) {
            DBUG_RETURN(error_num);
          }
          if ((error_num =
        ...
        if (result_list.bgs_phase > 0) {
          ...
          if (is_bulk_access_clone) {
            ...
            *found_range_p = &multi_range_ranges[multi_range_hit_point];
            ...
            if (result_list.tmp_table_join && bka_mode != 2 &&
            ...
            if (roop_count == link_ok) {
              if ((error_num =
                       spider_db_store_result(this, roop_count, table))) {
                if (error_num != HA_ERR_END_OF_FILE &&
                    share->monitoring_kind[roop_count] &&
                    need_mons[roop_count]) {
                  error_num = spider_ping_table_mon_from_table(
...
#ifdef HA_MRR_USE_DEFAULT_IMPL
int ha_spider::pre_multi_range_read_next(bool use_parallel) {
  DBUG_ENTER("ha_spider::pre_multi_range_read_next");
  DBUG_PRINT("info", ("spider this=%p", this));
  check_pre_call(use_parallel);
  if (use_pre_call) {
    store_error_num = multi_range_read_next_first(NULL);
    DBUG_RETURN(store_error_num);
  }
  DBUG_RETURN(0);
}
...
int ha_spider::multi_range_read_next(char **range_info)
...
#else
int ha_spider::pre_read_multi_range_first(KEY_MULTI_RANGE **found_range_p,
                                          KEY_MULTI_RANGE *ranges,
                                          uint range_count, bool sorted,
                                          HANDLER_BUFFER *buffer,
                                          bool use_parallel) {
  ...
  check_pre_call(use_parallel);
  if (use_pre_call) {
    store_error_num = read_multi_range_first_internal(
        NULL, found_range_p, ranges, range_count, sorted, buffer);
    DBUG_RETURN(store_error_num);
  }
  DBUG_RETURN(0);
}

int ha_spider::read_multi_range_first(KEY_MULTI_RANGE **found_range_p,
  ...
  DBUG_RETURN(read_multi_range_first_internal(
      table->record[0], found_range_p, ranges, range_count, sorted, buffer));
}
#endif

#ifdef HA_MRR_USE_DEFAULT_IMPL

int ha_spider::multi_range_read_next_next(char **range_info)

#else
int ha_spider::read_multi_range_next(KEY_MULTI_RANGE **found_range_p)
#endif
{
  int error_num, roop_count;
  SPIDER_CONN *conn;
...
#else
      if (multi_range_curr < multi_range_end) multi_range_curr++;
#endif
    } else {
      result_list.tmp_table_join_break_after_get_next = FALSE;
#ifdef HA_MRR_USE_DEFAULT_IMPL
...
int ha_spider::rnd_init_quick() {
  // rnd_scan_and_first = true;
  // spider_set_result_list_param(this);
  // result_list.quick_mode = 1;
  result_list.current_row_num = 0;
  return 0;
}

int ha_spider::rnd_init(bool scan) {
  int error_num;
  ...
  */
  ...
  if (scan && sql_command != SQLCOM_ALTER_TABLE) {
    spider_set_result_list_param(this);
    pk_update = FALSE;
    if (result_list.current && !result_list.low_mem_read &&
        ...
        for (roop_count = roop_start; roop_count < roop_end;
          ...
          if (quick_targets[roop_count]) {
            spider_db_free_one_quick_result(
                (SPIDER_RESULT *)result_list.current);
            assert(quick_targets[roop_count] == conn->quick_target);
...
#ifdef HA_CAN_BULK_ACCESS
int ha_spider::pre_rnd_init(bool scan) {
...
int ha_spider::rnd_next_internal(uchar *buf) {
  int error_num;
#ifdef WITH_PARTITION_STORAGE_ENGINE
  ...
  if (sql_command == SQLCOM_ALTER_TABLE) DBUG_RETURN(HA_ERR_END_OF_FILE);
#ifdef HANDLER_HAS_DIRECT_UPDATE_ROWS
  do_direct_update = FALSE;
#endif
  ...
  if (rnd_scan_and_first) {
...
#ifdef WITH_PARTITION_STORAGE_ENGINE
      if (direct_limit_offset_spider->direct_select_limit ==
          0) {  // mean has got all result
        DBUG_RETURN(check_error_mode_eof(HA_ERR_END_OF_FILE));
      }
      if (partition_handler_share->handlers &&
          direct_limit_offset_spider->direct_current_offset > 0) {
        ha_rows table_count = 0;
        this->records(&table_count);
        DBUG_PRINT("info", ("spider table_count=%lld", table_count));
        if (table_count <= (ha_rows)direct_limit_offset_spider->direct_current_offset) {
          // skip this spider(partition)
          direct_limit_offset_spider->direct_current_offset -= table_count;
          DBUG_PRINT("info",
                     ("spider direct_current_offset=%lld",
    ...
    if (this->result_list.direct_limit_offset) {
      if (buf &&
          (error_num = spider_db_seek_next(buf, this, search_link_idx, table)))
        DBUG_RETURN(check_error_mode_eof(error_num));
      DBUG_RETURN(0);
    }
  }
...
#ifdef WITH_PARTITION_STORAGE_ENGINE
  if (result_list.direct_limit_offset &&
...
int ha_spider::rnd_next_quick(uchar *buf [[maybe_unused]], THD *thd,
                              mem_root_deque<Item *> *fields, bool need_rewrite) {
  DBUG_ENTER("ha_spider::rnd_next_quick");
  int error_num = 0;
  if (result_list.record_num == 0) {
    SPIDER_CONN *conn = spider_get_conn_by_idx(0);
    if (!conn) {
      DBUG_RETURN(store_error_num);
    }
    spider_conn_set_state(conn, SPIDER_CONN_STATE_EXEC_QUERY);
    if ((error_num = spider_db_set_names(this, conn, 0))) {
      DBUG_RETURN(check_error_mode_eof(error_num));
    }
    // some pushdown sql need rewrite
    if (!need_rewrite) {
      if ((error_num = spider_db_query(conn, thd->query().str,
                                       thd->query().length, false, &need_mons[0]))) {
        error_num = spider_db_errorno(conn);
        DBUG_RETURN(check_error_mode_eof(error_num));
      }
    } else {
      Query_block *select = spider_get_select_lex(this);
      String query;
      // need write pushed cond in sql
      if (pushed_cond) {
        *(select->where_cond_ref()) = const_cast<Item*>(pushed_cond);
      }
      select->print_query_block(thd, &query, QT_PRINT_ALL_ITEMS);
      if ((error_num = spider_db_query(conn, query.ptr(),
                                       query.length(), false, &need_mons[0]))) {
        error_num = spider_db_errorno(conn);
        DBUG_RETURN(check_error_mode_eof(error_num));
      }
    }

    // SPIDER_RESULT_LIST *result_list = &result_list;
    if (!result_list.first) {
      if (!(result_list.first = (SPIDER_RESULT *)spider_malloc(
                spider_current_trx, 4, sizeof(*result_list.first),
                MYF(MY_WME | MY_ZEROFILL)))) {
        DBUG_RETURN(HA_ERR_OUT_OF_MEM);
      }
    } 
    if (!(result_list.first->result =
                  conn->db_conn->use_result(nullptr, &error_num))) {
    }
  }
  SPIDER_RESULT *current = result_list.first;
  SPIDER_DB_ROW *row;
  ...
  // store the data to fields passed from sql layer
  if (fields) {
...
#endif
      if ((error_num = spider_db_fetch_row(this, share, ((Item_field *)item)->field, row, 0)))
        DBUG_RETURN(error_num);
#ifndef DBUG_OFF
...
int ha_spider::direct_pushdown_update(THD *thd, ha_rows *update_rows, ha_rows *found_rows) {
  DBUG_ENTER("ha_spider::direct_pushdown_update"); 
  int error_num = 0;
  SPIDER_CONN *conn = spider_get_conn_by_idx(0);
  if (!conn) {
    DBUG_RETURN(store_error_num);
  ...
  if ((error_num = spider_db_query(conn, thd->query().str, thd->query().length, false, &need_mons[0]))) {
    error_num = spider_db_errorno(conn);
    DBUG_RETURN(check_error_mode_eof(error_num));
  }
...
int ha_spider::direct_pushdown_insert(THD *thd, ha_rows *insert_rows, ha_rows *found_rows) {
  DBUG_ENTER("ha_spider::direct_pushdown_insert"); 
  int error_num = 0;
  SPIDER_CONN *conn = spider_get_conn_by_idx(0);
  if (!conn) {
    DBUG_RETURN(store_error_num);
  ...
  *insert_rows = conn->db_conn->affected_rows();
  *found_rows = conn->db_conn->matched_rows();
  DBUG_RETURN(0);
}

int ha_spider::direct_pushdown_delete(THD *thd, ha_rows *delete_rows, ha_rows *found_rows) {
  DBUG_ENTER("ha_spider::direct_pushdown_delete"); 
  int error_num = 0;
  SPIDER_CONN *conn = spider_get_conn_by_idx(0);
  if (!conn) {
    DBUG_RETURN(store_error_num);
...
int ha_spider::pre_rnd_next(bool use_parallel) {
  DBUG_ENTER("ha_spider::pre_rnd_next");
  DBUG_PRINT("info", ("spider this=%p", this));
  check_pre_call(use_parallel);
  if (use_pre_call) {
    store_error_num = rnd_next_internal(NULL);
    DBUG_RETURN(store_error_num);
  }
  DBUG_RETURN(0);
}

int ha_spider::rnd_next(uchar *buf) {
  int error_num;
...
void ha_spider::position(const uchar *record) {
...
int ha_spider::rnd_pos(uchar *buf, uchar *pos) {
...
int ha_spider::cmp_ref(const uchar *ref1, const uchar *ref2) const {
  int ret = 0;
  DBUG_ENTER("ha_spider::cmp_ref");
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_PRINT("info", ("spider ref1=%p", ref1));
  DBUG_PRINT("info", ("spider ref2=%p", ref2));
  (const_cast<ha_spider*>(this))->result_list.in_cmp_ref = true;
  if (table_share->primary_key < MAX_KEY) {
    uchar table_key[MAX_KEY_LENGTH];
    KEY *key_info = &table->key_info[table_share->primary_key];
    DBUG_PRINT("info", ("spider cmp by primary key"));
    (const_cast<ha_spider*>(this))->rnd_pos(table->record[0], const_cast<uchar *>(ref2));
    key_copy(table_key, table->record[0], key_info, key_info->key_length);
    (const_cast<ha_spider*>(this))->rnd_pos(table->record[0], const_cast<uchar *>(ref1));
    ...
    (const_cast<ha_spider*>(this))->rnd_pos(table->record[1], const_cast<uchar *>(ref2));
    (const_cast<ha_spider*>(this))->rnd_pos(table->record[0], const_cast<uchar *>(ref1));
    for (field = table->field; *field; field++) {
      if ((ret = (*field)->cmp_binary_offset((uint)ptr_diff))) {
        DBUG_PRINT("info",
                   ("spider different at %s", (*field)->field_name));
        break;
      }
    }
  }
  (const_cast<ha_spider*>(this))->result_list.in_cmp_ref = false;
  DBUG_PRINT("info", ("spider ret=%d", ret));
  DBUG_RETURN(ret);
}

float spider_ft_find_relevance(FT_INFO *handler, uchar *record[[maybe_unused]], uint length[[maybe_unused]]) {
...
float spider_ft_get_relevance(FT_INFO *handler) {
...
void spider_ft_close_search(FT_INFO *handler[[maybe_unused]]) {
...
_ft_vft spider_ft_vft = {
    ...
    spider_ft_find_relevance, spider_ft_close_search, spider_ft_get_relevance,
...
int ha_spider::ft_init() {
  ...
  if (active_index == MAX_KEY && inited == NONE) {
    ...
    while (TRUE) {
      ...
      if (ft_info == ft_current) break;
      ft_info = ft_info->next;
    }
    if (ft_init_idx == MAX_KEY) {
      if ((error_num = rnd_init(TRUE))) DBUG_RETURN(error_num);
    }
  } else {
...
FT_INFO *ha_spider::ft_init_ext(uint flags, uint inx, String *key) {
  st_spider_ft_info *tmp_ft_info;
  backup_error_status();
  ...
  if (!ft_current) {
    if (!(ft_current = (st_spider_ft_info *)spider_malloc(
              spider_current_trx, 2, sizeof(st_spider_ft_info),
              MYF(MY_WME | MY_ZEROFILL)))) {
...
int ha_spider::ft_read_internal(uchar *buf) {
  int error_num;
  backup_error_status();
  ...
  if (ft_init_and_first) {
    ...
    if (ft_init_idx < MAX_KEY && share->key_hint &&
        (error_num =
    ...
    for (roop_count = roop_start; roop_count < roop_end;
      ...
      if (result_list.bgs_phase > 0) {
        ...
        if ((error_num = dbton_hdl->set_sql_for_exec(SPIDER_SQL_TYPE_SELECT_SQL,
                                                     roop_count))) {
          DBUG_RETURN(error_num);
        }
        if (!dbton_hdl->need_lock_before_set_sql_for_exec(
...
int ha_spider::pre_ft_read(bool use_parallel) {
  DBUG_ENTER("ha_spider::pre_ft_read");
  DBUG_PRINT("info", ("spider this=%p", this));
  check_pre_call(use_parallel);
  if (use_pre_call) {
    store_error_num = ft_read_internal(NULL);
    DBUG_RETURN(store_error_num);
  }
  DBUG_RETURN(0);
}

int ha_spider::ft_read(uchar *buf) {
  int error_num;
...
int ha_spider::info(uint flag) {
  ...
  if (flag & (HA_STATUS_TIME | HA_STATUS_CONST | HA_STATUS_VARIABLE |
    ...
    if (flag & HA_STATUS_TIME) stats.update_time = (ulong)share->update_time;
    ...
    if (flag & HA_STATUS_AUTO) {
      ...
      if (share->partition_share && table->next_number_field) {
        auto tmp_auto_increment_mode = tdsql_auto_increment_mode(thd, share->auto_increment_mode);
        ulonglong first_value, nb_reserved_values;
        if (tmp_auto_increment_mode == SpiderAutoIncrementMode::kGenerateBySelectMax &&
...
ha_rows ha_spider::records_in_range(uint inx, key_range *start_key,
                                    key_range *end_key) {
  THD *thd = ha_thd();
      ...
      if ((spider_init_error_table =
        ...
        if (difftime(tmp_time, spider_init_error_table->init_error_time) <
          ...
          if (check_error_mode(spider_init_error_table->init_error))
            ...
            set_my_errno(spider_init_error_table->init_error);
          DBUG_RETURN(HA_POS_ERROR);
        }
      }
      pthread_mutex_unlock(&share->crd_mutex);
      if (crd_mode == 3) crd_mode = 1;
    }
  }
  ...
  if (crd_mode == 1 || crd_mode == 2) {
    ...
    if (share->static_key_cardinality[inx] == -1) {
      for (key_part = key_info->key_part; tgt_key_part_map > 1;
           tgt_key_part_map >>= 1, key_part++) {
        ...
        if (share->cardinality[field->field_index()] == -1) {
          DBUG_PRINT("info", ("spider uninitialized column cardinality"));
          DBUG_RETURN(HA_POS_ERROR);
        }
        if ((rate = ((double)share->cardinality[field->field_index()]) /
                    weight) >= 1) {
          if ((rows = rows / rate) < 2) {
    ...
    if (start_key_part_map >= end_key_part_map &&
      ...
      if (share->static_key_cardinality[inx] == -1) {
        if ((rate = ((double)share->cardinality[field->field_index()]) / weight /
                    4) >= 1)
          rows = rows / rate;
      } else {
        if ((rate = ((double)share->static_key_cardinality[inx]) / 4) >= 1)
          rows = rows / rate;
      }
    } else {
      if (share->static_key_cardinality[inx] == -1) {
        ...
        if ((rate = ((double)share->static_key_cardinality[inx]) / 16) >= 1)
          rows = rows / rate;
      }
    }
    if (rows < 2) {
    ...
    DBUG_RETURN(
        spider_db_explain_select(start_key, end_key, this, search_link_idx));
  }
...
int ha_spider::check_crd() {
  ...
  DBUG_PRINT("info",
             ("spider difftime=%f", difftime(tmp_time, share->crd_get_time)));
  DBUG_PRINT("info", ("spider crd_interval=%f", crd_interval));
  if (difftime(tmp_time, share->crd_get_time) >= crd_interval) {
    if (crd_interval == 0 || !pthread_mutex_trylock(&share->crd_mutex)) {
      if (crd_interval == 0 || crd_bg_mode == 0) {
        if (crd_interval == 0) pthread_mutex_lock(&share->crd_mutex);
        if (difftime(tmp_time, share->crd_get_time) >= crd_interval) {
          if ((error_num = spider_get_crd(share, search_link_idx, tmp_time,
                                          ...
                                          share->crd_init ? 2 : 1))) {
            pthread_mutex_unlock(&share->crd_mutex);
            if (share->monitoring_kind[search_link_idx] &&
                need_mons[search_link_idx]) {
              error_num = spider_ping_table_mon_from_table(
                  trx, trx->thd, share, search_link_idx,
                  (uint32)share->monitoring_sid[search_link_idx],
                  share->table_name, share->table_name_length,
                  conn_link_idx[search_link_idx], NULL, 0,
                  share->monitoring_kind[search_link_idx],
                  share->monitoring_limit[search_link_idx],
                  share->monitoring_flag[search_link_idx], TRUE);
            }
            if (!share->crd_init) {
              if (spider_init_error_table ||
...
int ha_spider::pre_records() {
  ...
  if (!(share->additional_table_flags & HA_COUNT_ROWS_INSTANT)) {
    DBUG_RETURN(0);
  }
  THD *thd = trx->thd;
...
int ha_spider::records(ha_rows *num_rows) {
  int error_num;
  backup_error_status();
  ...
  if (!(share->additional_table_flags & HA_COUNT_ROWS_INSTANT) &&
      !this->result_list.direct_limit_offset) {
    DBUG_RETURN(handler::records(num_rows));
  }
...
ulonglong ha_spider::table_flags() const {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  ulonglong flags =
      HA_REC_NOT_IN_SEQ | HA_CAN_GEOMETRY | HA_NULL_IN_KEY |
      HA_CAN_INDEX_BLOBS | HA_AUTO_PART_KEY | HA_CAN_RTREEKEYS |
      HA_PRIMARY_KEY_REQUIRED_FOR_DELETE |
      /* HA_NO_PREFIX_CHAR_KEYS | */
      HA_CAN_FULLTEXT | HA_CAN_SQL_HANDLER | HA_FILE_BASED |
      /*HA_CAN_INSERT_DELAYED |*/ HA_CAN_BIT_FIELD | HA_NO_COPY_ON_ALTER |
      HA_BINLOG_ROW_CAPABLE | HA_BINLOG_STMT_CAPABLE | HA_PARTIAL_COLUMN_READ |
...
#endif
      SPIDER_CAN_BG_SEARCH | SPIDER_CAN_BG_INSERT | SPIDER_CAN_BG_UPDATE |
...
#endif
      HA_CAN_INDEX_VIRTUAL_GENERATED_COLUMN | // support index on virtual columns
      HA_GENERATED_COLUMNS | // support virtual columns
      HA_SUPPORTS_DEFAULT_EXPRESSION | // spport default func
...
const char *ha_spider::index_type(uint key_number) {
  ...
  DBUG_RETURN((key_info->flags & HA_FULLTEXT)
                  ? "FULLTEXT"
...
ulong ha_spider::index_flags(uint idx, uint part[[maybe_unused]], bool all_parts[[maybe_unused]]) const {
  DBUG_ENTER("ha_spider::index_flags");
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_RETURN((table_share->key_info[idx].algorithm == HA_KEY_ALG_FULLTEXT)
                  ? 0
...
uint ha_spider::max_supported_key_part_length(HA_CREATE_INFO *create_info MY_ATTRIBUTE((unused))) const {
  ...
  DBUG_PRINT("info",
             ("spider return=%s",
              (!share->lgtm_tblhnd_share->auto_increment_init ||
               (tdsql_auto_increment_mode(thd, share->auto_increment_mode) ==
                    SpiderAutoIncrementMode::kGenerateBySelectMax &&
                !info_auto_called))
                  ? "TRUE"
                  : "FALSE"));
  DBUG_RETURN((!share->lgtm_tblhnd_share->auto_increment_init ||
               (tdsql_auto_increment_mode(thd, share->auto_increment_mode) ==
                    SpiderAutoIncrementMode::kGenerateBySelectMax &&
                !info_auto_called)));
}
...
bool ha_spider::can_use_for_auto_inc_init() {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_PRINT("info",
             ("spider return=%s", (!auto_inc_temporary) ? "TRUE" : "FALSE"));
  DBUG_RETURN((!auto_inc_temporary));
}
#endif
...
int ha_spider::update_auto_increment() {
  ...
  */
  ...
  if (!check_partitioned()) {
    // no need update_auto_increment again if it's called from PartitionHandler.
...
void ha_spider::get_auto_increment(ulonglong offset[[maybe_unused]], ulonglong increment,
                                   ulonglong nb_desired_values,
                                   ulonglong *first_value,
                                   ulonglong *nb_reserved_values) {
  THD *thd = ha_thd();
  auto auto_increment_mode = tdsql_auto_increment_mode(thd, share->auto_increment_mode);
  DBUG_ENTER("ha_spider::get_auto_increment");
  DBUG_PRINT("info", ("spider this=%p", this));
  *nb_reserved_values = ULONG_LONG_MAX;
  using tdsql::common::CAutoInc;
  if (CAutoInc::distributed_auto_inc_enabled() &&
      table->s->tdsql_table_type == tdsql::ddl::TD_ALLSET_TABLE) {
    // implicit assurance via tdsql_auto_increment_mode().
    assert(auto_increment_mode == SpiderAutoIncrementMode::kGenerateByDistIDGen);  
    tdsql::common::TdString db_table =
        std::string(table->s->db.str, table->s->db.length) + "." +
        std::string(table->s->table_name.str, table->s->table_name.length);
    *first_value =
        tdsql::main::TdMainServer::Instance()->AutoInc()->get_auto_inc_id(
            db_table, nb_desired_values * increment);
  } else {
  if (auto_increment_mode == SpiderAutoIncrementMode::kGenerateBySelectMax) {
    /* strict mode */
    int error_num;
    extra(HA_EXTRA_KEYREAD);
    if (index_init(table_share->next_number_index, TRUE)) goto error_index_init;
    ...
    if (error_num)
      ...
      *first_value = ((ulonglong)table->next_number_field->val_int_offset(
  ...
  error_index_init:
    ...
    *first_value = ~(ulonglong)0;
    DBUG_VOID_RETURN;
  } else if (auto_increment_mode != SpiderAutoIncrementMode::kGenerateByDistIDGen) {
    if (auto_increment_mode != SpiderAutoIncrementMode::kGenerateByLocalCounter)
...
int ha_spider::reset_auto_increment(ulonglong value) {
...
//void ha_spider::start_bulk_insert(ha_rows rows, uint flags)
//#else
    void ha_spider::start_bulk_insert(ha_rows rows[[maybe_unused]])
//#endif
{
...
int ha_spider::get_bg_result() {
  ...
  if (result_list.bgs_error == HA_ERR_END_OF_FILE) result_list.bgs_error = 0;
...
int ha_spider::get_bg_result(ha_rows *update_rows, ha_rows *found_rows) {
  ...
  for (roop_count = spider_conn_link_idx_next(
           share->link_statuses, this->conn_link_idx, -1, share->link_count,
           SPIDER_LINK_STATUS_RECOVERY);
       roop_count < (int)share->link_count;
       roop_count = spider_conn_link_idx_next(
           share->link_statuses, this->conn_link_idx, roop_count,
           share->link_count, SPIDER_LINK_STATUS_RECOVERY)) {
    if (!counted) {
...
int ha_spider::get_bg_result(ha_rows *delete_rows) {
  ...
  for (roop_count = spider_conn_link_idx_next(
       ...
       roop_count = spider_conn_link_idx_next(
           ...
           share->link_count, SPIDER_LINK_STATUS_RECOVERY)) {
    if (!counted) {
      *delete_rows += this->conns[roop_count]->db_conn->affected_rows();
      DBUG_PRINT("info", ("spider delete_rows = %llu", *delete_rows));
      counted = TRUE;
    }
  }
  if ((this->direct_update_kinds & SPIDER_SQL_KIND_SQL) &&
      this->result_list.bgs_phase > 0) {
...
int ha_spider::write_row(uchar *buf) {
  int error_num;
  THD *thd = ha_thd();
  auto auto_increment_mode = tdsql_auto_increment_mode(thd, share->auto_increment_mode);
...
#ifdef HA_CAN_BULK_ACCESS
  if (bulk_access_executing &&
      ((!is_bulk_access_clone && bulk_access_link_exec_tgt->called) ||
       bulk_access_pre_called)) {
  ...
  if (!bulk_insert) store_last_insert_id = 0;
  if (auto_increment_flag) {
    if (auto_increment_mode == SpiderAutoIncrementMode::kForwardRemoteAsNull) {
      if (!table->autoinc_field_has_explicit_non_null_value) {
#ifndef DBUG_OFF
        my_bitmap_map *tmp_map =
            dbug_tmp_use_all_columns(table, table->write_set);
#endif
    ...
    } else if (auto_increment_mode == SpiderAutoIncrementMode::kForwardRemoteAsZero) {
#ifndef DBUG_OFF
      my_bitmap_map *tmp_map =
          dbug_tmp_use_all_columns(table, table->write_set);
#endif
      ...
      if ((error_num = update_auto_increment())) DBUG_RETURN(error_num);
    } else { 
...
#ifdef HA_CAN_BULK_ACCESS
int ha_spider::pre_write_row(uchar *buf) {
  int error_num;
  ulonglong option_backup = 0;
  THD *thd = trx->thd;
...
#ifdef HANDLER_HAS_DIRECT_UPDATE_ROWS
void ha_spider::direct_update_init(THD *thd[[maybe_unused]], bool hs_request[[maybe_unused]]) {
...
int ha_spider::exec_bulk_update(uint *dup_key_found) {
  int error_num;
  backup_error_status();
  DBUG_ENTER("ha_spider::exec_bulk_update");
  DBUG_PRINT("info", ("spider this=%p", this));
  ha_rows dup_key = 0;
  if ((error_num = spider_db_bulk_update_end(this, &dup_key)))
    DBUG_RETURN(check_error_mode(error_num));
  *dup_key_found = (uint)dup_key;
  DBUG_RETURN(0);
}
...
void ha_spider::end_bulk_update() {
  ...
  if ((error_num = check_and_end_bulk_update(SPD_BU_START_BY_BULK_INIT))) {
    if (check_error_mode(error_num)) return;
  }

  return;
}

int ha_spider::bulk_update_row(const uchar *old_data, uchar *new_data,
                               uint *dup_key_found) {
...
int ha_spider::update_row(const uchar *old_data, uchar *new_data) {
  int error_num;
  THD *thd = ha_thd();
  backup_error_status();
  ...
  if (bulk_access_executing && !is_bulk_access_clone &&
    ...
    error_num =
        bulk_access_link_exec_tgt->spider->ha_update_row(old_data, new_data);
  ...
  if (table->found_next_number_field && new_data == table->record[0] &&
    ...
    if (((Field_num *)table->found_next_number_field)->is_unsigned()) {
...
int ha_spider::direct_update_rows_init(uint mode [[maybe_unused]],
                                       KEY_MULTI_RANGE *ranges [[maybe_unused]],
                                       uint range_count,
...
#endif
  direct_update_init(thd, FALSE);
  if (!condition) cond_check = FALSE;
  spider_get_select_limit(this, &select_lex, &select_limit, &offset_limit);
  if (!range_count && direct_update_fields) {
    if (
...
#endif
        !select_lex || select_lex->table_list.elements != 1 ||
    ...
    if (thd->is_set_time()) {
      ...
      Field *field;
      Field **field_ptr;
      // if (table && (table->timestamp_field_type &
      ...
      for (field_ptr = table->field; *field_ptr; field_ptr++) {
        field = *field_ptr;
        if (field->is_flag_set(ON_UPDATE_NOW_FLAG)) {
          DBUG_RETURN(HA_ERR_WRONG_COMMAND);
        }
      }
    }
  ...
  if (!offset_limit && do_direct_update) {
    trx->direct_update_count++;
    DBUG_PRINT("info", ("spider OK"));
    DBUG_RETURN(0);
  }
...
#ifdef HA_CAN_BULK_ACCESS
#ifdef HANDLER_HAS_DIRECT_UPDATE_ROWS_WITH_HS
int ha_spider::pre_direct_update_rows_init(uint mode, KEY_MULTI_RANGE *ranges,
                                           uint range_count, bool sorted,
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  if (bulk_access_started) {
    error_num = bulk_access_link_current->spider->pre_direct_update_rows_init(
        mode, ranges, range_count, sorted, new_data);
    bulk_access_link_current->spider->bulk_access_pre_called = TRUE;
    bulk_access_link_current->called = TRUE;
    DBUG_RETURN(error_num);
...
#ifdef HANDLER_HAS_DIRECT_UPDATE_ROWS_WITH_HS
int ha_spider::direct_update_rows(KEY_MULTI_RANGE *ranges, uint range_count,
                                  bool sorted[[maybe_unused]], uchar *new_data[[maybe_unused]],
...
#endif
  if ((active_index != MAX_KEY && (error_num = index_handler_init())) ||
...
#else
int ha_spider::direct_update_rows(ha_rows *update_rows, ha_rows *found_rows) {
  int error_num;
  THD *thd = ha_thd();
  backup_error_status();
  DBUG_ENTER("ha_spider::direct_update_rows");
...
#ifdef HA_CAN_BULK_ACCESS
#ifdef HANDLER_HAS_DIRECT_UPDATE_ROWS_WITH_HS
int ha_spider::pre_direct_update_rows(KEY_MULTI_RANGE *ranges, uint range_count,
...
int ha_spider::delete_row(const uchar *buf) {
  THD *thd = ha_thd();
  int error_num;
  backup_error_status();
...
#ifdef HANDLER_HAS_DIRECT_UPDATE_ROWS
#ifdef HANDLER_HAS_DIRECT_UPDATE_ROWS_WITH_HS
int ha_spider::direct_delete_rows_init(uint mode[[maybe_unused]], KEY_MULTI_RANGE *ranges[[maybe_unused]],
                                       uint range_count, bool sorted[[maybe_unused]]) {
  Query_block *select_lex;
  longlong select_limit;
  longlong offset_limit;
  THD *thd = trx->thd;
...
#ifdef HA_CAN_BULK_ACCESS
#ifdef HANDLER_HAS_DIRECT_UPDATE_ROWS_WITH_HS
int ha_spider::pre_direct_delete_rows_init(uint mode, KEY_MULTI_RANGE *ranges,
                                           uint range_count, bool sorted) {
  int error_num;
  DBUG_ENTER("ha_spider::pre_direct_delete_rows_init");
  DBUG_PRINT("info", ("spider this=%p", this));
  if (bulk_access_started) {
    error_num = bulk_access_link_current->spider->pre_direct_delete_rows_init(
        mode, ranges, range_count, sorted);
    bulk_access_link_current->spider->bulk_access_pre_called = TRUE;
    bulk_access_link_current->called = TRUE;
    DBUG_RETURN(error_num);
...
#ifdef HANDLER_HAS_DIRECT_UPDATE_ROWS_WITH_HS
int ha_spider::direct_delete_rows(KEY_MULTI_RANGE *ranges, uint range_count,
                                  bool sorted[[maybe_unused]], ha_rows *delete_rows) {
  int error_num;
  THD *thd = ha_thd();
  backup_error_status();
...
#else
int ha_spider::direct_delete_rows(ha_rows *delete_rows) {
  int error_num;
  THD *thd = ha_thd();
  backup_error_status();
  DBUG_ENTER("ha_spider::direct_delete_rows");
  ...
  if (bulk_access_executing &&
    ...
    DBUG_RETURN(
        bulk_access_link_exec_tgt->spider->ha_direct_delete_rows(delete_rows));
  }
#endif
  if ((active_index != MAX_KEY && (error_num = index_handler_init())) ||
      (active_index == MAX_KEY && (error_num = rnd_handler_init())) ||
...
#ifdef HA_CAN_BULK_ACCESS
#ifdef HANDLER_HAS_DIRECT_UPDATE_ROWS_WITH_HS
int ha_spider::pre_direct_delete_rows(KEY_MULTI_RANGE *ranges, uint range_count,
...
int ha_spider::pre_direct_delete_rows() {
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_RETURN(
      bulk_access_link_current->spider->ha_direct_delete_rows(&delete_rows));
}
#endif
#endif
#endif
...
int ha_spider::truncate(dd::Table *table_def [[maybe_unused]]) {
  int error_num, roop_count;
  THD *thd = ha_thd();
  backup_error_status();
...
void ha_spider::bulk_req_exec() {
  ...
  while (conn) {
    ...
    if (conn->bulk_access_requests) {
      ...
      if (spider_db_query(conn, NULL, 0, -1, &need_mon)) {
...
double ha_spider::read_time(uint index, uint ranges[[maybe_unused]], ha_rows rows) {
  DBUG_ENTER("ha_spider::read_time");
  DBUG_PRINT("info", ("spider this=%p", this));
  if (keyread) {
    DBUG_PRINT(
        "info",
        ("spider read_time(keyread) = %.6f",
         share->read_rate * table->s->key_info[index].key_length * rows / 2 +
             2));
    DBUG_RETURN(
        share->read_rate * table->s->key_info[index].key_length * rows / 2 + 2);
  } else {
...
void ha_spider::print_error(int error, myf errflag) {
...
bool ha_spider::get_error_message(int error, String *buf) {
  ...
  switch (error) {
    case ER_SPIDER_REMOTE_SERVER_GONE_AWAY_NUM:
      if (buf->reserve(ER_SPIDER_REMOTE_SERVER_GONE_AWAY_LEN))
        DBUG_RETURN(TRUE);
      buf->append((const char*)ER_SPIDER_REMOTE_SERVER_GONE_AWAY_STR,
                    (uint32)ER_SPIDER_REMOTE_SERVER_GONE_AWAY_LEN);
      break;
    case ER_SPIDER_CON_COUNT_ERROR:
      if (buf->reserve(ER_SPIDER_CON_COUNT_ERROR_LEN)) DBUG_RETURN(TRUE);
      buf->append(ER_SPIDER_CON_COUNT_ERROR_STR,
                    ER_SPIDER_CON_COUNT_ERROR_LEN);
      break;
    case ER_SPIDER_COND_INVALID_NUM:
      if (buf->reserve(ER_SPIDER_COND_INVALID_LEN)) DBUG_RETURN(TRUE);
      buf->append(ER_SPIDER_COND_INVALID_STR, ER_SPIDER_COND_INVALID_LEN);
      break;
    case ER_SPIDER_XA_MAY_PARTIAL_COMMIT_NUM:
      if (buf->reserve(ER_SPIDER_XA_MAY_PARTIAL_COMMIT_LEN)) DBUG_RETURN(TRUE);
      buf->append(ER_SPIDER_XA_MAY_PARTIAL_COMMIT_STR,
                    ER_SPIDER_XA_MAY_PARTIAL_COMMIT_LEN);
      break;
    case ER_SPIDER_XA_TIMEOUT_NUM:
      if (buf->reserve(ER_SPIDER_XA_TIMEOUT_LEN)) DBUG_RETURN(TRUE);
      buf->append(ER_SPIDER_XA_TIMEOUT_STR, ER_SPIDER_XA_TIMEOUT_LEN);
      break;
    case ER_SPIDER_DRY_NUM:
      if (buf->reserve(ER_SPIDER_DRY_LEN)) DBUG_RETURN(TRUE);
      buf->append(ER_SPIDER_DRY_STR, ER_SPIDER_DRY_LEN);
      break;
    ...
    default:
      ...
      buf->append(ER_SPIDER_UNKNOWN_STR, ER_SPIDER_UNKNOWN_LEN);
      break;
  }
  DBUG_RETURN(FALSE);
}

SpiderAutoIncrementMode ha_spider::tdsql_auto_increment_mode(THD *thd, int auto_increment_mode) {
  if (unlikely(tdsql::common::tdsql_mode == tdsql::common::MODE_MYSQL)) {
    return spider_param_auto_increment_mode(thd, auto_increment_mode);
  }
  if (tdsql::ddl::NeedGenerateByDistIDGen(table->s->tdsql_table_type)) {
    return SpiderAutoIncrementMode::kGenerateByDistIDGen;
  } else {
    return SpiderAutoIncrementMode::kForwardRemoteAsNull;
  }
}

bool ha_spider::get_field_actual_type_string(TABLE *table, const char *field_name,
  ...
  for (ptr = table->field; (field = *ptr); ptr++) {
    ...
    if (field_len == (int)strlen(field->field_name) &&
        strncasecmp(field->field_name, field_name, strlen(field->field_name)) ==
            0) {
      field->sql_type(res);
      return true;
    }
...
std::string ha_spider::get_ddl_str(THD *thd, TABLE *form) {
  std::string ddl_str = std::string(thd->query().str, thd->query().length);

  if (thd->variables.opt_tdsql_compat_oracle_mode) {
    if (thd_sql_command(thd) == SQLCOM_ALTER_TABLE ||
      ...
      for (auto it = thd->ora_type_pos_list.rbegin();
           it != thd->ora_type_pos_list.rend(); ++it) {
        // get field name and length
        end = (*it).first - 1;
        while (query[end] == ' ') end--;
        start = end;
        ...
        if (get_field_actual_type_string(form, &query[start + 1], end - start,  type))
          ddl_str.replace((*it).first, (*it).second, type.ptr());
      }
    }
  }
...
// TODO get the defination from dd like show create table
std::string ha_spider::get_remote_ddl_str(THD *thd, TABLE *form) {
  std::string ddl_str;
  if (form) {
    ddl_str = get_ddl_str(thd, form) + " " +
                        std::string(form->s->partition_info_str,
                                    form->s->partition_info_str_len);
  }else{
    ddl_str = std::string(thd->query().str, thd->query().length);
  }
  // replace spider to inndb simplly 
  size_t start_pos = 0;
  while((start_pos = ddl_str.find("spider", start_pos)) != std::string::npos) {
    ddl_str.replace(start_pos, 6, "innodb");
    start_pos += 6;
  }
  start_pos = 0;
  while((start_pos = ddl_str.find("SPIDER", start_pos)) != std::string::npos) {
 ...
 */
int ha_spider::execute_remote_ddl(std::string srv_names, THD *thd,
                                  std::string db_name, TABLE *form) {
  // if spider handle ddl sql, send ddl to the backend shard
  std::string ddl_sql = get_remote_ddl_str(thd, form);
  int error_num = 0;
  auto main_server = tdsql::main::TdMainServer::Instance();
  assert(main_server);
  auto cluster_info = main_server->ClusterCache()->ClusterInfo();
  auto set_infos = cluster_info->SetInfos();
  auto iter = set_infos.find(srv_names);
  if (iter == set_infos.end()) {
    error_num = ER_FOREIGN_SERVER_DOESNT_EXIST;
    return error_num;
  }

  auto set_info = iter->second;
  auto master_host =
  ...
  auto m = mysql_real_connect(&db_conn, master_host.c_str(),
                              set_info->User().c_str(),
                              set_info->Password().c_str(), db_name.c_str(),
                              master_port, nullptr, 0);
  ...
  int ret = mysql_real_query(&db_conn, ddl_sql.c_str(), ddl_sql.length());
  log_spider_mysql_sql(&db_conn, ddl_sql.c_str(), ddl_sql.length(), COM_QUERY);
  if (ret) {
    //fprintf(stderr, "Failed to ddl: Error: %s\n", mysql_error(&db_conn));
    error_num = mysql_errno(&db_conn);
    mysql_close(&db_conn);
    return error_num;
  }
...
*/
int ha_spider::create(const char *name, TABLE *form, HA_CREATE_INFO *info, dd::Table *table_def [[maybe_unused]]) {
  int error_num/*, dummy*/;
  ...
  DBUG_PRINT("info", ("spider form->s->connect_string=%s",
                      form->s->connect_string.str));
  DBUG_PRINT("info",
             ("spider info->connect_string=%s", info->connect_string.str));
  if (sql_command == SQLCOM_CREATE_INDEX || sql_command == SQLCOM_DROP_INDEX)
    DBUG_RETURN(0);
  if (!(trx = spider_get_trx(thd, TRUE, &error_num))) goto error_get_trx;
  if (trx->locked_connections && sql_command == SQLCOM_ALTER_TABLE) {
    my_message(ER_SPIDER_ALTER_BEFORE_UNLOCK_NUM,
               ER_SPIDER_ALTER_BEFORE_UNLOCK_STR, MYF(0));
  ...
  tmp_share.table_name = const_cast<char *>(name);
  tmp_share.table_name_length = strlen(name);
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
  tmp_share.table_name_hash_value =
      my_calc_hash(&trx->trx_alter_table_hash, (uchar *)tmp_share.table_name,
                   tmp_share.table_name_length);
  tmp_share.lgtm_tblhnd_share = spider_get_lgtm_tblhnd_share(
      name, tmp_share.table_name_length, tmp_share.table_name_hash_value, FALSE,
      TRUE, &error_num);
#else
      tmp_share.lgtm_tblhnd_share = spider_get_lgtm_tblhnd_share(
  ...
  if (form->s->keys > 0) {
    if (!(tmp_share.static_key_cardinality = (longlong *)spider_bulk_malloc(
              spider_current_trx, 246, MYF(MY_WME),
  ...
  if ((sql_command == SQLCOM_CREATE_TABLE &&
    ...
    if (thd->lex->create_info->or_replace() &&
        (error_num = spider_delete_tables(table_tables, tmp_share.table_name,
                                          &dummy))) {
      goto error;
    }
#endif
    if ((error_num = spider_insert_tables(table_tables, &tmp_share))) {
      goto error;
    }
    spider_close_sys_table(current_thd, table_tables, &open_tables_backup,
                           FALSE);
    table_tables = NULL;
...
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
    if (!(alter_table = (SPIDER_ALTER_TABLE *)my_hash_search_using_hash_value(
              &trx->trx_alter_table_hash, tmp_share.table_name_hash_value,
              (uchar *)tmp_share.table_name, tmp_share.table_name_length)))
#else
        if (!(alter_table = (SPIDER_ALTER_TABLE *)my_hash_search(
                  &trx->trx_alter_table_hash, (uchar *)tmp_share.table_name,
                  tmp_share.table_name_length)))
#endif
    {
  ...
  if (opt_spider_auto_ddls) {
    error_num = execute_remote_ddl(std::string(*tmp_share.server_names), thd, form->s->db.str, form);
...
void ha_spider::update_create_info(HA_CREATE_INFO *create_info) {
  ...
  DBUG_PRINT("info", ("spider create_info->connect_string=%s",
...
int ha_spider::rename_table(const char *from, const char *to,
...
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
  my_hash_value_type from_hash_value =
      my_calc_hash(&spider_open_tables, (uchar *)(const_cast<char*>(from)), from_len);
  my_hash_value_type to_hash_value =
      my_calc_hash(&spider_open_tables, (uchar *)(const_cast<char*>(to)), to_len);
#endif
  THD *thd = ha_thd();
  ...
  if (sql_command == SQLCOM_CREATE_INDEX || sql_command == SQLCOM_DROP_INDEX)
    DBUG_RETURN(0);
  if (!(trx = spider_get_trx(thd, TRUE, &error_num))) goto error;
  if (trx->locked_connections &&
  ...
  if (sql_command == SQLCOM_RENAME_TABLE ||
      ...
      !(alter_table_from = (SPIDER_ALTER_TABLE *)my_hash_search(
            &trx->trx_alter_table_hash, (uchar *)(const_cast<char*>(from)), from_len))) {
    if (!(table_tables = spider_open_sys_table(
              current_thd, SPIDER_SYS_TABLES_TABLE_NAME_STR,
              SPIDER_SYS_TABLES_TABLE_NAME_LEN, TRUE, &open_tables_backup,
              FALSE, &error_num))) {
    ...
    if ((error_num = spider_update_tables_name(table_tables, from, to,
                                               &old_link_count))) {
      goto error;
    }
    spider_close_sys_table(current_thd, table_tables, &open_tables_backup,
    ...
    for (roop_count = 0; roop_count < old_link_count; roop_count++) {
      if ((error_num = spider_release_ping_table_mon_list(from, from_len,
                                                          roop_count))) {
        goto error;
      }
    }
    ...
    if ((alter_table_to = (SPIDER_ALTER_TABLE *)my_hash_search(
             &trx->trx_alter_table_hash, (uchar *)(const_cast<char*>(to)), to_len))) {
    ...
    if (alter_table_from->now_create) {
      SPIDER_SHARE tmp_share;
      tmp_share.table_name = const_cast<char *>(to);
      ...
      if ((error_num = spider_update_tables_priority(
...
#else
      from_lgtm_tblhnd_share =
          spider_get_lgtm_tblhnd_share(from, from_len, TRUE, FALSE, &error_num);
#endif
...
#else
        to_lgtm_tblhnd_share =
            spider_get_lgtm_tblhnd_share(to, to_len, TRUE, TRUE, &error_num);
#endif
    if (!to_lgtm_tblhnd_share) {
      pthread_mutex_unlock(&spider_lgtm_tblhnd_share_mutex);
      goto error;
    }
    DBUG_PRINT("info", ("spider auto_increment_init=%s",
                        from_lgtm_tblhnd_share->auto_increment_init ? "TRUE"
                                                                    : "FALSE"));
  ...
  //FIXME: need realize
  // to_table_def = const_cast<dd::Table*>(from_table_def);
  DBUG_RETURN(0);
...
#else
      to_lgtm_tblhnd_share =
          spider_get_lgtm_tblhnd_share(to, to_len, TRUE, FALSE, &tmp_error_num);
#endif
  if (to_lgtm_tblhnd_share)
...
int ha_spider::delete_table(const char *name, const dd::Table *table_def [[maybe_unused]]) {
  int error_num;
  THD *thd = ha_thd();
  SPIDER_TRX *trx;
  TABLE *table_tables = NULL;
  ...
  if (sql_command == SQLCOM_DROP_TABLE || sql_command == SQLCOM_DROP_DB ||
      sql_command == SQLCOM_ALTER_TABLE || sql_command == SQLCOM_CREATE_TABLE) {
    SPIDER_LGTM_TBLHND_SHARE *lgtm_tblhnd_share;
    int roop_count, old_link_count = 0, name_len = strlen(name);
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
    my_hash_value_type hash_value =
        my_calc_hash(&spider_open_tables, (uchar *)(const_cast<char*>(name)), name_len);
#endif
    if (sql_command == SQLCOM_ALTER_TABLE &&
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
        (alter_table = (SPIDER_ALTER_TABLE *)my_hash_search_using_hash_value(
             &trx->trx_alter_table_hash, hash_value, (uchar *)(const_cast<char*>(name)),
             name_len)) &&
#else
            (alter_table = (SPIDER_ALTER_TABLE *)my_hash_search(
                 &trx->trx_alter_table_hash, (uchar *)name, name_len)) &&
#endif
    ...
    if ((error_num =
             spider_delete_tables(table_tables, name, &old_link_count))) {
      goto error;
    }
    spider_close_sys_table(current_thd, table_tables, &open_tables_backup,
                           need_lock);
    ...
    pthread_mutex_lock(&spider_lgtm_tblhnd_share_mutex);
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
    lgtm_tblhnd_share = spider_get_lgtm_tblhnd_share(name, name_len, hash_value,
                                                     TRUE, FALSE, &error_num);
#else
        lgtm_tblhnd_share = spider_get_lgtm_tblhnd_share(name, name_len, TRUE,
                                                         FALSE, &error_num);
#endif
...
int ha_spider::disable_indexes(uint mode[[maybe_unused]]) {
  int error_num;
  backup_error_status();
...
int ha_spider::enable_indexes(uint mode[[maybe_unused]]) {
  int error_num;
  backup_error_status();
...
int ha_spider::check(THD *thd[[maybe_unused]], HA_CHECK_OPT *check_opt) {
  int error_num;
  backup_error_status();
...
int ha_spider::repair(THD *thd[[maybe_unused]], HA_CHECK_OPT *check_opt) {
  int error_num;
  backup_error_status();
...
bool ha_spider::check_and_repair(THD *thd[[maybe_unused]]) {
  ...
  if (spider_db_check_table(this, &check_opt)) {
    ...
    if (spider_db_repair_table(this, &check_opt)) DBUG_RETURN(TRUE);
  }
  DBUG_RETURN(FALSE);
}

int ha_spider::analyze(THD *thd[[maybe_unused]], HA_CHECK_OPT *check_opt[[maybe_unused]]) {
  int error_num;
  backup_error_status();
...
int ha_spider::optimize(THD *thd[[maybe_unused]], HA_CHECK_OPT *check_opt[[maybe_unused]]) {
  int error_num;
  backup_error_status();
...
Field *ha_spider::get_top_table_field(uint16 field_index) {
  Field *field;
...
Field *ha_spider::field_exchange(Field *field) {
  ...
  if (set_top_table_fields) {
    ...
    if (field->table != top_table) DBUG_RETURN(NULL);
    if (!(field = top_table_field[field->field_index()])) DBUG_RETURN(NULL);
  } else {
#endif
    DBUG_PRINT("info", ("spider table=%p", table));
    if (field->table != table) DBUG_RETURN(NULL);
#ifdef HANDLER_HAS_TOP_TABLE_FIELDS
  }
#endif
  DBUG_PRINT("info", ("spider out field=%p", field));
...
const Item *ha_spider::cond_push(const Item *cond, bool other_tbls_ok [[maybe_unused]]) {
  DBUG_ENTER("ha_spider::cond_push");
  cond_check = FALSE;
  if (cond) {
    SPIDER_CONDITION *tmp_cond;
    if (!(tmp_cond = (SPIDER_CONDITION *)spider_malloc(
              spider_current_trx, 3, sizeof(*tmp_cond), MYF(MY_WME))))
      DBUG_RETURN(cond);
    tmp_cond->cond = const_cast<COND *>(cond);
...
int ha_spider::info_push(uint info_type, void *info[[maybe_unused]]) {
  int error_num = 0;
  DBUG_ENTER("ha_spider::info_push");
  DBUG_PRINT("info", ("spider this=%p", this));
#ifdef HA_CAN_BULK_ACCESS
  if (info_type != INFO_KIND_BULK_ACCESS_BEGIN &&
      info_type != INFO_KIND_BULK_ACCESS_CURRENT &&
      info_type != INFO_KIND_BULK_ACCESS_END) {
    if (!is_bulk_access_clone) {
      if (bulk_access_executing && bulk_access_link_exec_tgt->called) {
        DBUG_RETURN(
            bulk_access_link_exec_tgt->spider->info_push(info_type, info));
      } else if (bulk_access_started) {
        DBUG_RETURN(
            bulk_access_link_current->spider->info_push(info_type, info));
      }
    }
  }
#endif
  ...
  switch (info_type) {
    ...
    case INFO_KIND_UPDATE_FIELDS:
      ...
      update_request = TRUE;
#ifdef WITH_PARTITION_STORAGE_ENGINE
      if (keyread && check_partitioned()) keyread = FALSE;
#endif
      break;
#endif
    ...
    case INFO_KIND_BULK_ACCESS_BEGIN:
      ...
      if ((error_num = bulk_access_link_current->spider->sync_from_clone_source(
               this)) ||
          (error_num = bulk_access_link_current->spider->check_access_kind(
               trx->thd, (lock_type >= TL_WRITE_ALLOW_WRITE)))) {
        DBUG_RETURN(error_num);
      }
      ...
      if (bulk_access_link_exec_tgt->spider->pt_clone_source_handler != this) {
        ...
        for (bulk_access_link_exec_tgt = bulk_access_link_first;
          ...
          if (bulk_access_link_exec_tgt->sequence_num >= sequence_num) {
            assert(bulk_access_link_exec_tgt->sequence_num ==
                        sequence_num);
            break;
          }
        }
...
#ifdef HANDLER_HAS_TOP_TABLE_FIELDS
  if (set_top_table_fields) DBUG_RETURN(top_table);
#endif
  DBUG_RETURN(table);
}
...
void ha_spider::set_ft_discard_bitmap() {
  ...
  if (table_list) {
    Query_block *select_lex = table_list->query_block;
    if (select_lex && select_lex->ftfunc_list) {
      uint roop_count;
      Field *field;
      Item *item = nullptr, *item_next = nullptr;
      ...
      Item_field *item_field;
      {
        ...
        while ((item_func_match = fmi++)) {
          ...
          for (roop_count = 1; roop_count < item_count; roop_count++) {
            ...
            if (!field || !(field = field_exchange(field))) continue;
...
void ha_spider::set_searched_bitmap() {
  ...
  for (roop_count = 0; roop_count < (int)((table_share->fields + 7) / 8);
    ...
    DBUG_PRINT("info", ("spider roop_count=%d", roop_count));
    DBUG_PRINT("info",
               ("spider searched_bitmap=%d", searched_bitmap[roop_count]));
    DBUG_PRINT("info", ("spider read_set=%d",
                        ((uchar *)table->read_set->bitmap)[roop_count]));
  ...
  if (sql_command == SQLCOM_UPDATE || sql_command == SQLCOM_UPDATE_MULTI) {
    ...
    // Item *item;
    Query_block *select_lex = spider_get_select_lex(this);
    // List_iterator_fast<Item> fi(select_lex->item_list);
    mem_root_deque<Item *> *fi = select_lex->get_fields_list();
    // while ((item = fi++)) {
    for (Item *item : *fi) {
      if (item->type() == Item::FIELD_ITEM) {
        ...
        spider_set_bit(searched_bitmap, field->field_index());
        DBUG_PRINT("info",
                   ("spider set searched_bitmap=%u", field->field_index()));
      } else {
...
#endif
  select_column_mode =
      spider_param_select_column_mode(thd, share->select_column_mode);
  if (select_column_mode) {
    DBUG_PRINT("info", ("spider searched_bitmap=%p", searched_bitmap));
#ifdef WITH_PARTITION_STORAGE_ENGINE
    if (partition_handler_share && partition_handler_share->searched_bitmap) {
...
#ifdef WITH_PARTITION_STORAGE_ENGINE
        uint part_num = 0;
        if (update_request) part_num = check_partitioned();
#endif
        if (
#ifdef WITH_PARTITION_STORAGE_ENGINE
            part_num ||
...
bool ha_spider::check_and_start_bulk_update(
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_PRINT("info",
             ("spider bulk_update_start=%d", result_list.bulk_update_start));
  if (result_list.bulk_update_start == SPD_BU_NOT_START ||
    ...
    */
    result_list.bulk_update_size =
        spider_param_bulk_update_size(thd, share->bulk_update_size);
...
int ha_spider::check_and_end_bulk_update(spider_bulk_upd_start bulk_upd_start) {
  int error_num = 0;
  ...
  DBUG_PRINT("info",
             ("spider bulk_update_start=%d", result_list.bulk_update_start));
  DBUG_PRINT("info",
             ("spider bulk_update_mode=%d", result_list.bulk_update_mode));
...
#ifdef WITH_PARTITION_STORAGE_ENGINE
  table->file->get_no_parts("", &part_num);
  if (part_num) DBUG_RETURN(part_num);
...
bool ha_spider::is_sole_projection_field(uint16 field_index) {
  // NOTE: It is assumed that spider_db_append_select_columns() has already been
  // called
  ...
  for (loop_index = 0; loop_index < share->use_sql_dbton_count; loop_index++) {
    ...
    if (dbton_hdl->first_link_idx >= 0) {
      is_ha_sole_projection_field =
          dbton_hdl->is_sole_projection_field(field_index);
...
int ha_spider::check_ha_range_eof() {
  ...
  DBUG_PRINT("info", ("spider sql_kind[%u]=%u", search_link_idx,
                      sql_kind[search_link_idx]));
  DBUG_PRINT("info", ("spider sql_command=%u", sql_command));
  if (result_list.use_both_key &&
      (sql_kind[search_link_idx] & SPIDER_SQL_KIND_HANDLER) &&
      sql_command != SQLCOM_HA_READ) {
    int cmp_result =
        key_cmp(result_list.key_info->key_part, end_key->key, end_key->length);
  ...
  if (result_list.tmp_tables_created) {
    ...
    for (roop_count = roop_start; roop_count < roop_end;
      ...
      if (spider_bit_is_set(result_list.tmp_table_created, roop_count)) {
        ...
        if ((error_num = dbton_hdl->set_sql_for_exec(SPIDER_SQL_TYPE_TMP_SQL,
                                                     roop_count))) {
          DBUG_RETURN(error_num);
        }
        if (!dbton_hdl->need_lock_before_set_sql_for_exec(
        ...
        if (!tmp_error_num) {
          spider_conn_set_timeout_from_share(conn, roop_count, trx->thd, share);
          if (dbton_hdl->execute_sql(SPIDER_SQL_TYPE_DROP_TMP_TABLE_SQL, conn,
                                     -1, &need_mons[roop_count])) {
            conn->mta_conn_mutex_lock_already = FALSE;
            conn->mta_conn_mutex_unlock_later = FALSE;
...
bool ha_spider::handler_opened(int link_idx,
                               uint tgt_conn_kind [[maybe_unused]]) {
...
void ha_spider::set_handler_opened(int link_idx) {
...
void ha_spider::clear_handler_opened(int link_idx, uint tgt_conn_kind[[maybe_unused]]) {
...
int ha_spider::close_opened_handler(int link_idx, bool release_conn) {
  ...
  if (spider_bit_is_set(m_handler_opened, link_idx)) {
    // cloase open handler is after ha_commit_low, can't register ha
    conn = spider_get_conn_by_idx(link_idx, false);
    if (!conn) {
      DBUG_RETURN(store_error_num);
    }
    if ((error_num2 = spider_db_close_handler(this, conn, link_idx,
                                              SPIDER_CONN_KIND_MYSQL))) {
      if (share->monitoring_kind[link_idx] && need_mons[link_idx]) {
        error_num2 = spider_ping_table_mon_from_table(
            trx, trx->thd, share, link_idx,
            (uint32)share->monitoring_sid[link_idx], share->table_name,
            share->table_name_length, conn_link_idx[link_idx], NULL, 0,
            share->monitoring_kind[link_idx], share->monitoring_limit[link_idx],
            share->monitoring_flag[link_idx], TRUE);
      }
      error_num = error_num2;
    }
    spider_clear_bit(m_handler_opened, link_idx);
  ...
  if (!init_index_handler) {
    ...
    for (roop_count = roop_start; roop_count < roop_end;
      ...
      if (spider_conn_use_handler(this, lock_mode, roop_count) &&
        ...
        if ((error_num = spider_db_open_handler(this, conn, roop_count))) {
          if (share->monitoring_kind[roop_count] && need_mons[roop_count]) {
            error_num = spider_ping_table_mon_from_table(
                trx, trx->thd, share, roop_count,
                (uint32)share->monitoring_sid[roop_count], share->table_name,
...
void ha_spider::backup_error_status() {
  ...
  if (thd) da_status = thd->is_error();
  DBUG_VOID_RETURN;
}

int ha_spider::check_error_mode(int error_num) {
  THD *thd = ha_thd();
  DBUG_ENTER("ha_spider::check_error_mode");
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_PRINT("info", ("spider error_num=%d", error_num));
  if (!thd || !error_mode) DBUG_RETURN(error_num);
  DBUG_PRINT("info", ("spider error reset"));
  SPIDER_RESTORE_DASTATUS;
  DBUG_RETURN(0);
}

int ha_spider::check_error_mode_eof(int error_num) {
  DBUG_ENTER("ha_spider::check_error_mode_eof");
  DBUG_PRINT("info", ("spider this=%p", this));
  DBUG_PRINT("info", ("spider error_num=%d", error_num));
  if (error_num == HA_ERR_END_OF_FILE) DBUG_RETURN(HA_ERR_END_OF_FILE);
  if (check_error_mode(error_num)) DBUG_RETURN(error_num);
  DBUG_PRINT("info", ("spider result_list.finish_flg = TRUE"));
  result_list.finish_flg = TRUE;
  if (result_list.current) {
    DBUG_PRINT("info", ("spider result_list.current->finish_flg = TRUE"));
...
void ha_spider::check_pre_call(bool use_parallel) {
  THD *thd = ha_thd();
  Query_block *select_lex = spider_get_select_lex(this);
  ...
  DBUG_PRINT("info", ("spider this=%p", this));
  if (((skip_parallel_search & 1) && thd->lex &&
       thd->lex->sql_command != SQLCOM_SELECT  // such like insert .. select ..
       ) ||
      ((skip_parallel_search & 2) /*&& select_lex &&
       select_lex->sql_cache == SELECT_LEX::SQL_NO_CACHE*/  //  for mysqldump
       )) {
    use_pre_call = FALSE;
    DBUG_VOID_RETURN;
  }
  ...
  if (use_pre_call && thd->lex->sql_command == SQLCOM_SELECT) {
    ...
    */
    ...
    if (select_lex->join->sum_funcs && *select_lex->join->sum_funcs)
      ...
      DBUG_VOID_RETURN;
    if (select_lex->join->grouped || select_lex->has_limit()) DBUG_VOID_RETURN;
    if (select_lex->where_cond()) DBUG_VOID_RETURN;
    use_pre_call = FALSE;
  }
...
void ha_spider::check_insert_dup_update_pushdown() {
  ...
  update_fields_list.clear();
  update_values_list.clear();
  if (thd->lex->m_sql_cmd && thd->lex->m_sql_cmd->sql_command_code() == SQLCOM_INSERT) {
    Sql_cmd_insert_base *insert_cmd = (Sql_cmd_insert_base *)thd->lex->m_sql_cmd; 
    for (Item *orig_field : insert_cmd->update_field_list) {
      update_fields_list.push_back(orig_field);
    }
    for (Item *value : insert_cmd->update_value_list) {
      update_values_list.push_back(value);
    }
  }
  direct_update_fields = &update_fields_list;
  direct_update_values = &update_values_list;
...
SPIDER_BULK_ACCESS_LINK *ha_spider::create_bulk_access_link() {
  ...
  */
  if (!(bulk_access_link = (SPIDER_BULK_ACCESS_LINK *)spider_bulk_malloc(
            spider_current_trx, 168, MYF(MY_WME), &bulk_access_link,
            sizeof(SPIDER_BULK_ACCESS_LINK), &ref, ALIGN_SIZE(ref_length) * 2,
            NullS))) {
...
int ha_spider::sync_from_clone_source(ha_spider *spider) {
  int error_num;
  ...
  if (!synced_from_clone_source) {
    ...
    memcpy(conns, spider->conns, sizeof(SPIDER_CONN *) * share->link_count);
    ...
    if ((error_num =
             spider_check_trx_and_get_conn(spider->trx->thd, this, TRUE))) {
      DBUG_RETURN(error_num);
    }
    external_lock_cnt = spider->external_lock_cnt;
  }
...
void ha_spider::sync_from_clone_source_base(ha_spider *spider) {
...
int ha_spider::reset_sql_sql(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
...
int ha_spider::append_tmp_table_and_sql_for_bka(const key_range *start_key) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
int ha_spider::append_union_table_and_sql_for_bka(const key_range *start_key) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
  ...
  for (roop_count = 0; roop_count < share->use_sql_dbton_count; roop_count++) {
    ...
    if (dbton_hdl->first_link_idx >= 0 &&
        (error_num =
             dbton_hdl->append_union_table_and_sql_for_bka(start_key))) {
      DBUG_RETURN(error_num);
    }
  }
  DBUG_RETURN(0);
...
#ifdef HANDLER_HAS_DIRECT_UPDATE_ROWS
int ha_spider::append_dup_update_pushdown_sql_part(const char *alias,
                                                   uint alias_length) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
  ...
  for (roop_count = 0; roop_count < share->use_sql_dbton_count; roop_count++) {
    ...
    if (dbton_hdl->first_link_idx >= 0 &&
        (error_num =
             dbton_hdl->append_dup_update_pushdown_part(alias, alias_length))) {
      DBUG_RETURN(error_num);
    }
  }
  DBUG_RETURN(0);
...
int ha_spider::append_update_columns_sql_part(const char *alias,
                                              uint alias_length) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
  ...
  for (roop_count = 0; roop_count < share->use_sql_dbton_count; roop_count++) {
    ...
    if (dbton_hdl->first_link_idx >= 0 &&
        (error_num =
             dbton_hdl->append_update_columns_part(alias, alias_length))) {
      DBUG_RETURN(error_num);
    }
  }
  DBUG_RETURN(0);
...
int ha_spider::append_select_sql_part(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
int ha_spider::append_table_select_sql_part(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
int ha_spider::append_key_select_sql_part(ulong sql_type, uint idx) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
int ha_spider::append_minimum_select_sql_part(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
int ha_spider::append_from_sql_part(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
int ha_spider::append_hint_after_table_sql_part(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
void ha_spider::set_where_pos_sql(ulong sql_type) {
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
  ...
  for (roop_count = 0; roop_count < share->use_sql_dbton_count; roop_count++) {
    dbton_id = share->use_sql_dbton_ids[roop_count];
    dbton_hdl = dbton_handler[dbton_id];
    if (dbton_hdl->first_link_idx >= 0) dbton_hdl->set_where_pos(sql_type);
  }
  DBUG_VOID_RETURN;
}

void ha_spider::set_where_to_pos_sql(ulong sql_type) {
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
  ...
  for (roop_count = 0; roop_count < share->use_sql_dbton_count; roop_count++) {
    dbton_id = share->use_sql_dbton_ids[roop_count];
    dbton_hdl = dbton_handler[dbton_id];
    if (dbton_hdl->first_link_idx >= 0) dbton_hdl->set_where_to_pos(sql_type);
  }
  DBUG_VOID_RETURN;
}

int ha_spider::check_item_type_sql(Item *item) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
int ha_spider::append_values_connector_sql_part(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
  ...
  for (roop_count = 0; roop_count < share->use_sql_dbton_count; roop_count++) {
    ...
    dbton_hdl = dbton_handler[dbton_id];
    if (dbton_hdl->first_link_idx >= 0 &&
        (error_num = dbton_hdl->append_values_connector_part(sql_type))) {
      DBUG_RETURN(error_num);
    }
  }
  DBUG_RETURN(0);
...
int ha_spider::append_values_terminator_sql_part(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
  ...
  for (roop_count = 0; roop_count < share->use_sql_dbton_count; roop_count++) {
    ...
    dbton_hdl = dbton_handler[dbton_id];
    if (dbton_hdl->first_link_idx >= 0 &&
        (error_num = dbton_hdl->append_values_terminator_part(sql_type))) {
      DBUG_RETURN(error_num);
    }
  }
  DBUG_RETURN(0);
...
int ha_spider::append_union_table_connector_sql_part(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
  ...
  for (roop_count = 0; roop_count < share->use_sql_dbton_count; roop_count++) {
    ...
    dbton_hdl = dbton_handler[dbton_id];
    if (dbton_hdl->first_link_idx >= 0 &&
        (error_num = dbton_hdl->append_union_table_connector_part(sql_type))) {
      DBUG_RETURN(error_num);
    }
  }
  DBUG_RETURN(0);
...
int ha_spider::append_union_table_terminator_sql_part(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
  ...
  for (roop_count = 0; roop_count < share->use_sql_dbton_count; roop_count++) {
    ...
    dbton_hdl = dbton_handler[dbton_id];
    if (dbton_hdl->first_link_idx >= 0 &&
        (error_num = dbton_hdl->append_union_table_terminator_part(sql_type))) {
      DBUG_RETURN(error_num);
    }
  }
  DBUG_RETURN(0);
...
int ha_spider::append_key_column_values_sql_part(const key_range *start_key,
                                                 ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
int ha_spider::append_key_column_values_with_name_sql_part(
  ...
  for (roop_count = 0; roop_count < share->use_sql_dbton_count; roop_count++) {
    ...
    dbton_hdl = dbton_handler[dbton_id];
    if (dbton_hdl->first_link_idx >= 0 &&
        (error_num = dbton_hdl->append_key_column_values_with_name_part(
             start_key, sql_type))) {
      DBUG_RETURN(error_num);
    }
  }
...
int ha_spider::append_key_where_sql_part(const key_range *start_key,
                                         const key_range *end_key,
                                         ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  ...
  for (roop_count = 0; roop_count < share->use_sql_dbton_count; roop_count++) {
    ...
    if (dbton_hdl->first_link_idx >= 0 &&
        (error_num =
             dbton_hdl->append_key_where_part(start_key, end_key, sql_type))) {
      DBUG_RETURN(error_num);
    }
  }
  DBUG_RETURN(0);
...
int ha_spider::append_match_where_sql_part(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
int ha_spider::append_condition_sql_part(const char *alias, uint alias_length,
...
#ifdef HANDLER_HAS_DIRECT_AGGREGATE
int ha_spider::append_sum_select_sql_part(ulong sql_type, const char *alias,
                                          uint alias_length) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
  ...
  for (roop_count = 0; roop_count < share->use_sql_dbton_count; roop_count++) {
    ...
    dbton_hdl = dbton_handler[dbton_id];
    if (dbton_hdl->first_link_idx >= 0 &&
        (error_num = dbton_hdl->append_sum_select_part(sql_type, alias,
                                                       alias_length))) {
      DBUG_RETURN(error_num);
    }
  }
...
int ha_spider::append_match_select_sql_part(ulong sql_type, const char *alias,
                                            uint alias_length) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
  ...
  for (roop_count = 0; roop_count < share->use_sql_dbton_count; roop_count++) {
    ...
    dbton_hdl = dbton_handler[dbton_id];
    if (dbton_hdl->first_link_idx >= 0 &&
        (error_num = dbton_hdl->append_match_select_part(sql_type, alias,
                                                         alias_length))) {
      DBUG_RETURN(error_num);
    }
  }
...
void ha_spider::set_order_pos_sql(ulong sql_type) {
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
  ...
  for (roop_count = 0; roop_count < share->use_sql_dbton_count; roop_count++) {
    dbton_id = share->use_sql_dbton_ids[roop_count];
    dbton_hdl = dbton_handler[dbton_id];
    if (dbton_hdl->first_link_idx >= 0) dbton_hdl->set_order_pos(sql_type);
  }
  DBUG_VOID_RETURN;
}

void ha_spider::set_order_to_pos_sql(ulong sql_type) {
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
  ...
  for (roop_count = 0; roop_count < share->use_sql_dbton_count; roop_count++) {
    dbton_id = share->use_sql_dbton_ids[roop_count];
    dbton_hdl = dbton_handler[dbton_id];
    if (dbton_hdl->first_link_idx >= 0) dbton_hdl->set_order_to_pos(sql_type);
  }
  DBUG_VOID_RETURN;
}

#ifdef HANDLER_HAS_DIRECT_AGGREGATE
int ha_spider::append_group_by_sql_part(const char *alias, uint alias_length,
                                        ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
  ...
  for (roop_count = 0; roop_count < share->use_sql_dbton_count; roop_count++) {
    ...
    if (dbton_hdl->first_link_idx >= 0 &&
        (error_num =
             dbton_hdl->append_group_by_part(alias, alias_length, sql_type))) {
      DBUG_RETURN(error_num);
    }
  }
  DBUG_RETURN(0);
...
int ha_spider::append_key_order_for_merge_with_alias_sql_part(const char *alias,
                                                              uint alias_length,
                                                              ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
...
int ha_spider::append_key_order_for_direct_order_limit_with_alias_sql_part(
  ...
  spider_db_handler *dbton_hdl;
  DBUG_ENTER(
      "ha_spider::append_key_order_for_direct_order_limit_with_alias_sql_part");
  for (roop_count = 0; roop_count < share->use_sql_dbton_count; roop_count++) {
    dbton_id = share->use_sql_dbton_ids[roop_count];
    dbton_hdl = dbton_handler[dbton_id];
    if (dbton_hdl->first_link_idx >= 0 &&
...
int ha_spider::append_key_order_with_alias_sql_part(const char *alias,
                                                    uint alias_length,
                                                    ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
...
int ha_spider::append_limit_sql_part(longlong offset, longlong limit,
                                     ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
int ha_spider::reappend_limit_sql_part(longlong offset, longlong limit,
                                       ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
int ha_spider::append_insert_terminator_sql_part(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
int ha_spider::append_insert_values_sql_part(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
int ha_spider::append_into_sql_part(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
void ha_spider::set_insert_to_pos_sql(ulong sql_type) {
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
  ...
  for (roop_count = 0; roop_count < share->use_sql_dbton_count; roop_count++) {
    dbton_id = share->use_sql_dbton_ids[roop_count];
    dbton_hdl = dbton_handler[dbton_id];
    if (dbton_hdl->first_link_idx >= 0) dbton_hdl->set_insert_to_pos(sql_type);
  }
  DBUG_VOID_RETURN;
}

bool ha_spider::is_bulk_insert_exec_period(bool bulk_end) {
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
int ha_spider::append_select_lock_sql_part(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
int ha_spider::append_union_all_start_sql_part(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
int ha_spider::append_union_all_sql_part(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
int ha_spider::append_union_all_end_sql_part(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
int ha_spider::append_multi_range_cnt_sql_part(ulong sql_type,
...
int ha_spider::append_multi_range_cnt_with_name_sql_part(ulong sql_type,
                                                         uint multi_range_cnt) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
int ha_spider::append_delete_all_rows_sql_part(ulong sql_type) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
int ha_spider::append_update_sql(const TABLE *table, my_ptrdiff_t ptr_diff,
                                 bool bulk) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
int ha_spider::append_delete_sql(const TABLE *table, my_ptrdiff_t ptr_diff,
                                 bool bulk) {
  int error_num;
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
bool ha_spider::sql_is_filled_up(ulong sql_type) {
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
bool ha_spider::sql_is_empty(ulong sql_type) {
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
  ...
  for (roop_count = 0; roop_count < share->use_sql_dbton_count; roop_count++) {
    dbton_id = share->use_sql_dbton_ids[roop_count];
    dbton_hdl = dbton_handler[dbton_id];
    if (dbton_hdl->first_link_idx >= 0 && !dbton_hdl->sql_is_empty(sql_type)) {
      DBUG_RETURN(FALSE);
    }
  }
  DBUG_RETURN(TRUE);
...
bool ha_spider::support_bulk_update_sql() {
  ...
  for (roop_count = 0; roop_count < share->use_sql_dbton_count; roop_count++) {
    dbton_id = share->use_sql_dbton_ids[roop_count];
    dbton_hdl = dbton_handler[dbton_id];
    if (dbton_hdl->first_link_idx >= 0 && !dbton_hdl->support_bulk_update()) {
      DBUG_RETURN(FALSE);
    }
  }
  DBUG_RETURN(TRUE);
...
int ha_spider::bulk_tmp_table_end_bulk_insert() {
  ...
  for (roop_count = 0; roop_count < share->link_count; roop_count++) {
    if (tmp_table[roop_count]) {
      if ((error_num2 = tmp_table[roop_count]->file->ha_end_bulk_insert())) {
        error_num = error_num2;
      }
    }
  }
...
int ha_spider::bulk_tmp_table_rnd_init() {
  ...
  for (roop_count = 0; roop_count < share->link_count; roop_count++) {
    if (tmp_table[roop_count]) {
      tmp_table[roop_count]->file->ha_extra(HA_EXTRA_CACHE);
      if ((error_num = tmp_table[roop_count]->file->ha_rnd_init(TRUE)))
        goto error_2;
    }
  }
  DBUG_RETURN(0);
...
error_1:
  for (; roop_count > 0; roop_count--) {
    ...
    dbton_hdl = dbton_handler[dbton_id];
    if (dbton_hdl->first_link_idx >= 0) dbton_hdl->bulk_tmp_table_rnd_end();
  }
  DBUG_RETURN(error_num);
}
...
int ha_spider::mk_bulk_tmp_table_and_bulk_start() {
  ...
  for (roop_count = 0; roop_count < share->link_count; roop_count++) {
    ...
    if (dbton_hdl->first_link_idx >= 0 &&
      ...
      tmp_table[roop_count]->file->ha_extra(HA_EXTRA_WRITE_CACHE);
...
void ha_spider::rm_bulk_tmp_table() {
  ...
  for (roop_count = share->use_sql_dbton_count; roop_count > 0; roop_count--) {
    dbton_id = share->use_sql_dbton_ids[roop_count - 1];
    dbton_hdl = dbton_handler[dbton_id];
    if (dbton_hdl->first_link_idx >= 0) dbton_hdl->rm_bulk_tmp_table();
  }
  DBUG_VOID_RETURN;
}
...
int ha_spider::print_item_type(Item *item, spider_string *str,
  ...
  for (roop_count = 0; roop_count < share->use_sql_dbton_count; roop_count++) {
    ...
    if (dbton_hdl->first_link_idx >= 0 &&
        (error_num = spider_db_print_item_type(
             item, this, str, alias, alias_length, dbton_id, FALSE, NULL))) {
      DBUG_RETURN(error_num);
    }
  }
  DBUG_RETURN(0);
...
bool ha_spider::support_use_handler_sql(int use_handler) {
  uint roop_count, dbton_id;
  spider_db_handler *dbton_hdl;
...
SPIDER_CONN *ha_spider::spider_get_conn_by_idx(int link_idx, bool need_register_ha) {
  int error_num = 0;
  assert(this->conns);
  // if (this && this->conns && this->conns[link_idx]) {
  if (this->conns && this->conns[link_idx]) {
    if (need_register_ha) {
      trans_register_ha(trx->thd, FALSE, spider_hton_ptr, nullptr);
      if (thd_test_options(trx->thd, OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN))
        trans_register_ha(trx->thd, TRUE, spider_hton_ptr, nullptr);
    }
    return this->conns[link_idx];
  } else {
    /* create conn */
    this->conns[link_idx] =
        spider_get_conn(share, link_idx, this->trx, this, FALSE, TRUE,
                        SPIDER_CONN_KIND_MYSQL, &error_num);
    if (!this->conns[link_idx]) { /* failed to create conn */
      //			share->init_error = TRUE;
...
SPIDER_CONN *ha_spider::get_spider_conn_for_pq(int link_idx) {
  auto *spider_conn = spider_get_conn_by_idx(link_idx);
  if (!spider_conn || (!SPIDER_CONN_get_mysql(spider_conn) &&
                       spider_db_set_names(this, spider_conn, 0) != 0)) {
    return nullptr;
  }
  return spider_conn;
}

std::string ha_spider::get_spider_conn_hashkey(int link_idx) const {
  return std::string(share->conn_keys[link_idx],
...
int ha_spider::spider_set_trx_status_info() {
  ...
  // trx->set_spider_gtid(thd->server_id, thd->query_id, part_no >= 0 ? part_no : 0);
  for (roop_count = spider_conn_link_idx_next(
           share->link_statuses, conn_link_idx, -1, share->link_count,
           SPIDER_LINK_STATUS_RECOVERY);
       roop_count < (int)share->link_count;
    ...
    if (sql_command != SQLCOM_UNLOCK_TABLES) {
      DBUG_PRINT("info", ("spider conns[%d]->join_trx=%u", roop_count,
                          conns[roop_count]->join_trx));
      if ((!conns[roop_count]->join_trx &&
      ...
      if (conns[roop_count]->semi_trx_isolation == -2 &&
        ...
        {
        */
        spider_conn_queue_semi_trx_isolation(
            conns[roop_count], spider_param_semi_trx_isolation(trx->thd));
        /*
        }
    ...
    if (conns[roop_count]->table_lock >= 2) {
      if (conns[roop_count]->db_conn->have_lock_table_list() &&
      ...
      if (conns[roop_count]->table_lock == 2) conns[roop_count]->table_lock = 1;
...
int ha_spider::spider_fix_pins() {
  return 0;
  /* return conn_pins ? 0 : (conn_pins =
   * lf_hash_get_pins(&spider_open_connections)) == 0;*/
}
...
int ha_spider::check_dry_run() {
  ...
  THD *thd = (trx ? trx->thd : NULL);
   Query_block *root_select_lex =
      ((thd && thd->lex) ? thd->lex->query_block : NULL);
  if (root_select_lex &&
      (dry_run = (root_select_lex->active_options() & OPTION_SPIDER_DRY_RUN))) {
    /*
    ...
    */
    ...
    if (!trx->dry_run_src_logged) {
      ...
      log_spider_resultf(SPIDER_LOG_DRY_RUN_SOURCE, "THD: %llu, Query: %.*s",
                         thd->thread_id(), thd->query().length,
                         thd->query());
    }
  }
  DBUG_RETURN(0);
...
int ha_spider::fast_update(THD *thd[[maybe_unused]],
                           mem_root_deque<Item *> &update_fields[[maybe_unused]],
                           mem_root_deque<Item *> &update_values[[maybe_unused]],
                           Item *cond[[maybe_unused]],
                           ha_rows *update_rows, ha_rows *found_rows) {
  if (condition == nullptr) { // not pushed yet
    cond_push(cond, false);
  }
  update_fields_list.clear();
  update_values_list.clear();

  for (Item *field : update_fields) {
    update_fields_list.push_back(field);
  }
  for (Item *value : update_values) {
    update_values_list.push_back(value);
  }
  ...
  if (direct_update_rows_init()) return ENOTSUP;

  if (direct_update_rows(update_rows, found_rows)) {
    reset_sql_sql(SPIDER_SQL_TYPE_UPDATE_SQL);
    return ENOTSUP;
  }
...
/*
 * use create to send the thd alter query to remote srv
 */
bool ha_spider::prepare_inplace_alter_table(TABLE *altered_table,
                                            Alter_inplace_info *ha_alter_info,
                                            const dd::Table *old_dd_tab[[maybe_unused]],
                                            dd::Table *new_dd_tab) {
  std::string new_name;
  if (ha_alter_info->part_index == -1) {
    new_name = altered_table->s->path.str;
  } else {
    // FIXME: sup-partition
    new_name = altered_table->s->path.str
               + std::string("#P#p") + std::to_string(ha_alter_info->part_index);
  }
  return create(new_name.c_str(), altered_table, ha_alter_info->create_info, new_dd_tab);
}

std::string ha_spider::explain_extra() const {
  std::string str("");
...
bool ha_spider::check_fast_update_and_delete(bool has_multi_parts) {
  DBUG_ENTER("ha_spider::check_fast_update_and_delete");
  if (table->triggers) DBUG_RETURN(false);

  Query_block *select = spider_get_select_lex(this);

  if (select == nullptr) DBUG_RETURN(false);

  for (uint roop_count = 0; roop_count < share->use_sql_dbton_count;
       roop_count++) {
    uint dbton_id = share->use_sql_dbton_ids[roop_count];
    spider_db_handler *dbton_hdl = dbton_handler[dbton_id];

    if (dbton_hdl == nullptr) DBUG_RETURN(false);

    /**
     * Multiple partitions with limit offset or order by can not do
     * fast update/delete.
     */
    if (dbton_hdl->first_link_idx >= 0 && has_multi_parts &&
        (select->has_limit() || select->is_ordered())) {
      DBUG_RETURN(false);
    }
  }
