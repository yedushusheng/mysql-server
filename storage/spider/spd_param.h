...
/**
 * @brief SpiderAutoIncrementMode controls how auto_inc generated in spider.
 *
 *  - kUseTableParameter: use DSN parameter "aim" in table parameter.
 *  - kGenerateBySelectMax: uyses a counter that gets from the remote backend server with an exclusive lock for the auto-increment value.
 *  - kGenerateByLocalCounter: generate by local counter, it is possible for duplicates to occur when updating from multiple proxies.
 *  - kForwardRemoteAsZero: sets the auto_inc column to 0 and send to backend to generate auto_inc value.
 *  - kForwardRemoteAsNull: sets the auto_inc column to NULL and send to backend to generate auto_inc value.
 *  - kGenerateByDistIDGen: use distributed id generator based on database/zookeeper.
 * 
 * @see https://mariadb.com/kb/en/spider-server-system-variables/+simple/dash/#spider_auto_increment_mode
 */
enum SpiderAutoIncrementMode : int {
  kUseTableParameter = -1,
  kGenerateBySelectMax = 0,
  kGenerateByLocalCounter = 1,
  kForwardRemoteAsZero = 2,
  kForwardRemoteAsNull = 3,
  kGenerateByDistIDGen = 4, 
};
...
SpiderAutoIncrementMode spider_param_auto_increment_mode(THD *thd, int auto_increment_mode);
