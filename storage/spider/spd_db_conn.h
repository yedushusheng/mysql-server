...
extern ulonglong opt_tdsql_log_ignore_err_nums;
...
int spider_db_query(SPIDER_CONN *conn, const char *query, uint length,
                    int quick_mode, int *need_mon, bool sync = true);
...
int spider_db_xa_end(SPIDER_CONN *conn, tdsql::Gtid *gtid);

int spider_db_xa_prepare(SPIDER_CONN *conn, tdsql::Gtid *gtid);

int spider_db_xa_end_and_prepare(SPIDER_CONN *conn, tdsql::Gtid *gtid);
...
int spider_db_fetch_table(ha_spider *spider, uchar *buf, TABLE *table,
                          SPIDER_RESULT_LIST *result_list, mem_root_deque<Item *> *fields = nullptr);

int spider_db_fetch_key(ha_spider *spider, uchar *buf, TABLE *table,
                        const KEY *key_info, SPIDER_RESULT_LIST *result_list, mem_root_deque<Item *> *fields = nullptr);
...
int spider_db_fetch(uchar *buf, ha_spider *spider, TABLE *table, mem_root_deque<Item *> *fields = nullptr);
...
int spider_db_seek_next(uchar *buf, ha_spider *spider, int link_idx,
                        TABLE *table, mem_root_deque<Item *> *fields = nullptr);
