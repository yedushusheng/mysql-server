...
int spider_udf_direct_sql_create_table_list(SPIDER_DIRECT_SQL *direct_sql,
  ...
  if (!(direct_sql->db_names = (char **)spider_bulk_malloc(
            spider_current_trx, 31, MYF(MY_WME | MY_ZEROFILL),
  ...
  roop_count = 0;
  while (TRUE) {
    if ((tmp_ptr2 = strchr(tmp_ptr, ' '))) *tmp_ptr2 = '\0';
    ...
    if ((tmp_ptr3 = strchr(tmp_ptr, '.'))) {
      ...
      *tmp_ptr3 = '\0';
      ...
      if (thd->db().str) {
        memcpy(tmp_name_ptr, thd->db().str, thd->db().length + 1);
        tmp_name_ptr += thd->db().length + 1;
      } else {
        direct_sql->db_names[roop_count] = const_cast<char *>("");
      }
    }
    ...
    DBUG_PRINT("info", ("spider db=%s", direct_sql->db_names[roop_count]));
    DBUG_PRINT("info",
               ("spider table_name=%s", direct_sql->table_names[roop_count]));
...
int spider_udf_direct_sql_create_conn_key(SPIDER_DIRECT_SQL *direct_sql) {
  ...
  direct_sql->conn_key_length =
      1 + direct_sql->tgt_wrapper_length + 1 + direct_sql->tgt_host_length + 1 +
      5 + 1 + direct_sql->tgt_socket_length + 1 +
      direct_sql->tgt_username_length + 1 + direct_sql->tgt_password_length +
      1 + direct_sql->tgt_ssl_ca_length + 1 +
      direct_sql->tgt_ssl_capath_length + 1 + direct_sql->tgt_ssl_cert_length +
      1 + direct_sql->tgt_ssl_cipher_length + 1 +
      direct_sql->tgt_ssl_key_length + 1 + 1 + 1 +
      direct_sql->tgt_default_file_length + 1 +
      direct_sql->tgt_default_group_length;
  if (!(direct_sql->conn_key = (char *)spider_malloc(
            spider_current_trx, 9, direct_sql->conn_key_length + 1,
            MYF(MY_WME | MY_ZEROFILL))))
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
 
  DBUG_PRINT("info", ("spider tgt_wrapper=%s", direct_sql->tgt_wrapper));
  tmp_name = strmov(direct_sql->conn_key, direct_sql->tgt_wrapper);
  ...
  if (direct_sql->tgt_ssl_capath) {
    DBUG_PRINT("info",
               ("spider tgt_ssl_capath=%s", direct_sql->tgt_ssl_capath));
  ...
  if (direct_sql->tgt_ssl_cipher) {
    DBUG_PRINT("info",
               ("spider tgt_ssl_cipher=%s", direct_sql->tgt_ssl_cipher));
  ...
  *tmp_name = '0' + ((char)direct_sql->tgt_ssl_vsc);
  if (direct_sql->tgt_default_file) {
    DBUG_PRINT("info",
               ("spider tgt_default_file=%s", direct_sql->tgt_default_file));
    tmp_name = strmov(tmp_name + 1, direct_sql->tgt_default_file);
  } else
    tmp_name++;
  if (direct_sql->tgt_default_group) {
    DBUG_PRINT("info",
               ("spider tgt_default_group=%s", direct_sql->tgt_default_group));
  ...
  DBUG_PRINT("info",
             ("spider direct_sql->tgt_wrapper=%s", direct_sql->tgt_wrapper));
...
SPIDER_CONN *spider_udf_direct_sql_create_conn(
  ...
  if (!(conn = (SPIDER_CONN *)spider_bulk_malloc(
            spider_current_trx, 32, MYF(MY_WME | MY_ZEROFILL), &conn,
            sizeof(*conn), &tmp_name, direct_sql->conn_key_length + 1,
            &tmp_host, direct_sql->tgt_host_length + 1, &tmp_username,
            direct_sql->tgt_username_length + 1, &tmp_password,
            direct_sql->tgt_password_length + 1, &tmp_socket,
            direct_sql->tgt_socket_length + 1, &tmp_wrapper,
            direct_sql->tgt_wrapper_length + 1, &tmp_ssl_ca,
            direct_sql->tgt_ssl_ca_length + 1, &tmp_ssl_capath,
            direct_sql->tgt_ssl_capath_length + 1, &tmp_ssl_cert,
            direct_sql->tgt_ssl_cert_length + 1, &tmp_ssl_cipher,
            direct_sql->tgt_ssl_cipher_length + 1, &tmp_ssl_key,
            direct_sql->tgt_ssl_key_length + 1, &tmp_default_file,
            direct_sql->tgt_default_file_length + 1, &tmp_default_group,
            direct_sql->tgt_default_group_length + 1, &need_mon, sizeof(int),
            NullS))) {
...
SPIDER_CONN *spider_udf_direct_sql_get_conn(const SPIDER_DIRECT_SQL *direct_sql,
...
#else
  if (!(conn = (SPIDER_CONN *)my_hash_search(&trx->trx_conn_hash,
  ...
  if (conn->queued_ping) conn->queued_ping = FALSE;
...
int spider_udf_direct_sql_get_server(SPIDER_DIRECT_SQL *direct_sql) {
  ...
  auto set_info = get_server_by_name_or_alias_name(direct_sql->server_name);
  tdsql::common::TdString master_host;
  tdsql::common::TdString servers_db;
  ...
  if (!direct_sql->tgt_wrapper && !set_info->ServerSchema().empty()) {
    direct_sql->tgt_wrapper_length = set_info->ServerSchema().size();
    if (!(direct_sql->tgt_wrapper =
              spider_create_string(set_info->ServerSchema().c_str(),
                                   direct_sql->tgt_wrapper_length))) {
      error_num = HA_ERR_OUT_OF_MEM;
  ...
  if (!direct_sql->tgt_host && !master_host.empty()) {
    direct_sql->tgt_host_length = master_host.size();
    if (!(direct_sql->tgt_host = spider_create_string(
              master_host.c_str(), direct_sql->tgt_host_length))) {
      error_num = HA_ERR_OUT_OF_MEM;
      my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
      goto error;
    }
  ...
  if (direct_sql->tgt_port == -1) {
    direct_sql->tgt_port = master_port;
    DBUG_PRINT("info", ("spider tgt_port=%ld", direct_sql->tgt_port));
  }

  if (!direct_sql->tgt_socket && !set_info->ServersSocket().empty()) {
    direct_sql->tgt_socket_length = set_info->ServersSocket().size();
    if (!(direct_sql->tgt_socket =
              spider_create_string(set_info->ServersSocket().c_str(),
                                   direct_sql->tgt_socket_length))) {
      error_num = HA_ERR_OUT_OF_MEM;
      my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
      goto error;
  ...
  servers_db = set_info->DefaultDb();
  if (!direct_sql->tgt_default_db_name && !servers_db.empty()) {
    direct_sql->tgt_default_db_name_length = servers_db.size();
    if (!(direct_sql->tgt_default_db_name =
              spider_create_string(servers_db.c_str(), servers_db.size()))) {
      error_num = HA_ERR_OUT_OF_MEM;
      my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
      goto error;
    }
  ...
  if (!direct_sql->tgt_username && !set_info->User().empty()) {
    direct_sql->tgt_username_length = set_info->User().size();
    if (!(direct_sql->tgt_username = spider_create_string(
              set_info->User().c_str(), direct_sql->tgt_username_length))) {
      error_num = HA_ERR_OUT_OF_MEM;
      my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
      goto error;
    }
  ...
  if (!direct_sql->tgt_password && !set_info->Password().empty()) {
    direct_sql->tgt_password_length = set_info->Password().size();
    if (!(direct_sql->tgt_password = spider_create_string(
              set_info->Password().c_str(), direct_sql->tgt_password_length))) {
      error_num = HA_ERR_OUT_OF_MEM;
      my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
      goto error;
    }
...
#define SPIDER_PARAM_STR(title_name, param_name)                        \
  if (!strncasecmp(tmp_ptr, title_name, title_length)) {                \
    ...
    if (!direct_sql->param_name) {                                      \
      ...
      DBUG_PRINT("info",                                                \
                 ("spider " title_name "=%s", direct_sql->param_name)); \
    }                                                                   \
    break;                                                              \
  }
#define SPIDER_PARAM_HINT_WITH_MAX(title_name, param_name, check_length,      \
                                   max_size, min_val, max_val)                \
  if (!strncasecmp(tmp_ptr, title_name, check_length)) {                      \
    ...
    DBUG_PRINT("info",                                                        \
               ("spider direct_sql->param_name=%p", direct_sql->param_name)); \
    if (direct_sql->param_name) {                                             \
...
#define SPIDER_PARAM_INT_WITH_MAX(title_name, param_name, min_val, max_val)  \
  if (!strncasecmp(tmp_ptr, title_name, title_length)) {                     \
    ...
    if (direct_sql->param_name == -1) {                                      \
      ...
      DBUG_PRINT("info",                                                     \
                 ("spider " title_name "=%d", (int)direct_sql->param_name)); \
    }                                                                        \
    break;                                                                   \
  }
#define SPIDER_PARAM_INT(title_name, param_name, min_val)                    \
  if (!strncasecmp(tmp_ptr, title_name, title_length)) {                     \
    ...
    if (direct_sql->param_name == -1) {                                      \
      ...
      DBUG_PRINT("info",                                                     \
                 ("spider " title_name "=%d", direct_sql->param_name));      \
    }                                                                        \
    break;                                                                   \
  }
    ...
    if (direct_sql->param_name == -1) {                                      \
      if ((tmp_ptr2 = spider_get_string_between_quote(start_ptr, FALSE))) {  \
        direct_sql->param_name =                                             \
            my_strtoll10(tmp_ptr2, nullptr, &error_num);               \
        if (direct_sql->param_name < min_val)                                \
          direct_sql->param_name = min_val;                                  \
        param_string_parse.set_param_value(tmp_ptr2,                         \
                                           tmp_ptr2 + strlen(tmp_ptr2) + 1); \
      ...
      DBUG_PRINT("info",                                                     \
                 ("spider " title_name "=%lld", direct_sql->param_name));    \
    }                                                                        \
    break;                                                                   \
  }
  ...
  while (sprit_ptr[0]) {
    ...
    switch (title_length) {
      ...
      case 6:
        ...
        SPIDER_PARAM_HINT_WITH_MAX("iop", iop, 3, direct_sql->table_count, 0,
                                   2);
...
set_default:
  if ((error_num = spider_udf_set_direct_sql_param_default(trx, direct_sql)))
    goto error;
...
int spider_udf_set_direct_sql_param_default(SPIDER_TRX *trx,
  ...
  if (!direct_sql->tgt_default_db_name) {
    ...
    if (!(direct_sql->tgt_default_db_name = spider_create_string(
              trx->thd->db().str, direct_sql->tgt_default_db_name_length))) {
      my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
      DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    }
  }
  ...
  if (!direct_sql->tgt_wrapper) {
    ...
    if (!(direct_sql->tgt_wrapper = spider_create_string(
              SPIDER_DB_WRAPPER_STR, direct_sql->tgt_wrapper_length))) {
      my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
      DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    }
  }
  ...
  if (!direct_sql->tgt_host) {
    ...
    if (!(direct_sql->tgt_host = spider_create_string(
              my_localhost, direct_sql->tgt_host_length))) {
      my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
      DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    }
  }

  if (!direct_sql->tgt_default_file && direct_sql->tgt_default_group &&
    ...
    if (*spd_defaults_extra_file) {
      ...
      if (!(direct_sql->tgt_default_file =
                spider_create_string(*spd_defaults_extra_file,
                                     direct_sql->tgt_default_file_length))) {
        my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
        DBUG_RETURN(HA_ERR_OUT_OF_MEM);
      }
      ...
      if (!(direct_sql->tgt_default_file = spider_create_string(
                *spd_defaults_file, direct_sql->tgt_default_file_length))) {
        my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
        DBUG_RETURN(HA_ERR_OUT_OF_MEM);
      }
    }
  ...
  if (direct_sql->tgt_ssl_vsc == -1) direct_sql->tgt_ssl_vsc = 0;

  if (!direct_sql->tgt_socket && !strcmp(direct_sql->tgt_host, my_localhost)) {
    DBUG_PRINT("info", ("spider create default tgt_socket"));
    direct_sql->tgt_socket_length = strlen(const_cast<char *>(MYSQL_UNIX_ADDR));
    if (!(direct_sql->tgt_socket = spider_create_string(
              const_cast<char *>(MYSQL_UNIX_ADDR), direct_sql->tgt_socket_length))) {
      my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
      DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    }
  }

  if (direct_sql->table_loop_mode == -1) direct_sql->table_loop_mode = 0;
  if (direct_sql->priority == -1) direct_sql->priority = 1000000;
  if (direct_sql->connect_timeout == -1) direct_sql->connect_timeout = 6;
  if (direct_sql->net_read_timeout == -1) direct_sql->net_read_timeout = 600;
  if (direct_sql->net_write_timeout == -1) direct_sql->net_write_timeout = 600;
  if (direct_sql->bulk_insert_rows == -1) direct_sql->bulk_insert_rows = 3000;


  if (direct_sql->use_real_table == -1) direct_sql->use_real_table = 0;

  if (direct_sql->error_rw_mode == -1) direct_sql->error_rw_mode = 0;
  for (roop_count = 0; roop_count < direct_sql->table_count; roop_count++) {
    if (direct_sql->iop[roop_count] == -1) direct_sql->iop[roop_count] = 0;
  }
  DBUG_RETURN(0);
}
...
void spider_udf_free_direct_sql_alloc(SPIDER_DIRECT_SQL *direct_sql,
  ...
  if (bg) {
    ...
    if (direct_sql->next) direct_sql->next->prev = direct_sql->prev;
    if (direct_sql->prev) direct_sql->prev->next = direct_sql->next;
...
long long spider_direct_sql_body(UDF_INIT *initid, UDF_ARGS *args,
                                 char *is_null __attribute__((unused)), char *error, bool bg) {
  int error_num, roop_count;
  ...
  if (!(direct_sql = (SPIDER_DIRECT_SQL *)spider_bulk_malloc(
            spider_current_trx, 34, MYF(MY_WME | MY_ZEROFILL), &direct_sql,
            sizeof(SPIDER_DIRECT_SQL), &sql, sizeof(char) * args->lengths[0],
            NullS))) {
    error_num = HA_ERR_OUT_OF_MEM;
    my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
    goto error;
  ...
  if (args->args[1]) {
    if ((error_num = spider_udf_direct_sql_create_table_list(
             direct_sql, args->args[1], args->lengths[1]))) {
      if (error_num == HA_ERR_OUT_OF_MEM)
        my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
      goto error;
    }
  } else {
    if ((error_num = spider_udf_direct_sql_create_table_list(direct_sql,
                                                             const_cast<char *>(""), 0))) {
      if (error_num == HA_ERR_OUT_OF_MEM)
        my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
      goto error;
  ...
  if (args->args[2]) {
    if ((error_num = spider_udf_parse_direct_sql_param(
             trx, direct_sql, args->args[2], args->lengths[2]))) {
      goto error;
    }
  } else {
    if ((error_num =
             spider_udf_parse_direct_sql_param(trx, direct_sql, "", 0))) {
      goto error;
    }
  }
  ...
  use_real_table =
      spider_param_udf_ds_use_real_table(thd, direct_sql->use_real_table);
  ...
  for (roop_count = 0; roop_count < direct_sql->table_count; roop_count++) {
    ...
    MDL_REQUEST_INIT(&table_list.mdl_request, MDL_key::TABLE, table_list.db,
                               table_list.table_name, MDL_SHARED_WRITE,
                               MDL_TRANSACTION);
#else
    table_list.db = direct_sql->db_names[roop_count];
    table_list.table_name = direct_sql->table_names[roop_count];
    ...
    if (!(direct_sql->tables[roop_count] =
      ...
      my_printf_error(ER_SPIDER_UDF_TMP_TABLE_NOT_FOUND_NUM,
                      ER_SPIDER_UDF_TMP_TABLE_NOT_FOUND_STR, MYF(0),
      ...
      //table_list.init_one_table(&table_list.db, &table_list.table_name, 0,
      //                          TL_WRITE);
      // tables->mdl_request.init(MDL_key::TABLE, table_list.db.str,
      //                        table_list.table_name.str, MDL_SHARED_WRITE,
      //                         MDL_TRANSACTION);
      tables = new (current_thd->mem_root) TABLE_LIST(table_list.db, table_list.table_name, TL_WRITE);
      MDL_REQUEST_INIT(&tables->mdl_request, MDL_key::TABLE, table_list.db,
                               table_list.table_name, MDL_SHARED_WRITE,
                               MDL_TRANSACTION); 
  ...
  if (bg) {
    ...
    if (conn->bg_init) pthread_mutex_lock(&conn->bg_conn_mutex);
    if ((error_num = spider_db_udf_direct_sql(direct_sql))) {
      if (conn->bg_init) pthread_mutex_unlock(&conn->bg_conn_mutex);
...
bool spider_direct_sql_init_body(UDF_INIT *initid, UDF_ARGS *args,
                                    char *message, bool bg) {
  SPIDER_BG_DIRECT_SQL *bg_direct_sql;
  ...
  if (bg) {
    if (!(bg_direct_sql = (SPIDER_BG_DIRECT_SQL *)spider_malloc(
              spider_current_trx, 10, sizeof(SPIDER_BG_DIRECT_SQL),
              MYF(MY_WME | MY_ZEROFILL)))) {
      strcpy(message, "spider_bg_direct_sql() out of memory");
      goto error;
    }

    if (mysql_mutex_init(spd_key_mutex_bg_direct_sql, &bg_direct_sql->bg_mutex,
                         MY_MUTEX_INIT_FAST))
    ...
    if (mysql_cond_init(spd_key_cond_bg_direct_sql, &bg_direct_sql->bg_cond))
...
int spider_udf_bg_direct_sql(SPIDER_DIRECT_SQL *direct_sql) {
  ...
  if ((error_num = spider_create_conn_thread(conn))) DBUG_RETURN(error_num);
  if (!pthread_mutex_trylock(&conn->bg_conn_mutex)) {
    ...
    if (conn->bg_job_stack.max_element > old_elements) {
      spider_alloc_calc_mem(spider_current_trx, conn->bg_job_stack,
                            (conn->bg_job_stack.max_element - old_elements) *
                                conn->bg_job_stack.size_of_element);
    }
    if (!conn->bg_get_job_stack_off) bg_get_job_stack = TRUE;
    pthread_mutex_unlock(&conn->bg_job_stack_mutex);
    if (bg_get_job_stack) {
      DBUG_PRINT("info", ("spider get bg_conn_mutex"));
