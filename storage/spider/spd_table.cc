...
#include "sql/sql_thd_internal_api.h" // destory_thd
#include "spd_err.h"
#include "spd_param.h"
...
#include "sql/tdsql/rpc/td_rpc_statistics.h"

extern HASH spider_conn_meta_info;
extern HASH spider_for_sts_conns;

#undef SPIDER_HAS_NEXT_THREAD_ID
...
inline void SPIDER_set_next_thread_id(THD *A) {
  pthread_mutex_lock(&LOCK_spider_thread_count);
  // A->thread_id = (*spd_db_att_thread_id)++;
  if (A->thread_id() == 0) {
    A->set_new_thread_id(Global_THD_manager::get_instance()->get_new_thread_id());
  }
  pthread_mutex_unlock(&LOCK_spider_thread_count);
}
MYSQL_THD create_thd() {
...
//void destroy_thd(MYSQL_THD thd) { delete thd; }
#endif
inline MYSQL_THD spider_create_sys_thd(SPIDER_THREAD *thread) {
  ...
  if (thd) {
    ...
    thd->current_cond = &thread->cond;
    thd->current_mutex = &thread->mutex;
  }
  return thd;
}
inline void spider_destroy_sys_thd(MYSQL_THD thd) { delete thd; }
inline MYSQL_THD spider_create_thd() {
  THD *thd;
  my_thread_init();
  // if (!(thd = new THD(next_thread_id())))
  if (!(thd = create_thd(false, true, false, 0)))
    my_thread_end();
  else {
...
extern void log_spider_mysql_sql(MYSQL *m, const char *query, uint length,
                                 int command);
...
struct CHARSET_INFO *spd_charset_utf8_bin;
const char **spd_defaults_extra_file;
const char **spd_defaults_file;
std::atomic<int32> volatile *spd_abort_loop;
...
static PSI_mutex_info all_spider_mutexes[] __attribute__((unused)) = {
    {&spd_key_mutex_tbl, "tbl", PSI_FLAG_SINGLETON, 0, ""},
    {&spd_key_mutex_init_error_tbl, "init_error_tbl", PSI_FLAG_SINGLETON, 0, ""},
#ifdef WITH_PARTITION_STORAGE_ENGINE
    {&spd_key_mutex_pt_share, "pt_share", PSI_FLAG_SINGLETON, 0, ""},
#endif
    {&spd_key_mutex_lgtm_tblhnd_share, "lgtm_tblhnd_share", PSI_FLAG_SINGLETON, 0, ""},
    {&spd_key_mutex_conn, "conn", PSI_FLAG_SINGLETON, 0, ""},
    {&spd_key_mutex_conn_meta, "conn_meta", PSI_FLAG_SINGLETON, 0, ""},
    {&spd_key_mutex_open_conn, "open_conn", PSI_FLAG_SINGLETON, 0, ""},
    {&spd_key_mutex_allocated_thds, "allocated_thds", PSI_FLAG_SINGLETON, 0, ""},
    {&spd_key_mutex_mon_table_cache, "mon_table_cache", PSI_FLAG_SINGLETON, 0, ""},
    {&spd_key_mutex_udf_table_mon, "udf_table_mon", PSI_FLAG_SINGLETON, 0, ""},
    {&spd_key_mutex_mem_calc, "mem_calc", PSI_FLAG_SINGLETON, 0, ""},
    {&spd_key_thread_id, "thread_id", PSI_FLAG_SINGLETON, 0, ""},
    {&spd_key_conn_id, "conn_id", PSI_FLAG_SINGLETON, 0, ""},
    {&spd_key_rwlock_ipport_conn, "ipport_count", PSI_FLAG_SINGLETON, 0, ""},
    {&spd_key_mutex_xid, "xid", PSI_FLAG_SINGLETON, 0, ""},
    {&spd_key_mutex_xid_cache, "xid_cache", PSI_FLAG_SINGLETON, 0, ""},
    {&spd_key_mutex_bg_stss, "bg_stss", PSI_FLAG_SINGLETON, 0, ""},
    {&spd_key_mutex_bg_crds, "bg_crds", PSI_FLAG_SINGLETON, 0, ""},
    {&spd_key_mutex_conn_i, "conn_i", 0, 0, ""},
    {&spd_key_mutex_mta_conn, "mta_conn", 0, 0, ""},
    {&spd_key_mutex_bg_conn_chain, "bg_conn_chain", 0, 0, ""},
    {&spd_key_mutex_bg_conn_sync, "bg_conn_sync", 0, 0, ""},
    {&spd_key_mutex_bg_conn, "bg_conn", 0, 0, ""},
    {&spd_key_mutex_conn_finish_exec, "bg_conn_finish_exec", 0, 0, ""},
    {&spd_key_mutex_bg_job_stack, "bg_job_stack", 0, 0, ""},
    {&spd_key_mutex_bg_mon, "bg_mon", 0, 0, ""},
    {&spd_key_mutex_bg_direct_sql, "bg_direct_sql", 0, 0, ""},
    {&spd_key_mutex_mon_list_caller, "mon_list_caller", 0, 0, ""},
    {&spd_key_mutex_mon_list_receptor, "mon_list_receptor", 0, 0, ""},
    {&spd_key_mutex_mon_list_monitor, "mon_list_monitor", 0, 0, ""},
    {&spd_key_mutex_mon_list_update_status, "mon_list_update_status", 0, 0, ""},
    {&spd_key_mutex_share, "share", 0, 0, ""},
    {&spd_key_mutex_share_sts, "share_sts", 0, 0, ""},
    {&spd_key_mutex_share_crd, "share_crd", 0, 0, ""},
    {&spd_key_mutex_share_auto_increment, "share_auto_increment", 0, 0, ""},
#ifdef WITH_PARTITION_STORAGE_ENGINE
    {&spd_key_mutex_pt_share_sts, "pt_share_sts", 0, 0, ""},
    {&spd_key_mutex_pt_share_crd, "pt_share_crd", 0, 0, ""},
    {&spd_key_mutex_pt_handler, "pt_handler", 0, 0, ""},
#endif
    {&spd_key_mutex_udf_table, "udf_table", 0, 0, ""},
    {&spd_key_mutex_conn_status, "conn_status", 0, 0, ""},
    {&spd_key_mutex_active_conns, "active_conns", 0, 0, ""},
};
...
static PSI_cond_info all_spider_conds[] __attribute__((unused)) = {
    {&spd_key_cond_bg_conn_sync, "bg_conn_sync", 0, 0, ""},
    {&spd_key_cond_bg_conn, "bg_conn", 0, 0, ""},
    {&spd_key_cond_conn_finish_exec, "bg_conn_finish_exec", 0, 0, ""},
    {&spd_key_cond_bg_sts, "bg_sts", 0, 0, ""},
    {&spd_key_cond_bg_sts_sync, "bg_sts_sync", 0, 0, ""},
    {&spd_key_cond_bg_crd, "bg_crd", 0, 0, ""},
    {&spd_key_cond_bg_crd_sync, "bg_crd_sync", 0, 0, ""},
    {&spd_key_cond_bg_mon, "bg_mon", 0, 0, ""},
    {&spd_key_cond_bg_mon_sleep, "bg_mon_sleep", 0, 0, ""},
    {&spd_key_cond_bg_direct_sql, "bg_direct_sql", 0, 0, ""},
    {&spd_key_cond_udf_table_mon, "udf_table_mon", 0, 0, ""},
    {&spd_key_cond_conn_i, "conn_i", 0, 0, ""},
    {&spd_key_cond_bg_stss, "bg_stss", 0, 0, ""},
    {&spd_key_cond_bg_sts_syncs, "bg_sts_syncs", 0, 0, ""},
    {&spd_key_cond_bg_crds, "bg_crds", 0, 0, ""},
    {&spd_key_cond_bg_crd_syncs, "bg_crd_syncs", 0, 0, ""},
};
...
PSI_thread_key spd_key_thd_get_status;

static PSI_thread_info all_spider_threads[] __attribute__((unused)) = {
    {&spd_key_thd_bg, "bg", 0, 0, ""},
    {&spd_key_thd_bg_sts, "bg_sts", 0, 0, ""},
    {&spd_key_thd_bg_crd, "bg_crd", 0, 0, ""},
    {&spd_key_thd_bg_mon, "bg_mon", 0, 0, ""},
    {&spd_key_thd_bg_stss, "bg_stss", 0, 0, ""},
    {&spd_key_thd_bg_crds, "bg_crds", 0, 0, ""},
    {&spd_key_thd_conn_rcyc, "conn_rcyc", 0, 0, ""},
};
#endif
...
uchar *spider_tbl_get_key(SPIDER_SHARE *share, size_t *length,
  ...
  *length = share->table_name_with_version_length;
  DBUG_RETURN((uchar *)share->table_name_with_version);
}

#ifdef WITH_PARTITION_STORAGE_ENGINE
uchar *spider_pt_share_get_key(SPIDER_PARTITION_SHARE *share, size_t *length,
                               bool not_used __attribute__((unused))) {
...
uchar *spider_lgtm_tblhnd_share_hash_get_key(SPIDER_LGTM_TBLHND_SHARE *share,
                                             size_t *length,
                                             bool not_used
                                             __attribute__((unused))) {
...
uchar *spider_link_get_key(SPIDER_LINK_FOR_HASH *link_for_hash, size_t *length,
  ...
  *length = link_for_hash->db_table_str->length();
  char* ptmp = const_cast<char*>(link_for_hash->db_table_str->ptr());
  DBUG_RETURN(reinterpret_cast<uchar *>(ptmp));
}
...
uchar *spider_udf_tbl_mon_list_key(SPIDER_TABLE_MON_LIST *table_mon_list,
                                   size_t *length,
                                   bool not_used __attribute__((unused))) {
...
uchar *spider_allocated_thds_get_key(THD *thd, size_t *length,
                                     bool not_used __attribute__((unused))) {
...
#if 0
  if (PSI_server == NULL) DBUG_VOID_RETURN;
...
int spider_get_server(SPIDER_SHARE *share, int link_idx) {
  ...
  tdsql::common::TdString master_host;
  uint32_t master_port = 0;

  auto set_info =
      get_server_by_name_or_alias_name(share->server_names[link_idx]);
  if (!set_info) {
    error_num = ER_FOREIGN_SERVER_DOESNT_EXIST;
    goto error;
  }
  master_host =
      set_info->MasterNodeInfo() ? set_info->MasterNodeInfo()->host : "";
  master_port = set_info->MasterNodeInfo() ? set_info->MasterNodeInfo()->port : 0;

  if (!share->tgt_wrappers[link_idx] && !set_info->ServerSchema().empty()) {
    auto server_schema = set_info->ServerSchema();
    share->tgt_wrappers_lengths[link_idx] = server_schema.size();
    if (!(share->tgt_wrappers[link_idx] = spider_create_string(
              server_schema.c_str(), share->tgt_wrappers_lengths[link_idx]))) {
      error_num = HA_ERR_OUT_OF_MEM;
      goto error;
    }
    DBUG_PRINT("info",
               ("spider tgt_wrappers=%s", share->tgt_wrappers[link_idx]));
  }

  if (!share->tgt_hosts[link_idx] && !master_host.empty()) {
    share->tgt_hosts_lengths[link_idx] = master_host.size();
    if (!(share->tgt_hosts[link_idx] = spider_create_string(
              master_host.c_str(), share->tgt_hosts_lengths[link_idx]))) {
      error_num = HA_ERR_OUT_OF_MEM;
      goto error;
    }
  ...
  if (share->tgt_ports[link_idx] == -1) {
    share->tgt_ports[link_idx] = master_port;
    DBUG_PRINT("info", ("spider tgt_ports=%ld", share->tgt_ports[link_idx]));
  }

  if (!share->tgt_sockets[link_idx] && !set_info->ServersSocket().empty()) {
    auto server_socket = set_info->ServersSocket();
    share->tgt_sockets_lengths[link_idx] = server_socket.size();
    if (!(share->tgt_sockets[link_idx] = spider_create_string(
              server_socket.c_str(), share->tgt_sockets_lengths[link_idx]))) {
      error_num = HA_ERR_OUT_OF_MEM;
      goto error;
    }
  ...
  if (!share->tgt_dbs[link_idx] && !set_info->DefaultDb().empty()) {
    auto server_db = set_info->DefaultDb();
    share->tgt_dbs_lengths[link_idx] = server_db.size();
    if (!(share->tgt_dbs[link_idx] =
              spider_create_string(server_db.c_str(), server_db.size()))) {
      error_num = HA_ERR_OUT_OF_MEM;
      goto error;
    }
  ...
  if (!share->tgt_usernames[link_idx] && !set_info->User().empty()) {
    auto server_user = set_info->User();
    share->tgt_usernames_lengths[link_idx] = server_user.size();
    if (!(share->tgt_usernames[link_idx] = spider_create_string(
              server_user.c_str(), share->tgt_usernames_lengths[link_idx]))) {
      error_num = HA_ERR_OUT_OF_MEM;
      goto error;
    }
    DBUG_PRINT("info",
               ("spider tgt_usernames=%s", share->tgt_usernames[link_idx]));
  }

  if (!share->tgt_passwords[link_idx] && !set_info->Password().empty()) {
    auto password = set_info->Password();
    share->tgt_passwords_lengths[link_idx] = password.size();
    if (!(share->tgt_passwords[link_idx] = spider_create_string(
              password.c_str(), share->tgt_passwords_lengths[link_idx]))) {
      error_num = HA_ERR_OUT_OF_MEM;
      goto error;
    }
    DBUG_PRINT("info",
               ("spider tgt_passwords=%s", share->tgt_passwords[link_idx]));
  }
...
int spider_free_share_alloc(SPIDER_SHARE *share) {
  ...
  if (share->tgt_shard_keys) {
    for (roop_count = 0; roop_count < (int)share->tgt_shard_keys_length;
      ...
      if (share->tgt_shard_keys[roop_count])
        spider_free(spider_current_trx, share->tgt_shard_keys[roop_count],
                    MYF(0));
    }
  ...
  if (share->tgt_config_table) {
    for (roop_count = 0; roop_count < (int)share->tgt_config_table_length;
      ...
      if (share->tgt_config_table[roop_count])
        spider_free(spider_current_trx, share->tgt_config_table[roop_count],
                    MYF(0));
    }
  ...
  if (share->tgt_sockets) {
    for (roop_count = 0; roop_count < (int)share->tgt_sockets_length;
      ...
      if (share->tgt_sockets[roop_count])
        spider_free(spider_current_trx, share->tgt_sockets[roop_count], MYF(0));
    }
  ...
  if (share->tgt_ssl_cas) {
    for (roop_count = 0; roop_count < (int)share->tgt_ssl_cas_length;
      ...
      if (share->tgt_ssl_cas[roop_count])
        spider_free(spider_current_trx, share->tgt_ssl_cas[roop_count], MYF(0));
    }
  ...
  if (share->monitoring_binlog_pos_at_failing)
    spider_free(spider_current_trx, share->monitoring_binlog_pos_at_failing,
                MYF(0));
...
#ifdef WITH_PARTITION_STORAGE_ENGINE
  if (share->partition_share) spider_free_pt_share(share->partition_share);
#endif
  DBUG_RETURN(0);
}
...
char *spider_get_string_between_quote(
    char *ptr, bool alloc, SPIDER_PARAM_STRING_PARSE *param_string_parse) {
  ...
  if (start_ptr && (!end_ptr || start_ptr < end_ptr)) {
    tmp_ptr = ++start_ptr;
    while (!find_flg) {
      if (!(end_ptr = strchr(tmp_ptr, '\''))) DBUG_RETURN(NULL);
      esc_ptr = tmp_ptr;
      while (!find_flg) {
  ...
  *end_ptr = '\0';
  ...
  if (param_string_parse)
    param_string_parse->set_param_value(start_ptr,
                                        start_ptr + strlen(start_ptr) + 1);

  if (alloc) {
    DBUG_RETURN(spider_create_string(start_ptr, strlen(start_ptr)));
  } else {
...
int spider_create_string_list(char ***string_list, uint **string_length_list,
                              uint *list_length, char *str, uint length __attribute__((unused)),
  ...
  *list_length = 0;
  ...
  if (!(*string_list = (char **)spider_bulk_malloc(
            spider_current_trx, 37, MYF(MY_WME | MY_ZEROFILL), string_list,
            sizeof(char *) * (*list_length), string_length_list,
            sizeof(int) * (*list_length), NullS))) {
...
int spider_create_long_list(long **long_list, uint *list_length, char *str,
                            uint length __attribute__((unused)), long min_val, long max_val,
                            SPIDER_PARAM_STRING_PARSE *param_string_parse) {
  int roop_count;
  char *tmp_ptr;
  ...
  *list_length = 0;
  ...
  if (!(*long_list = (long *)spider_bulk_malloc(
            spider_current_trx, 38, MYF(MY_WME | MY_ZEROFILL), long_list,
            sizeof(long) * (*list_length), NullS))) {
    my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  }
  ...
  for (roop_count = 0; roop_count < (int)*list_length; roop_count++) {
    if (roop_count != 0) tmp_ptr = strchr(tmp_ptr, ' ');
  ...
  param_string_parse->set_param_value(tmp_ptr, tmp_ptr + strlen(tmp_ptr) + 1);
...
int spider_create_longlong_list(longlong **longlong_list, uint *list_length,
                                char *str, uint length __attribute__((unused)), longlong min_val,
                                longlong max_val,
                                SPIDER_PARAM_STRING_PARSE *param_string_parse) {
  ...
  *list_length = 0;
  ...
  if (!(*longlong_list = (longlong *)spider_bulk_malloc(
            spider_current_trx, 39, MYF(MY_WME | MY_ZEROFILL), longlong_list,
            sizeof(longlong) * (*list_length), NullS))) {
    my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  }
  ...
  for (roop_count = 0; roop_count < (int)*list_length; roop_count++) {
    ...
    (*longlong_list)[roop_count] =
        my_strtoll10(tmp_ptr, nullptr, &error_num);
...
int spider_increase_string_list(char ***string_list, uint **string_length_list,
  ...
  if (!(tmp_str_list = (char **)spider_bulk_malloc(
            spider_current_trx, 40, MYF(MY_WME | MY_ZEROFILL), &tmp_str_list,
            sizeof(char *) * link_count, &tmp_length_list,
            sizeof(uint) * link_count, NullS))) {
    my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  }
  ...
  for (roop_count = 0; roop_count < (int)link_count; roop_count++) {
    ...
    if (tmp_str) {
      if (!(tmp_str_list[roop_count] =
                spider_create_string(tmp_str, tmp_length)))
        goto error;
      DBUG_PRINT("info", ("spider string_list[%d]=%s", roop_count,
...
error:
  ...
  if (tmp_str_list) spider_free(spider_current_trx, tmp_str_list, MYF(0));
  my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
  DBUG_RETURN(HA_ERR_OUT_OF_MEM);
}

int spider_increase_null_string_list(char ***string_list,
                                     uint **string_length_list,
                                     uint *list_length, uint *list_charlen __attribute__((unused)),
                                     uint link_count) {
  int roop_count;
  char **tmp_str_list;
  ...
  if (!(tmp_str_list = (char **)spider_bulk_malloc(
            spider_current_trx, 247, MYF(MY_WME | MY_ZEROFILL), &tmp_str_list,
            sizeof(char *) * link_count, &tmp_length_list,
            sizeof(uint) * link_count, NullS))) {
    my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
...
int spider_increase_long_list(long **long_list, uint *list_length,
  ...
  if (!(tmp_long_list = (long *)spider_bulk_malloc(
            spider_current_trx, 41, MYF(MY_WME | MY_ZEROFILL), &tmp_long_list,
            sizeof(long) * link_count, NullS))) {
    my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  }
  ...
  for (roop_count = 0; roop_count < (int)link_count; roop_count++) {
    ...
    DBUG_PRINT("info", ("spider long_list[%d]=%ld", roop_count, tmp_long));
  }
  if (*long_list) spider_free(spider_current_trx, *long_list, MYF(0));
  *list_length = link_count;
...
int spider_increase_longlong_list(longlong **longlong_list, uint *list_length,
  ...
  if (!(tmp_longlong_list = (longlong *)spider_bulk_malloc(
            spider_current_trx, 42, MYF(MY_WME | MY_ZEROFILL),
            &tmp_longlong_list, sizeof(longlong) * link_count, NullS))) {
    my_error(ER_OUT_OF_RESOURCES, MYF(0), HA_ERR_OUT_OF_MEM);
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  }
  ...
  for (roop_count = 0; roop_count < (int)link_count; roop_count++) {
    ...
    DBUG_PRINT("info",
               ("spider longlong_list[%d]=%lld", roop_count, tmp_longlong));
  }
  if (*longlong_list) spider_free(spider_current_trx, *longlong_list, MYF(0));
  *list_length = link_count;
...
static int spider_set_ll_value(longlong *value, char *str) {
  int error_num = 0;
  DBUG_ENTER("spider_set_ll_value");
  *value = my_strtoll10(str, nullptr, &error_num);
  DBUG_RETURN(error_num);
}
  ...
  if (start_title_ptr) {
    ...
    switch (error_num) {
      case ER_SPIDER_INVALID_UDF_PARAM_NUM:
        my_printf_error(error_num, ER_SPIDER_INVALID_UDF_PARAM_STR, MYF(0),
                        start_title_ptr);
        break;
      case ER_SPIDER_INVALID_CONNECT_INFO_NUM:
      default:
        my_printf_error(error_num, ER_SPIDER_INVALID_CONNECT_INFO_STR, MYF(0),
                        start_title_ptr);
    }
...
#define SPIDER_PARAM_HINT(title_name, param_name, check_length, max_size,      \
                          append_method)                                       \
...
#define SPIDER_PARAM_NUMHINT(title_name, param_name, check_length, max_size,   \
                             append_method)                                    \
  if (!strncasecmp(tmp_ptr, title_name, check_length)) {                       \
    DBUG_PRINT("info", ("spider " title_name " start"));                       \
    DBUG_PRINT("info", ("spider max_size=%d", max_size));                      \
...
#define SPIDER_PARAM_LONG_LIST_WITH_MAX(title_name, param_name, min_val,      \
                                        max_val)                              \
  if (!strncasecmp(tmp_ptr, title_name, title_length)) {                      \
    DBUG_PRINT("info", ("spider " title_name " start"));                      \
    if (!share->param_name) {                                                 \
      if ((tmp_ptr2 = spider_get_string_between_quote(start_ptr, FALSE))) {   \
        if ((error_num = spider_create_long_list(                             \
                 &share->param_name,                                          \
                 &share->SPIDER_PARAM_LONG_LEN(param_name), tmp_ptr2,         \
                 strlen(tmp_ptr2), min_val, max_val, &connect_string_parse))) \
          goto error;                                                         \
      } else {                                                                \
        error_num = connect_string_parse.print_param_error();                 \
        goto error;                                                           \
...
#define SPIDER_PARAM_LONGLONG_LIST_WITH_MAX(title_name, param_name, min_val,  \
                                            max_val)                          \
  if (!strncasecmp(tmp_ptr, title_name, title_length)) {                      \
    DBUG_PRINT("info", ("spider " title_name " start"));                      \
    if (!share->param_name) {                                                 \
      if ((tmp_ptr2 = spider_get_string_between_quote(start_ptr, FALSE))) {   \
        if ((error_num = spider_create_longlong_list(                         \
                 &share->param_name,                                          \
                 &share->SPIDER_PARAM_LONGLONG_LEN(param_name), tmp_ptr2,     \
                 strlen(tmp_ptr2), min_val, max_val, &connect_string_parse))) \
          goto error;                                                         \
      } else {                                                                \
        error_num = connect_string_parse.print_param_error();                 \
...
#define SPIDER_PARAM_INT(title_name, param_name, min_val)                      \
  if (!strncasecmp(tmp_ptr, title_name, title_length)) {                       \
    ...
    if (share->param_name == -1) {                                             \
      if ((tmp_ptr2 = spider_get_string_between_quote(start_ptr, FALSE))) {    \
        share->param_name = atoi(tmp_ptr2);                                    \
        if (share->param_name < min_val) share->param_name = min_val;          \
        connect_string_parse.set_param_value(tmp_ptr2,                         \
                                             tmp_ptr2 + strlen(tmp_ptr2) + 1); \
      } else {                                                                 \
        error_num = connect_string_parse.print_param_error();                  \
...
#define SPIDER_PARAM_LONGLONG(title_name, param_name, min_val)                 \
  if (!strncasecmp(tmp_ptr, title_name, title_length)) {                       \
    ...
    if (share->param_name == -1) {                                             \
      if ((tmp_ptr2 = spider_get_string_between_quote(start_ptr, FALSE))) {    \
        share->param_name = my_strtoll10(tmp_ptr2, nullptr, &error_num); \
        if (share->param_name < min_val) share->param_name = min_val;          \
        connect_string_parse.set_param_value(tmp_ptr2,                         \
                                             tmp_ptr2 + strlen(tmp_ptr2) + 1); \
      } else {                                                                 \
...
static char *get_noshard_table_connect_string() {
  auto main_sever = tdsql::main::TdMainServer::Instance();
  assert(main_sever);
  auto cluster_cache = main_sever->ClusterCache();
  assert(main_sever);
  auto set_infos = cluster_cache->ClusterInfo()->GetFirstSetInfo();
  if (!set_infos) {
    LogCommError("get first set info failed, nullptr, cluster info: %s",
                 cluster_cache->ClusterInfo()->ToString().c_str());
    return nullptr;
  }

  tdsql::common::TdString str = "SERVER \"" + set_infos->AliasName() + "\"";
  char *connect_string = nullptr;
  if (!(connect_string = spider_create_string(str.c_str(), str.size()))) {
    LogCommError("create noshard table connect str failed, info: %s",
                 str.c_str());
    return nullptr;
  }
  LogCommDebug("get noshard table route info: %s", connect_string);
  return connect_string;
}

static char *get_broadcast_table_connect_string() {
  auto main_sever = tdsql::main::TdMainServer::Instance();
  assert(main_sever);
  auto cluster_cache = main_sever->ClusterCache();
  assert(main_sever);
  auto set_infos = cluster_cache->ClusterInfo()->SetInfos();
  tdsql::common::TdString str = "SERVER \"";
  if (set_infos.empty()) {
    LogCommError("get all set info failed, nullptr, cluster info: %s",
                 cluster_cache->ClusterInfo()->ToString().c_str());
    return nullptr;
  }

  for (const auto &item : set_infos) {
    str += item.second->AliasName() + " ";
  }
  str[str.size() - 1] = '\"';
  char *connect_string = nullptr;
  if (!(connect_string = spider_create_string(str.c_str(), str.size()))) {
    LogCommError("create all sets table connect str failed, info: %s",
                 str.c_str());
    return nullptr;
  }
  LogCommDebug("get broadcast table route info: %s", connect_string);
  return connect_string;
}
...
int spider_parse_connect_info(SPIDER_SHARE *share, TABLE_SHARE *table_share,
  ...
  DBUG_PRINT("info",
             ("spider partition_info=%s", table_share->partition_info_str));
  ...
  DBUG_PRINT("info", ("spider s->normalized_path=%s",
...
#ifdef WITH_PARTITION_STORAGE_ENGINE
  for (roop_count = 5; roop_count > 0; roop_count--)
#else
...
#endif
  {
    ...
    switch (roop_count) {
#ifdef WITH_PARTITION_STORAGE_ENGINE
      case 5:
        if (table_share->tdsql_table_type == tdsql::ddl::TD_NOSHARD_TABLE) {
          if (!(connect_string = get_noshard_table_connect_string())) {
            error_num = HA_ERR_OUT_OF_MEM;
            goto error_alloc_conn_string;
          }
          DBUG_PRINT("info", ("spider comment string=%s", connect_string));
          break;
        } else if (table_share->tdsql_table_type ==
                   tdsql::ddl::TD_ALLSET_TABLE) {
          if (!(connect_string = get_broadcast_table_connect_string())) {
            error_num = HA_ERR_OUT_OF_MEM;
            goto error_alloc_conn_string;
          }
          DBUG_PRINT("info", ("spider comment string=%s", connect_string));
        ...
        if (!part_elem || !part_elem->partition_name) continue;
        if (check_server_exist_by_alias(part_elem->partition_name)) {
          char buffer[1024];
          snprintf(buffer, 1024, "SERVER \"%s\"", part_elem->partition_name);
          if (!(connect_string =
                    spider_create_string(buffer, strlen(buffer)))) {
            error_num = HA_ERR_OUT_OF_MEM;
            goto error_alloc_conn_string;
          }
          LogDebug("use partition name as route, str: %s", buffer);
          break;
        } else {
          LogDebug("partition does not server info, name: %s",
      ...
      case 2:
        ...
        break;
      default:
        if (table_share->connect_string.length == 0) continue;
        DBUG_PRINT("info", ("spider create connect_string string"));
        if (!(connect_string =
                  spider_create_string(table_share->connect_string.str,
    ...
    connect_string_parse.init(connect_string,
    ...
    while (sprit_ptr[0]) {
      ...
      switch (title_length) {
        ...
        case 3:
          ...
          SPIDER_PARAM_LONG_LIST_WITH_MAX("btt", bka_table_name_types, 0, 1);
          ...
          SPIDER_PARAM_LONGLONG_LIST_WITH_MAX("mbi", monitoring_bg_interval, 0,
                                              4294967295LL);
          SPIDER_PARAM_LONG_LIST_WITH_MAX("mbk", monitoring_bg_kind, 0, 3);
          SPIDER_PARAM_LONG_LIST_WITH_MAX(
              "mbp", monitoring_binlog_pos_at_failing, 0, 2);
        ...
        case 15:
          ...
          SPIDER_PARAM_LONG_LIST_WITH_MAX("monitoring_flag", monitoring_flag, 0,
                                          1);
          SPIDER_PARAM_LONG_LIST_WITH_MAX("monitoring_kind", monitoring_kind, 0,
                                          3);
          ...
          goto error;
        case 16:
          SPIDER_PARAM_INT_WITH_MAX("multi_split_read", multi_split_read, 0,
                                    2147483647);
          SPIDER_PARAM_INT_WITH_MAX("selupd_lock_mode", selupd_lock_mode, 0, 2);
          SPIDER_PARAM_INT_WITH_MAX("internal_delayed", internal_delayed, 0, 1);
          ...
          SPIDER_PARAM_LONG_LIST_WITH_MAX("net_read_timeout", net_read_timeouts,
                                          0, 2147483647);
          SPIDER_PARAM_INT_WITH_MAX("error_write_mode", error_write_mode, 0, 1);
#ifdef HA_CAN_BULK_ACCESS
          SPIDER_PARAM_INT_WITH_MAX("bulk_access_free", bulk_access_free, 0, 1);
#endif
          SPIDER_PARAM_INT_WITH_MAX("query_cache_sync", query_cache_sync, 0, 3);
          error_num = connect_string_parse.print_param_error();
          goto error;
        case 17:
          SPIDER_PARAM_INT_WITH_MAX("internal_optimize", internal_optimize, 0,
                                    1);
          SPIDER_PARAM_INT_WITH_MAX("use_table_charset", use_table_charset, 0,
                                    1);
          SPIDER_PARAM_INT_WITH_MAX("direct_dup_insert", direct_dup_insert, 0,
                                    1);
...
#ifdef HA_CAN_FORCE_BULK_DELETE
          SPIDER_PARAM_INT_WITH_MAX("force_bulk_delete", force_bulk_delete, 0,
                                    1);
#endif
#ifdef HA_CAN_FORCE_BULK_UPDATE
          SPIDER_PARAM_INT_WITH_MAX("force_bulk_update", force_bulk_update, 0,
                                    1);
#endif
          error_num = connect_string_parse.print_param_error();
          goto error;
        case 18:
          SPIDER_PARAM_INT_WITH_MAX("select_column_mode", select_column_mode, 0,
                                    1);
          SPIDER_PARAM_LONG_LIST_WITH_MAX("monitoring_bg_flag",
                                          monitoring_bg_flag, 0, 1);
          SPIDER_PARAM_LONG_LIST_WITH_MAX("monitoring_bg_kind",
                                          monitoring_bg_kind, 0, 3);
          SPIDER_PARAM_LONGLONG("direct_order_limit", direct_order_limit, 0);
          error_num = connect_string_parse.print_param_error();
          goto error;
        case 19:
          SPIDER_PARAM_INT("init_sql_alloc_size", init_sql_alloc_size, 0);
          SPIDER_PARAM_INT_WITH_MAX("auto_increment_mode", auto_increment_mode,
                                    0, 3);
          SPIDER_PARAM_LONG_LIST_WITH_MAX("bka_table_name_type",
                                          bka_table_name_types, 0, 1);
          SPIDER_PARAM_INT_WITH_MAX("load_crd_at_startup", load_crd_at_startup,
                                    0, 1);
          SPIDER_PARAM_INT_WITH_MAX("load_sts_at_startup", load_sts_at_startup,
                                    0, 1);
          error_num = connect_string_parse.print_param_error();
          goto error;
        case 20:
          SPIDER_PARAM_LONGLONG_LIST_WITH_MAX("monitoring_server_id",
                                              monitoring_sid, 0, 4294967295LL);
          SPIDER_PARAM_INT_WITH_MAX("delete_all_rows_type",
                                    delete_all_rows_type, 0, 1);
          SPIDER_PARAM_INT_WITH_MAX("skip_parallel_search",
                                    skip_parallel_search, 0, 3);
          error_num = connect_string_parse.print_param_error();
          goto error;
        case 21:
          SPIDER_PARAM_LONGLONG("semi_split_read_limit", semi_split_read_limit,
                                0);
          error_num = connect_string_parse.print_param_error();
          goto error;
        case 22:
          SPIDER_PARAM_LONG_LIST_WITH_MAX("ssl_verify_server_cert",
                                          tgt_ssl_vscs, 0, 1);
          SPIDER_PARAM_LONGLONG_LIST_WITH_MAX("monitoring_bg_interval",
                                              monitoring_bg_interval, 0,
                                              4294967295LL);
          SPIDER_PARAM_INT_WITH_MAX("skip_default_condition",
                                    skip_default_condition, 0, 1);
          SPIDER_PARAM_LONGLONG("static_mean_rec_length",
                                static_mean_rec_length, 0);
          error_num = connect_string_parse.print_param_error();
          goto error;
        case 23:
          SPIDER_PARAM_INT_WITH_MAX("internal_optimize_local",
                                    internal_optimize_local, 0, 1);
          error_num = connect_string_parse.print_param_error();
          goto error;
        case 25:
          ...
          goto error;
        case 26:
          SPIDER_PARAM_INT_WITH_MAX("semi_table_lock_connection",
                                    semi_table_lock_conn, 0, 1);
          error_num = connect_string_parse.print_param_error();
          goto error;
        case 32:
          SPIDER_PARAM_LONG_LIST_WITH_MAX("monitoring_binlog_pos_at_failing",
                                          monitoring_binlog_pos_at_failing, 0,
                                          2);
          error_num = connect_string_parse.print_param_error();
          goto error;
        default:
    ...
    // tdsql connection string only parse once
    if (table_share->tdsql_table_type != tdsql::ddl::TD_INIT_TABLE) {
      break;
    }
  }
...
//#ifndef SPIDER_DISABLE_LINK // enable multi links
...
//#endif
  if ((error_num = spider_increase_string_list(
           &share->server_names, &share->server_names_lengths,
           &share->server_names_length, &share->server_names_charlen,
           share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_string_list(
           &share->tgt_table_names, &share->tgt_table_names_lengths,
           &share->tgt_table_names_length, &share->tgt_table_names_charlen,
           share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_string_list(
           &share->tgt_dbs, &share->tgt_dbs_lengths, &share->tgt_dbs_length,
           &share->tgt_dbs_charlen, share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_string_list(
           &share->tgt_hosts, &share->tgt_hosts_lengths,
           &share->tgt_hosts_length, &share->tgt_hosts_charlen,
           share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_string_list(
           &share->tgt_usernames, &share->tgt_usernames_lengths,
           &share->tgt_usernames_length, &share->tgt_usernames_charlen,
           share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_string_list(
           &share->tgt_passwords, &share->tgt_passwords_lengths,
           &share->tgt_passwords_length, &share->tgt_passwords_charlen,
           share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_string_list(
           &share->tgt_sockets, &share->tgt_sockets_lengths,
           &share->tgt_sockets_length, &share->tgt_sockets_charlen,
           share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_string_list(
           &share->tgt_config_table, &share->tgt_config_table_lengths,
           &share->tgt_config_table_length, &share->tgt_config_table_charlen,
           share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_string_list(
           &share->tgt_wrappers, &share->tgt_wrappers_lengths,
           &share->tgt_wrappers_length, &share->tgt_wrappers_charlen,
           share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_string_list(
           &share->tgt_ssl_cas, &share->tgt_ssl_cas_lengths,
           &share->tgt_ssl_cas_length, &share->tgt_ssl_cas_charlen,
           share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_string_list(
           &share->tgt_ssl_capaths, &share->tgt_ssl_capaths_lengths,
           &share->tgt_ssl_capaths_length, &share->tgt_ssl_capaths_charlen,
           share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_string_list(
           &share->tgt_ssl_certs, &share->tgt_ssl_certs_lengths,
           &share->tgt_ssl_certs_length, &share->tgt_ssl_certs_charlen,
           share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_string_list(
           &share->tgt_ssl_ciphers, &share->tgt_ssl_ciphers_lengths,
           &share->tgt_ssl_ciphers_length, &share->tgt_ssl_ciphers_charlen,
           share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_string_list(
           &share->tgt_ssl_keys, &share->tgt_ssl_keys_lengths,
           &share->tgt_ssl_keys_length, &share->tgt_ssl_keys_charlen,
           share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_string_list(
    ...
    goto error;
  if ((error_num = spider_increase_string_list(
           &share->tgt_default_groups, &share->tgt_default_groups_lengths,
           &share->tgt_default_groups_length,
           &share->tgt_default_groups_charlen, share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_string_list(
           &share->tgt_pk_names, &share->tgt_pk_names_lengths,
           &share->tgt_pk_names_length, &share->tgt_pk_names_charlen,
           share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_string_list(
           &share->tgt_sequence_names, &share->tgt_sequence_names_lengths,
           &share->tgt_sequence_names_length,
           &share->tgt_sequence_names_charlen, share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_null_string_list(
           &share->static_link_ids, &share->static_link_ids_lengths,
           &share->static_link_ids_length, &share->static_link_ids_charlen,
           share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_long_list(
           &share->tgt_ports, &share->tgt_ports_length, share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_long_list(&share->tgt_ssl_vscs,
                                             &share->tgt_ssl_vscs_length,
                                             share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_long_list(&share->link_statuses,
                                             &share->link_statuses_length,
                                             share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_long_list(&share->monitoring_bg_flag,
                                             &share->monitoring_bg_flag_length,
                                             share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_long_list(&share->monitoring_bg_kind,
  ...
  if ((error_num = spider_increase_long_list(
           ...
           share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_long_list(&share->monitoring_flag,
                                             &share->monitoring_flag_length,
                                             share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_long_list(&share->monitoring_kind,
                                             &share->monitoring_kind_length,
                                             share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_longlong_list(
           &share->monitoring_bg_interval,
           &share->monitoring_bg_interval_length, share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_longlong_list(
           &share->monitoring_limit, &share->monitoring_limit_length,
           share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_longlong_list(&share->monitoring_sid,
                                                 &share->monitoring_sid_length,
                                                 share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_long_list(&share->use_handlers,
                                             &share->use_handlers_length,
                                             share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_long_list(&share->connect_timeouts,
                                             &share->connect_timeouts_length,
                                             share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_long_list(&share->net_read_timeouts,
                                             &share->net_read_timeouts_length,
                                             share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_long_list(&share->net_write_timeouts,
                                             &share->net_write_timeouts_length,
                                             share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_long_list(&share->access_balances,
                                             &share->access_balances_length,
                                             share->all_link_count)))
    goto error;
  if ((error_num = spider_increase_long_list(
           &share->bka_table_name_types, &share->bka_table_name_types_length,
           share->all_link_count)))
    goto error;
  ...
  if (!(share_alter->tmp_server_names = (char **)spider_bulk_malloc(
            spider_current_trx, 43, MYF(MY_WME | MY_ZEROFILL),
            ...
            &share_alter->tmp_tgt_ports, sizeof(long) * share->all_link_count,
            ...
            sizeof(long) * share->all_link_count, NullS))) {
    error_num = HA_ERR_OUT_OF_MEM;
    goto error;
  }
  ...
  memcpy(share_alter->tmp_tgt_dbs, share->tgt_dbs,
         sizeof(char *) * share->all_link_count);
  share_alter->tmp_tgt_hosts = share_alter->tmp_tgt_dbs + share->all_link_count;
  ...
  memcpy(share_alter->tmp_server_names_lengths, share->server_names_lengths,
  ...
  memcpy(share_alter->tmp_tgt_table_names_lengths,
         share->tgt_table_names_lengths, sizeof(uint) * share->all_link_count);
  ...
  memcpy(share_alter->tmp_tgt_usernames_lengths, share->tgt_usernames_lengths,
         sizeof(uint) * share->all_link_count);
  share_alter->tmp_tgt_passwords_lengths =
      share_alter->tmp_tgt_usernames_lengths + share->all_link_count;
  memcpy(share_alter->tmp_tgt_passwords_lengths, share->tgt_passwords_lengths,
         sizeof(uint) * share->all_link_count);
  ...
  memcpy(share_alter->tmp_tgt_wrappers_lengths, share->tgt_wrappers_lengths,
         sizeof(uint) * share->all_link_count);
  share_alter->tmp_tgt_ssl_cas_lengths =
      share_alter->tmp_tgt_wrappers_lengths + share->all_link_count;
  memcpy(share_alter->tmp_tgt_ssl_cas_lengths, share->tgt_ssl_cas_lengths,
         sizeof(uint) * share->all_link_count);
  ...
  memcpy(share_alter->tmp_tgt_ssl_capaths_lengths,
         share->tgt_ssl_capaths_lengths, sizeof(uint) * share->all_link_count);
  share_alter->tmp_tgt_ssl_certs_lengths =
      share_alter->tmp_tgt_ssl_capaths_lengths + share->all_link_count;
  memcpy(share_alter->tmp_tgt_ssl_certs_lengths, share->tgt_ssl_certs_lengths,
         sizeof(uint) * share->all_link_count);
  ...
  memcpy(share_alter->tmp_tgt_ssl_ciphers_lengths,
         share->tgt_ssl_ciphers_lengths, sizeof(uint) * share->all_link_count);
  share_alter->tmp_tgt_ssl_keys_lengths =
      share_alter->tmp_tgt_ssl_ciphers_lengths + share->all_link_count;
  memcpy(share_alter->tmp_tgt_ssl_keys_lengths, share->tgt_ssl_keys_lengths,
         sizeof(uint) * share->all_link_count);
  ...
  memcpy(share_alter->tmp_static_link_ids_lengths,
         share->static_link_ids_lengths, sizeof(uint) * share->all_link_count);
  ...
  share_alter->tmp_tgt_default_files_charlen = share->tgt_default_files_charlen;
  share_alter->tmp_tgt_default_groups_charlen =
      share->tgt_default_groups_charlen;
  share_alter->tmp_static_link_ids_charlen = share->static_link_ids_charlen;
  ...
  share_alter->tmp_tgt_default_groups_length = share->tgt_default_groups_length;
  share_alter->tmp_static_link_ids_length = share->static_link_ids_length;
  ...
  if ((error_num = spider_set_connect_info_default(share,
#ifdef WITH_PARTITION_STORAGE_ENGINE
                                                   part_elem, sub_elem,
#endif
                                                   table_share)))
  ...
  if (create_table) {
    for (roop_count = 0; roop_count < (int)share->all_link_count;
      ...
      for (roop_count2 = 0; roop_count2 < SPIDER_DBTON_SIZE; roop_count2++) {
        if (spider_dbton[roop_count2].wrapper &&
      ...
      DBUG_PRINT("info", ("spider server_names_lengths[%d] = %u", roop_count,
                          share->server_names_lengths[roop_count]));
      if (share->server_names_lengths[roop_count] >
      ...
      DBUG_PRINT("info", ("spider tgt_table_names_lengths[%d] = %u", roop_count,
      ...
      DBUG_PRINT("info", ("spider tgt_dbs_lengths[%d] = %u", roop_count,
      ...
      DBUG_PRINT("info", ("spider tgt_hosts_lengths[%d] = %u", roop_count,
      ...
      if (share->tgt_config_table_lengths &&
          share->tgt_config_table_lengths[roop_count] >
              SPIDER_CONNECT_INFO_MAX_LEN) {
        error_num = ER_SPIDER_INVALID_CONNECT_INFO_TOO_LONG_NUM;
        my_printf_error(error_num, ER_SPIDER_INVALID_CONNECT_INFO_TOO_LONG_STR,
                        MYF(0), share->tgt_config_table[roop_count],
                        "config_table");
        goto error;
      }

      DBUG_PRINT("info", ("spider tgt_usernames_lengths[%d] = %u", roop_count,
      ...
      DBUG_PRINT("info", ("spider tgt_passwords_lengths[%d] = %u", roop_count,
      ...
      DBUG_PRINT("info", ("spider tgt_sockets_lengths[%d] = %u", roop_count,
      ...
      DBUG_PRINT("info", ("spider tgt_wrappers_lengths[%d] = %u", roop_count,
      ...
      DBUG_PRINT("info", ("spider tgt_ssl_cas_lengths[%d] = %u", roop_count,
      ...
      DBUG_PRINT("info", ("spider tgt_ssl_capaths_lengths[%d] = %u", roop_count,
                          share->tgt_ssl_capaths_lengths[roop_count]));
      if (share->tgt_ssl_capaths_lengths[roop_count] >
          SPIDER_CONNECT_INFO_PATH_MAX_LEN) {
        error_num = ER_SPIDER_INVALID_CONNECT_INFO_TOO_LONG_NUM;
        my_printf_error(error_num, ER_SPIDER_INVALID_CONNECT_INFO_TOO_LONG_STR,
                        MYF(0), share->tgt_ssl_capaths[roop_count],
                        "ssl_capath");
        goto error;
      }

      DBUG_PRINT("info", ("spider tgt_ssl_certs_lengths[%d] = %u", roop_count,
      ...
      DBUG_PRINT("info", ("spider tgt_ssl_ciphers_lengths[%d] = %u", roop_count,
                          share->tgt_ssl_ciphers_lengths[roop_count]));
      if (share->tgt_ssl_ciphers_lengths[roop_count] >
          SPIDER_CONNECT_INFO_MAX_LEN) {
        error_num = ER_SPIDER_INVALID_CONNECT_INFO_TOO_LONG_NUM;
        my_printf_error(error_num, ER_SPIDER_INVALID_CONNECT_INFO_TOO_LONG_STR,
                        MYF(0), share->tgt_ssl_ciphers[roop_count],
                        "ssl_cipher");
        goto error;
      }

      DBUG_PRINT("info", ("spider tgt_ssl_keys_lengths[%d] = %u", roop_count,
      ...
      if (share->tgt_default_files_lengths[roop_count] >
        ...
        error_num = ER_SPIDER_INVALID_CONNECT_INFO_TOO_LONG_NUM;
        my_printf_error(error_num, ER_SPIDER_INVALID_CONNECT_INFO_TOO_LONG_STR,
                        MYF(0), share->tgt_default_files[roop_count],
                        "default_file");
        goto error;
      }
      ...
      if (share->tgt_default_groups_lengths[roop_count] >
        ...
        error_num = ER_SPIDER_INVALID_CONNECT_INFO_TOO_LONG_NUM;
        my_printf_error(error_num, ER_SPIDER_INVALID_CONNECT_INFO_TOO_LONG_STR,
                        MYF(0), share->tgt_default_groups[roop_count],
                        "default_group");
        goto error;
      }

      DBUG_PRINT("info", ("spider tgt_pk_names_lengths[%d] = %u", roop_count,
      ...
      if (share->tgt_sequence_names_lengths[roop_count] >
        ...
        error_num = ER_SPIDER_INVALID_CONNECT_INFO_TOO_LONG_NUM;
        my_printf_error(error_num, ER_SPIDER_INVALID_CONNECT_INFO_TOO_LONG_STR,
                        MYF(0), share->tgt_sequence_names[roop_count],
                        "sequence_name");
        goto error;
      }

      DBUG_PRINT("info", ("spider static_link_ids_lengths[%d] = %u", roop_count,
      ...
      if (share->static_link_ids[roop_count]) {
        if (share->static_link_ids_lengths[roop_count] > 0 &&
          ...
          my_printf_error(
              error_num, ER_SPIDER_INVALID_CONNECT_INFO_START_WITH_NUM_STR,
              MYF(0), share->static_link_ids[roop_count], "static_link_id");
          goto error;
        }
        for (roop_count2 = roop_count + 1;
             roop_count2 < (int)share->all_link_count; roop_count2++) {
          if (share->static_link_ids_lengths[roop_count] ==
            ...
            my_printf_error(error_num, ER_SPIDER_INVALID_CONNECT_INFO_SAME_STR,
                            MYF(0), share->static_link_ids[roop_count],
                            "static_link_id");
            goto error;
          }
...
int spider_set_connect_info_default(SPIDER_SHARE *share,
  ...
  for (roop_count = 0; roop_count < (int)share->all_link_count; roop_count++) {
    ...
    if (!share->tgt_wrappers[roop_count]) {
      ...
      if (!(share->tgt_wrappers[roop_count] = spider_create_string(
    ...
    if (!share->tgt_hosts[roop_count]) {
      ...
      if (!(share->tgt_hosts[roop_count] = spider_create_string(
                my_localhost, share->tgt_hosts_lengths[roop_count]))) {
        DBUG_RETURN(HA_ERR_OUT_OF_MEM);
      }
    }
    ...
    if (!share->tgt_dbs[roop_count] && table_share) {
      ...
      if (!(share->tgt_dbs[roop_count] = spider_create_string(
    ...
    if (!share->tgt_table_names[roop_count] && table_share) {
      ...
      if (!(share->tgt_table_names[roop_count] =
                spider_create_table_name_string(
...
//                    (sub_elem ? sub_elem->partition_name : NULL)
      ...
      // set the length of the table_name not ./db_name/table_name
      share->tgt_table_names_lengths[roop_count] = strlen(share->tgt_table_names[roop_count]);
    }
    ...
    if (!share->tgt_default_files[roop_count] &&
      ...
      if (*spd_defaults_extra_file) {
        ...
        if (!(share->tgt_default_files[roop_count] = spider_create_string(
    ...
    if (!share->tgt_pk_names[roop_count]) {
      ...
      if (!(share->tgt_pk_names[roop_count] = spider_create_string(
    ...
    if (!share->tgt_sequence_names[roop_count]) {
      ...
      if (!(share->tgt_sequence_names[roop_count] = spider_create_string(
    ...
    */
    ...
    if (!share->tgt_sockets[roop_count] &&
      ...
      share->tgt_sockets_lengths[roop_count] = strlen(const_cast<char *>(MYSQL_UNIX_ADDR));
      if (!(share->tgt_sockets[roop_count] =
                spider_create_string(const_cast<char *>(MYSQL_UNIX_ADDR),
                                     share->tgt_sockets_lengths[roop_count]))) {
        DBUG_RETURN(HA_ERR_OUT_OF_MEM);
      }
    }
  ...
  if (share->sts_bg_mode == -1) share->sts_bg_mode = 2;
  if (share->sts_interval == -1) share->sts_interval = 3600;
  if (share->sts_mode == -1) share->sts_mode = 1;
#ifdef WITH_PARTITION_STORAGE_ENGINE
  if (share->sts_sync == -1) share->sts_sync = 0;
#endif
  if (share->store_last_sts == -1) share->store_last_sts = 1;
  if (share->load_sts_at_startup == -1) share->load_sts_at_startup = 1;
  if (share->crd_bg_mode == -1) share->crd_bg_mode = 0;
  if (share->crd_interval == -1) share->crd_interval = 51;
  if (share->crd_mode == -1) share->crd_mode = 1;
#ifdef WITH_PARTITION_STORAGE_ENGINE
  if (share->crd_sync == -1) share->crd_sync = 0;
#endif
  if (share->store_last_crd == -1) share->store_last_crd = 1;
  if (share->load_crd_at_startup == -1) share->load_crd_at_startup = 1;
  if (share->crd_type == -1) share->crd_type = 2;
  ...
  if (share->max_order == -1) share->max_order = 32767;
  if (share->semi_table_lock == -1) share->semi_table_lock = 0;
  if (share->semi_table_lock_conn == -1) share->semi_table_lock_conn = 1;
  if (share->selupd_lock_mode == -1) share->selupd_lock_mode = 1;
  if (share->query_cache == -1) share->query_cache = 0;
  if (share->query_cache_sync == -1) share->query_cache_sync = 0;
  if (share->internal_delayed == -1) share->internal_delayed = 0;
  if (share->bulk_size == -1) share->bulk_size = 16000;
  if (share->bulk_update_mode == -1) share->bulk_update_mode = 0;
  if (share->bulk_update_size == -1) share->bulk_update_size = 16000;
  if (share->internal_optimize == -1) share->internal_optimize = 0;
  if (share->internal_optimize_local == -1) share->internal_optimize_local = 0;
  if (share->scan_rate == -1) share->scan_rate = 1;
  if (share->read_rate == -1) share->read_rate = 0.0002;
  if (share->priority == -1) share->priority = 1000000;
  if (share->quick_mode == -1) share->quick_mode = 1;
  if (share->quick_page_size == -1) share->quick_page_size = 100;
  if (share->low_mem_read == -1) share->low_mem_read = 1;
  if (share->table_count_mode == -1) share->table_count_mode = 0;
  if (share->select_column_mode == -1) share->select_column_mode = 1;
  if (share->bgs_mode == -1) share->bgs_mode = 0;
  if (share->bgs_first_read == -1) share->bgs_first_read = 2;
  if (share->bgs_second_read == -1) share->bgs_second_read = 100;
  if (share->first_read == -1) share->first_read = 0;
  if (share->second_read == -1) share->second_read = 0;
  if (share->auto_increment_mode == -1) share->auto_increment_mode = 0;
  if (share->use_table_charset == -1) share->use_table_charset = 1;
  if (share->use_pushdown_udf == -1) share->use_pushdown_udf = 1;
  if (share->skip_default_condition == -1) share->skip_default_condition = 0;
...
#ifdef HA_CAN_BULK_ACCESS
  if (share->bulk_access_free == -1) share->bulk_access_free = 0;
#endif
#ifdef HA_CAN_FORCE_BULK_UPDATE
  if (share->force_bulk_update == -1) share->force_bulk_update = 0;
#endif
#ifdef HA_CAN_FORCE_BULK_DELETE
  if (share->force_bulk_delete == -1) share->force_bulk_delete = 0;
#endif
  if (share->casual_read == -1) share->casual_read = 0;
  ...
  if (share->bka_mode == -1) share->bka_mode = 1;
  if (!share->bka_engine) {
    DBUG_PRINT("info", ("spider create default bka_engine"));
    share->bka_engine_length = SPIDER_SQL_TMP_BKA_ENGINE_LEN;
    if (!(share->bka_engine = spider_create_string(
              SPIDER_SQL_TMP_BKA_ENGINE_STR, SPIDER_SQL_TMP_BKA_ENGINE_LEN))) {
      DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    }
  }
  DBUG_RETURN(0);
...
int spider_set_connect_info_default_db_table(SPIDER_SHARE *share,
  ...
  for (roop_count = 0; roop_count < (int)share->link_count; roop_count++) {
    if (!share->tgt_dbs[roop_count] && db_name) {
      ...
      if (!(share->tgt_dbs[roop_count] =
                spider_create_string(db_name, db_name_length))) {
        DBUG_RETURN(HA_ERR_OUT_OF_MEM);
      }
    }
    ...
    if (!share->tgt_table_names[roop_count] && table_name) {
      ...
      if (!(share->tgt_table_names[roop_count] =
                spider_create_string(table_name, table_name_length))) {
        DBUG_RETURN(HA_ERR_OUT_OF_MEM);
      }
    }
  }
...
int spider_set_connect_info_default_dbtable(SPIDER_SHARE *share,
  ...
  DBUG_PRINT("info",
             ("spider ptr_diff_table = %lld", (longlong)ptr_diff_table));
  DBUG_RETURN(spider_set_connect_info_default_db_table(
      share, ptr_db, (uint)(ptr_diff_table - 1), ptr_table,
...
void spider_print_keys(const char *key, uint length) {
  ...
  char *tmp = (char *)my_malloc(PSI_NOT_INSTRUMENTED, length + 1, MY_ZEROFILL | MY_WME);
...
int spider_create_conn_keys(SPIDER_SHARE *share) {
  ...
  for (roop_count = 0; roop_count < (int)share->all_link_count; roop_count++) {
    /* tgt_db not use */
    conn_keys_lengths[roop_count] = share->server_names_lengths[roop_count];
    share->conn_keys_charlen += conn_keys_lengths[roop_count];
  }
  if (!(share->conn_keys = (char **)spider_bulk_alloc_mem(
            spider_current_trx, 45, __func__, __FILE__, __LINE__,
            MYF(MY_WME | MY_ZEROFILL), &share->conn_keys,
            sizeof(char *) * share->all_link_count, &share->conn_keys_lengths,
            sizeof(uint) * share->all_link_count,
  ...
  for (roop_count = 0; roop_count < (int)share->all_link_count; roop_count++) {
    ...
    if (share->server_names[roop_count])
      tmp_name = strmov(tmp_name, share->server_names[roop_count]);
    ...
    for (roop_count2 = 0; roop_count2 < SPIDER_DBTON_SIZE; roop_count2++) {
      ...
      if (spider_dbton[roop_count2].wrapper &&
        ...
        if (!get_sql_id && spider_dbton[roop_count2].db_access_type ==
    ...
    if (!get_sql_id) share->sql_dbton_ids[roop_count] = SPIDER_DBTON_SIZE;
  }
  for (roop_count2 = 0; roop_count2 < SPIDER_DBTON_SIZE; roop_count2++) {
...
int spider_update_conn_keys(SPIDER_SHARE *share, int link_idx) {
  ulong conn_key_version;
  DBUG_ENTER("spider_update_conn_keys");

  ulong server_version =
      get_server_version_by_name(share->server_names[link_idx]);
  pthread_mutex_lock(&spider_tbl_mutex);
  conn_key_version = share->conn_key_version;
  if (server_version !=
      conn_key_version) { /* update share conn key with version */
    share->conn_key_version = server_version;
    LogCommInfo(
        "set info change, old set version: %lu, new set version: %lu, conn key: "
        "%s, conn hash key: "
        "%u",
        conn_key_version, server_version,
        std::string(share->conn_keys[link_idx],
                    share->conn_keys_lengths[link_idx])
            .c_str(),
        share->conn_keys_hash_value[link_idx]);
  } else {
    LogCommInfo(
        "set info not change, set version: %lu, conn key: %s, conn hash key: "
        "%u",
        conn_key_version,
        std::string(share->conn_keys[link_idx],
                    share->conn_keys_lengths[link_idx])
...
SPIDER_SHARE *spider_create_share(const char *table_name,
  ...
  uint length, length_with_version;
  int use_table_charset;
  SPIDER_SHARE *share;
  char *tmp_name;
  char *tmp_name_with_version;
  ...
  length_with_version = (uint)strlen(table_name_with_version);
  bitmap_size = spider_bitmap_size(table_share->fields);
  if (!(share = (SPIDER_SHARE *)spider_bulk_malloc(
            spider_current_trx, 46, MYF(MY_WME | MY_ZEROFILL), &share,
            sizeof(*share), &tmp_name, length + 1, &tmp_name_with_version,
            length_with_version + 1, &tmp_static_key_cardinality,
            sizeof(*tmp_static_key_cardinality) * table_share->keys,
            ...
            sizeof(*tmp_table_mon_mutex_bitmap) *
  ...
  share->table_name_with_version_length = length_with_version;
  share->table_name_with_version = tmp_name_with_version;
  strmov(share->table_name_with_version, table_name_with_version);
  ...
  share->modify_server_version = 0;
  share->conn_key_version = 0;
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
  share->table_name_hash_value =
      my_calc_hash(&spider_open_tables, (uchar *)share->table_name,
                   share->table_name_length);
#ifdef WITH_PARTITION_STORAGE_ENGINE
  share->table_path_hash_value =
      my_calc_hash(&spider_open_tables, (uchar *)table_share->path.str,
  ...
  use_table_charset = spider_param_use_table_charset(share->use_table_charset);
  if (table_share->table_charset && use_table_charset)
    share->access_charset = const_cast<CHARSET_INFO*>(table_share->table_charset);
  else
  ...
  if (share->table_count_mode & 2)
    share->additional_table_flags |= HA_COUNT_ROWS_INSTANT;


  if (mysql_mutex_init(spd_key_mutex_share, &share->mutex, MY_MUTEX_INIT_FAST))
  ...
  if (mysql_mutex_init(spd_key_mutex_share_sts, &share->sts_mutex,
                       MY_MUTEX_INIT_FAST))
  ...
  if (mysql_mutex_init(spd_key_mutex_share_crd, &share->crd_mutex,
                       MY_MUTEX_INIT_FAST))
...
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
  if (!(share->lgtm_tblhnd_share = spider_get_lgtm_tblhnd_share(
            tmp_name, length, hash_value, FALSE, TRUE, error_num)))
#else
  if (!(share->lgtm_tblhnd_share = spider_get_lgtm_tblhnd_share(
            tmp_name, length, FALSE, TRUE, error_num)))
#endif
  {
...
SPIDER_SHARE *spider_get_share(const char *table_name, TABLE *table, THD *thd,
  ...
  char *tmp_cid;
  int roop_count;
  char first_byte __attribute__((unused));
  ...
  char table_name_with_version[FN_REFLEN];
  char ts_version[40];
  long long table_share_version_tmp = thd->flush_no_block_version;
  DBUG_ENTER("spider_get_share");

  if (table_share_version_tmp == 0) {
    strmov(table_name_with_version, table_name);
  } else {
    ullstr(table_share_version_tmp, ts_version);
    strmov(strmov(strmov(table_name_with_version, table_name), "#"),
           ts_version);
  }
  length = (uint)strlen(table_name_with_version);
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
  my_hash_value_type hash_value = my_calc_hash(
      &spider_open_tables, (uchar *)table_name_with_version, length);
#endif
  pthread_mutex_lock(&spider_tbl_mutex);
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
  if (!(share = (SPIDER_SHARE *)my_hash_search_using_hash_value(
            &spider_open_tables, hash_value, (uchar *)table_name_with_version,
            length)))
#else
  if (!(share = (SPIDER_SHARE *)my_hash_search(
            &spider_open_tables, (uchar *)table_name_with_version, length)))
#endif
  {
    if (!(share = spider_create_share(table_name, table_name_with_version,
                                      table_share,
#ifdef WITH_PARTITION_STORAGE_ENGINE
                                      table->part_info,
#endif
    ...
    if (spider_open_tables.array.max_element > old_elements) {
      spider_alloc_calc_mem(
          spider_current_trx, spider_open_tables,
    ...
    if (!share->link_status_init) {
      ...
      if (!share->link_status_init) {
        if ((table_share->tmp_table == NO_TMP_TABLE &&
          ...
          init_mem_root = TRUE;
          if (!(table_tables = spider_open_sys_table(
          ...
          if (*error_num) {
            if (*error_num != HA_ERR_KEY_NOT_FOUND &&
              ...
              for (roop_count = 0;
                   roop_count < (int)spider_param_udf_table_mon_mutex_count();
                   roop_count++) {
                if (spider_bit_is_set(share->table_mon_mutex_bitmap,
                                      roop_count))
                  pthread_mutex_unlock(
                      &spider_udf_table_mon_mutexes[roop_count]);
              }
              pthread_mutex_unlock(&share->mutex);
              share->init_error = TRUE;
              share->init_error_time = (time_t)time((time_t *)0);
        ...
        if (table_tables) {
          spider_close_sys_table(thd, table_tables, &open_tables_backup, FALSE);
          table_tables = NULL;
        }
        if (init_mem_root) {
    ...
    semi_table_lock_conn = spider_param_semi_table_lock_connection(
        thd, share->semi_table_lock_conn);
    if (semi_table_lock_conn)
      first_byte =
          '0' + spider_param_semi_table_lock(thd, share->semi_table_lock);
    else
      first_byte = '0';
    ...
    if (!(spider->conns = (SPIDER_CONN **)spider_bulk_alloc_mem(
              spider_current_trx, 47, __func__, __FILE__, __LINE__,
              MYF(MY_WME | MY_ZEROFILL), &spider->conns,
              sizeof(SPIDER_CONN *) * share->link_count, &spider->conn_link_idx,
              sizeof(uint) * share->link_count, &spider->conn_can_fo,
              sizeof(uchar) * share->link_bitmap_size, &spider->sql_kind,
              sizeof(uint) * share->link_count, &spider->connection_ids,
              sizeof(ulonglong) * share->link_count, &spider->conn_kind,
              sizeof(uint) * share->link_count, &spider->db_request_id,
              sizeof(ulonglong) * share->link_count, &spider->db_request_phase,
              sizeof(uchar) * share->link_bitmap_size,
              &spider->m_handler_opened,
              sizeof(uchar) * share->link_bitmap_size, &spider->m_handler_id,
              sizeof(uint) * share->link_count, &spider->m_handler_cid,
              sizeof(char *) * share->link_count, &tmp_cid,
              sizeof(char) * (SPIDER_SQL_HANDLER_CID_LEN + 1) *
                  share->link_count,
              ...
              sizeof(Temp_table_param) * share->link_count,
              ...
              sizeof(spider_db_handler *) * SPIDER_DBTON_SIZE, NullS))) {
      share->init_error = TRUE;
      share->init_error_time = (time_t)time((time_t *)0);
      share->init = TRUE;
      spider_free_share(share);
    ...
    for (roop_count = 0; roop_count < (int)share->link_count; roop_count++) {
      ...
      result_list->upd_tmp_tbl_prms[roop_count].func_count = 1;
    ...
    for (roop_count = 0; roop_count < (int)share->use_dbton_count;
      ...
      if (!(spider->dbton_handler[dbton_id] =
                spider_dbton[dbton_id].create_db_handler(
    ...
    search_link_idx = spider_conn_first_link_idx(
    ...
    if (search_link_idx == -1) {
      char *db = (char *)my_alloca(table_share->db.length + 1 +
    ...
    if (sql_command != SQLCOM_DROP_TABLE && sql_command != SQLCOM_ALTER_TABLE &&
        sql_command != SQLCOM_SHOW_CREATE && !spider->error_mode &&
        (!same_server_link)) {
    ...
    if (!(spider->conns = (SPIDER_CONN **)spider_bulk_alloc_mem(
              spider_current_trx, 49, __func__, __FILE__, __LINE__,
              MYF(MY_WME | MY_ZEROFILL), &spider->conns,
              sizeof(SPIDER_CONN *) * share->link_count, &spider->conn_link_idx,
              sizeof(uint) * share->link_count, &spider->conn_can_fo,
              sizeof(uchar) * share->link_bitmap_size, &spider->sql_kind,
    ...
    if (sql_command != SQLCOM_DROP_TABLE && sql_command != SQLCOM_ALTER_TABLE &&
      ...
      for (roop_count = spider_conn_link_idx_next(
        ...
        if (!spider_param_get_conn_from_idx(thd)) {
          if (!(spider->conns[roop_count] = spider_get_conn(
                    share, roop_count, spider->trx, spider, FALSE, TRUE,
                    SPIDER_CONN_KIND_MYSQL, error_num))) {
            if (share->monitoring_kind[roop_count] &&
                spider->need_mons[roop_count]) {
              *error_num = spider_ping_table_mon_from_table(
                  spider->trx, spider->trx->thd, share, roop_count,
                  (uint32)share->monitoring_sid[roop_count], share->table_name,
                  share->table_name_length, spider->conn_link_idx[roop_count],
                  NULL, 0, share->monitoring_kind[roop_count],
                  share->monitoring_limit[roop_count],
                  share->monitoring_flag[roop_count], FALSE);
            }
            spider_free_share(share);
            goto error_but_no_delete;
          }
...
int spider_free_share(SPIDER_SHARE *share) {
  ...
  if (do_delete_thd) spider_destroy_thd(thd);
  pthread_mutex_unlock(&spider_tbl_mutex);
  DBUG_RETURN(0);
}
...
void spider_update_link_status_for_share(const char *table_name,
                                         ...
                                         long link_status) {
  SPIDER_SHARE *share;
  char* ptable_name = const_cast<char*>(table_name);
  DBUG_ENTER("spider_update_link_status_for_share");

#ifdef SPIDER_HAS_HASH_VALUE_TYPE
  my_hash_value_type hash_value =
      my_calc_hash(&spider_open_tables, reinterpret_cast<uchar *>(ptable_name), table_name_length);
#endif
  pthread_mutex_lock(&spider_tbl_mutex);
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
  if ((share = (SPIDER_SHARE *)my_hash_search_using_hash_value(
           &spider_open_tables, hash_value, reinterpret_cast<uchar *>(ptable_name),
           table_name_length)))
#else
  if ((share = (SPIDER_SHARE *)my_hash_search(
           &spider_open_tables, reinterpret_cast<uchar *>(ptable_name), table_name_length)))
#endif
  {
    DBUG_PRINT("info", ("spider share->link_status_init=%s",
                        share->link_status_init ? "TRUE" : "FALSE"));
    if (share->link_status_init) {
      DBUG_PRINT("info", ("spider share->link_statuses[%d]=%ld", link_idx,
                          link_status));
...
SPIDER_LGTM_TBLHND_SHARE *spider_get_lgtm_tblhnd_share(
    ...
    my_hash_value_type hash_value, bool locked, bool need_to_create __attribute__((unused)),
    int *error_num)
#else
...
#endif
{
  ...
  if (!(lgtm_tblhnd_share =
            (SPIDER_LGTM_TBLHND_SHARE *)my_hash_search_using_hash_value(
                &spider_lgtm_tblhnd_share_hash, hash_value, reinterpret_cast<uchar *>(ptable_name),
                table_name_length)))
#else
  if (!(lgtm_tblhnd_share = (SPIDER_LGTM_TBLHND_SHARE *)my_hash_search(
            &spider_lgtm_tblhnd_share_hash, reinterpret_cast<uchar *>(ptable_name),
            table_name_length)))
#endif
  {
    DBUG_PRINT("info", ("spider create new lgtm tblhnd share"));
    if (!(lgtm_tblhnd_share = (SPIDER_LGTM_TBLHND_SHARE *)spider_bulk_malloc(
              spider_current_trx, 244, MYF(MY_WME | MY_ZEROFILL),
              &lgtm_tblhnd_share, sizeof(*lgtm_tblhnd_share), &tmp_name,
              table_name_length + 1, NullS))) {
      *error_num = HA_ERR_OUT_OF_MEM;
      goto error_alloc_share;
    }
    ...
    if (mysql_mutex_init(spd_key_mutex_share_auto_increment,
                         &lgtm_tblhnd_share->auto_increment_mutex,
                         MY_MUTEX_INIT_FAST))
...
#ifdef WITH_PARTITION_STORAGE_ENGINE
SPIDER_PARTITION_SHARE *spider_get_pt_share(SPIDER_SHARE *share,
                                            TABLE_SHARE *table_share,
                                            int *error_num) {
...
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
  if (!(partition_share =
            (SPIDER_PARTITION_SHARE *)my_hash_search_using_hash_value(
  ...
  if (!(partition_share = (SPIDER_PARTITION_SHARE *)my_hash_search(
            &spider_open_pt_share, (uchar *)table_share->path.str,
            table_share->path.length)))
#endif
  {
    DBUG_PRINT("info", ("spider create new pt share"));
    if (!(partition_share = (SPIDER_PARTITION_SHARE *)spider_bulk_malloc(
              spider_current_trx, 51, MYF(MY_WME | MY_ZEROFILL),
              &partition_share, sizeof(*partition_share), &tmp_name,
              table_share->path.length + 1, &tmp_cardinality,
              sizeof(*tmp_cardinality) * table_share->fields, NullS))) {
      *error_num = HA_ERR_OUT_OF_MEM;
      goto error_alloc_share;
    }
    ...
    if (mysql_mutex_init(spd_key_mutex_pt_handler,
                         &partition_share->pt_handler_mutex,
                         MY_MUTEX_INIT_FAST))
    ...
    if (my_hash_init(&partition_share->pt_handler_hash, spd_charset_utf8_bin,
                     32, 0, 0, (my_hash_get_key)spider_pt_handler_share_get_key,
                     0, 0)) {
      *error_num = HA_ERR_OUT_OF_MEM;
    ...
    spider_alloc_calc_mem(
        spider_current_trx, partition_share->pt_handler_hash,
    ...
    if (spider_open_pt_share.array.max_element > old_elements) {
      spider_alloc_calc_mem(
          spider_current_trx, spider_open_pt_share,
...
error_hash_insert:
  spider_free_mem_calc(
      spider_current_trx, partition_share->pt_handler_hash_id,
      partition_share->pt_handler_hash.array.max_element *
          partition_share->pt_handler_hash.array.size_of_element);
...
int spider_free_pt_share(SPIDER_PARTITION_SHARE *partition_share) {
...
void spider_copy_sts_to_pt_share(SPIDER_PARTITION_SHARE *partition_share,
                                 SPIDER_SHARE *share) {
  DBUG_ENTER("spider_copy_sts_to_pt_share");
  memcpy(&partition_share->data_file_length, &share->data_file_length,
         sizeof(ulonglong) * 4 + sizeof(ha_rows) + sizeof(ulong) +
             sizeof(time_t) * 3);
  /*
...
void spider_copy_crd_to_pt_share(SPIDER_PARTITION_SHARE *partition_share,
...
int spider_open_all_tables(SPIDER_TRX *trx, bool lock) {
  ...
  if (!(table_tables =
            spider_open_sys_table(thd, SPIDER_SYS_TABLES_TABLE_NAME_STR,
                                  SPIDER_SYS_TABLES_TABLE_NAME_LEN, TRUE,
                                  &open_tables_backup, TRUE, &error_num)))
    DBUG_RETURN(error_num);
  if ((error_num = spider_sys_index_first(table_tables, 1))) {
    if (error_num != HA_ERR_KEY_NOT_FOUND && error_num != HA_ERR_END_OF_FILE) {
      table_tables->file->print_error(error_num, MYF(0));
      spider_close_sys_table(thd, table_tables, &open_tables_backup, TRUE);
      DBUG_RETURN(error_num);
    } else {
      spider_close_sys_table(thd, table_tables, &open_tables_backup, TRUE);
      DBUG_RETURN(0);
  ...
  memset((void*)&tmp_connect_info, 0,
  ...
  do {
    if ((error_num = spider_get_sys_tables(table_tables, &db_name, &table_name,
                                           &mem_root)) ||
        (error_num = spider_get_sys_tables_connect_info(
             table_tables, &tmp_share, 0, &mem_root)) ||
        (error_num = spider_set_connect_info_default(&tmp_share,
#ifdef WITH_PARTITION_STORAGE_ENGINE
                                                     NULL, NULL,
#endif
                                                     NULL))) {
    ...
    if ((error_num = spider_set_connect_info_default_db_table(
             &tmp_share, db_name, db_name_length, table_name,
             table_name_length)) ||
    ...
    /* create conn */
    if (!(conn = spider_get_conn(&tmp_share, 0, trx, NULL, FALSE, FALSE,
                                 SPIDER_CONN_KIND_MYSQL, &error_num))) {
      spider_sys_index_end(table_tables);
      spider_close_sys_table(thd, table_tables, &open_tables_backup, TRUE);
    ...
    if (lock && spider_param_use_snapshot_with_flush_tables(thd) == 2) {
      ...
      if (!(share = (SPIDER_SHARE *)spider_bulk_malloc(
                spider_current_trx, 52, MYF(MY_WME | MY_ZEROFILL), &share,
                sizeof(*share), &connect_info,
                sizeof(char *) * SPIDER_TMP_SHARE_CHAR_PTR_COUNT,
                &connect_info_length,
                sizeof(uint) * SPIDER_TMP_SHARE_UINT_COUNT, &long_info,
                sizeof(long) * SPIDER_TMP_SHARE_LONG_COUNT, &longlong_info,
                sizeof(longlong) * SPIDER_TMP_SHARE_LONGLONG_COUNT, &conns,
                sizeof(SPIDER_CONN *), &need_mon, sizeof(int),
                &spider->conn_link_idx, sizeof(uint), &spider->conn_can_fo,
                sizeof(uchar), NullS))) {
        delete spider;
        spider_sys_index_end(table_tables);
        spider_close_sys_table(thd, table_tables, &open_tables_backup, TRUE);
      ...
      spider_set_tmp_share_pointer(share, connect_info, connect_info_length,
                                   long_info, longlong_info);
      memcpy(connect_info, &tmp_connect_info,
             sizeof(char *) * SPIDER_TMP_SHARE_CHAR_PTR_COUNT);
      memcpy(connect_info_length, &tmp_connect_info_length,
             sizeof(uint) * SPIDER_TMP_SHARE_UINT_COUNT);
      memcpy(long_info, &tmp_long, sizeof(long) * SPIDER_TMP_SHARE_LONG_COUNT);
      memcpy(longlong_info, &tmp_longlong,
             sizeof(longlong) * SPIDER_TMP_SHARE_LONGLONG_COUNT);
      spider->share = share;
      spider->trx = trx;
      spider->conns = conns;
      ...
      /* create another conn */
      if ((!(conn = spider_get_conn(&tmp_share, 0, trx, spider, TRUE, FALSE,
                                    SPIDER_CONN_KIND_MYSQL, &error_num)))) {
      ...
      int appended = 0;
      if ((error_num =
               spider->dbton_handler[conn->dbton_id]->append_lock_tables_list(
                   conn, 0, &appended))) {
        spider_free_tmp_dbton_handler(spider);
        spider_free(trx, share, MYF(0));
        delete spider;
...
bool spider_flush_logs(handlerton *hton __attribute__((unused)), bool binlog_group_flush[[maybe_unused]]) {
  int error_num;
  THD *thd = current_thd;
  SPIDER_TRX *trx;
  ...
  if (spider_param_use_flash_logs(trx->thd) &&
...
handler *spider_create_handler(handlerton *hton, TABLE_SHARE *table,
  ...
  if (partitioned || opt_spider_auto_ddls) {
    ha_spiderpart *file = new (mem_root) ha_spiderpart(hton, table);
...
int spider_close_connection(handlerton *hton __attribute__((unused)), THD *thd) {
  int roop_count = 0;
  SPIDER_CONN *conn;
  SPIDER_TRX *trx;
  ...
  while ((
    ...
    if (conn->table_lock > 0) {
      if (!conn->trx_start) conn->disable_reconnect = FALSE;
...
/**
 * @brief to be deprecated. used by old direct ddl
 * @return  0 Suceese, or >0 error_num
 */
int spider_create_database(handlerton *hton __attribute__((unused)), char *path __attribute__((unused))) {
  DBUG_ENTER("spider_create_database");
  if (!opt_spider_auto_ddls) {
    DBUG_RETURN(0);
  }
  std::string create_database_sql = "CREATE DATABASE IF NOT EXISTS ";
  create_database_sql += path;
  std::vector<std::string> srv_names;
  get_all_server_name(srv_names);
  std::shared_ptr<tdsql::meta_data::TdSetInfo> set_info;
  tdsql::common::TdString master_host;
  uint32_t master_port = 0;
  ...
  for (auto srv : srv_names) {
    set_info = get_server_by_name_or_alias_name(srv.c_str());
    if (!set_info) {
      error_num = ER_FOREIGN_SERVER_DOESNT_EXIST;
      goto error;
    }
    master_host =
        set_info->MasterNodeInfo() ? set_info->MasterNodeInfo()->host : "";
    master_port =
        set_info->MasterNodeInfo() ? set_info->MasterNodeInfo()->port : 0;
    MYSQL db_conn;
    mysql_init(&db_conn);
    auto m = mysql_real_connect(&db_conn, master_host.c_str(),
                             set_info->User().c_str(),
                             set_info->Password().c_str(), NULL, master_port,
                             set_info->ServersSocket().empty()
                                 ? nullptr
                                 : set_info->ServersSocket().c_str(),
                             0);
    ...
    ret = mysql_real_query(&db_conn, create_database_sql.c_str(),
                           create_database_sql.length());
    log_spider_mysql_sql(&db_conn, create_database_sql.c_str(),
                         create_database_sql.length(), COM_CREATE_DB);
    if (ret) {
 ...
 * @return  0 Suceese, or >0 error_num
 */
void spider_drop_database(handlerton *hton __attribute__((unused)), char *path __attribute__((unused))) {
  DBUG_ENTER("spider_drop_database");
  if (!opt_spider_auto_ddls) DBUG_VOID_RETURN;
  std::string db_name;
  db_name = path; // path is "./database_name/", we need to substr
  db_name = db_name.substr(2,db_name.length()-3);
  std::string drop_database_sql = "DROP DATABASE IF EXISTS ";
  drop_database_sql += db_name;
  std::vector<std::string> srv_names;
  get_all_server_name(srv_names);
  std::shared_ptr<tdsql::meta_data::TdSetInfo> set_info;
  tdsql::common::TdString master_host;
  ...
  for (auto srv : srv_names) {
    ...
    MYSQL db_conn;
    mysql_init(&db_conn);
    auto m = mysql_real_connect(
        &db_conn, master_host.c_str(), set_info->User().c_str(),
        set_info->Password().c_str(), NULL, master_port,
        set_info->ServersSocket().empty() ? nullptr
                                          : set_info->ServersSocket().c_str(),
        0);
    log_spider_mysql_sql(&db_conn, "", 0, COM_CONNECT);
    if (!m) {
    ...
    ret = mysql_real_query(&db_conn, drop_database_sql.c_str(),
                           drop_database_sql.length());
    log_spider_mysql_sql(&db_conn, drop_database_sql.c_str(),
                         drop_database_sql.length(), COM_DROP_DB);
    if (ret) {
...
bool spider_show_status(handlerton *hton __attribute__((unused)), THD *thd __attribute__((unused)), stat_print_fn *stat_print __attribute__((unused)),
                        enum ha_stat_type stat_type) {
  DBUG_ENTER("spider_show_status");
  switch (stat_type) {
    case HA_ENGINE_STATUS:
      DBUG_RETURN(tdsql::rpc::ShowStatus(hton, thd, stat_print));
    default:
      DBUG_RETURN(FALSE);
  }
}

int spider_db_done(void *p __attribute__((unused))) {
  int roop_count;
  bool do_delete_thd;
  THD *thd = current_thd, *tmp_thd;
  SPIDER_CONN *conn __attribute__((unused));
  SPIDER_INIT_ERROR_TABLE *spider_init_error_table;
  ...
  if (thd)
    ...
    thd = spider_create_thd();
    if (!thd) DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    do_delete_thd = TRUE;
  }
  ...
  for (roop_count = spider_param_table_crd_thread_count() - 1; roop_count >= 0;
       roop_count--) {
    spider_free_crd_threads(&spider_table_crd_threads[roop_count]);
  }
  for (roop_count = spider_param_table_sts_thread_count() - 1; roop_count >= 0;
       roop_count--) {
  ...
  for (roop_count = spider_param_udf_table_mon_mutex_count() - 1;
    ...
    spider_free_mem_calc(
        spider_current_trx, spider_udf_table_mon_list_hash_id,
  ...
  while ((tmp_thd = (THD *)my_hash_element(&spider_allocated_thds, 0))) {
    SPIDER_TRX *trx = (SPIDER_TRX *)thd_get_ha_data(tmp_thd, spider_hton_ptr);
    if (trx) {
      assert(tmp_thd == trx->thd);
  ...
  spider_free_mem_calc(spider_current_trx, spider_mon_table_cache_id,
                       spider_mon_table_cache.max_element *
                           spider_mon_table_cache.size_of_element);
  delete_dynamic(&spider_mon_table_cache);
  spider_free_mem_calc(spider_current_trx, spider_allocated_thds_id,
...
#ifdef WITH_PARTITION_STORAGE_ENGINE
  spider_free_mem_calc(spider_current_trx, spider_open_pt_share_id,
                       spider_open_pt_share.array.max_element *
                           spider_open_pt_share.array.size_of_element);
  my_hash_free(&spider_open_pt_share);
#endif
  pthread_mutex_lock(&spider_init_error_tbl_mutex);
  while ((spider_init_error_table = (SPIDER_INIT_ERROR_TABLE *)my_hash_element(
              &spider_init_error_tables, 0))) {
  ...
  spider_free_mem_calc(spider_current_trx, spider_init_error_tables_id,
  ...
  spider_free_mem_calc(spider_current_trx, spider_open_tables_id,
  ...
  for (roop_count = 0; roop_count < SPIDER_MEM_CALC_LIST_NUM; roop_count++) {
    if (spider_alloc_func_name[roop_count])
      DBUG_PRINT("info", ("spider %d %s %s %lu %llu %lld %llu %llu %s",
                          roop_count, spider_alloc_func_name[roop_count],
...
int spider_panic(handlerton *hton __attribute__((unused)), ha_panic_function type __attribute__((unused))) {
  DBUG_ENTER("spider_panic");
  DBUG_RETURN(0);
}

static uint spider_partition_flags() { return (HA_CANNOT_PARTITION_FK); }

/**
 * @brief init spider storage plugin
 */
int spider_db_init(void *p) {
  ...
  /**
   * TDSQL v2.5 should make log_bin disabled.
   * The TABLE's write_set and read_set might be changed if log_bin
   * enabled in mark_columns_per_binlog_row_image, which make some 
   * mistakes in partition_key_modified().
   * FQS depends on partition_key_modified()
   */
  spider_hton->flags = HTON_NO_BINLOG_ROW_OPT;
  ...
  */
  ...
  spd_db_att_thread_id =
      (ulong *)GetProcAddress(current_module, "?thread_id@@3KA");
#endif
#ifdef SPIDER_XID_USES_xid_cache_iterate
#else
#ifdef XID_CACHE_IS_SPLITTED
  spd_db_att_xid_cache_split_num =
      (uint *)GetProcAddress(current_module, "?opt_xid_cache_split_num@@3IA");
...
#endif
#endif
  spd_charset_utf8_bin = (struct charset_info_st *)GetProcAddress(
      current_module, "my_charset_utf8_bin");
  spd_defaults_extra_file =
      (const char **)GetProcAddress(current_module, "my_defaults_extra_file");
  spd_defaults_file =
      (const char **)GetProcAddress(current_module, "my_defaults_file");
  spd_abort_loop =
      (bool volatile *)GetProcAddress(current_module, "?abort_loop@@3_NC");
  spd_tz_system =
      *(Time_zone **)
#ifdef _WIN64
  ...
  spd_db_att_thread_id = (ulong*)current_thd->thread_id_ptr(); 
#endif
#ifdef SPIDER_XID_USES_xid_cache_iterate
#else
#ifdef XID_CACHE_IS_SPLITTED
...
// spd_db_att_xid_cache = &xid_cache;
  ...
  spd_abort_loop = &connection_events_loop_aborted_flag;
  ...
  */
  ...
  if (mysql_mutex_init(spd_key_mutex_tbl, &spider_tbl_mutex,
                       MY_MUTEX_INIT_FAST))
  ...
  if (mysql_mutex_init(spd_key_thread_id, &spider_thread_id_mutex,
                       MY_MUTEX_INIT_FAST))
...
// #else
//   if (mysql_mutex_init(spd_key_conn_id, &spider_conn_id_mutex,
//                        MY_MUTEX_INIT_FAST))
// #endif
//   {
//     error_num = HA_ERR_OUT_OF_MEM;
...
//   }
  ...
  if (mysql_mutex_init(spd_key_mutex_xid_cache, &spd_db_att_xid_cache_mutex,
                       MY_MUTEX_INIT_FAST))
  ...
  if (mysql_mutex_init(spd_key_mutex_xid, &spider_xid_mutex,
                       MY_MUTEX_INIT_FAST))
  ...
  if (mysql_mutex_init(spd_key_mutex_pt_share, &spider_pt_share_mutex,
                       MY_MUTEX_INIT_FAST))
  ...
  if (mysql_mutex_init(spd_key_mutex_open_conn, &spider_open_conn_mutex,
                       MY_MUTEX_INIT_FAST))
  ...
  if (mysql_mutex_init(spd_key_mutex_mem_calc, &spider_mem_calc_mutex,
                       MY_MUTEX_INIT_FAST))
  ...
  spider_alloc_calc_mem(NULL, spider_open_tables,
                        spider_open_tables.array.max_element *
                            spider_open_tables.array.size_of_element);
#if 0
  ...
  spider_alloc_calc_mem(NULL, spider_init_error_tables,
                        spider_init_error_tables.array.max_element *
                            spider_init_error_tables.array.size_of_element);
#endif
#ifdef WITH_PARTITION_STORAGE_ENGINE
  ...
  spider_alloc_calc_mem(NULL, spider_open_pt_share,
                        spider_open_pt_share.array.max_element *
                            spider_open_pt_share.array.size_of_element);
#endif
  if (my_hash_init(&spider_lgtm_tblhnd_share_hash, spd_charset_utf8_bin, 32, 0,
                   0, (my_hash_get_key)spider_lgtm_tblhnd_share_hash_get_key, 0,
                   0)) {
    error_num = HA_ERR_OUT_OF_MEM;
  ...
  if (my_hash_init(&spider_for_sts_conns, spd_charset_utf8_bin, 32, 0, 0,
                   (my_hash_get_key)spider_for_sts_conn_get_key,
                   spider_free_for_sts_conn, 0)) {
    error_num = HA_ERR_OUT_OF_MEM;
    goto error_sts_for_conn_hash_init;
  }
  ...
  if (my_hash_init(&spider_active_conns, spd_charset_utf8_bin,
                   (spider_param_enable_conns_view() ? 128 : 0), 0, 0,
                   NULL, NULL, 0)) {
    error_num = HA_ERR_OUT_OF_MEM;
  ...
  spider_alloc_calc_mem(NULL, spider_allocated_thds,
                        spider_allocated_thds.array.max_element *
                            spider_allocated_thds.array.size_of_element);
  ...
  spider_alloc_calc_mem(NULL, spider_mon_table_cache,
                        spider_mon_table_cache.max_element *
                            spider_mon_table_cache.size_of_element);

  if (!(spider_udf_table_mon_mutexes = (pthread_mutex_t *)spider_bulk_malloc(
            NULL, 53, MYF(MY_WME | MY_ZEROFILL), &spider_udf_table_mon_mutexes,
            sizeof(pthread_mutex_t) * spider_param_udf_table_mon_mutex_count(),
            &spider_udf_table_mon_conds,
            sizeof(pthread_cond_t) * spider_param_udf_table_mon_mutex_count(),
            &spider_udf_table_mon_list_hash,
            sizeof(HASH) * spider_param_udf_table_mon_mutex_count(), NullS)))
  ...
  for (roop_count = 0;
    ...
    if (mysql_cond_init(spd_key_cond_udf_table_mon,
                        &spider_udf_table_mon_conds[roop_count]))
  ...
  for (roop_count = 0;
    ...
    spider_alloc_calc_mem(
        NULL, spider_udf_table_mon_list_hash,
        spider_udf_table_mon_list_hash[roop_count].array.max_element *
            spider_udf_table_mon_list_hash[roop_count].array.size_of_element);
  }
  ...
  error_num = spider_create_get_status_thread();
  if (error_num) {
...
error_conn_recycle_thd_init:
  spider_free_conn_recycle_thread();
error_conn_get_status_thd_init:
  spider_free_get_status_thread();
error_init_udf_table_mon_list_hash:
  for (; roop_count >= 0; roop_count--) {
    spider_free_mem_calc(
        NULL, spider_udf_table_mon_list_hash_id,
        spider_udf_table_mon_list_hash[roop_count].array.max_element *
            spider_udf_table_mon_list_hash[roop_count].array.size_of_element);
    my_hash_free(&spider_udf_table_mon_list_hash[roop_count]);
  }
...
error_alloc_mon_mutxes:
  spider_free_mem_calc(NULL, spider_mon_table_cache_id,
                       spider_mon_table_cache.max_element *
                           spider_mon_table_cache.size_of_element);
  delete_dynamic(&spider_mon_table_cache);
...
#ifdef WITH_PARTITION_STORAGE_ENGINE
  spider_free_mem_calc(NULL, spider_open_pt_share_id,
                       spider_open_pt_share.array.max_element *
                           spider_open_pt_share.array.size_of_element);
  my_hash_free(&spider_open_pt_share);
error_open_pt_share_hash_init:
#endif
  spider_free_mem_calc(NULL, spider_init_error_tables_id,
                       spider_init_error_tables.array.max_element *
                           spider_init_error_tables.array.size_of_element);
  my_hash_free(&spider_init_error_tables);
/*
error_init_error_tables_hash_init:
  spider_free_mem_calc(NULL, spider_open_tables_id,
                       spider_open_tables.array.max_element *
                           spider_open_tables.array.size_of_element);
  my_hash_free(&spider_open_tables);
*/
...
char *spider_create_table_name_string(const char *table_name,
  ...
  if (part_name) {
    ...
    if (sub_name) length += sizeof("#SP#") - 1 + strlen(sub_name);
  }
...
void spider_get_partition_info(const char *table_name, uint table_name_length,
  ...
  *part_elem = NULL;
  *sub_elem = NULL;
  if (!part_info) DBUG_VOID_RETURN;

  if (!memcmp(table_name + table_name_length - 5, "#TMP#", 5)) tmp_flg = TRUE;
    ...
    if ((*part_elem)->subpartitions.elements) {
      ...
      while ((*sub_elem = sub_it++)) {
        ...
        if (!memcmp(table_name, tmp_name, table_name_length + 1))
          DBUG_VOID_RETURN;
        if (tmp_flg && *(tmp_name + table_name_length - 5) == '\0' &&
...
int spider_get_sts(SPIDER_SHARE *share, int link_idx, time_t tmp_time,
                   ha_spider *spider, double sts_interval __attribute__((unused)), int sts_mode,
#ifdef WITH_PARTITION_STORAGE_ENGINE
                   int sts_sync,
#endif
                   int sts_sync_level __attribute__((unused)), uint flag) {
  ...
  if (sts_sync == 0) {
    ...
    get_type = 1;
#ifdef WITH_PARTITION_STORAGE_ENGINE
  } else if (!share->partition_share->sts_init) {
  ...
  } else if (difftime(share->sts_get_time,
                      share->partition_share->sts_get_time) < sts_interval) {
    /* copy */
    get_type = 0;
  } else if (!pthread_mutex_trylock(&share->partition_share->sts_mutex)) {
  ...
  if (!share->sts_init &&
    ...
    error_num = spider_sys_get_table_sts(
        current_thd, share->lgtm_tblhnd_share->table_name,
        share->lgtm_tblhnd_share->table_name_length, &share->data_file_length,
        &share->max_data_file_length, &share->index_file_length,
        &share->records, &share->mean_rec_length, &share->check_time,
        &share->create_time, &share->update_time, FALSE);
    if (!error_num ||
...
#ifdef WITH_PARTITION_STORAGE_ENGINE
  if (get_type >= 2) pthread_mutex_unlock(&share->partition_share->sts_mutex);
#endif
  if (error_num) {
#ifdef WITH_PARTITION_STORAGE_ENGINE
    SPIDER_PARTITION_HANDLER_SHARE *partition_handler_share =
        spider->partition_handler_share;
    if (!share->partition_share->sts_init && sts_sync >= sts_sync_level &&
        get_type > 1 && partition_handler_share &&
      ...
      for (roop_count = 1; roop_count < (int)partition_handler_share->use_count;
           roop_count++) {
        tmp_spider = (ha_spider *)partition_handler_share->handlers[roop_count];
        ...
        spider_get_sts(tmp_share, tmp_spider->search_link_idx, tmp_time,
                       tmp_spider, tmp_sts_interval, tmp_sts_mode, tmp_sts_sync,
                       1, flag);
        if (share->partition_share->sts_init) {
          error_num = 0;
          thd->clear_error();
...
int spider_get_crd(SPIDER_SHARE *share, int link_idx, time_t tmp_time,
                   ha_spider *spider, TABLE *table, double crd_interval __attribute__((unused)),
                   int crd_mode,
#ifdef WITH_PARTITION_STORAGE_ENGINE
                   int crd_sync,
#endif
                   int crd_sync_level __attribute__((unused))) {
  int get_type __attribute__((unused));
  int error_num = 0;
  bool need_to_get = TRUE;
  ...
  if (crd_sync == 0) {
    ...
    get_type = 1;
#ifdef WITH_PARTITION_STORAGE_ENGINE
  } else if (!share->partition_share->crd_init) {
  ...
  } else if (difftime(share->crd_get_time,
                      share->partition_share->crd_get_time) < crd_interval) {
    /* copy */
    get_type = 0;
  } else if (!pthread_mutex_trylock(&share->partition_share->crd_mutex)) {
    /* get after mutex_trylock */
    get_type = 3;
  } else {
    /* copy */
...
#ifdef WITH_PARTITION_STORAGE_ENGINE
    if (get_type == 0)
      spider_copy_crd_to_share(share, share->partition_share, table->s->fields);
    else {
#endif
...
#ifdef WITH_PARTITION_STORAGE_ENGINE
  if (get_type >= 2) pthread_mutex_unlock(&share->partition_share->crd_mutex);
#endif
  if (error_num) {
#ifdef WITH_PARTITION_STORAGE_ENGINE
    SPIDER_PARTITION_HANDLER_SHARE *partition_handler_share =
        spider->partition_handler_share;
    if (!share->partition_share->crd_init && crd_sync >= crd_sync_level &&
        get_type > 1 && partition_handler_share &&
        partition_handler_share->handlers &&
        partition_handler_share->handlers[0] == spider) {
      int roop_count;
...
void spider_set_result_list_param(ha_spider *spider) {
  ...
  result_list->max_order = spider_param_max_order(thd, share->max_order);
  if (spider_param_quick_mode_only_select() && thd && thd->lex &&
      thd->lex->sql_command !=
          SQLCOM_SELECT) { /*
                           set quick_mode = 0;
                           if quick_mode=1, "delete from t1 where c1 in select
                           c1 from t2" would cause error result
                           */
...
SPIDER_INIT_ERROR_TABLE *spider_get_init_error_table(SPIDER_TRX *trx,
...
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
  if (!(spider_init_error_table =
            (SPIDER_INIT_ERROR_TABLE *)my_hash_search_using_hash_value(
  ...
  if (!(spider_init_error_table = (SPIDER_INIT_ERROR_TABLE *)my_hash_search(
            &spider_init_error_tables, (uchar *)share->table_name,
            share->table_name_length)))
#endif
  {
    if (!create) {
    ...
    if (!(spider_init_error_table =
              (SPIDER_INIT_ERROR_TABLE *)spider_bulk_malloc(
                  spider_current_trx, 54, MYF(MY_WME | MY_ZEROFILL),
                  &spider_init_error_table, sizeof(*spider_init_error_table),
                  &tmp_name, share->table_name_length + 1, NullS))) {
      pthread_mutex_unlock(&spider_init_error_tbl_mutex);
      DBUG_RETURN(NULL);
    }
    ...
    if (spider_init_error_tables.array.max_element > old_elements) {
      spider_alloc_calc_mem(
          spider_current_trx, spider_init_error_tables,
...
void spider_delete_init_error_table(const char *name) {
...
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
  my_hash_value_type hash_value =
      my_calc_hash(&spider_open_tables, reinterpret_cast<uchar *>(pname), length);
#endif
  ...
  if ((spider_init_error_table =
           (SPIDER_INIT_ERROR_TABLE *)my_hash_search_using_hash_value(
               &spider_init_error_tables, hash_value, reinterpret_cast<uchar *>(pname), length)))
#else
  if ((spider_init_error_table = (SPIDER_INIT_ERROR_TABLE *)my_hash_search(
           &spider_init_error_tables, reinterpret_cast<uchar *>(pname), length)))
#endif
  {
    my_hash_delete(&spider_init_error_tables, (uchar *)spider_init_error_table);
...
bool spider_check_pk_update(TABLE *table) {
  ...
  if (table_share->primary_key == MAX_KEY) DBUG_RETURN(FALSE);
...
int spider_create_tmp_dbton_handler(ha_spider *tmp_spider) {
  ...
  if (!(tmp_spider->dbton_handler[dbton_id] =
            spider_dbton[dbton_id].create_db_handler(
                tmp_spider, tmp_share->dbton_share[dbton_id]))) {
    DBUG_RETURN(HA_ERR_OUT_OF_MEM);
  }
...
Query_block *spider_get_select_lex(ha_spider *spider) {
  TABLE_LIST *table_list = spider_get_parent_table_list(spider);
  DBUG_ENTER("spider_get_select_lex");
  if (table_list) {
    DBUG_RETURN(table_list->query_block);
  }
  DBUG_RETURN(NULL);
}

void spider_get_select_limit_from_select_lex(Query_block *select_lex,
                                             longlong *select_limit,
                                             longlong *offset_limit) {
  DBUG_ENTER("spider_get_select_limit_from_select_lex");
  *select_limit = INT_MAX64;
  *offset_limit = 0;
  if (select_lex && select_lex->has_limit()) {
    *select_limit =
        select_lex->select_limit ? select_lex->select_limit->val_int() : 0;
    *offset_limit =
        select_lex->offset_limit ? select_lex->offset_limit->val_int() : 0;
  }
  DBUG_VOID_RETURN;
}
...
bool is_spider_select_limit_x_y(ha_spider *spider) {
  ...
  if (select_lex && select_lex->select_limit && select_lex->has_limit() &&
      select_lex->offset_limit && select_limit && offset_limit) {
    DBUG_RETURN(true);
  }
  DBUG_RETURN(false);
}
...
void spider_get_select_limit(ha_spider *spider, Query_block **select_lex,
...
longlong spider_split_read_param(ha_spider *spider) {
  ...
  spider_get_select_limit(spider, &select_lex, &select_limit, &offset_limit);
  DBUG_PRINT("info", ("spider result_list->set_split_read=%s",
                      result_list->set_split_read ? "TRUE" : "FALSE"));
  if (!result_list->set_split_read) {
    int bulk_update_mode =
        spider_param_bulk_update_mode(thd, share->bulk_update_mode);
...
#ifdef SPIDER_HAS_GROUP_BY_HANDLER
    bool inserting = (spider->sql_command == SQLCOM_INSERT ||
                      spider->sql_command == SQLCOM_INSERT_SELECT);
#endif
    bool updating = (spider->sql_command == SQLCOM_UPDATE ||
                     spider->sql_command == SQLCOM_UPDATE_MULTI);
    bool deleting = (spider->sql_command == SQLCOM_DELETE ||
                     spider->sql_command == SQLCOM_DELETE_MULTI);
    bool replacing = (spider->sql_command == SQLCOM_REPLACE ||
    ...
    if (
#ifdef SPIDER_HAS_GROUP_BY_HANDLER
        (inserting && spider->use_fields) ||
#endif
        replacing ||
        ((updating || deleting) &&
         (bulk_update_mode != 2 || !spider->support_bulk_update_sql() ||
          (updating && table->triggers &&
#ifdef HA_CAN_FORCE_BULK_UPDATE
           !(table->file->ha_table_flags() & HA_CAN_FORCE_BULK_UPDATE) &&
#endif
           table->triggers->has_triggers(TRG_EVENT_UPDATE, TRG_ACTION_AFTER)) ||
          (deleting && table->triggers &&
#ifdef HA_CAN_FORCE_BULK_DELETE
    ...
    if (explain) {
      ...
      if (select_lex) {
        ...
        DBUG_PRINT("info",
                   ("spider select_number=%u", select_lex->select_number));
        explain_select = explain->get_select(select_lex->select_number);
      }
      if (explain_select) {
...
#endif
    result_list->first_read = spider_param_first_read(thd, share->first_read);
    result_list->second_read =
        spider_param_second_read(thd, share->second_read);
    result_list->semi_split_read_base = 0;
    result_list->set_split_read = TRUE;
  }
  DBUG_PRINT("info", ("spider result_list->semi_split_read=%f",
                      result_list->semi_split_read));
  DBUG_PRINT("info", ("spider select_lex->has_limit=%d",
                      select_lex ? select_lex->has_limit() : 0));
  DBUG_PRINT("info", ("spider OPTION_FOUND_ROWS=%s",
                      select_lex && (select_lex->active_options() & OPTION_FOUND_ROWS)
                          ? "TRUE"
                          : "FALSE"));
  ...
  if (result_list->semi_split_read > 0 && select_lex &&
      select_lex->has_limit() &&
      !(select_lex->active_options() & OPTION_FOUND_ROWS) &&
      !select_lex->group_list.elements && !select_lex->with_sum_func) {
    semi_split_read =
        result_list->semi_split_read * (select_limit + offset_limit);
...
void spider_next_split_read_param(ha_spider *spider) {
  ...
  if (result_list->semi_split_read_base)
    result_list->split_read = result_list->semi_split_read_base;
  else if (result_list->set_split_read_count == 1 &&
...
bool spider_check_direct_order_limit(ha_spider *spider) {
  ...
  if (spider->sql_command != SQLCOM_HA_READ && !spider->use_index_merge &&
    ...
    if (select_lex && (select_lex->active_options() & SELECT_DISTINCT)) {
...
#ifdef HANDLER_HAS_DIRECT_AGGREGATE
      spider->result_list.direct_aggregate = FALSE;
    } else if (!select_lex->group_list.elements && !select_lex->with_sum_func) {
    ...
    /*  don't do direct limit to data node when distinct limit */
    /* example query: select distinct id from t where name > 'a' limit 10  */
    // (select_lex && (select_lex->options & SELECT_DISTINCT) && select_lex->explicit_limit) 
    if (direct_order_limit &&
          !(select_lex && (select_lex->active_options() & SELECT_DISTINCT) &&
          select_lex->has_limit())) {
          if (!first_check || !select_lex->has_limit() ||
          (select_lex->active_options() & OPTION_FOUND_ROWS) ||
          (
#ifdef HANDLER_HAS_DIRECT_AGGREGATE
              !spider->result_list.direct_aggregate &&
          ...
          (!opt_tdsql_direct_limit_in_group &&
...
int spider_set_direct_limit_offset(ha_spider *spider) {
  ...
  longlong select_limit;
  longlong offset_limit;
  TABLE_LIST *table_list __attribute__((unused));
  DBUG_ENTER("spider_set_direct_limit_offset");

  if (spider->result_list.direct_limit_offset) DBUG_RETURN(TRUE);
  ...
  /******
   may cause crash
    if (
      ...
      if (spider->pt_handler_share_creator->result_list.direct_limit_offset ==
  TRUE)
      {
  ...
  ***********/
  ...
  if (spider->sql_command != SQLCOM_SELECT ||
      ...
      spider->prev_index_rnd_init !=
          SPD_RND  // must be RND_INIT and not be INDEX_INIT
  )
    DBUG_RETURN(FALSE);
  ...
  if (!(select_limit && offset_limit)) DBUG_RETURN(FALSE);

  // more than one table
  if (!select_lex || select_lex->table_list.elements != 1) DBUG_RETURN(FALSE);
#if 0
  ...
  if (
      ...
      spider->condition  // conditions is null may be no where condition in
                         // rand_init
  )
    DBUG_RETURN(FALSE);

  // ignore condition like 1=1
  // if (select_lex->where_cond() && select_lex->where_cond()->with_subquery())
  if (select_lex->where_cond() && select_lex->where_cond()->type() == Item::SUBSELECT_ITEM)
    DBUG_RETURN(FALSE);

  if (select_lex->group_list.elements || select_lex->with_sum_func ||
      select_lex->having_cond() || select_lex->order_list.elements)
    DBUG_RETURN(FALSE);

  // must not be derived table
  if (thd->lex->query_block != select_lex) DBUG_RETURN(FALSE);
...
bool spider_check_index_merge(TABLE *table, Query_block *select_lex) {
  uint roop_count;
  JOIN *join;
  ...
  for (roop_count = 0; roop_count < spider_join_table_count(join);
       ++roop_count) {
...
int spider_compare_for_sort(SPIDER_SORT *a, SPIDER_SORT *b) {
  ...
  if (a->sort > b->sort) DBUG_RETURN(-1);
  if (a->sort < b->sort) DBUG_RETURN(1);
  DBUG_RETURN(0);
}
...
int spider_discover_table_structure_internal(SPIDER_TRX *trx,
  ...
  for (roop_count = 0; roop_count < SPIDER_DBTON_SIZE; roop_count++) {
    if (spider_bit_is_set(spider_share->dbton_bitmap, roop_count)) {
      if ((error_num =
               spider_share->dbton_share[roop_count]->discover_table_structure(
...
int spider_discover_table_structure(handlerton *hton, THD *thd,
  ...
  char tmp_name_with_version[FN_LEN];
  char ts_version[40];
  long long table_share_version_tmp = thd->flush_no_block_version;
  ullstr(table_share_version_tmp, ts_version);
  ...
  str.length(0);
  if (str.reserve(SPIDER_SQL_CREATE_TABLE_LEN + share->db.length +
                  SPIDER_SQL_DOT_LEN + share->table_name.length +
                  /* SPIDER_SQL_LCL_NAME_QUOTE_LEN */ 4 +
...
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
  my_hash_value_type hash_value =
      my_calc_hash(&spider_open_tables, (uchar *)table_name, table_name_length);
#endif
  if (!(trx = spider_get_trx(thd, TRUE, &error_num))) {
  ...
  if (!part_info) {
#endif
    if (table_share_version_tmp == 0) {
      strmov(tmp_name_with_version, table_name);
    } else {
      strmov(strmov(strmov(tmp_name_with_version, table_name), "#"),
             ts_version);
    }
    if (!(spider_share =
              spider_create_share(table_name, tmp_name_with_version, share,
#ifdef WITH_PARTITION_STORAGE_ENGINE
                                  NULL,
#endif
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
    ...
    error_num =
        spider_discover_table_structure_internal(trx, spider_share, &str);
    ...
    while ((part_elem = part_it++)) {
      if ((part_elem)->subpartitions.elements) {
        ...
        while ((sub_elem = sub_it++)) {
          ...
          if (!(spider_share = spider_create_share(
                    tmp_name, tmp_name_with_version, share, part_info,
#ifdef SPIDER_HAS_HASH_VALUE_TYPE
                    hash_value,
#endif
                    &error_num))) {
          ...
          while ((sub_elem = sub_it++)) {
            ...
            if (thd->lex->create_info.or_replace()) {
              error_num = spider_delete_tables(
                  table_tables, spider_share->table_name, &dummy);
            }
            if (!error_num) {
#endif
              error_num = spider_insert_tables(table_tables, spider_share);
  ...
  if (part_info) {
    ...
    if (!(part_syntax = SPIDER_generate_partition_syntax(
              thd, part_info, &part_syntax_len, FALSE, TRUE, info, NULL,
              NULL))) {
      DBUG_RETURN(HA_ERR_OUT_OF_MEM);
    }
  ...
  error_num =
      share->init_from_sql_statement_string(thd, TRUE, str.ptr(), str.length());
  DBUG_RETURN(error_num);
}
#endif
...
int spider_create_spider_object_for_share(SPIDER_TRX *trx, SPIDER_SHARE *share,
  ...
  DBUG_PRINT("info", ("spider spider=%p", (*spider)));
  if (!(need_mons = (int *)spider_bulk_malloc(
            spider_current_trx, 255, MYF(MY_WME | MY_ZEROFILL), &need_mons,
            (sizeof(int) * share->link_count), &conns,
            (sizeof(SPIDER_CONN *) * share->link_count), &conn_link_idx,
            (sizeof(uint) * share->link_count), &conn_can_fo,
            (sizeof(uchar) * share->link_bitmap_size), &conn_keys,
            (sizeof(char *) * share->link_count), &dbton_hdl,
            (sizeof(spider_db_handler *) * SPIDER_DBTON_SIZE), NullS))) {
    error_num = HA_ERR_OUT_OF_MEM;
...
int spider_create_sts_threads(SPIDER_THREAD *spider_thread) {
  ...
  if (mysql_mutex_init(spd_key_mutex_bg_stss, &spider_thread->mutex,
                       MY_MUTEX_INIT_FAST))
  ...
  if (mysql_cond_init(spd_key_cond_bg_stss, &spider_thread->cond))
  ...
  if (mysql_cond_init(spd_key_cond_bg_sts_syncs, &spider_thread->sync_cond))
...
void spider_free_sts_threads(SPIDER_THREAD *spider_thread) {
  ...
  pthread_join(spider_thread->thread.thread, NULL);
  pthread_cond_destroy(&spider_thread->sync_cond);
  pthread_cond_destroy(&spider_thread->cond);
  pthread_mutex_destroy(&spider_thread->mutex);
...
int spider_create_crd_threads(SPIDER_THREAD *spider_thread) {
  ...
  if (mysql_mutex_init(spd_key_mutex_bg_crds, &spider_thread->mutex,
                       MY_MUTEX_INIT_FAST))
  ...
  if (mysql_cond_init(spd_key_cond_bg_crds, &spider_thread->cond))
  ...
  if (mysql_cond_init(spd_key_cond_bg_crd_syncs, &spider_thread->sync_cond))
...
void *spider_table_bg_sts_action(void *arg) {
  ...
  while (TRUE) {
    ...
    if (!thread->queue_first) {
      ...
      thread->thd_wait = FALSE;
      if (thd->killed) thread->killed = TRUE;
      continue;
    }
    ...
    if (spider->search_link_idx >= 0) {
      DBUG_PRINT("info", ("spider difftime=%f", difftime(share->bg_sts_try_time,
                                                         share->sts_get_time)));
      DBUG_PRINT("info", ("spider bg_sts_interval=%f", share->bg_sts_interval));
...
void *spider_table_bg_crd_action(void *arg) {
  ...
  while (TRUE) {
    ...
    if (spider->search_link_idx >= 0) {
      DBUG_PRINT("info", ("spider difftime=%f", difftime(share->bg_crd_try_time,
                                                         share->crd_get_time)));
      DBUG_PRINT("info", ("spider bg_crd_interval=%f", share->bg_crd_interval));
...
void spider_table_add_share_to_sts_thread(SPIDER_SHARE *share) {
  ...
  if (!share->sts_wait && !pthread_mutex_trylock(&spider_thread->mutex)) {
    if (!share->sts_wait) {
...
void spider_table_add_share_to_crd_thread(SPIDER_SHARE *share) {
  ...
  if (!share->crd_wait && !pthread_mutex_trylock(&spider_thread->mutex)) {
    if (!share->crd_wait) {
      if (spider_thread->queue_last) {
        DBUG_PRINT("info", ("spider add to last"));
...
void spider_print_timestamp(FILE *file) /*!< in: file where to print */
{
  ...
  fprintf(file, "%02d%02d%02d %2d:%02d:%02d", (int)cal_tm.wYear % 100,
          (int)cal_tm.wMonth, (int)cal_tm.wDay, (int)cal_tm.wHour,
          (int)cal_tm.wMinute, (int)cal_tm.wSecond);
#else
  struct tm cal_tm __attribute__((unused));
...
#endif
  fprintf(file, "%02d%02d%02d %2d:%02d:%02d", cal_tm_ptr->tm_year % 100,
...
bool spider_time_to_str(char *dst, size_t len, void *tm) {
  DBUG_ENTER("spider_time_to_str");
#ifdef __WIN__
  SYSTEMTIME *cal_tm = (SYSTEMTIME *)tm;
  if (snprintf(dst, len, "%02d%02d%02d %2d:%02d:%02d",
                  (int)cal_tm->wYear % 100, (int)cal_tm->wMonth,
                  (int)cal_tm->wDay, (int)cal_tm->wHour, (int)cal_tm->wMinute,
...
#endif
  if (snprintf(dst, len, "%02d%02d%02d %2d:%02d:%02d",
                  cal_tm_ptr->tm_year % 100, cal_tm_ptr->tm_mon + 1,
                  cal_tm_ptr->tm_mday, cal_tm_ptr->tm_hour, cal_tm_ptr->tm_min,
                  cal_tm_ptr->tm_sec) >= (int)len) {
    DBUG_RETURN(true);
  }
#endif
  DBUG_RETURN(false);
...
void spider_make_mysql_time(MYSQL_TIME *ts, time_t *tm) {
...
int spider_replace_table_status_up(char *table_name, uint table_name_length,
                                   char *tgt_table_names, char *tgt_dbs,
                                   ulonglong data_file_length,
                                   ulonglong max_data_file_length,
                                   ulonglong index_file_length, ha_rows records,
                                   ulong mean_rec_length, time_t check_time,
                                   time_t create_time, time_t update_time) {
  TABLE *status_table = NULL;
  int error_num = 0;
  Open_tables_backup open_tables_backup;
  DBUG_ENTER("spider_replace_table_status_up");
  if (!(status_table =
            spider_open_sys_table(current_thd, SPIDER_SYS_TABLE_STATUS_NAME_STR,
                                  SPIDER_SYS_TABLE_STATUS_NAME_LEN, TRUE,
                                  &open_tables_backup, FALSE, &error_num))) {
    DBUG_RETURN(error_num);
  }

  if ((error_num = spider_replace_table_status(
           status_table, table_name, table_name_length, tgt_table_names,
           tgt_dbs, data_file_length, max_data_file_length, index_file_length,
           records, mean_rec_length, check_time, create_time, update_time))) {
    DBUG_RETURN(error_num);
  }
  spider_close_sys_table(current_thd, status_table, &open_tables_backup, FALSE);
  status_table = NULL;
