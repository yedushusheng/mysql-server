...
#include "sql/tdsql/common/config/td_config.h"

// #include "sql/tdsql/xa/xa_log.h"
#include "spd_err.h"
#include "spd_param.h"
...
#endif
extern pthread_mutex_t spd_db_att_xid_cache_mutex;
extern HASH spd_db_att_xid_cache;
#endif
extern pthread_mutex_t spider_xid_mutex;
...
class spider_db_mysql;
extern void log_spider_sql_tdsql(THD *thd, spider_db_mysql *db_conn,
                                 const char *query, uint length,
                                 my_hrtime_t &start, bool has_error);

// for spider_alter_tables
uchar *spider_alter_tbl_get_key(SPIDER_ALTER_TABLE *alter_table, size_t *length,
...
// for SPIDER_TRX_HA
uchar *spider_trx_ha_get_key(SPIDER_TRX_HA *trx_ha, size_t *length,
                             bool not_used __attribute__((unused))) {
...
int spider_trx_all_start_trx(SPIDER_TRX *trx) {
  ...
  tmp_spider.trx = trx;
  tmp_spider.need_mons = &need_mon;
  // FIXME: set set the first partition 0 here
  // trx->set_spider_gtid(thd->server_id, thd->query_id, 0);
  while ((
      conn = (SPIDER_CONN *)my_hash_element(&trx->trx_conn_hash, roop_count))) {
    if ((spider_param_sync_trx_isolation(trx->thd) &&
...
void spider_free_trx_alter_table_alloc(SPIDER_TRX *trx,
  ...
  if (alter_table->tmp_char) spider_free(trx, alter_table->tmp_char, MYF(0));
...
int spider_free_trx_alter_table(SPIDER_TRX *trx) {
  ...
  while ((alter_table = (SPIDER_ALTER_TABLE *)my_hash_element(
              &trx->trx_alter_table_hash, 0))) {
...
int spider_create_trx_alter_table(SPIDER_TRX *trx, SPIDER_SHARE *share,
  ...
  if (!(alter_table = (SPIDER_ALTER_TABLE *)spider_bulk_malloc(
            spider_current_trx, 55, MYF(MY_WME | MY_ZEROFILL), &alter_table,
            sizeof(*alter_table), &tmp_name,
            sizeof(char) * (share->table_name_length + 1),
            ...
            sizeof(uint) * share->all_link_count, &tmp_static_link_ids_lengths,
            sizeof(uint) * share->all_link_count,
            ...
            sizeof(long) * share->all_link_count, &tmp_link_statuses,
            sizeof(long) * share->all_link_count,

            &tmp_server_names_char,
            sizeof(char) * (share_alter->tmp_server_names_charlen + 1),
            &tmp_tgt_table_names_char,
            sizeof(char) * (share_alter->tmp_tgt_table_names_charlen + 1),
            &tmp_tgt_dbs_char,
            sizeof(char) * (share_alter->tmp_tgt_dbs_charlen + 1),
            &tmp_tgt_hosts_char,
            sizeof(char) * (share_alter->tmp_tgt_hosts_charlen + 1),
            &tmp_tgt_usernames_char,
            sizeof(char) * (share_alter->tmp_tgt_usernames_charlen + 1),
            &tmp_tgt_passwords_char,
            sizeof(char) * (share_alter->tmp_tgt_passwords_charlen + 1),
            &tmp_tgt_sockets_char,
            sizeof(char) * (share_alter->tmp_tgt_sockets_charlen + 1),
            &tmp_tgt_wrappers_char,
            sizeof(char) * (share_alter->tmp_tgt_wrappers_charlen + 1),
            &tmp_tgt_ssl_cas_char,
            sizeof(char) * (share_alter->tmp_tgt_ssl_cas_charlen + 1),
            &tmp_tgt_ssl_capaths_char,
            sizeof(char) * (share_alter->tmp_tgt_ssl_capaths_charlen + 1),
            &tmp_tgt_ssl_certs_char,
            sizeof(char) * (share_alter->tmp_tgt_ssl_certs_charlen + 1),
            &tmp_tgt_ssl_ciphers_char,
            sizeof(char) * (share_alter->tmp_tgt_ssl_ciphers_charlen + 1),
            &tmp_tgt_ssl_keys_char,
            sizeof(char) * (share_alter->tmp_tgt_ssl_keys_charlen + 1),
            &tmp_tgt_default_files_char,
            sizeof(char) * (share_alter->tmp_tgt_default_files_charlen + 1),
            &tmp_tgt_default_groups_char,
            sizeof(char) * (share_alter->tmp_tgt_default_groups_charlen + 1),
            &tmp_static_link_ids_char,
            sizeof(char) * (share_alter->tmp_static_link_ids_charlen + 1),
            NullS))) {
  ...
  DBUG_PRINT("info",
             ("spider table_name_hash_value=%u", share->table_name_hash_value));
  ...
  for (roop_count = 0; roop_count < (int)share->all_link_count; roop_count++) {
    ...
    tmp_tgt_dbs_char += share_alter->tmp_tgt_dbs_lengths[roop_count] + 1;
    ...
    tmp_tgt_hosts_char += share_alter->tmp_tgt_hosts_lengths[roop_count] + 1;
  ...
  memcpy(tmp_tgt_default_groups_lengths,
         ...
         sizeof(uint) * share->all_link_count);
  memcpy(tmp_static_link_ids_lengths, share_alter->tmp_static_link_ids_lengths,
         sizeof(uint) * share->all_link_count);

  alter_table->tmp_server_names_length = share_alter->tmp_server_names_length;
  alter_table->tmp_tgt_table_names_length =
      share_alter->tmp_tgt_table_names_length;
  alter_table->tmp_tgt_dbs_length = share_alter->tmp_tgt_dbs_length;
  alter_table->tmp_tgt_hosts_length = share_alter->tmp_tgt_hosts_length;
  alter_table->tmp_tgt_usernames_length = share_alter->tmp_tgt_usernames_length;
  alter_table->tmp_tgt_passwords_length = share_alter->tmp_tgt_passwords_length;
  alter_table->tmp_tgt_sockets_length = share_alter->tmp_tgt_sockets_length;
  alter_table->tmp_tgt_wrappers_length = share_alter->tmp_tgt_wrappers_length;
  alter_table->tmp_tgt_ssl_cas_length = share_alter->tmp_tgt_ssl_cas_length;
  alter_table->tmp_tgt_ssl_capaths_length =
      share_alter->tmp_tgt_ssl_capaths_length;
  alter_table->tmp_tgt_ssl_certs_length = share_alter->tmp_tgt_ssl_certs_length;
  alter_table->tmp_tgt_ssl_ciphers_length =
      share_alter->tmp_tgt_ssl_ciphers_length;
  alter_table->tmp_tgt_ssl_keys_length = share_alter->tmp_tgt_ssl_keys_length;
  ...
  alter_table->tmp_tgt_ports_length = share_alter->tmp_tgt_ports_length;
  alter_table->tmp_tgt_ssl_vscs_length = share_alter->tmp_tgt_ssl_vscs_length;
  alter_table->tmp_monitoring_binlog_pos_at_failing_length =
      share_alter->tmp_monitoring_binlog_pos_at_failing_length;
  alter_table->tmp_link_statuses_length = share_alter->tmp_link_statuses_length;
  ...
  if (trx->trx_alter_table_hash.array.max_element > old_elements) {
    spider_alloc_calc_mem(
        spider_current_trx, trx->trx_alter_table_hash,
...
bool spider_cmp_trx_alter_table(SPIDER_ALTER_TABLE *cmp1,
  ...
  if (cmp1->tmp_priority != cmp2->tmp_priority ||
  ...
  for (roop_count = 0; roop_count < (int)cmp1->all_link_count; roop_count++) {
    if ((cmp1->tmp_server_names[roop_count] !=
        ...
        (cmp1->tmp_tgt_table_names[roop_count] !=
         ...
         (!cmp1->tmp_tgt_table_names[roop_count] ||
          ...
          strcmp(cmp1->tmp_tgt_table_names[roop_count],
                 cmp2->tmp_tgt_table_names[roop_count]))) ||
        (cmp1->tmp_tgt_dbs[roop_count] != cmp2->tmp_tgt_dbs[roop_count] &&
         (!cmp1->tmp_tgt_dbs[roop_count] || !cmp2->tmp_tgt_dbs[roop_count] ||
          strcmp(cmp1->tmp_tgt_dbs[roop_count],
                 cmp2->tmp_tgt_dbs[roop_count]))) ||
        (cmp1->tmp_tgt_hosts[roop_count] != cmp2->tmp_tgt_hosts[roop_count] &&
...
int spider_free_trx_alloc(SPIDER_TRX *trx) {
  ...
  spider_free_mem_calc(spider_current_trx, trx->trx_conn_hash_id,
                       trx->trx_conn_hash.array.max_element *
                           trx->trx_conn_hash.array.size_of_element);
  my_hash_free(&trx->trx_conn_hash);
  spider_free_mem_calc(spider_current_trx, trx->trx_another_conn_hash_id,
  ...
  spider_free_mem_calc(spider_current_trx, trx->trx_ha_hash_id,
                       trx->trx_ha_hash.array.max_element *
                           trx->trx_ha_hash.array.size_of_element);
  my_hash_free(&trx->trx_ha_hash);
  spider_free_mem_calc(spider_current_trx, trx->trx_alter_table_hash_id,
...
SPIDER_TRX *spider_get_trx(THD *thd, bool regist_allocated_thds,
  ...
  if (!thd || !(trx = (SPIDER_TRX *)thd_get_ha_data(thd, spider_hton_ptr))) {
    DBUG_PRINT("info", ("spider create new trx"));
    if (!(trx = (SPIDER_TRX *)spider_bulk_malloc(
              NULL, 56, MYF(MY_WME | MY_ZEROFILL), &trx, sizeof(*trx),
              &tmp_share, sizeof(SPIDER_SHARE), &udf_table_mutexes,
    ...
    for (roop_count = 0;
      ...
      if (mysql_mutex_init(spd_key_mutex_udf_table,
                           &trx->udf_table_mutexes[roop_count],
                           MY_MUTEX_INIT_FAST))

        goto error_init_udf_table_mutex;
    }

    if (my_hash_init(&trx->trx_conn_hash, spd_charset_utf8_bin, 128, 0, 0,
    ...
    if (my_hash_init(&trx->trx_another_conn_hash, spd_charset_utf8_bin, 32, 0,
    ...
    if (my_hash_init(&trx->trx_alter_table_hash, spd_charset_utf8_bin, 32, 0, 0,
    ...
    if (my_hash_init(&trx->trx_ha_hash, spd_charset_utf8_bin, 32, 0, 0,
    ...
    if (thd)
      trx->thd_hash_value =
          my_calc_hash(&spider_allocated_thds, (uchar *)thd, sizeof(THD *));
    else
    ...
    if (thd) {
      spider_set_tmp_share_pointer(trx->tmp_share, trx->tmp_connect_info,
                                   trx->tmp_connect_info_length, trx->tmp_long,
                                   trx->tmp_longlong);
      if (spider_set_connect_info_default(trx->tmp_share,
#ifdef WITH_PARTITION_STORAGE_ENGINE
                                          NULL, NULL,
#endif
                                          NULL) ||
          spider_set_connect_info_default_db_table(trx->tmp_share, "", 0, "",
                                                   0) ||
      ...
      for (roop_count2 = 0; roop_count2 < SPIDER_DBTON_SIZE; ++roop_count2) {
        if (!spider_dbton[roop_count2].init) continue;
        ...
        if (!(trx->tmp_spider->dbton_handler[roop_count2] =
                  spider_dbton[roop_count2].create_db_handler(
                      trx->tmp_spider,
      ...
      // log gtid commit
      native_mutex_init(&trx->m_log_done_mutex, nullptr);
      native_cond_init(&trx->m_log_done_cond);
      native_mutex_init(&trx->m_start_gts_mutex, nullptr);
      native_cond_init(&trx->m_start_gts_cond);
      native_mutex_init(&trx->m_end_gts_mutex, nullptr);
      native_cond_init(&trx->m_end_gts_cond);
...
error_set_connect_info_default:
  ...
  spider_free_mem_calc(trx, trx->trx_ha_hash_id,
                       trx->trx_ha_hash.array.max_element *
                           trx->trx_ha_hash.array.size_of_element);
  my_hash_free(&trx->trx_ha_hash);
...
int spider_free_trx(SPIDER_TRX *trx, bool need_lock) {
  DBUG_ENTER("spider_free_trx");
  if (trx->thd) {
    if (trx->registed_allocated_thds) {
      if (need_lock) pthread_mutex_lock(&spider_allocated_thds_mutex);
      my_hash_delete(&spider_allocated_thds, (uchar *)trx->thd);
      if (need_lock) pthread_mutex_unlock(&spider_allocated_thds_mutex);
    }
    thd_set_ha_data(trx->thd, spider_hton_ptr, NULL);
  }
  native_mutex_destroy(&trx->m_log_done_mutex);
  native_cond_destroy(&trx->m_log_done_cond);
  native_mutex_destroy(&trx->m_start_gts_mutex);
  native_cond_destroy(&trx->m_start_gts_cond);
  native_mutex_destroy(&trx->m_end_gts_mutex);
  native_cond_destroy(&trx->m_end_gts_cond);
...
int spider_check_and_set_trx_isolation(SPIDER_CONN *conn, int *need_mon[[maybe_unused]]) {
  int trx_isolation;
...
int spider_check_and_set_autocommit(THD *thd, SPIDER_CONN *conn,
                                    int *need_mon[[maybe_unused]]) {
  bool autocommit;
...
int spider_check_and_set_time_zone(THD *thd, SPIDER_CONN *conn, int *need_mon[[maybe_unused]]) {
  Time_zone *time_zone;
...
int spider_xa_lock(XID_STATE *xid_state) {
  return 0;
  THD *thd = current_thd;
  int error_num;
  ...
  my_hash_value_type hash_value =
      my_calc_hash(&spd_db_att_xid_cache, (uchar *)xid_state->get_xid()->key(),
...
#endif
#else
  if (my_hash_search(&spd_db_att_xid_cache, xid_state->get_xid()->key(),
                     xid_state->get_xid()->key_length()))
#endif
  {
...
int spider_xa_unlock(XID_STATE *xid_state) {
  return 0;
  THD *thd = current_thd;
  const char *old_proc_info;
...
bool trx_need_start_gts(SPIDER_TRX *trx) {
  return (tdsql::tdsql_internal_consistent_read_mode &&
          tdsql::consistent_read_optimize &&
          trx->thd->variables.transaction_isolation == ISO_REPEATABLE_READ);
}

int spider_request_gts(SPIDER_TRX *trx) {
  int ret = -1;
  THD *thd = trx->thd;
  auto read_time = thd->start_time; // timeval
  ret =
      tdsql::g_meta_cluster->AsyncRequest([trx, read_time](
                                           const uint64_t &gts, const timespec &req_time) -> int {
        auto read_fuzzy_time = read_time.tv_usec;
        auto req_fuzzy_time = req_time.tv_nsec;
        if (likely(tdsql::gts_fuzzy_time)) {
          read_fuzzy_time = read_fuzzy_time / tdsql::gts_fuzzy_time;
          req_fuzzy_time = req_fuzzy_time / (tdsql::gts_fuzzy_time * 1000);
        }
        // FIXME: query start time is real time
        // if (likely(((read_time.tv_sec == req_time.tv_sec && read_fuzzy_time <= req_fuzzy_time) ||
        //             read_time.tv_sec < req_time.tv_sec))) {
          if (trx->internal_xid_state.get_state() == XID_STATE::XA_ACTIVE) {
            native_mutex_lock(&(trx->m_start_gts_mutex));
            trx->m_start_gts = gts;
            native_cond_signal(&(trx->m_start_gts_cond));
            native_mutex_unlock(&(trx->m_start_gts_mutex));
          } else if (trx->internal_xid_state.get_state() == XID_STATE::XA_PREPARED) {
            native_mutex_lock(&(trx->m_end_gts_mutex));
            trx->m_end_gts = gts;
            native_cond_signal(&(trx->m_end_gts_cond));
...
int spider_internal_start_trx(ha_spider *spider, SPIDER_CONN *conn,
                              int link_idx, int part_no) {
  int error_num;
  ...
  if (!trx->trx_start) {
    if (!trx->trx_consistent_snapshot) {
      trx->use_consistent_snapshot = spider_param_use_consistent_snapshot(thd);
      trx->internal_xa = tdsql_xa;
      trx->internal_xa_snapshot = spider_param_internal_xa_snapshot(thd);
    }
  }
  if ((error_num = spider_check_and_set_sql_log_off(
           thd, conn, &spider->need_mons[link_idx])) ||
      (sync_time_zone && (error_num = spider_check_and_set_time_zone(
                              thd, conn, &spider->need_mons[link_idx]))) ||
      (sync_autocommit && (error_num = spider_check_and_set_autocommit(
                               thd, conn, &spider->need_mons[link_idx]))))
    goto error;
  if (trx->trx_consistent_snapshot) {
    if (trx->internal_xa && trx->internal_xa_snapshot < 2) {
      ...
      if ((error_num = spider_start_internal_consistent_snapshot(
               trx, conn, &spider->need_mons[link_idx])))
        goto error;
    }
  }
  DBUG_PRINT("info",
             ("spider trx->trx_start= %s", trx->trx_start ? "TRUE" : "FALSE"));
  if (!trx->trx_start) {
    ...
    if (thd->get_transaction()->xid_state()->has_state(XID_STATE::XA_ACTIVE) &&
      ...
      tdsql::GTM::instance()->get_gtid(trx->m_spider_gtid, trx, thd->query_id, part_no >= 0 ? part_no : 0);
    }

    if (!trx->trx_xa && trx->internal_xa &&
        (thd_test_options(thd, OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN) ||
         (
          // FIXME: two tables use same partition thd->sql_use_partition_count == 2 ?
          thd->sql_use_partition_count > 1 &&
        ...
        // truncate can't be in xa
        spider->sql_command != SQLCOM_TRUNCATE) {
      trx->trx_xa = TRUE;
      // trx->set_spider_gtid(thd->server_id, thd->query_id, part_no >= 0 ? part_no : 0);
      tdsql::GTM::instance()->get_gtid(trx->m_spider_gtid, trx, thd->query_id, part_no >= 0 ? part_no : 0);
      trx->xid.formatID = opt_tdsql_auto_increment_mode_value
                              ? opt_tdsql_auto_increment_mode_value
                              : 1;
      if (spider_param_internal_xa_id_type(thd) == 0) {
        pthread_mutex_lock(&spider_xid_mutex);
        if (spider_xid_gtrid == ULLONG_MAX) spider_xid_gtrid = 1;
        trx->xid.gtrid_length = my_sprintf(
            trx->xid.data, (trx->xid.data, "%llx%llx", my_interval_timer(),
                            spider_xid_gtrid++));
        pthread_mutex_unlock(&spider_xid_mutex);
      } else {
        trx->xid.gtrid_length =
            my_sprintf(trx->xid.data, (trx->xid.data, "%lx%016lx",
                                       thd_get_thread_id(thd), thd->query_id));
      }

      trx->xid.bqual_length = my_sprintf(
          trx->xid.data + trx->xid.gtrid_length,
          (trx->xid.data + trx->xid.gtrid_length, "%x", thd->server_id));

      //trx->internal_xid_state.xa_state = XID_STATE::XA_ACTIVE;
      trx->internal_xid_state.set_state(XID_STATE::XA_ACTIVE);
      //trx->internal_xid_state.xid.set(&trx->xid);
      trx->internal_xid_state.get_xid()->set(&trx->xid);
      // request gts at start xa
      trx->m_start_gts = 0;
      if (trx_need_start_gts(trx)) spider_request_gts(trx); 
    ...
    if (!trx->trx_consistent_snapshot) {
      trans_register_ha(thd, false, spider_hton_ptr, nullptr);
      if (thd_test_options(thd, OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN))
        trans_register_ha(thd, true, spider_hton_ptr, nullptr);
    }
    trx->trx_start = TRUE;
    trx->trx_xa_prepared = FALSE;
    trx->updated_in_this_trx = FALSE;
    trx->trx_xa_rollbacking = false;
  ...
  if (conn->table_lock == 3) {
    ...
    if (sync_autocommit && !conn->table_lock &&
    ...
    spider_conn_queue_xa_start(conn, spider->trx->m_spider_gtid.get(), trx_need_start_gts(trx));
    conn->disable_xa = FALSE;
  } else if (!trx->trx_consistent_snapshot &&
             !thd_test_options(thd, OPTION_BEGIN) && sync_autocommit &&
             !conn->table_lock &&
             ((!conn->queued_autocommit && conn->autocommit == 1) ||
              (conn->queued_autocommit &&
               conn->queued_autocommit_val == TRUE)) &&
             spider_param_semi_trx(thd)) {
    DBUG_PRINT("info", ("spider semi_trx is set"));
...
error:
  if (xa_lock) spider_xa_unlock(&trx->internal_xid_state);
  DBUG_RETURN(error_num);
}

int spider_internal_xa_commit(THD *thd, SPIDER_TRX *trx, XID *xid[[maybe_unused]],
  ...
  if (trx->updated_in_this_trx || spider_param_xa_register_mode(thd) == 0) {
    ...
    */
    if (!spider_param_ignore_xa_log(thd)) {
      if (!(table_xa = spider_open_sys_table(
      ...
      if ((error_num = spider_check_sys_table(table_xa, xa_key))) {
      ...
      */
      if ((error_num =
               spider_update_xa(table_xa, &trx->xid, SPIDER_SYS_XA_COMMIT_STR)))
        goto error;
  ...
  if ((conn = spider_tree_first(trx->join_trx_top))) {
    // loop xa commit all conn
    do {
      if (conn->bg_search) spider_bg_conn_break(conn, NULL);
      DBUG_PRINT("info", ("spider conn=%p", conn));
      DBUG_PRINT("info", ("spider conn->join_trx=%u", conn->join_trx));
      if (conn->join_trx) {
        if ((tmp_error_num = spider_db_xa_commit(conn, trx->m_spider_gtid.get()))) {
          if (force_commit == 0 ||
              (force_commit == 1 &&
               tmp_error_num !=
                   ER_XAER_NOTA)) { /*If the pending transaction is commit or
                                       rollback in a timely manner, it can be
                                       considered that the transaction at this
                                       stage will only return success; It is
                                       reasonable that  return success to the
                                       application even if a pritition commit
                                       error occurs; However, the current
                                       suspension transaction is not perfect
                                       enough to temporarily return
                                       ER_SPIDER_XA_TIMEOUT_NUM, timeout*/
        ...
        if ((tmp_error_num = spider_end_trx(trx, conn))) {
          SPIDER_CONN_RESTORE_DASTATUS_AND_RESET_TMP_ERROR_NUM;
          if (!error_num && tmp_error_num) error_num = ER_SPIDER_XA_TIMEOUT_NUM;
          // error_num = tmp_error_num;
        }
        conn->join_trx = 0;
      }
  ...
  if (error_num) {
    /*if commit partial,the error information is as follows */
    thd->clear_error();
  ...
  if (trx->updated_in_this_trx || spider_param_xa_register_mode(thd) == 0) {
    ...
    */
    if (!spider_param_ignore_xa_log(thd)) {
      if (!(table_xa_member =
                spider_open_sys_table(thd, SPIDER_SYS_XA_MEMBER_TABLE_NAME_STR,
                                      SPIDER_SYS_XA_MEMBER_TABLE_NAME_LEN, TRUE,
      ...
      */
      if (!(table_xa = spider_open_sys_table(
        ...
        goto error_open_table;
      table_xa_opened = TRUE;
      if ((error_num = spider_delete_xa(table_xa, &trx->xid))) goto error;
      spider_close_sys_table(thd, table_xa, &open_tables_backup, TRUE);
      table_xa_opened = FALSE;
    }
  }
  ...
  trx->internal_xid_state.set_state(XID_STATE::XA_NOTR);
  DBUG_RETURN(0);
...
int spider_internal_xa_commit_one_phase(THD *thd, SPIDER_TRX *trx) {
  int error_num = 0;
  int tmp_error_num = 0;
  SPIDER_CONN *conn;
  DBUG_ENTER("spider_internal_xa_commit_one_phase");
  ...
  if ((conn = spider_tree_first(trx->join_trx_top))) {
    do {
      ...
      DBUG_PRINT("info", ("spider conn->join_trx=%u", conn->join_trx));
      if (conn->join_trx) {
        if ((tmp_error_num = spider_db_xa_commit_one_phase(conn, trx->m_spider_gtid.get()))) {
          if (tmp_error_num != ER_XAER_NOTA) {
            if (!error_num && tmp_error_num)
              error_num = ER_SPIDER_XA_TIMEOUT_NUM;
            /*   error_num = tmp_error_num;*/
          }
...
int spider_internal_xa_rollback(THD *thd, SPIDER_TRX *trx) {
  ...
  if (trx->trx_xa_prepared && !spider_param_ignore_xa_log(thd) &&
      (trx->updated_in_this_trx || spider_param_xa_register_mode(thd) == 0)) {
    /*
      select
        status
      from
    ...
    */
    ...
    if (force_commit != 2 &&
        (error_num = spider_check_sys_xa_status(
             table_xa, SPIDER_SYS_XA_PREPARED_STR, SPIDER_SYS_XA_ROLLBACK_STR,
             NULL, ER_SPIDER_XA_NOT_PREPARED_NUM, &mem_root))) {
      free_root(&mem_root, MYF(0));
      if (error_num == ER_SPIDER_XA_NOT_PREPARED_NUM)
        my_message(error_num, ER_SPIDER_XA_NOT_PREPARED_STR, MYF(0));
    ...
    */
    if ((error_num =
             spider_update_xa(table_xa, &trx->xid, SPIDER_SYS_XA_ROLLBACK_STR)))
      goto error;
    spider_close_sys_table(thd, table_xa, &open_tables_backup, TRUE);
    table_xa_opened = FALSE;
  }
  ...
  if ((conn = spider_tree_first(trx->join_trx_top))) {
    do {
      ...
      if (conn->join_trx) {
        if (conn->disable_xa) {
          if (conn->table_lock != 3 && !trx->trx_xa_prepared) {
            if (!conn->server_lost &&
              ...
              SPIDER_CONN_RESTORE_DASTATUS_AND_RESET_TMP_ERROR_NUM;
              if (!error_num && tmp_error_num) error_num = tmp_error_num;
            }
          }
        } else {
          if (!conn->server_lost) {
            if (!trx->trx_xa_prepared &&
                (tmp_error_num = spider_db_xa_end(conn, trx->m_spider_gtid.get()))) {
              if (force_commit == 0 ||
                  (force_commit == 1 && (tmp_error_num != ER_XAER_NOTA &&
                                         tmp_error_num != ER_XA_RBTIMEOUT &&
                                         tmp_error_num != ER_XA_RBDEADLOCK)) ||
                  tmp_error_num == ER_XAER_RMFAIL /* have done xa end */
              ) {
                SPIDER_CONN_RESTORE_DASTATUS_AND_RESET_TMP_ERROR_NUM;
                if (!error_num && tmp_error_num) error_num = tmp_error_num;
              }
            }
            // if xa end is fail, don't do xa rollback
            if (!error_num && (tmp_error_num = spider_db_xa_rollback(
                                   conn, trx->m_spider_gtid.get()))) {
              if (force_commit == 0 ||
                  (force_commit == 1 && (tmp_error_num != ER_XAER_NOTA &&
                                         tmp_error_num != ER_XA_RBTIMEOUT &&
                                         tmp_error_num != ER_XA_RBDEADLOCK))) {
        ...
        conn->join_trx = 0;
        if (conn->server_lost) server_lost = TRUE;
      }
    } while ((conn = spider_tree_next(conn)));
    trx->join_trx_top = NULL;
  }
  if (error_num) goto error_in_rollback;
...
int spider_free_prepared_trx(THD *thd[[maybe_unused]], SPIDER_TRX *trx) {
  SPIDER_CONN *conn, *conn_next;
  DBUG_ENTER("spider_free_prepared_trx");
  trx->trx_start = FALSE;
  trx->trx_xa = FALSE;
  // release the gtid
  trx->is_set_gtid = FALSE;
  if (trx->m_spider_gtid.get()) {
    tdsql::GTM::instance()->delete_gtid(*trx->m_spider_gtid.get());
    trx->m_spider_gtid.reset(nullptr);
  }
  ...
  if ((conn = spider_tree_first(trx->join_trx_top))) {
    do {
      conn_next = spider_tree_next(conn);
      spider_tree_delete(conn, trx->join_trx_top);
...
int spider_internal_xa_prepare(THD *thd, SPIDER_TRX *trx, TABLE *table_xa,
    ...
    */
    if (!spider_param_ignore_xa_log(thd)) {
      if (!(table_xa = spider_open_sys_table(
        ...
        goto error_open_table;
      table_xa_opened = TRUE;
      if ((error_num = spider_insert_xa(table_xa, &trx->xid,
      ...
      if ((conn = spider_tree_first(trx->join_trx_top))) {
        do {
          if ((error_num =
                   spider_insert_xa_member(table_xa_member, &trx->xid, conn,
                                           SPIDER_SYS_XA_NOT_YET_STR))) {
            SPIDER_CONN_RESTORE_DASTATUS_AND_RESET_ERROR_NUM;
  ...
  if ((conn = spider_tree_first(trx->join_trx_top))) {
    do {
      ...
      if (conn->disable_xa) {
        ...
        /*if (trx->updated_in_this_trx || spider_param_xa_register_mode(thd) ==
        0)
        {*/
        /*
        ...
        */
        ...
        error_num = spider_db_xa_end_and_prepare(conn, trx->m_spider_gtid.get());
        if (error_num) { /*prepare error,then rollback*/
          goto error;
        }
        /*
                if (!internal_xa)
...
#if 0
  if ((error_num = spider_db_xa_commit_one_phase(conn, &(trx->m_spider_gtid.get())))) {
    goto error;
  }
#endif
  if (trx->updated_in_this_trx || spider_param_xa_register_mode(thd) == 0) {
    ...
    */
    if (!spider_param_ignore_xa_log(thd)) {
      if (!(table_xa = spider_open_sys_table(
        ...
        goto error_open_table;
      table_xa_opened = TRUE;
      if ((error_num = spider_update_xa(table_xa, &trx->xid,
  ...
  if (internal_xa) // request commit gts after set state 
    trx->internal_xid_state.set_state(XID_STATE::XA_PREPARED);
  trx->m_end_gts = 0;
  if (trx_need_end_gts()) spider_request_gts(trx);
  // here to insert xa log
  // FIXME: update gts in xa_gtid log asynchronuslly, the get gts and log commit
  // can be processed concurrentlly
  // when tdsql mode is mode mysql, does not write xa log.
  // mode mysql does not init mysql xa log table
  if (tdsql::common::tdsql_mode != tdsql::common::MODE_MYSQL) {
    my_hrtime_t start = my_hrtime();
    error_num = tdsql::GTM::instance()->log_gt_async("", trx, "commit", true);
    TdString gtid_query = "insert xa.t_gtid_log gtid: ";
    gtid_query += trx->m_spider_gtid->to_string();
    log_spider_sql_tdsql(thd, (spider_db_mysql*)(spider_tree_first(trx->join_trx_top)->db_conn),
                         gtid_query.c_str(), gtid_query.length(), start,
                         error_num);
    if (error_num) goto error;
  }
  DBUG_RETURN(0);
...
int spider_internal_xa_recover(THD *thd, XID *xid_list, uint len) {
  ...
  /*
    ...
    where
      status = 'PREPARED'
  */
  if (!(table_xa = spider_open_sys_table(thd, SPIDER_SYS_XA_TABLE_NAME_STR,
                                         SPIDER_SYS_XA_TABLE_NAME_LEN, FALSE,
                                         &open_tables_backup, TRUE, &my_errno)))
    goto error_open_table;
  spider_store_xa_status(table_xa, SPIDER_SYS_XA_PREPARED_STR);
  if ((my_errno = spider_get_sys_table_by_idx(table_xa, xa_key, 1,
...
int spider_initinal_xa_recover(XID *xid_list[[maybe_unused]], uint len[[maybe_unused]]) {
#if 0
  int error_num;
  static THD *thd = NULL;
  ...
  */
  if (!table_xa) {
    if (!(table_xa = spider_open_sys_table(
      ...
      goto error_open_table;
    SPIDER_init_read_record(read_record, thd, table_xa, NULL, NULL, TRUE, FALSE,
                            FALSE);
  }
  SPD_INIT_ALLOC_ROOT(&mem_root, 4096, 0, MYF(MY_WME));
...
int spider_internal_xa_commit_by_xid(THD *thd, SPIDER_TRX *trx, XID *xid[[maybe_unused]]) {
  ...
  */
  ...
  if (force_commit != 2 &&
      (error_num = spider_check_sys_xa_status(
           table_xa, SPIDER_SYS_XA_PREPARED_STR, SPIDER_SYS_XA_COMMIT_STR, NULL,
           ER_SPIDER_XA_NOT_PREPARED_NUM, &mem_root))) {
    free_root(&mem_root, MYF(0));
    if (error_num == ER_SPIDER_XA_NOT_PREPARED_NUM)
      my_message(error_num, ER_SPIDER_XA_NOT_PREPARED_STR, MYF(0));
    goto error;
  ...
  /*
    ...
    where
      ...
      data = trx->xid.data
  */
  if ((error_num = spider_update_xa(table_xa, xid, SPIDER_SYS_XA_COMMIT_STR))) {
    free_root(&mem_root, MYF(0));
    goto error;
  }
  spider_close_sys_table(thd, table_xa, &open_tables_backup, TRUE);
  ...
  */
  ...
  if ((error_num = spider_get_sys_table_by_idx(table_xa_member, xa_member_key,
                                               0, SPIDER_SYS_XA_PK_COL_CNT))) {
    if (error_num != HA_ERR_KEY_NOT_FOUND && error_num != HA_ERR_END_OF_FILE) {
      free_root(&mem_root, MYF(0));
  ...
  do {
    ...
    if ((error_num = spider_db_xa_commit(conn, trx->m_spider_gtid.get())) &&
        (force_commit == 0 ||
         (force_commit == 1 && error_num != ER_XAER_NOTA))) {
      SPIDER_CONN_RESTORE_DASTATUS_AND_RESET_ERROR_NUM;
      if (error_num) {
  ...
  /*
    ...
    where
      ...
      data = xid->data
  */
  if ((error_num = spider_delete_xa_member(table_xa_member, xid))) goto error;
  spider_close_sys_table(thd, table_xa_member, &open_tables_backup, TRUE);
  table_xa_member_opened = FALSE;
...
xa_delete:
  ...
  */
  if (!(table_xa = spider_open_sys_table(
    ...
    goto error_open_table;
  table_xa_opened = TRUE;
  if ((error_num = spider_delete_xa(table_xa, xid))) goto error;
  spider_close_sys_table(thd, table_xa, &open_tables_backup, TRUE);
  table_xa_opened = FALSE;
  DBUG_RETURN(0);
...
int spider_internal_xa_rollback_by_xid(THD *thd, SPIDER_TRX *trx, XID *xid) {
  ...
  */
  ...
  do {
    ...
    conn->error_mode &= spider_param_error_read_mode(thd, 0);
    conn->error_mode &= spider_param_error_write_mode(thd, 0);
    if ((error_num = spider_db_xa_rollback(conn, trx->m_spider_gtid.get())) &&
        (force_commit == 0 ||
         (force_commit == 1 && error_num != ER_XAER_NOTA))) {
      SPIDER_CONN_RESTORE_DASTATUS_AND_RESET_ERROR_NUM;
      if (error_num) {
...
int spider_start_consistent_snapshot(handlerton *hton[[maybe_unused]], THD *thd) {
  int error_num;
  SPIDER_TRX *trx;
  DBUG_ENTER("spider_start_consistent_snapshot");

  if (!(trx = spider_get_trx(thd, TRUE, &error_num))) DBUG_RETURN(error_num);
...
int spider_commit(handlerton *hton, THD *thd, bool all) {
  ...
  if (all || (!thd_test_options(thd, OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN))) {
    if (trx->trx_start) {
      if (trx->trx_xa) {
        if (trx->internal_xa && !trx->trx_xa_prepared) {
          int tmp_error_num;
          uint tree_conn_num = spider_tree_num(trx->join_trx_top);
          if (tree_conn_num == 1) {
            // if only one connection, we can xa commit one phase
            if ((tmp_error_num =
                     spider_internal_xa_commit_one_phase(thd, trx))) {
              if (tmp_error_num) error_num = tmp_error_num;
            }
          } else {
            if ((error_num = spider_internal_xa_prepare(
                     thd, trx, table_xa, table_xa_member, TRUE))) {
              switch (error_num) {
                // if timeout, let agent deal xa trx
                case ER_SPIDER_XA_TIMEOUT_NUM:
                case ER_PROXY_XA_WRITE_GTID_LOG_TIMEOUT:
                  my_message(ER_PROXY_XA2PC_UNCERTAIN, ER_SPIDER_XA_TIMEOUT_STR,
                             MYF(0));
                  spider_free_prepared_trx(thd, trx);
                  break;
                case ER_PROXY_XA_WRITE_GTID_LOG_FAILED:
                  my_error(ER_PROXY_XA_WRITE_GTID_LOG_FAILED, MYF(0),
                           trx->m_spider_gtid.get()->to_string().c_str());
          ...
          if ((tmp_error_num = spider_internal_xa_commit(
        ...
        // here not xa transaction
        if ((conn = spider_tree_first(trx->join_trx_top))) {
          SPIDER_BACKUP_DASTATUS;
          int tmp_error_num;
          do {
            if ((conn->autocommit != 1 || conn->trx_start) &&
    ...
    trx->trx_consistent_snapshot = FALSE;
             ...
             spider_param_enable_conns_view()) {
    /*
...
int spider_rollback(handlerton *hton[[maybe_unused]], THD *thd, bool all) {
  SPIDER_TRX *trx;
  int error_num = 0;
  SPIDER_CONN *conn;
  ...
  // avoid rollback call rollback recursively
  if (trx->trx_xa && trx->trx_xa_rollbacking) {
    LogCommInfo("trx:%s is rollback recursively",
                trx->m_spider_gtid->to_string().c_str());
    DBUG_RETURN(ER_PROXY_XA2PC_UNCERTAIN);
  }
  if (trx->trx_xa) trx->trx_xa_rollbacking = true;
  ...
  if (thd->killed != THD::NOT_KILLED) {
    ...
    if ((conn = spider_tree_first(trx->join_trx_top))) {
      do {
        ...
        spider_send_kill(conn, thd->killed);
      } while ((conn = spider_tree_next(conn)));
    }
  ...
  if (all || (!thd_test_options(thd, OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN))) {
    if (trx->trx_start) {
      if (trx->trx_xa) {
        int tmp_error_num;
        if ((tmp_error_num = spider_internal_xa_rollback(thd, trx))) {
          if (tmp_error_num) error_num = tmp_error_num;
        }
        trx->trx_xa = FALSE;
        trx->join_trx_top = NULL;
  ...
  spider_merge_mem_calc(trx, FALSE);
  if (!all && spider_param_trans_rollback(thd) &&
      thd->is_error()) { /*one query error in the procedure of transaction,then
                            need to rollback*/
...
int spider_xa_prepare(handlerton *hton[[maybe_unused]], THD *thd, bool all) {
  int error_num;
  SPIDER_TRX *trx;
  TABLE *table_xa = NULL;
  TABLE *table_xa_member = NULL;
  ...
  if (all || (!thd_test_options(thd, OPTION_NOT_AUTOCOMMIT | OPTION_BEGIN))) {
    ...
    DBUG_PRINT("info",
               ("spider trx_start=%s", trx->trx_start ? "TRUE" : "FALSE"));
    DBUG_PRINT("info", ("spider trx_xa=%s", trx->trx_xa ? "TRUE" : "FALSE"));
    if (trx->trx_start && trx->trx_xa) {
      if ((error_num = spider_internal_xa_prepare(thd, trx, table_xa,
                                                  table_xa_member, FALSE)))
        goto error;
      trx->trx_xa_prepared = TRUE;
    }
  }
...
int spider_xa_recover(handlerton *hton, XA_recover_txn *xa_trx_list, uint len,
                         MEM_ROOT *mem_root) {
  XID *xid_list = (XID*)mem_root->Alloc(sizeof(XID) * len);
  for (uint i = 0; i < len; i++) {
    xid_list[i] = xa_trx_list[i].id;
  }

  return spider_xa_recover_low(hton, xid_list, len);
}

int spider_xa_recover_low(handlerton *hton[[maybe_unused]], XID *xid_list, uint len) {
  THD *thd = current_thd;
  DBUG_ENTER("spider_xa_recover");
  /* do not support spider do xa */
  DBUG_RETURN(0);
  if (len == 0 || xid_list == NULL) DBUG_RETURN(0);
...
xa_status_code spider_xa_commit_by_xid(handlerton *hton[[maybe_unused]], XID *xid) {
  SPIDER_TRX *trx;
  int error_num;
  THD *thd = current_thd;
  ...
  if ((error_num = spider_internal_xa_commit_by_xid(thd, trx, xid))) goto error;

  DBUG_RETURN(XA_OK);

error:
error_get_trx:
  DBUG_RETURN((xa_status_code)error_num);
}

xa_status_code spider_xa_rollback_by_xid(handlerton *hton[[maybe_unused]], XID *xid) {
  SPIDER_TRX *trx;
  int error_num;
  THD *thd = current_thd;
...
int spider_end_trx(SPIDER_TRX *trx, SPIDER_CONN *conn) {
  ...
  if (conn->table_lock == 3) {
    ...
    if (!conn->server_lost &&
      ...
      if (error_num == ER_SPIDER_REMOTE_SERVER_GONE_AWAY_NUM) error_num = 0;
    }
  } else if (!conn->table_lock)
    conn->disable_reconnect = FALSE;
  if (conn->semi_trx_isolation >= 0 &&
      conn->trx_isolation != conn->semi_trx_isolation) {
    DBUG_PRINT("info", ("spider conn=%p", conn));
    DBUG_PRINT("info", ("spider conn->trx_isolation=%d", conn->trx_isolation));
    if (!conn->server_lost && !conn->queued_semi_trx_isolation &&
        (error_num = spider_db_set_trx_isolation(conn, conn->trx_isolation,
                                                 &need_mon))) {
...
int spider_check_trx_and_get_conn(THD *thd, ha_spider *spider,
                                  bool use_conn_kind[[maybe_unused]]) {
  int error_num, roop_count, search_link_idx;
  SPIDER_TRX *trx;
  SPIDER_SHARE *share = spider->share;
  // SPIDER_CONN *conn = nullptr;
  int semi_table_lock_conn MY_ATTRIBUTE((unused)) =
  ...
  if (spider->sql_command != SQLCOM_DROP_TABLE &&
    ...
    DBUG_PRINT("info",
               ("spider semi_table_lock_conn = %d", semi_table_lock_conn));
    DBUG_PRINT("info",
               ("spider semi_table_lock = %d",
    ...
    if (!trx_ha || trx_ha->wait_for_reusing ||
      ...
      if (spider->spider_thread_id != trx->spider_thread_id ||
        ...
        if (search_link_idx == -1) {
          ...
          my_printf_error(ER_SPIDER_ALL_LINKS_FAILED_NUM,
                          ER_SPIDER_ALL_LINKS_FAILED_STR, MYF(0), db,
                          table_name);
          my_afree(db);
      ...
      DBUG_PRINT(
          ...
          ("spider link_status = %ld",
           share->link_statuses[spider
                                    ->conn_link_idx[spider->search_link_idx]]));
      ...
      for (roop_count = spider_conn_link_idx_next(
        ...
        if (!search_link_idx_is_checked) {
          ...
          my_printf_error(ER_SPIDER_LINK_MON_JUST_NG_NUM,
                          ER_SPIDER_LINK_MON_JUST_NG_STR, MYF(0), db,
                          table_name);
          my_afree(db);
...
THD *spider_create_tmp_thd() {
  ...
  if (!(thd = SPIDER_new_THD((my_thread_id)0))) DBUG_RETURN(NULL);
...
int spider_create_trx_ha(SPIDER_TRX *trx, ha_spider *spider,
  ...
  if (spider_param_enable_trx_ha()) {
    ...
    if (need_create) {
      if (!(trx_ha = (SPIDER_TRX_HA *)spider_bulk_malloc(
                spider_current_trx, 58, MYF(MY_WME), &trx_ha,
                sizeof(SPIDER_TRX_HA), &tmp_name,
                sizeof(char *) * (share->table_name_length + 1), &conn_link_idx,
                sizeof(uint) * share->link_count, &conn_can_fo,
                sizeof(uchar) * share->link_bitmap_size, NullS))) {
        DBUG_RETURN(HA_ERR_OUT_OF_MEM);
      }
      ...
      if (trx->trx_ha_hash.array.max_element > old_elements) {
        spider_alloc_calc_mem(
            spider_current_trx, trx->trx_ha_hash,
...
void spider_trx_set_link_idx_for_all(ha_spider *spider) {
  ...
  for (roop_count = 0; roop_count < link_count; roop_count++) {
    for (roop_count2 = roop_count; roop_count2 < all_link_count;
      ...
      if (link_statuses[roop_count2] <= SPIDER_LINK_STATUS_RECOVERY) break;
    }
    if (roop_count2 < all_link_count) {
      ...
      DBUG_PRINT("info",
                 ("spider set conn_link_idx[%d]=%d", roop_count, roop_count2));
    } else {
      conn_link_idx[roop_count] = roop_count;
      DBUG_PRINT("info",
                 ("spider set2 conn_link_idx[%d]=%d", roop_count, roop_count));
    }
  }
  DBUG_VOID_RETURN;
}
...
int spider_trx_check_link_idx_failed(ha_spider *spider) {
  ...
  for (roop_count = 0; roop_count < link_count; roop_count++) {
    if (link_statuses[conn_link_idx[roop_count]] == SPIDER_LINK_STATUS_NG &&
...
void spider_trx_add_bulk_access_conn(SPIDER_TRX *trx, SPIDER_CONN *conn) {
  ...
  DBUG_PRINT("info",
             ("spider conn->bulk_access_sended=%u", conn->bulk_access_sended));
  DBUG_PRINT("info", ("spider trx->bulk_access_conn_first=%p",
                      trx->bulk_access_conn_first));
