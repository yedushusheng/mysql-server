...
struct hstcpcli : public hstcpcli_i, private noncopyable {
  ...
  virtual bool stable_point() override;
  virtual void request_buf_open_index(size_t pst_id, const char *dbn,
                                      const char *tbl, const char *idx,
                                      const char *retflds, const char *filflds) override;
  virtual void request_buf_auth(const char *secret, const char *typ) override;
  virtual void request_buf_exec_generic(
      ...
      size_t invalueslen) override;
  virtual size_t request_buf_append(const char *start, const char *finish) override;
  virtual void request_reset() override;
  virtual int request_send() override;
  virtual int response_recv(size_t &num_flds_r) override;
  virtual int get_result(hstresult &result) override;
  virtual const string_ref *get_next_row() override;
  virtual const string_ref *get_next_row_from_result(hstresult &result) override;
  virtual void response_buf_remove() override;
  virtual int get_error_code() override;
  virtual String &get_error() override;
  virtual void clear_error() override;
  virtual int set_timeout(int send_timeout, int recv_timeout) override;
  virtual size_t get_num_req_bufd() override { return num_req_bufd; }
  virtual size_t get_num_req_sent() override { return num_req_sent; }
  virtual size_t get_num_req_rcvd() override { return num_req_rcvd; }
  virtual size_t get_response_end_offset() override { return response_end_offset; }
  virtual const char *get_readbuf_begin() override { return readbuf.begin(); }
  virtual const char *get_readbuf_end() override { return readbuf.end(); }
  virtual const char *get_writebuf_begin() override { return writebuf.begin(); }
  virtual size_t get_writebuf_size() override { return writebuf.size(); }
  virtual void write_error_to_log(const char *func_name, const char *file_name,
                                  ulong line_no) override;
