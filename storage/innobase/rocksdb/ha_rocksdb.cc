...
#include "sql/tdsql/route/route_mgr.h"
...
#include "storage/rocksdb/p_s.h"
#include "include/mysql/psi/mysql_data_lock.h"
...
static Rocksdb_data_lock_inspector rocksdb_data_lock_inspector;
...
static int rocksdb_init_func(void *const p) {
  ...
  mysql_data_lock_register(&rocksdb_data_lock_inspector);
...
int ha_rocksdb::convert_record_from_storage_format(
  ...
  return convert_record_from_storage_format(key, &m_retrieved_record, buf, &m_retrieved_header);
}
...
int ha_rocksdb::convert_record_from_storage_format(
    ...
    uchar *const buf, rocksdb::Slice *meta) {
  ...
  */
  ...
  return m_tbl_def->ConvertRecordFromStorageFormat(table,key,value,buf,m_key_requested,m_decoders_vect,&unpack_arg,meta);
}
...
int ha_rocksdb::CovertRecordPush(const rocksdb::Slice *const key,
  ...
  return m_tbl_def->ConvertRecordFromStorageFormat(
      table, key, value, buf, m_key_requested, m_decoders_vect, unpack_arg, nullptr);
}

int ha_rocksdb::tdsql_convert_record_from_storage_format(
    rocksdb::Slice key, rocksdb::Slice value, uchar *const buf) {

  m_last_rowkey.copy(key.data(), key.size(), &my_charset_bin);
  return convert_record_from_storage_format(&key, &value, buf, &m_retrieved_header);
}
...
bool ha_rocksdb::tdsql_key_in_pk(const std::string &key) const {
  ...
  if (!tdsql_get_storage_format_pk(&pk)) {
    assert(pk.size() == 1);
    // Both of them can't be empty key, so it's OK to assume them as start keys.
    if (CompareStartKey(key, pk.back().key) >= 0 &&
        CompareStartKey(key, pk.back().successor_key) < 0) {
      return true;
    }
  }
  return false;
...
int ha_rocksdb::read_row_from_primary_key(uchar *const buf) {
  ...
  return convert_record_from_storage_format(&rkey, &value, buf, &m_retrieved_header);
}
...
int ha_rocksdb::get_row_by_rowid(uchar *const buf, const char *const rowid,
  ...
  if (m_lock_rows == RDB_LOCK_NONE) {
    // pk point loopup
    rc = rpc_get_record(tx, key_slice, pk_point_select);
    if (rc == HA_EXIT_SUCCESS) {
      found = true;
    ...
    rc = rpc_get_record(tx, key_slice);
    if (rc == HA_EXIT_SUCCESS) {
      found = true;
    }
    else {
...
bool ha_rocksdb::need_retry_to_generate_autoinc(int errcode) const {
  if (errcode != HA_ERR_FOUND_DUPP_KEY) return false;
  if (m_dupp_errkey >= (int)table->s->keys) return false;

  auto dup_keyinfo = table->s->key_info[m_dupp_errkey];
  if (dup_keyinfo.tindex_id == table->tindex_id
    && dup_keyinfo.actual_key_parts == 1
    && (dup_keyinfo.key_part[0].field->auto_flags & Field::NEXT_NUMBER)) {           
    return true;
  }
  return false;
}
...
int ha_rocksdb::write_row(uchar *const buf) {
  ...
  int retry_times = tdsql_autoinc_retry_times_on_dupkey_err;
  for (int i = 0; i <= retry_times; ++i) {
    ...
    */
    ...
    if (i < retry_times
        && need_retry_to_generate_autoinc(rv)
        && need_auto_incr && orig_next_number == 0
...
int ha_rocksdb::HandleStmtOptimizeLoadDataDuplicateEntry(int ret) {
  ...
  while (!tx->StmtOptimizeLoadDataPopDuplicateEntry(m_part_pos, &dup_entry)) {
    ...
    if (convert_record_from_storage_format(&dup_entry_key, &dup_entry_val, table->record[0], nullptr)) {
      assert(0);
      LogError(
...
int ha_rocksdb::rnd_range_init(const tdsql::parallel::PartKeyRangeInfo& range_info) {
  ...
  DBUG_EXECUTE_IF("rnd_range_init_meet_close_conn_before_init_td_iter", {
    DBUG_SET("+d, update_route_bypass_cache");
    current_thd->awake(THD::killed_state::KILL_CONNECTION);
  });
  ...
  if (flag & HA_STATUS_VARIABLE) {
    ...
    */
    ...
    if (need_compute_statistic) {
      ...
      bool need_broadcast = (!m_table_handler->IsSystemTable()) && (!tdstore_auto_stat_skip_send_reload);
      bool async = true;
...
std::string GetIndexIdAddress(uint32 index_id) {
  ...
  auto *rm = tdsql::GetRegionManager(range.start);
  if (!rm) return "";

  RegionPtrs rgns;  // Regions to be accessed.

  if (rm->GetMultiRegion(range.start, range.limit, &rgns)) return "";

  if (!rgns.size()) return "";

  return rgns.front()->rep_group->hyper_node->node_rpc_addr;
}
...
int ha_rocksdb::index_fillback_sk_by_range(const rocksdb::Slice &start,
  ...
  if (ha_rnd_range_init(range_info)) {
    ...
    if (!current_thd->is_error()) {
      my_error(ER_SQLENGINE_SYSTEM, MYF(0), "Failed to init rnd range scan for parallel index fillback");
    }
    DBUG_RETURN(HA_EXIT_FAILURE);
  }
...
int ha_rocksdb::rpc_get_record(
  ...
  if (ReadWithoutTrans(thd)) {
    ...
    assert(cntl.get_tx() == tx);
    ret = cntl.Execute();
    SetValueSlice(&m_retrieved_record, ret, &cntl);
  } else {
...
int ha_rocksdb::rpc_del_record(
  ...
  */
  ...
  if (with_extra_info && !tdsql::IsSystemTable(table->tindex_id)) {
    rocksdb::Slice extra_info = m_retrieved_header;
    rocksdb::Slice value = extra_info;
    /*
   ...
   if (table->correlate_src_table) {
      ...
      value = rocksdb::Slice(m_storage_record.ptr(), m_storage_record.length());
    } else if (extra_info.empty()) {
      value = m_retrieved_record;
    }
    if (extra_info.data() != value.data()) {
      ret = m_tbl_def->GetExtraInfo(table, &value, extra_info);
    }

    if (ret) return ret;
    
    cntl.SetExtraInfo(extra_info.data(), extra_info.size());
  }
  assert(cntl.get_thd() == tx->thd());
  assert(cntl.get_tx() == tx);
...
int ha_rocksdb::request_row_count(THD *thd, uint index MY_ATTRIBUTE((unused)),
                                  uint64_t *rows, const std::string &start,
...
void ha_rocksdb::FixParallelScanRangeIfCrossIndex(const Rdb_key_def& kd) {
  if (KeyRangeCrossIndexId(m_scan_it_lower_bound_slice,
                           m_scan_it_upper_bound_slice)) {
  ...
  assert(!KeyRangeCrossIndexId(m_scan_it_lower_bound_slice,
                               m_scan_it_upper_bound_slice));
}

int ha_rocksdb::GetParallelScanRegionListRefOrNull(
    uint keynr, key_range *min_key, key_range *max_key, RegionPtrs *ref_rgns,
    RegionPtrs *null_rgns, bool preassign_ranges,
  ...
  assert(!KeyRangeCrossIndexId(m_scan_it_lower_bound_slice,
                               m_scan_it_upper_bound_slice));
  rocksdb::PinnableSlice null_upper_bound;
  if (paral_scan_handle) {
    ...
    null_upper_bound.PinSelf(m_scan_it_upper_bound_slice);
  }

  auto *rgn_mgr = tdsql::GetRegionManager(m_scan_it_lower_bound_slice);
  assert(rgn_mgr);

  // Get route of null.
  int ret = rgn_mgr->GetMultiRegion(
    m_scan_it_lower_bound_slice, m_scan_it_upper_bound_slice, null_rgns);
  if (ret != TDSQL_EXIT_SUCCESS) {
    LogError(
  ...
  if (preassign_ranges) {
    // Parallel scan on inner table always construct the whole range of index.
    // See the comment in MyRocksParallelScan::Init.
    //
    // Each field of a secondary index is NULL in the null part of RefOrNull.
    // If each field of a secondary index is NULL, the high order bytes
    // (excluding the tindex id prefix) encoded in the index record will be 0.
    // E.g. CREATE TABLE t(a INT PRIMARY KEY, b INT, INDEX idxb(b));
    //      | a |   b  |
    //      | 1 | NULL | --> 00002701 00 80000001
    //                       ^        ^  ^
    //                       |        |  |
    //        tindex id prefix        |  a=1
    //                                |
    //                                b's null byte
    //
    // There can't be any records to the left of null records.
    // E.g. [00002701 ~ 0000270100) is definitely empty.
    //
    // So, the ref parts of parallel RefOrNull on inner table should be
    // (null_upper_bound, tindex_id.successor).
    const rocksdb::Slice &null_upper_bound_ref =
        paral_scan_handle ? paral_scan_handle->null_upper_bound()
                          : null_upper_bound;
    assert(null_upper_bound_ref.size() > 0);
    memcpy(m_scan_it_lower_bound, null_upper_bound_ref.data(),
           null_upper_bound_ref.size());
    memcpy(m_scan_it_upper_bound, kd.get_index_number_storage_form(),
           Rdb_key_def::INDEX_NUMBER_SIZE);
    kd.successor(m_scan_it_upper_bound, Rdb_key_def::INDEX_NUMBER_SIZE);
    m_scan_it_lower_bound_slice = rocksdb::Slice(
        (const char *)m_scan_it_lower_bound, null_upper_bound_ref.size());
    m_scan_it_upper_bound_slice = rocksdb::Slice(
        (const char *)m_scan_it_upper_bound, Rdb_key_def::INDEX_NUMBER_SIZE);
    assert(!KeyRangeCrossIndexId(m_scan_it_lower_bound_slice,
                                 m_scan_it_upper_bound_slice));
    ...
    ret = rgn_mgr->GetMultiRegion(
      m_scan_it_lower_bound_slice, m_scan_it_upper_bound_slice, ref_rgns);
    if (ret != TDSQL_EXIT_SUCCESS) {
      LogError(
          "[GetParallelScanRegionListRefOrNull failed]"
          "[GetMultiRegion failed]"
...
int ha_rocksdb::GetParallelScanRegionList(
    uint keynr, key_range *min_key, key_range *max_key, RegionPtrs *rgns,
    bool preassign_ranges, MyRocksParallelScan *paral_scan_handle) {
  if (preassign_ranges) min_key = max_key = nullptr;
  ...
  int ret = rgn_mgr->GetMultiRegion(
    m_scan_it_lower_bound_slice, m_scan_it_upper_bound_slice, rgns);
  if (ret != TDSQL_EXIT_SUCCESS) {
    LogError(
...
int ha_rocksdb::attach_parallel_scan(parallel_scan_handle_t scan_handle) {
  ...
  if (m_parallel_scan_handle->preassign_ranges()) {
    m_current_job = 0;
    m_parallel_scan_handle->AttachPreassignJobs(&m_parallel_scan_jobs);
#ifndef NDEBUG
    for (const auto &job : m_parallel_scan_jobs) {
      LogInfo("[Preassign][job:%s]", job->ToString().data());
    }
#endif
  }
...
int ha_rocksdb::estimate_parallel_scan_ranges(uint keynr, key_range *min_key,
  ...
  RegionPtrs rgns, null_rgns;
  bool preassign_ranges = flags & parallel_scan_flags::preassign_ranges;
  int ret = flags & parallel_scan_flags::ref_or_null
                ? GetParallelScanRegionListRefOrNull(keynr, min_key, max_key,
                                                     &rgns, &null_rgns,
                                                     preassign_ranges, nullptr)
                : GetParallelScanRegionList(keynr, min_key, max_key, &rgns,
                                            preassign_ranges, nullptr);
...
int ha_rocksdb::ha_estimate_rows() {
  return m_table_handler->m_mtcache_count;
}
...
int ha_rocksdb::FlushStmtOptimizeLoadDataIfPossible(tdsql::Transaction *txn,
  ...
  if (txn && use_optimize_load_data(txn) &&
    ...
    } else if (ret == HA_ERR_LOCK_WAIT_TIMEOUT) {
      print_error(ret, MYF(0));
...
int ha_rocksdb::get_distribution_info_by_scan_range(
  ...
  if (unlikely(flags & parallel_scan_flags::ref_or_null))
    res = GetParallelScanRegionListRefOrNull(
        keynr, min_key, max_key, &rgns, &null_rgns, preassign_ranges, nullptr);
  else
    res = GetParallelScanRegionList(keynr, min_key, max_key, &rgns,
                                    preassign_ranges, nullptr);
  ...
  for (auto &rgn : rgns) {
    table_node_storage_info &tnsi =
        aggregated_rgninfo[rgn->rep_group->hyper_node->node_rpc_addr];
    if (tnsi.node_addr.empty())
      tnsi.node_addr = rgn->rep_group->hyper_node->node_rpc_addr;
    ++tnsi.num_region;
  }
