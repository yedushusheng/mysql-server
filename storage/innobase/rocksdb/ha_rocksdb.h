...
extern bool tdstore_auto_stat_skip_send_reload;
...
struct Rdb_table_handler : public boost::noncopyable{
 public:
  Rdb_table_handler(const std::string &name) :m_table_name(name){
    ...
    is_system_table_ = is_system_table(m_real_db_name.c_str(), m_real_table_name.c_str());
...
class ha_rocksdb : public my_core::handler {
  ...
  */
  ...
  rocksdb::Slice m_retrieved_header;
...
public:
  ...
  */
  ...
  bool need_retry_to_generate_autoinc(int errcode) const;
 ...
 public:
  ...
  virtual int tdsql_convert_record_from_storage_format(rocksdb::Slice key,
                                                       rocksdb::Slice value,
 ...
 public:
  ...
  void clone_parallel_scan(ha_rocksdb *from) {
    ...
    m_parallel_scan_jobs = from->m_parallel_scan_jobs;
    if (m_parallel_scan_handle && m_parallel_scan_handle->preassign_ranges()) {
      m_current_job = 0;
    }
  }
  ...
  void AttachAParallelScanJob() {
    if (m_current_job == -1) {
      ...
      if (!m_parallel_scan_handle->preassign_ranges())
        m_parallel_scan_jobs.emplace_back(m_parallel_scan_job);
    } else if ((size_t)m_current_job < m_parallel_scan_jobs.size()) {
      m_parallel_scan_job = m_parallel_scan_jobs[m_current_job++];
    } else {
  ...
  bool use_parallel_scan() const {
    return m_parallel_scan_handle != nullptr &&
           !is_parallel_scan_exec_flag_set(PARALLEL_SCAN_EXEC_NOT_PSCAN);
  }

  int GetParallelScanRegionList(uint keynr, key_range *min_key,
                                key_range *max_key, RegionPtrs *rgns,
                                bool preassign_ranges,
 ...
 private:
  ...
  // Preassign jobs for inner table.
  std::vector<MyRocksParallelScanJob *> m_parallel_scan_jobs;
  int64_t m_current_job{-1};
