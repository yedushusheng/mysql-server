...
class Item_cond : public Item_bool_func {
 ...
 public:
  ...
  bool init_from(const Item *from, Item_clone_context *context) override;
  Item_parallel_safe parallel_safe() const override {
    auto safe = super::parallel_safe();
    if (safe == Item_parallel_safe::Unsafe) return safe;
    for (auto &item : list) {
      if (item.parallel_safe() < safe) continue;
      if ((safe = item.parallel_safe()) == Item_parallel_safe::Unsafe)
        return safe;
