...
class Item_func_trig_cond final : public Item_bool_func {
  ...
  */
  ...
  Item_parallel_safe parallel_safe() const override {
    auto safe = Item_bool_func::parallel_safe();
    if (safe != Item_parallel_safe::Safe) return safe;

    // Don't support this yet if trig_var is used
    return trig_var ? Item_parallel_safe::Unsafe : Item_parallel_safe::Safe;
  }
...
class in_double final : public in_vector {
 ...
 public:
  ...
  in_vector *new_in_vector(Item_clone_context *context) const override {
    auto *mem_root = context->mem_root();
    return new (mem_root) in_double(mem_root, used_count);
  }
  bool init_from(const in_vector *from, Item_clone_context *context) override;
