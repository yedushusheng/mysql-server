...
bool Item_sum::init_from(const Item *from, Item_clone_context *context) {
  ...
  THD *thd = context->thd();
  const Item_sum *item = down_cast<const Item_sum *>(from);
  ...
  base_query_block = context->target_query_block();
  if (!m_window) aggr_query_block = context->target_query_block();

  return context->fix_cloned_sum(this, nullptr);
}
...
void Item_udf_sum::clear() {
  DBUG_TRACE;
  DBUG_ASSERT(udf.is_initialized());
  udf.clear();
}
...
inline Item *Item_rollup_sum_switcher::current_arg() const {
  DBUG_ASSERT(m_current_rollup_level >= 0 &&
              m_current_rollup_level < m_num_levels);
