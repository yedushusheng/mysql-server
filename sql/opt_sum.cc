...
bool optimize_aggregated_query(THD *thd, SELECT_LEX *select,
  ...
  *decision = AGGR_REGULAR;  // Default return value
  ...
  /*
    ...
    of returned rows.
  */
  /** NOTE:统计所有叶子结点上的单表,如果做笛卡尔积,其最大的连接后的行数
   * 入口参数tables的输入时select_lex->leaf_tables,查询语句中的所有的单表对象,都存于叶子结点select_lex->leaf_tables
   * 尽管带有MAX函数且没有GROUPBY子句,但不支持的优化形式如下(带有外连接但WHERE子句的条件限制了内表)
   * select max(t1.a) from t1 left join t2 join-condition where t2.field is null
  */
  for (TABLE_LIST *tl = tables; tl; tl = tl->next_leaf) {
    /** NOTE:尽量利用存储引擎的功能(有的存储引擎提供直接对表的行数计数的功能,所以不需要额外计算,直接获取即可)
     * 完成求多表连接后笛卡尔积的行数计算
    */
  ...
  */
  //NOTE:遍历传入的每个列对象,如果列上存在COUNT/MIN/MAX函数,则尽可能优化
  for (Item *item : fields) {
    if (item->type() == Item::SUM_FUNC_ITEM && !item->m_is_window_function) {
    //NOTE:如果是聚合函数(非窗口函数)则进行优化
      ...
      switch (item_sum->sum_func()) {
        case Item_sum::COUNT_FUNC: {  //NOTE:聚合函数中的COUNT函数
          ...
          */
          /** NOTE:没有WHERE子句、函数参数不可为NULL、不存在外连接、能够精确计数,
           * 则调用get_exact_record_count函数求精确值
          */
        ...
        case Item_sum::MAX_FUNC: {
          ...
          */
          ...
          if (expr->type() == Item::FIELD_ITEM) {
            ...
            */
            /** NOTE:调用find_key_for_maxmin查看是否可以利用索引得到MAX/MIN的值,
             * 如果能利用索引,则会大大加快执行速度.
            */
            ...
            */
            ...
            /** NOTE:如果是MAC则调用get_index_max_value获得最大值;否则调用get_index_min_value获得最小值.
             * 注意,此阶段求得最值的真实值,有利于后续利用常量进行其他技术的优化.
            */
            ...
            */
            ...
            //NOTE:MIN/MAX函数不是基于列对象,如SELECT MAX(1) from table...
            /*
