...
bool JOIN::optimize() {
  ...
  */
  ...
  if ((where_cond || !group_list.empty() || !order.empty()) &&
    ...
    count_field_types(select_lex, &tmp_table_param, select_lex->fields, false,
                      false);
  }
  ...
  */
  ...
  if (const int ret = replace_index_subquery()) {
    ...
    // set_plan_state(PLAN_READY) has been moved to end of parallel_optimize();
    /*
...
bool JOIN::parallel_optimize() {
  // This could be plan we don't support, e.g. ZERO_RESULT, NO_TABLE or plan
  // generated by hypergraph_optimizer.
  if (plan_state != NO_PLAN) return false;

  Opt_trace_context *const trace = &thd->opt_trace;
  Opt_trace_object trace_wrapper(trace);
  Opt_trace_object trace_optimize(trace, "parallel_optimization");
...
static void semijoin_types_allow_materialization(TABLE_LIST *sj_nest) {
  ...
  DBUG_ASSERT(sj_nest->nested_join->sj_outer_exprs.size() ==
...
*/
/** NOTE:把列上的非NULL语义加入到条件中,有利于条件判断(语义优化的技术使用)
 * 为条件子句增加非空条件函数,目的是应用语义优化的技术,对SQL查询语句进一步优化.
 * 该函数为等式条件子句增加非空条件,但不是所有的等式条件子句都可以增加非空条件.
 * 非空条件是DDL语句为表增加的属性信息,包括因主键索引创建暗含的列值非空、列定义为非空约束等情况.
 * 
 ...
 * 上述表达式表达的含义如下:
 * 访问t1表(any_access)，引用(ref)了t2.key=t1.field这个条件.
 * 但因为t2表有索引列key,所以可以增加判断条件以便为整体的查询计划做优化.增加条件后的SQL查询的含义可如下:
 * select * from t1,t2 where t2.key=t1.field and t1.field is not null;
 * t2.key=t1.field中t2.key列是一个存在主键索引的列,故key不可为NULL,所以根据等式,
 * 能够判断t1.field也不能为NULL,所以可以为SQL查询增加条件t1.field IS NOT NULL.
 * MYSQL因为能够使用非空约束支持语义优化,所以MySQL有多处代码在收集列的非空信息.对于列约束为非NULL的情况,MySQL的代码处理方式总结如下:
 * 1.update_ref_and_keys函数:收集了空值拒绝(null-rejection)的情况(在Key_field::null_rejection).
 * 有利于进行外连接消除的优化(即明确了哪些列是非空的).
 * 2.add_key_part函数:保存收集到的空值拒绝信息到Key_use.
 * 3.create_ref_for_key函数:复制空值拒绝信息到TABLE_REF,这种方式有利于表数据的快速访问.
 * 4.add_not_null_conds函数:增加x IS NOT NULL到连接树(JOIN_TAB)的成员上的join_tab->m_condition.
 * 这样有利于条件的判断,这是利用了语义优化技术对SQL进行优化.
 * 
 * 由此可知,add_not_null_conds函数被make_join_select函数调用,利用非NULL信息优化表达式
 * 调用关系:
 * JOIN::optimzie -> make_join_select -> add_not_null_conds -> and_with_condition
*/
static bool add_not_null_conds(JOIN *join) {
  DBUG_TRACE;
  ASSERT_BEST_REF_IN_JOIN_ORDER(join);
  for (uint i = join->const_tables; i < join->tables; i++) {
  //NOTE:遍历连接树中除常量表外的每个表
    JOIN_TAB *const tab = join->best_ref[i];
    if ((tab->type() != JT_REF && tab->type() != JT_EQ_REF &&
         tab->type() != JT_REF_OR_NULL) ||
        tab->table()->is_nullable()) {//NOTE:不是所有的等式条件子句都可以增加非空条件
      continue;
    }
    //NOTE:遍历表上的每个key
 ...
 * 调用关系:
 * JOIN::optimize -> make_join_select -> add_not_null_conds
 *                                    -> make_cond_for_table
 *                                    -> and_conditions
*/
static bool make_join_select(JOIN *join, Item *cond) {
  ...
  //NOTE:优化点:重要操作.把列上的非NULL语义加入到条件中,有利于条件判断(语义优化的技术使用)
  ...
  */
  /** NOTE:步骤一:抽取表达式中的常量.调用的函数包括:make_cond_for_table/and_conditions/Item_func_trig_cond
  ...
  *all_order_by_fields_used = true;
  ...
  for (const auto &item_and_order : bit_fields_to_add) {
    item_and_order.second->item =
        thd->lex->current_select()->add_hidden_item(item_and_order.first);
...
static void calculate_materialization_costs(JOIN *join, TABLE_LIST *sj_nest,
  ...
  if (sj_nest) {
    ...
    */
    ...
    inner_expr_list = &join->select_lex->fields;
  }
