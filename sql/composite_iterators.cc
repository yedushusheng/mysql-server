      ...
      if (err == -1) {
        ...
        if (m_join->grouped || m_join->group_optimized_away) {
          ...
          */
          ...
          // If this is the first stage of two-phase-grouping in a parallel
          // query don't output a extra row, we handle it in the second phase.
          if (first_phase_of_grouping) return -1;          
          return 0;
        }
      }
      if (err != 0) return err;
